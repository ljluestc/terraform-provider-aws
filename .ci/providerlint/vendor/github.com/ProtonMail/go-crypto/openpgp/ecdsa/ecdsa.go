...
PublicKey(curve ecc.ECDSACurve) *PublicKey {
	return &PublicKey{
		curve: curve,
	}
}
 ...
 *PublicKey) GetCurve() ecc.ECDSACurve {
	return pk.curve
}
 ...
 *PrivateKey) MarshalIntegerSecret() []byte {
	return sk.curve.MarshalIntegerSecret(sk.D)
}


 *PrivateKey) UnmarshalIntegerSecret(d []byte) error {
...
erateKey(rand io.Reader, c ecc.ECDSACurve) (priv *PrivateKey, err error) {
	priv = new(PrivateKey)
...
n(rand io.Reader, priv *PrivateKey, hash []byte) (r, s *big.Int, err error) {
	return priv.PublicKey.curve.Sign(rand, priv.X, priv.Y, priv.D, hash)
}


ify(pub *PublicKey, hash []byte, r, s *big.Int) bool {
