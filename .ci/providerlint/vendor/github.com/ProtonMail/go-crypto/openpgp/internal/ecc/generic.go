...
GenericCurve(c elliptic.Curve) *genericCurve {
	return &genericCurve{
		Curve: c,
	}
}
...
*genericCurve) GetCurveName() string {
	return c.Curve.Params().Name
}


*genericCurve) MarshalBytePoint(point []byte) []byte {
	return point
}


*genericCurve) UnmarshalBytePoint(point []byte) []byte {
	return point
}


*genericCurve) MarshalIntegerPoint(x, y *big.Int) []byte {
	return elliptic.Marshal(c.Curve, x, y)
}


*genericCurve) UnmarshalIntegerPoint(point []byte) (x, y *big.Int) {
	return elliptic.Unmarshal(c.Curve, point)
}


*genericCurve) MarshalByteSecret(d []byte) []byte {
	return d
}


*genericCurve) UnmarshalByteSecret(d []byte) []byte {
	return d
}


*genericCurve) MarshalIntegerSecret(d *big.Int) []byte {
	return d.Bytes()
}


*genericCurve) UnmarshalIntegerSecret(d []byte) *big.Int {
	return new(big.Int).SetBytes(d)
}


*genericCurve) GenerateECDH(rand io.Reader) (point, secret []byte, err error) {
...
*genericCurve) GenerateECDSA(rand io.Reader) (x, y, secret *big.Int, err error) {
...
*genericCurve) Encaps(rand io.Reader, point []byte) (ephemeral, sharedSecret []byte, err error) {
...
*genericCurve) Decaps(ephemeral, secret []byte) (sharedSecret []byte, err error) {
...
*genericCurve) Sign(rand io.Reader, x, y, d *big.Int, hash []byte) (r, s *big.Int, err error) {
	priv := &ecdsa.PrivateKey{D: d, PublicKey: ecdsa.PublicKey{X: x, Y: y, Curve: c.Curve}}
	return ecdsa.Sign(rand, priv, hash)
}


*genericCurve) Verify(x, y *big.Int, hash []byte, r, s *big.Int) bool {
	pub := &ecdsa.PublicKey{X: x, Y: y, Curve: c.Curve}
	return ecdsa.Verify(pub, hash, r, s)
}


*genericCurve) validate(xP, yP *big.Int, secret []byte) error {
...
*genericCurve) ValidateECDSA(xP, yP *big.Int, secret []byte) error {
	return c.validate(xP, yP, secret)
}


*genericCurve) ValidateECDH(point []byte, secret []byte) error {
	xP, yP := elliptic.Unmarshal(c.Curve, point)
	if xP == nil {
		return errors.KeyInvalidError(fmt.Sprintf("ecc (%s): invalid point", c.Curve.Params().Name))
	}
