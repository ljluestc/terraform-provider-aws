...
*x448) MarshalBytePoint(point []byte) []byte {
...
*x448) UnmarshalBytePoint(point []byte) []byte {
...
*x448) MarshalByteSecret(d []byte) []byte {
...
*x448) UnmarshalByteSecret(d []byte) []byte {
...
*x448) generateKeyPairBytes(rand io.Reader) (sk, pk x448lib.Key, err error) {
...
*x448) GenerateECDH(rand io.Reader) (point []byte, secret []byte, err error) {
...
*x448) Encaps(rand io.Reader, point []byte) (ephemeral, sharedSecret []byte, err error) {
...
*x448) Decaps(ephemeral, secret []byte) (sharedSecret []byte, err error) {
...
*x448) ValidateECDH(point []byte, secret []byte) error {
