...
// Cache stores keys derived with s2k 
s from one passphrase
...
// If there is no hit, it derives the key with the s2k 
 from the passphrase,
// updates the cache, and returns the key.

*Cache) GetOrComputeDerivedKey(passphrase []byte, params *Params, expectedKeySize int) ([]byte, error) {
	key, found := (*c)[*params]
