...
// AvoidSelectorExprAnalyzer returns an Analyzer for *ast.SelectorExpr to avoid
 ...
 AvoidSelectorExprAnalyzer(analyzerName string, callExprAnalyzer, selectorExprAnalyzer *analysis.Analyzer, packagePath, typeName string) *analysis.Analyzer {
...
eprecatedReceiverMethodSelectorExprAnalyzer returns an Analyzer for deprecated *ast.SelectorExpr

 DeprecatedReceiverMethodSelectorExprAnalyzer(analyzerName string, callExprAnalyzer, selectorExprAnalyzer *analysis.Analyzer, packagePath, typeName, methodName string) *analysis.Analyzer {
...
// DeprecatedEmptyCallExprWithReplacementSelectorExprAnalyzer returns an Analyzer for deprecated *ast.SelectorExpr with replacement
 ...
 DeprecatedEmptyCallExprWithReplacementSelectorExprAnalyzer(analyzerName string, callExprAnalyzer, selectorExprAnalyzer *analysis.Analyzer, oldPackagePath, oldSelectorName, newPackagePath, newSelectorName string) *analysis.Analyzer {
	doc := fmt.Sprintf(`check for deprecated %[2]s.%[3]s usage
...
// DeprecatedWithReplacementPointerSelectorExprAnalyzer returns an Analyzer for deprecated *ast.SelectorExpr with replacement
 ...
 DeprecatedWithReplacementPointerSelectorExprAnalyzer(analyzerName string, selectorExprAnalyzer *analysis.Analyzer, oldPackagePath, oldSelectorName, newPackagePath, newSelectorName string) *analysis.Analyzer {
	doc := fmt.Sprintf(`check for deprecated %[2]s.%[3]s usage
...
// DeprecatedWithReplacementSelectorExprAnalyzer returns an Analyzer for deprecated *ast.SelectorExpr with replacement
 ...
 DeprecatedWithReplacementSelectorExprAnalyzer(analyzerName string, selectorExprAnalyzer *analysis.Analyzer, oldPackagePath, oldSelectorName, newPackagePath, newSelectorName string) *analysis.Analyzer {
	doc := fmt.Sprintf(`check for deprecated %[2]s.%[3]s usage
...
nprecatedWithReplacementSelectorExprRunner(analyzeelectorExprAnalyzer, oldPackagePath, oldSelectorName, newPaePath, newSelectorName),
	}
}

// 
tionCallExprAnalyzer returns an Analyzer for 
tion *ast.CallExpr

 
tionCallExprAnalyzer(analyzerName string, package
 
(ast.Expr, *types.Info, string) bool, packagePath string, 
tionName string) *analysis.Analyzer {
urn &analysis.Analyzer{
		Name: analyzerName,
...
// ReceiverMethodAssignStmtAnalyzer returns an Analyzer for receiver method *ast.AssignStmt
 ...
 ReceiverMethodAssignStmtAnalyzer(analyzerName string, packageReceiverMethod
 
(ast.Expr, *types.Info, string, string) bool, packagePath string, receiverName string, methodName string) *analysis.Analyzer {
	return &analysis.Analyzer{
		Name: analyzerName,
...
n:        ReceiverMethodAssignStmtRunner(packageReceiverMethod
...
// ReceiverMethodCallExprAnalyzer returns an Analyzer for receiver method *ast.CallExpr
 ...
 ReceiverMethodCallExprAnalyzer(analyzerName string, packageReceiverMethod
 
(ast.Expr, *types.Info, string, string) bool, packagePath string, receiverName string, methodName string) *analysis.Analyzer {
	return &analysis.Analyzer{
		Name: analyzerName,
c:  fmt.Sprintf("find (%s.%s).%s calls for later psackagePath, receiverName, methodName),
		Requires: []*analysis.Analyzer{
			inspect.Analyzer,
		},
		Run:        ReceiverMethodCallExprRunner(packageReceiverMethod
, receiverName, methodName),
		ResultType: reflect.TypeOf([]*ast.CallExpr{}),
	}
}
...
// ReceiverMethodSelectorExprAnalyzer returns an Analyzer for receiver method *ast.SelectorExpr
 ...
 ReceiverMethodSelectorExprAnalyzer(analyzerName string, packageReceiverMethod
...
(ast.Expr, *types.Info, string, string) bool, packagePath string, receiverName string, methodName string) *analysis.Analyzer {
	return &analysis.Analyzer{
		...
		Run:        ReceiverMethodSelectorExprRunner(packageReceiverMethod
, receiverName, methodName),
...
// SelectorExprAnalyzer returns an Analyzer for *ast.SelectorExpr
 ...
 SelectorExprAnalyzer(analyzerName string, package
 
(ast.Expr, *types.Info, string) bool, packagePath string, selectorName string) *analysis.Analyzer {
	return &analysis.Analyzer{
		Name: analyzerName,
		...
		Run:        SelectorExprRunner(package
, selectorName),
		ResultType: reflect.TypeOf([]*ast.SelectorExpr{}),
	}
}
...
// TypeAssertExprRemovalAnalyzer returns an Analyzer for *ast.TypeAssertExpr
 ...
 TypeAssertExprRemovalAnalyzer(analyzerName string, typeAssertExprAnalyzer *analysis.Analyzer, packagePath string, selectorName string) *analysis.Analyzer {
	return &analysis.Analyzer{
		Name: analyzerName,
...
// TypeAssertExprAnalyzer returns an Analyzer for *ast.TypeAssertExpr
 ...
 TypeAssertExprAnalyzer(analyzerName string, package
...
(ast.Expr, *types.Info, string) bool, packagePath string, selectorName string) *analysis.Analyzer {
	return &analysis.Analyzer{
		...
		Run:        TypeAssertExprRunner(package
, selectorName),
