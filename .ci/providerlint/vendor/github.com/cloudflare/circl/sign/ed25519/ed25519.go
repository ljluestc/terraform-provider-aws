...
//
// These 
s are compatible with the “Ed25519” 
 defined in
...
iv PrivateKey) Sign(
	...
	switch true {
	case scheme == ED25519 && opts.Hash
= crypto.Hash(0):
		return Sign(priv, message), nil
	case scheme == ED25519Ph && opts.Hash
= crypto.SHA512:
		return SignPh(priv, message, ctx), nil
	case scheme == ED25519Ctx && opts.Hash
= crypto.Hash(0) && len(ctx) > 0:
...
// This 
 supports the signature variant defined in RFC-8032: Ed25519,
// also known as the pure version of EdDSA.
// It will panic if len(privateKey) is not PrivateKeySize.

n(privateKey PrivateKey, message []byte) []byte {
...
// This 
 supports the signature variant defined in RFC-8032: Ed25519ph,
...
nPh(privateKey PrivateKey, message []byte, ctx string) []byte {
...
// This 
 supports the signature variant defined in RFC-8032: Ed25519ctx,
// meaning it accepts a non-empty context string.
// It will panic if len(privateKey) is not PrivateKeySize.
// Context must be passed to this 
, which length should be no more than
// ContextMaxSize=255 and cannot be empty.

nWithCtx(privateKey PrivateKey, message []byte, ctx string) []byte {
...
// This 
 supports all the three signature variants defined in RFC-8032,
...
ify(public PublicKey, message, signature []byte) bool {
...
ifyWithCtx(public PublicKey, message, signature []byte, ctx string) bool {
	if len(ctx) == 0 || len(ctx) > ContextMaxSize {
		return false
	}
