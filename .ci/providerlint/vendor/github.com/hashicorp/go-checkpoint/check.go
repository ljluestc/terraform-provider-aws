...
)var magicBytes = [4]byte{0x35, 0x77, 0x69, 0xFB}// CheckParams are the parameters for configuring a check request.
type CheckParams struct {
	...
	// a version check.
	Product string
	Version string	// Arch and OS are used to filter alerts potentially only to things
	// affecting a specific os/arch combination. If these aren't specified,
	// they'll be automatically filled in.
	Arch string
	OS   string	// Signature is some random signature that should be stored and used
	...
	Signature     string
	SignatureFile string	// CacheFile, if specified, will cache the result of a check. The
	...
	// permissions 0755.
	CacheFile     string
	CacheDuration time.Duration	// Force, if true, will force the check even if CHECKPOINT_DISABLE
...
type CheckAlert struct {
...
}// Check checks for alerts and new version information. Check(p *CheckParams) (*CheckResponse, error) {
	...
	v.Set("arch", p.Arch)
	v.Set("os", p.OS)
	v.Set("signature", signature)	u.Scheme = "https"
	u.Host = "checkpoint-api.hashicorp.com"
	u.Path = fmt.Sprintf("/v1/check/%s", p.Product)
	u.RawQuery = v.Encode()	req, err := http.NewRequest("GET", u.String(), nil)
	if err != nil {
		return nil, err
	}
	req.Header.Set("Accept", "application/json")
	req.Header.Set("User-Agent", "HashiCorp/go-checkpoint")	client := cleanhttp.DefaultClient()	// We use a short timeout since checking for new versions is not critical
	// enough to block on if checkpoint is broken/slow.
	client.Timeout = time.Duration(timeout) * time.Millisecond	resp, err := client.Do(req)
	if err != nil {
		return nil, err
	}
...
nterval. The returned channel may be closed to stop backgr checks. CheckInterval(p *CheckParams, interval time.Duration, cb 
(*CheckResponse, error)) chan struct{} {
	doneCh := make(chan struct{})	if disabled := os.Getenv("CHECKPOINT_DISABLE"); disabled != "" {
		re doneCh
	}	go 
() {
		for {
...
// the given interval. The expected value is the interval.domStagger(interval time.Duration) time.Duration {
...
Close()
 ...
 checkSignature(path string) (string, error) {
	...
	signature := fmt.Sprintf(
		"%x-%x-%x-%x-%x", b[0:4], b[4:6], b[6:8], b[8:10], b[10:])	// Make sure the directory holding our signature exists.
