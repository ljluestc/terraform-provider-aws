...
type ReportParams struct {
	...
	SignatureFile string `json:"-"`	StartTime     time.Time   `json:"start_time"`
	EndTime       time.Time   `json:"end_time"`
	Arch string      `json:"arch"`
	OS   string      `json:"os"`
	Payload       interface{} `json:"payload,omitempty"`
	Product       string      `json:"product"`
	RunIDstring      `json:"run_id"`
 ...
 (i *ReportParams) signature() string {
	...
	return signature
}eport sends telemetry information to checkpoint Report(ctx context.Context, r *ReportParams) error {
...
}// ReportRequest creates a request object for making a report ReportRequest(r *ReportParams) (*http.Request, error) {
	...
	req.Header.Set("User-Agent", "HashiCorp/go-checkpoint")	return req, nil
}
