...
// Each field is a reference to a 
 that can either be nil or can be
// set to an implementation of the corresponding operation. If an operation
// 
 is nil then it isn't supported for the given capsule type.
...
// 
 that can be optionally implemented for a capsule type. It will
...
// If CapsuleExtensionData is called on a non-capsule type then it will panic.
 ...
 Type) CapsuleExtensionData(key interface{}) interface{} {
	ops := ty.CapsuleOps()
