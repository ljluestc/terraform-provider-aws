...
// which is always supported) then the 
 may panic or produce undefined
// results.
type Conversion 
...
Conversion(in cty.Type, out cty.Type) Conversion {
...
ConversionUnsafe(in cty.Type, out cty.Type) Conversion {
...
vert(in cty.Value, want cty.Type) (cty.Value, error) {
...
fy(types []cty.Type) (cty.Type, []Conversion) {
...
fyUnsafe(types []cty.Type) (cty.Type, []Conversion) {
