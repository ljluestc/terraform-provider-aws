...
SinkAdapter(opts *LoggerOptions) SinkAdapter {
	l := newLogger(opts)
	if l.callerOffset > 0 {
...
Logger(opts *LoggerOptions) *intLogger {
	if opts == nil {
		opts = &LoggerOptions{}
	}
...
ntityHook(logger Logger) Logger {
	return logger
}
...
*intLogger) log(name string, level Level, msg string, args ...interface{}) {
...
mCallerPath(path string) string {
...
// logPlain is the non-JSON logging format 
 which writes directly
// to the underlying writer the logger was initialized with.
//
// If the logger was initialized with a color 
, it also handles
...
*intLogger) logPlain(t time.Time, name string, level Level, msg string, args ...interface{}) {
...
teIndent(w *writer, str string, indent string) {
	for {
...
teEscapedForOutput(w io.Writer, str string, escapeQuotes bool) {
...
*intLogger) renderSlice(v reflect.Value) string {
...
*intLogger) logJSON(t time.Time, name string, level Level, msg string, args ...interface{}) {
...
intLogger) jsonMapEntry(t time.Time, name string, level Level, msg string) map[string]interface{} {
...
*intLogger) With(args ...interface{}) Logger {
	var extra interface{}
...
*intLogger) Named(name string) Logger {
	sl := l.copy()
...
*intLogger) ResetNamed(name string) Logger {
	sl := l.copy()
...
*intLogger) ResetOutput(opts *LoggerOptions) error {
	if opts.Output == nil {
...
*intLogger) ResetOutputWithFlush(opts *LoggerOptions, flushable Flushable) error {
	if opts.Output == nil {
		return errors.New("given output is nil")
	}
	if flushable == nil {
...
*intLogger) SetLevel(level Level) {
	atomic.StoreInt32(l.level, int32(level))
}

// Returns the current level

*intLogger) GetLevel() Level {
...
*intLogger) StandardLogger(opts *StandardLoggerOptions) *log.Logger {
	if opts == nil {
...
*intLogger) StandardWriter(opts *StandardLoggerOptions) io.Writer {
	newLog := *l
	if l.callerOffset > 0 {
		// the stack is
...
*intLogger) Accept(name string, level Level, msg string, args ...interface{}) {
	i.log(name, level, msg, args...)
}

// ImpliedArgs returns the loggers implied args

*intLogger) ImpliedArgs() []interface{} {
	return i.implied
}

// Name returns the loggers name

*intLogger) Name() string {
	return i.name
}
