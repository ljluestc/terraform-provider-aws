...
// SPDX-License-Identifier: MPL-2.0package pluginimport (
	...
	"sync"
	"sync/atomic"
	"time"	"github.com/hashicorp/go-hclog"
...
var Killed uint32 = 0// This is a slice of the "managed" clients which are cleaned up when
// calling Cleanup
var managedClients = make([]*Client, 0, 5)
var managedClientsLock sync.Mutex// Error types
var (
	// ErrProcessNotFound is returned when a client is instantiated to
	// reattach to an existing process and it isn't found.
	ErrProcessNotFound = cmdrunner.ErrProcessNotFound	// ErrChecksumsDoNotMatch is returned when binary's checksum doesn't match
	// the one provided in the SecureConfig.
	ErrChecksumsDoNotMatch = errors.New("checksums did not match")	// ErrSecureNoChecksum is returned when an empty checksum is provided to the
	// SecureConfig.
	ErrSecureConfigNoChecksum = errors.New("no checksum provided")	// ErrSecureNoHash is returned when a nil Hash object is provided to the
	// SecureConfig.
	ErrSecureConfigNoHash = errors.New("no hash implementation provided")	// ErrSecureConfigAndReattach is returned when both Reattach and
...
// dispense a plugin type, use the `Client.Client` 
tion, and then
...
type Client struct {
	...
	protocol Protocol
	logger   hclog.Logger
	doneCtx  context.Contex
	ctxCancelcontext.Cancel	negotiatedVersion int	// clientWaitGroup is used to manage the lifecycle of the plugin management
	// goroutines.
	clientWaitGroup sync.WaitGroup	// stderrWaitGroup is used to prevent the command's Wait() 
tion from
	// being called before we've finished reading from the stderr pipe.
	stderrWaitGroup sync.WaitGroup	// processKilled is used for testing only, to flag when the process was
	// forcefully killed.
	processKilled bool	unixSocketCfg UnixSocketConfig
}// NegotiatedVersion returns the protocol version negotiated with the server.
// This is only valid after Start() is called. (c *Client) NegotiatedVersion() int {
	return c.negotiatedVersion
// ID returns a unique ID for the running plugin. By default this is the process
// ID (pid), but it could take other forms if Runner
 was provided. (c *Client) ID() string {
	c.l.Lock()
	defer c.l.Unlock()	if c.runner != nil {
		return c.runner.ID()
	}	return ""
...
type ClientConfig struct {
	...
	HandshakeConfig	// Plugins are the plugins that can be consumed.
	...
	VersionedPlugins map[int]PluginSet	// One of the following must be set, but not both.
	//
	...
	Cmd      *exec.Cmd
	Reattach *ReattachConfig	// Runner
 allows consumers to provide their own implementation of
...
(l hclog.Logger, cmd *exec.Cmd, tmpDir string) (runner.Runner, error)	// SecureConfig is configuration for verifying the integrity of the
	// executable. It can not be used with Reattach.
	SecureConfig *SecureConfig	// TLSConfig is used to enable TLS on the RPC client.
	TLSConfig *tls.Config	// Managed represents if the client should be managed by the
	// plugin package or not. If true, then by calling CleanupClients,
	// it will automatically be cleaned up. Otherwise, the client
	// user is fully responsible for making sure to Kill all plugin
	// clients. By default the client is _not_ managed.
	Managed bool	// The minimum and maximum port to use for communicating with
	// the subprocess. If not set, this defaults to 10,000 and 25,000
	// respectively.
	MinPort, MaxPort uint	// StartTimeout is the timeout to wait for the plugin to say it
	// has started successfully.
	StartTimeout time.Duration	// If non-nil, then the stderr of the client will be written to here
	// (as well as the log). This is the original os.Stderr of the subprocess.
	// This isn't the output of synced stderr.
	Stderr io.Writer	// SyncStdout, SyncStderr can be set to override the
	// respective os.Std* values in the plugin. Care should be taken to
	// avoid races here. If these are nil, then this will be set to
	// ioutil.Discard.
	SyncStdout io.Writer
	SyncStderr io.Writer	// AllowedProtocols is a list of allowed protocols. If this isn't set,
	...
	// new protocols.
	AllowedProtocols []Protocol	// Logger is the logger that the client will used. If none is provided,
	// it will default to hclog's default logger.
	Logger hclog.Logger	// AutoMTLS has the client and server automatically negotiate mTLS for
	...
	AutoMTLS bool	// GRPCDialOptions allows plugin users to pass custom grpc.DialOption
	// to create gRPC connections. This only affects plugins using the gRPC
	// protocol.
	GRPCDialOptions []grpc.DialOption	// SkipHostEnv allows plugins to run without inheriting the parent process'
	// environment variables.
	SkipHostEnv bool	// UnixSocketConfig configures additional options for any Unix sockets
...
this group, and set them as group-writable. Can be a name or gid. The
	...
	Group string	// The directory to create Unix sockets in. Internally managed by go-plugin
	// and deleted when the plugin is killed.
	directory string
}
 unixSocketConfigFromEnv() UnixSocketConfig {
	return UnixSocketConfig{
		Group:     os.Getenv(EnvUnixSocketGroup),
		directory: os.Getenv(EnvUnixSocketDir),
	}
}// ReattachConfig is used to configure a client to reattach to an
// already-running plugin process. can retrieve this information by
// callinattachConfig on nt.
type ReattachConfig struct {
 ...
 allows consumers to provide their own implementation of
	...
	// At least one of Pid or Reattach
 must be set.
	Reattach
 runner.Reattach
...
he file.  The configuration is verified by the client by calling the
// SecureConfig.Check() 
tion.
//
...
// the file matches the checksum provided in the SecureConfig. (s *SecureConfig) Check(filePath string) (bool, error) {
	if len(s.Checksum) == 0 {
	...
	defer file.Close()	_, err = io.Copy(s.Hash, file)
	if err != nil {
		return false, err
	}	sum := s.Hash.Sum(nil)	return subtle.ConstantTimeCompare(sum, s.Checksum) == 1, nil
}// Thakes sure all the managed subprocesses are killed and properly
// logged. This should be called before the parent process running the
// plugins exits.
//
// This must only be called _once_. CleanupClients() {
	// Set the killed to true so that we don't get unexpected panics
	atomic.StoreUint32(&Killed, 1)	// Kill all the managed clients in parallel and use a WaitGroup
...
(client *Client) {
	...
	managedClientsLock.Unlock()	wg.Wait()
...
// be properly cleaned. NewClient(config *ClientConfig) (c *Client) {
...
//
// Subsequent calls to this will return the same client. (c *Client) Client() (ClientProtocol, error) {
	_, err := c.Start()
	if err != nil {
		return nil, err
	}	c.l.Lock()
	defer c.l.Unlock()	if c.client != nil {
		return c.client, nil
	}tch c.protocol {
	case ProtocolNetRPC:
		c.client, err = newRPCClient(c)	case ProtocolGRPC:
		c.client, err = newGRPCClient(c.doneCtx, c)	default:
turn nil, fmt.Errorf("unknown server protocol: %s", c.protocol)
	}	if err != nil {
		c.client = nil
		return nil, err
	}	return c.client, nil
}// Tells whether or not the underlying process has exited. (c *Client) Exited() bool {
	c.l.Lock()
	defer c.l.Unlock()
	return c.exited
}// killed is used in tests to check if a process failed to exit gracefully, and
// needed to be killed. (c *Client) killed() bool {
	c.l.Lock()
	defer c.l.Unlock()
	returnrocessKilled
...
//
// This method can safely be called multiple times. (c *Client) Kill() {
	// Grab a lock to read some private fields.
	c.l.Lock()
	runner := c.runner
	addr := c.address
	hostSocketDir := c.unixSocketCfg.directory
	c.l.Unlock()	// If there is no runner or ID, there is nothing to kill.
	if runner == nil || runner.ID() == "" {
		return
	}	defer 
() {
		// Wait for the all client goroutines to finish.
		c.clientWaitGroup.Wait()		if hostSocketDir != "" {
	...
	if addr != nil {
		...
		client, err := c.Client()
		if err == nil {
			err = client.Close()			// If there is no error, then we attempt to wait for a graceful
...
// a port for RPC cotions, and returning the address to connect via RPC.
//
// This method is safe to call multiple times. Subsequent calls have no effect.
// Once a client has been started once, it cannot be started again, even if
// it was killed. (c *Client) Start() (addr net.Addr, err error) {
	c.l.Lock()
	defer c.l.Unlock()	if c.address != nil {
 ...
 != nil {
		...
		if mutuallyExclusiveOptions != 1 {
			return nil, fmt.Errorf("exactly one of Cmd, or Reattach, or Runner
 must be set")
		...
		c.config.VersionedPlugins = make(map[int]PlSet)
	}	// handle all plugins as versioned, using the handshake config as the default.
	version := int(c.config.ProtocolVersion)	// Make sure we're not overwriting a real version 0. If ProtocolVersion was
	...
	if cmd == nil {
		// It's only possible to get here if Runner
 is non-nil, but we'll
	...
	cmd.Stdin = os.Stdin	if c.config.SecureConfig != nil {
	...
	if c.config.AutoMTLS {
		...
		if err != nil {
			...
			return nil, err
		}		cmd.Env = append(cmd.Env, fmt.Sprintf("PLUGIN_CLIENT_CERT=%s", certPEM))		c.config.TLSConfig = &tls.Config{
 ...
 != nil:
		...
		if c.unixSocketCfg.Group != "" {
			err tGroupWritable(c.unixSocketCfg.directory, c.unixSocketCfg.Group, 0o770)
			if err != nil {
				return nil, err
			}
		}
		cmd.Env = append(cmd.Env, fmt.Sprintf("%s=%s", EnvUnixSocketDir, c.unixSocketCfg.directory))
		c.logger.Trace("created temporary directory for unix sockets", "dir", c.unixSocketCfg.directory)		runner, err = c.config.Runner
(c.logger, cmd, c.unixSocketCfg.directory)
		if err != nil {
			return nil, err
		}
	default:
	...
	stax, startCtxCancel := context.WithTimeout(context.Background(), c.config.StartTimeout)
	...
	defer 
() {
		rErr := recover()		if err != nil || rErr != nil {
			runner.Kill(context.Background())
		}		if rErr != nil {
			panic(rErr)
		}
	}()	// Create a context for when we kill
	c.doneCtx, c.ctxCancel = context.WithCancel(context.Background())	// Start goroutine that logs the stderr
	c.clientWaitGroup.Add(1)
	c.stderrWaitGroup.Add(1)
	// logStderr calls Done()
	go c.logStderr(runner.Name(), runner.Stderr())	c.clientWaitGroup.Add(1)
	go 
() {
		// ensure the context is cancelled when we're done
		defer c.ctxCancel()		defer c.clientWaitGroup.Done()		// wait to finish reading from stderr since the stderr pipe reader
		// will be closed by the subsequent call to cmd.Wait().
		c.stderrWaitGroup.Wait()		// Wait for the command to end.
		...
		}		os.Stderr.Sync()		// Set that we exited, which takes a lock
		c.l.Lock()
		defer c.l.Unlock()
		c.exited = true
...
() {
		...
		defer close(linesCh)		scanner := bufio.NewScanner(runner.Stdout())
		for scanner.Scan() {
...
() {
	...
	timeout := time.After(c.config.StartTimeout)	// Start looking for the address
	...
	case line, ok := <-linesCh:
		...
		c.logger.Debug("using plugin", "version", version)		network, address, err := runner.PluginToHost(parts[2], parts[3])
		if err != nil {
			return addr, err
		}		switch network {
		case "tcp":
...
protocol = ProtocolNetRPC
 ...
 data returned from some older implementations.
		if len(s) >= 6 && len(parts[5]) > 50 
			err := c.loadServerCert(parts[5])
			if err != {
				return, fmt.Errorf("error png server cert: %s", err)
			}
		}
	}	c.address = addr
	return
}// loadServerCert is used by AutoMTLS to read an x.509 cert returned by the
// server, and load it as the RootCA and ClientCA for the client TLSConfig. (c *Client) loadServerCert(cert string) error {
	cerl := x509.NewCertPool()	asn1, err := base64.RawStdEncoding.DecodeString(cert)
	if err != nil {
		return err
	}	x509Cert, err := x509.ParseCertificate([]byte(asn1))
	if err != nil {
		return err
	}	certPool.AddCert(x509Cert)	c.config.TLSConfig.RootCAs = certPool
	c.config.TLSConfig.ClientCAs = certPool
	return nil
}
 (c *Client) reattach() (net.Addr, error) {
	reattach
 := c.config.Reattach.Reattach	// For backwards compatibility default to cmdrunner.Reattach	if reattach
 == nil {
		reattach
 = cmdrunner.Reattach
(c.config.Reattach.Pid, c.config.Reattach.Addr)
	}	r, err := reattach
()
	if err != nil {
		return nil, err
	}	// Create a context for when we kill
	c.doneCtx, c.ctxCancel = context.WithCancel(context.Background())	c.clientWaitGroup.Add(1)
Goroutine to mark exit status
	go 
(r runner.AttachedRunner) {
		defer c.clientWaitGroup.Done()		// ensure the context is cancelled when we're done
		defer c.ctxCancel()		// Wait for the process to die
		r.Wait(context.Background())		// Log so we can see it
		c.logger.Debug("reattached plugin process exited")		// Mark it
		c.l.Lock()
		defer c.l.Unlock()
		c.exited = true
	}(r)	// Set the address and protocol
	...
	}	// If we'r test mode, we do NOT set the process. This avoids the
...
// version, or an invalid handshake response. (c *Client) checkProtoVersion(protoVersion string) (int, PluginSet, error) {
	...
	var clientVersions []int	// all versions, including the legacy ProtocolVersion have been added to
	// the versions set
	for version, plugins := range c.config.VersionedPlugins {
		clientVersions = append(clientVersions, version)		if serverVersion != version {
			continue
		}
		return version, plugins, nil
	}	return 0, nil, fmt.Errorf("Incompatible API version with plugin. "+
lugin version: %d, Client versions: %d", serverVersion, clientVersions)
...
// call Start or Client before calling this.// Clienho specified a Runner
 will need to populate their own
// Reattach
 in the returned ReattachConfig before it can be used. (c *Client) ReattachConfig() *ReattachConfig {
	c.l.Lock()
	defer c.l.Unlock()	if c.address == nil {
		return nil
...
}rotocol returns the protocol of server on the remote end. This will
...
// no errors occur. (c *Client) Protocol() Protocol {
	_, err := c.Start()
	if err != nil {
		return ProtocolInvalid
	}	return c.protocol
}
 netAddrDialer(addr net.Addr) 
(string, time.Duration) (net.Conn, error) {
	return 
(_ string, _ time.Duration) (net.Conn, error) {
...
// to the plugin. (c *Client) dialer(_ string, timeout time.Duration) (net.Conn, error) {
 ...
 (c *Client) logStderr(name string, r io.Reader) {
	defer c.clientWaitGroup.Done()
	defer c.stderrWaitGroup.Done()
	l := c.logger.Named(filepath.Base(name))	reader := bufio.NewReaderSize(r, stdErrBufferSize)
	// continuation indicates the previous line was a prefix
	continuation := false	for {
		...
		case err != nil:
			...
			return
		}		c.config.Stderr.Write(line)		// The line was longer than our max token size, so it's likely
		// incomplete and won't unmarshal.
		if isPrefix || continuation {
			l.Debug(string(line))			// if we're finishing a continued line, add the newline back in
			if !isPrefix {
		...
		if err != nil {
			...
			out := flattenKVPairs(entry.KVPairs)			out = append(out, "timestamp", entry.Timestamp.Format(hclog.TimeFormat))
