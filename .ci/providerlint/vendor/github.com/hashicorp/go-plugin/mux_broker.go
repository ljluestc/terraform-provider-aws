...
// SPDX-License-Identifier: MPL-2.0package pluginimport (
	...
	"sync"
	"sync/atomic"
	"time"	"github.com/hashicorp/yamux"
...
type MuxBroker struct {
	...
	streams map[uint32]*muxBrokerPending	sync.Mutex
...
//
his should not be called multiple times with the same ID at one time. (m *MuxBroker) Accept(id uint32) (net.Conn, error) {
	var c net.Conn
	p := m.getStream(id)
	select {
	case c = <-p.ch:
	...
	case <-time.After(5 * time.Second):
		m.Lock()
		defer m.Unlock()
		delete(m.streams, id)		return nil, fmt.Errorf("timeout waiting for accept")
	}	// Ack our connection
...
// complex arguments.// The served interface is always registered to the "Plugin" name. (m *MuxBroker) AcceptAndServe(id uint32, v interface{}) {
	conn, err := m.Accept(id)
	if err != nil {
		log.Printf("[ERR] plugin: plugin acceptAndServe error: %s", err)
		return
	}	serve(conn, "Plugin", v)
// Close closes the connection and all sub-connections. (m *MuxBroker) Close() error {
urn m.session.Close()
}// Dial opens a connection by ID. (m *MuxBroker) Dial(id uint32) (net.Conn, error) {
	// Open the stream
...
// the plugin host/client. (m *MuxBroker) Run() {
 ...
 (m *MuxBroker) timeoutWait(id uint32, p *muxBrokerPending) {
	...
	delete(m.streams, id)	// If we timed out, then check if we have a channel in the buffer,
	// and if so, close it.
	if timeout {
		select {
		case s := <-p.ch:
