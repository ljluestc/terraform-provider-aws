...
"io"
"net"
"net/rpc""github.com/hashicorp/yamux"
)// RPCClient connects to an RPCServer over net/rpc to dispense plugin types.
type RPCClient struct {
broker  *MuxBroker
control *rpc.Client
plugins map[string]Plugin// These are the streams used for the various stdout/err overrides
stdout, stderr net.Conn
}// newRPCClient creates a new RPCClient. The Client argument is expected
// to be successfully started already with a lock held. newRPCClient(c *Client) (*RPCClient, error) {
...
ial is typically used instead. NewRPCClient(conn io.ReadWriteCloser, plugins map[string]Plugin) (*RPCClient, error) {
...
go broker.Run()// Build the client using our broker and control channel.
return &RPCClient{
broker:  broker,
...
// in the background. You do not need to launch this in a goroutine itself.// This should never be called multiple times. (c *RPCClient) SyncStreams(stdout io.Writer, stderr io.Writer) error {
...
// is called. (c *RPCClient) Close() error {
...
var empty struct{}
returnErr := c.control.Call("Control.Quit", true, &empty)// Close the other streams we have
...
// that the connection to the plugin is not healthy. (c *RPCClient) Ping() error {
var empty struct{}
