...
"net"
"net/rpc"
"sync""github.com/hashicorp/yamux"
...
Plugins map[string]Plugin// Stdout, Stderr are what this server will use instead of the
...
Stdout io.Reader
Stderr io.Reader// DoneCh should be set to a non-nil channel that will be closed
// when the control requests the RPC server to end.
DoneCh chan<- struct{}lock sync.Mutex
}// ServerProtocol impl. (s *RPCServer) Init() error { return nil }erverProtocol impl. (s *RPCServer) Config() string { return "" }// ServerProtocol impl. (s *RPCServer) Serve(lis net.Listener) {
defer s.done()for {
conn, err := lis.Accept()
if err != nil {
severity := "ERR"
...
//
// ServeConn blocks, serving the connection until the client hangs up. (s *RPCServer) ServeConn(conn io.ReadWriteCloser) {
...
go copyStream("stderr", stdstream[1], s.Stderr)// Create the broker and start it up
broker := newMuxBroker(mux)
go broker.Run()// Use the control connection to build the dispenser and serve the
// connection.
...
// exit. (s *RPCServer) done() {
s.lock.Lock()
defer s.lock.Unlock()if s.DoneCh != nil {
close(s.DoneCh)
s.DoneCh = nil
}
...
// is still alive to a plugin. (c *controlServer) Ping(
...
// End the server
c.server.done()// Always return true
*response = struct{}{}return nil
}// dispenseServer dispenses variousinterface implementations for Terraform.
...
id := d.broker.NextId()
*response = id// Run the rest in a goroutine since it can only happen once this RPC
