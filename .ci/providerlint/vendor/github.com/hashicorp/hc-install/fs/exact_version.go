...
type ExactVersion struct {
 ...
 (*ExactVersion) IsSourceImpl() src.InstallSrcSigil {
...
urn ev.logger
 ...
 (ev *Exersion) Find(ctx context.Context) (string, error) {
	timeout := dltTimeout
	if ev.Timeout > 0 {
		timeout = ev.Timeout
	}
...
()
	...
	execPath, err := findFile(lookupDirs(ev.ExtraPaths), ev.Product.BinaryName(), 
(file string) error {
