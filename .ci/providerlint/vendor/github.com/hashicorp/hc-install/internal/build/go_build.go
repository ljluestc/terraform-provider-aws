...
// SPDX-License-Identifier: MPL-2.0package buildimport (
	...
	"os"
	"os/exec"
	"path/filepath"	"github.com/hashicorp/go-version"
	"golang.org/x/mod/modfile"
)var discardLogger = log.New(ioutil.Discard, "", 0)// GoBuild represents a Go builder (to run "go build")
type GoBuild struct {
	Version*version.Version
	DetectVendoring bool	pathToRemove string
	logger       *log.Logger
}
 ...
 (gb *GoBuild) log() *log.Logger {
	...
	return gb.logger
}// Build runs "go build" within a given repo to produce binaryName in targetDir (gb *GoBuild) Build(ctx context.Context, repoDir, targetDir, binaryName string) (string, error) {
	reqGo, err := gb.ensureRequiredGoVersion(ctx, repoDir)
	if err != nil {
		return "", err
	}
	defer reqGo.Cleanup
(ctx)	if reqGo.Version == nil {
	...
	}	buildArgs := []string{"build", "-o", filepath.Join(targetDir, binaryName)}	if gb.DetectVendoring {
	...
	}	binPath := filepath.Join(targetDir, binaryName)	gb.pathToRemove = binPath	return binPath, nil
}
...
string
	Cleanup
 Cleanup	Version     *version.Version
}
 (gb *GoBuild) ensureRequiredGoVersion(ctx context.Context, repoDir string) (Go, error) {
	cmdName := "go"
	noopCleanup
 := 
(contexntext) {}	var installedVersion *version.Version	if gb.Version != nil {
