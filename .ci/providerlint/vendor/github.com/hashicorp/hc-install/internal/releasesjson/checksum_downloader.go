...
// SPDX-License-Identifier: MPL-2.0package releasesjsonimport (
	"context"
	"crypto/sha256"
	"encoding/hex"
	"fmt"
	...
	"net/http"
	"net/url"
	"strings"	"github.com/ProtonMail/go-crypto/openpgp"
	"github.com/hashicorp/hc-install/internal/httpclient"
)type ChecksumDownloader struct {
	ProductVersion   *ProductVersion
	Logger  *log.Logger
	ArmoredPublicKey string	BaseURL string
}type ChecksumFileMap map[string]HashSumtype HashSum []byte
 (hs HashSum) Size() int {
	return len(hs)
}
 (hs HashSum) String() string {
	return hex.EncodeToString(hs) HashSumFromHexDigest(hexDigest string) (HashSum, error) {
...
urn HashSum(sumBytes), nil
 ...
 (cd *ChecksumDownloader) DownloadAndVerifyChecksums(ctx context.Context) (ChecksumFileMap, error) {
	...
	cd.Logger.Printf("downloading signature from %s", sigURL)	req, err := http.NewRequestWithContext(ctx, http.MethodGet, sigURL, nil)
	if err != nil {
	...
	}	defer sigResp.Body.Close()	shasumsURL := fmt.Sprintf("%s/%s/%s/%s", cd.BaseURL,
		url.PathEscape(cd.ProductVersion.Name),
		url.PathEscape(cd.ProductVersion.RawVersion),
		url.PathEscape(cd.ProductVersion.SHASUMS))
	cd.Logger.Printf("downloading checksums from %s", shasumsURL)	req, err = http.NewRequestWithContext(ctx, http.MethodGet, shasumsURL, nil)
	if err != nil {
	...
	}	defer sumsResp.Body.Close()	var shaSums strings.Builder
	sumsReader := io.TeeReader(sumsResp.Body, &shaSums)	err = cd.verifySumsSignature(sumsReader, sigResp.Body)
	if err != nil {
		return nil, err
	}	return fileMapFromChecksums(shaSums)
}
 fileMapFromChecksums(checksums strings.Builder) (ChecksumFileMap, error) {
	csMap := make(ChecksumFileMap, 0)	lines := strings.Split(checksums.String(), "\n")
 ...
 (cd *ChecksumDownloader) verifySumsSignature(checksums, signature io.Reader) error {
	...
	}	cd.Logger.Printf("checksum signature is valid")	return nil
}
