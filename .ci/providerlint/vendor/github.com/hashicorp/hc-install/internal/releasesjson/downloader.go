...
// SPDX-License-Identifier: MPL-2.0package releasesjsonimport (
	...
	"path/filepath"
	"runtime"
	"strings"	"github.com/hashicorp/hc-install/internal/httpclient"
 ...
 (d *Downloader) DownloadAndUnpack(ctx context.Context, pv *ProductVersion, binDir string, licenseDir string) (zipFilePath string, err error) {
	...
	}	client := httpclient.NewHTTPClient()	archiveURL := pb.URL
	if d.BaseURL != "" {
		...
		u.Host = baseURL.Host
		archiveURL = u.String()
	}	d.Logger.Printf("downloading archive from %s", archiveURL)	req, err := http.NewRequestWithContext(ctx, http.MethodGet, archiveURL, nil)
	if err != nil {
	...
	}	defer resp.Body.Close()	pkgReader := resp.Body	contentType := resp.Header.Get("content-type")
	if !contentTypeIsZip(contentType) {
		return "", fmt.Errorf("unexpected content-type: %s (expected any of %q)",
			contentType, zipMimeTypes)
	}	expectedSize := resp.ContentLength	pkgFile, err := ioutil.TempFile("", pb.Filename)
	if err != nil {
		return "", err
	}
	defer pkgFile.Close()
	pkgFilePath, err := filepath.Abs(pkgFile.Name())	d.Logger.Printf("copying %q (%d bytes) to %s", pb.Filename, expectedSize, pkgFile.Name())	var bytesCopied int64
	if d.VerifyChecksum {
		d.Logger.Printf("verifying checksum of %q", pb.Filename)
		h := sha256.New()
		r := io.TeeReader(resp.Body, pkgFile)		bytesCopied, err = io.Copy(h, r)
		if err != nil {
			return "", err
	...
	}	d.Logger.Printf("copied %d bytes to %s", bytesCopied, pkgFile.Name())	if expectedSize != 0 && bytesCopied != int64(expectedSize) {
		return pkgFilePath, fmt.Errorf(
	...
	defer r.Close()	for _, f := range r.File {
...
var licenseFiles = []string{
	"EULA.txt",
	"TermsOfEvaluation.txt", isLicenseFile(filename string) bool {
