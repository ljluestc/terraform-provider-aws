...
)var consulVersionOutputRe = regexp.MustCompile(`Consul ` + simpleVersionRe)var (
...
(ctx context.Context, path string) (*version.Version, error) {
		...
		if err != nil {
			return nil, err
		}		stdout := strings.TrimSpace(string(out))		submatches := consulVersionOutputRe.FindStringSubmatch(stdout)
		if len(submatches) != 2 {
