...
)var nomadVersionOutputRe = regexp.MustCompile(`Nomad ` + simpleVersionRe)var Nomad = Product{
	Name: "nomad",
	BinaryName: 
() string {
...
(ctx context.Context, path string) (*version.Version, error) {
		...
		if err != nil {
			return nil, err
		}		stdout := strings.TrimSpace(string(out))		submatches := nomadVersionOutputRe.FindStringSubmatch(stdout)
		if len(submatches) != 2 {
