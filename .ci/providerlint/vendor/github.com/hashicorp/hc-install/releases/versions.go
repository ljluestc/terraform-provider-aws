...
type Versions struct {
	...
	Enterprise  *EnterpriseOptions // require enterprise version if set (leave nil for OSS)	ListTimeout time.Duration	// Install represents configuration for installation of any listed version
	Install InstallationOptions
}type InstallationOptions struct {
	Timeout time.Duration
	Dir     string	SkipChecksumVerification bool	// ArmoredPublicKey is a public PGP key in ASCII/armor format to use
...
()	r := rjson.NewReleases()
	...
	sort.Stable(versions)	expectedMetadata := enterpriseVersionMetadata(v.Enterprise)	installables := make([]src.Source, 0)
	for _, pv := range versions {
		if !v.Constraints.Check(pv.Version) {
			...
			Timeout:    v.Install.Timeout,			ArmoredPublicKey:v.Install.ArmoredPublicKey,
