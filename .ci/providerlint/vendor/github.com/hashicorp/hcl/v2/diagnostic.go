...
type DiagnosticSeverity intconst (
	// DiagInvalid is the invalid zero value of DiagnosticSeverity
	DiagInvalid DiagnosticSeverity = iota	// DiagError indicates that the problem reported by a diagnostic prevents
	// further progress in parsing and/or evaluating the subject.
	DiagError	// DiagWarning indicates that the problem reported by a diagnostic warrants
...
type Diagnostic struct {
	Severity DiagnosticSeverity	// Summary and Detail contain the English-language description of the
	// problem. Summary is a terse description of the general problem and
	// detail is a more elaborate, often-multi-sentence description of
	// the problem and what might be done to solve it.
	Summary string
	Detail  string	// Subject and Context are both source ranges relating to the diagnostic.
	//
	...
	Subject *Range
	Context *Range	// For diagnostics that occur when evaluating an expression, Expression
...
tions the caller must walk up this
	...
	EvalContext *EvalContext	// Extra is an extension point for additional machine-readable information
	// about this problem.
	//
...
type Diagnostics []*Diagnostic// error implementation, so that diagnostics can be returned via APIs
// that normally deal in vanilla Go errors.
//
his presents only minimal context about the error, for compatibility
// with usual expectations about how errors will present as strings. (d *Diagnostic) Error() string {
...
// APIs that normally deal in vanilla Go errors. (d Diagnostics) Error() string {
	count := len(d)
	switch {
	case count == 0:
...
// to extend a diagnostics created by a different subsystem. (d Diagnostics) Append(diag *Diagnostic) Diagnostics {
...
// all the same caveats and constraints. (d Diagnostics) Extend(diags Diagnostics) Diagnostics {
	return append(d, diags...)
}// HasErrors returns true if the receiver contains any diagnostics of
// severity DiagError. (d Diagnostics) HasErrors() bool {
 _, diag := range d {
