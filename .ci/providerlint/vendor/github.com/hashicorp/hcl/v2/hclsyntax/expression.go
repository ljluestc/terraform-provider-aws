...
ExpandFinal boolNameRange       hcl.Range
...
var diags hcl.Diagnosticsvar f 
tion.
tion
exists := false
hasNonNilMap := false
...
diagExtra.
tionCallErroerrswitch terr := err.(type) {
case 
tion.ArgError:
i := terr.Index
var param *
...
FalseResult ExpressionSrcRange hcl.Range
}
...
var diags hcl.DiagnosticsresultType := cty.DynamicPseudoType
...
Key        ExpressionSrcRange     hcl.Range
OpenRange    hcl.Range
...
Exprs []ExpressionSrcRange  hcl.Range
OpenRange hcl.Range
}
...
var vals []cty.Value
var diags hcl.Diagnosticsvals = make([]cty.Value, len(e.Exprs))
...
Items []ObjectConsItemSrcRange  hcl.Range
OpenRange hcl.Range
...
known := truevals = make(map[string]cty.Value, len(e.Items))
for _, item := range e.Items {
...
ValVar stringCollExpr ExpressionKeyExpr  Expression // nil when producing a tuple
ValExpr  Expression
CondExpr Expression // null if no "if" clause is presentGroup bool // set if the ellipsis is used on the value in an object forSrcRange   hcl.Range
OpenRange  hcl.Range
CloseRange hcl.Range
}
 (e *ForExpr) Value(ctx *hcl.EvalContext) (cty.Value, hcl.Diagnostics) {
var diags hcl.Diagnostics
var marks []cty.ValueMarkscollVal, collDiags := e.CollExpr.Value(ctx)
...
childCtx.Variables[e.ValVar] = cty.DynamicValresult, condDiags := e.CondExpr.Value(childCtx)
diags = append(diags, condDiags...)
if result.IsNull() {
diags = append(diags, &hcl.Diagnostic{
Severity:    hcl.DiagError,
...
childCtx.Variables[e.ValVar] = vif e.CondExpr != nil {
...
Source Expression
Each   Expression
Item   *AnonSymbolExprSrcRange    hcl.Range
...
e.Item.clearValue(ctx) // clean up our temporary valueif upgradedUnknown {
return cty.DynamicVal, diags
}if !isKnown {
