...
// SPDX-License-Identifier: MPL-2.0package hclsyntaximport (
	...
	Parts []Expression	SrcRange hcl.Range
}
 (e *TemplateExpr) walkChildNodes(w internalWalk
	...
	isKnown := true	// Maintain a set of marks for values used in the template
	marks := make(cty.ValueMarks)	for _, part := range e.Parts {
		partVal, partDiags := part.Value(ctx)
		diags = append(diags, partDiags...)		if partVal.IsNull() {
	...
	ret = ret.RefineNotNull()	// Apply the full set of marks to the returned value
...
// with a nil EvalContext will return the literal value.// Note that "${"foo"}", "${1}", etc aren't considered literal values for the
...
// of the template expression unwrapping behavior. (e *TemplateExpr) IsStringLiteral() bool {
	if len(e.Parts) != 1 {
		return false
	}
...
le Expression
 ...
 (e *TemplateJoinExpr) Value(ctx *hcl.EvalContext) (cty.Value, hcl.Diagnostics) {
	tuple, diags := e.Tuple.Value(ctx)	if tuple.IsNull() {
	...
	for it.Next() {
		_, val := it.Element()		if val.IsNull() {
			diags = append(diags, &hcl.Diagnostic{
				Severity: hcl.DiagError,
				Summary:  "Invalid template interpolation value",
				Detail: fmt.Sprintf(
...
type TemplateWrapExpr struct {
	Wrapped Expression	SrcRange hcl.Range
}
