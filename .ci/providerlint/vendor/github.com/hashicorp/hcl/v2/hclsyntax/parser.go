...
attrs := Attributes{}
blocks := Blocks{}
var diags hcl.DiagnosticsstartRange := p.PrevRange()
...
endRange = p.PrevRange() // arbitrary, but somewhere inside the body means better diagnosticsp.recover(end) // attempt to recover to the token after the end of this body
break Token
}
}return &Body{
Attributes: attrs,
...
var diags hcl.Diagnosticsnext := p.Peek()switch next.Type {
case TokenEqual:
...
}var endRange hcl.Rangeexpr, diags := p.ParseExpression()
...
var labels []string
var labelRanges []hcl.Rangevar oBrace TokenToken:
for {
tok := p.Peek()switch tok.Type {case TokenOBrace:
oBrace = p.Read()
break Tokencase TokenOQuote:
...
var diags hcl.DiagnosticscondExpr, condDiags := p.parseBinaryOps(binaryOps)
...
return condExpr, diags
}p.Read() // eat question marktrueExpr, trueDiags := p.ParseExpression()
...
return condExpr, diags
}p.Read() // eat colonfalseExpr, falseDiags := p.ParseExpression()
...
case TokenOParen:
oParen := p.Read() // eat open parenp.PushIncludeNewlines(false)expr, diags := p.ParseExpression()
if diags.HasErrors() {
...
}return expr, diagscase TokenNumberLit:
tok := p.Read() // eat number tokennumVal, diags := p.numberLitValue(tok)
return &LiteralValueExpr{
Val:      numVal,
SrcRange: tok.Range,
}, diagscase TokenIdent:
tok := p.Read() // eat identifier tokenif p.Peek().Type == TokenOParen {
...
expandFinal = trueif p.Peek().Type != TokenCParen {
if !p.recovery {
diags = append(diags, &hcl.Diagnostic{
Severity: hcl.DiagError,
...
}var close Tokenvar diags hcl.Diagnostics
var exprs []Expressionfor {
next := p.Peek()
...
defer p.PopIncludeNewlines()var close Tokenvar diags hcl.Diagnostics
var items []ObjectConsItemfor {
next := p.Peek()
...
close = p.recover(TokenCBrace)
break
}p.Read() // eat equals sign or colonvalue, valueDiags := p.ParseExpression()
...
var keyName, valName stringif p.Peek().Type != TokenIdent {
if !p.recovery {
diags = append(diags, &hcl.Diagnostic{
Severity: hcl.DiagError,
...
keyName = valName
p.Read() // eat commaif p.Peek().Type != TokenIdent {
if !p.recovery {
diags = append(diags, &hcl.Diagnostic{
Severity: hcl.DiagError,
Summary:  "Invalid 'for' expression",
...
p.Read() // eat 'in' keywordcollExpr, collDiags := p.ParseExpression()
...
p.Read() // eat colonvar keyExpr, valExpr Expression
...
keyExpr, keyDiags = valExpr, valDiagsvalExpr, valDiags = p.ParseExpression()
}
diags = append(diags, keyDiags...)
...
endRange = tok.Range
break Tokencase TokenQuotedLit:
...
ret.WriteString("{ ... }")p.recover(TokenTemplateSeqEnd) // we'll try to keep parsing after the sequence endscase TokenEOF:
diags = append(diags, &hcl.Diagnostic{
Severity: hcl.DiagError,
...
var diags hcl.Diagnosticsret := make([]byte, 0, len(tok.Bytes))
...
rb := ret[len(ret)-l:]
utf8.EncodeRune(rb, r)continue Slicesdefault:
diags = append(diags, &hcl.Diagnostic{
Severity: hcl.DiagError,
Summary:  "Invalid escape sequence",
...
break Tokencase TokenOBrace, TokenOBrack, TokenOParen, TokenOQuote, TokenOHeredoc, TokenTemplateInterp, TokenTemplateControl:
...
return TokenCQuote
case TokenOHeredoc:
return TokenCHeredoccase TokenCBrace:
return TokenOBrace
case TokenCBrack:
return TokenOBrack
case TokenCParen:
...
return TokenOQuote
case TokenCHeredoc:
return TokenOHeredoccase TokenTemplateControl:
...
return TokenTemplateInterpdefault:
return TokenNil
}
}
