...
}, nilcase *templateInterpToken:
p.Read() // eat interp
return tok.Expr, nilcase *templateIfToken:
...
var endifRange hcl.RangecurrentExprs := &ifExprs
Token:
for {
next := p.Peek()
...
p.Read() // eat end directiveswitch end.Type {case templateElse:
...
var diags hcl.DiagnosticsstartRange := p.NextRange()
ltrimNext := false
...
nextCanTrimPrev = falseswitch next.Type {
...
nextCanTrimPrev = truecase TokenTemplateInterp:
...
p.Read() // eat keyword tokenswitch {case ifKeyword.TokenMatches(kw):
