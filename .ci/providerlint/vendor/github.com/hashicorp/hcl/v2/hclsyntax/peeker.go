...
var tracePeekerNewlinesStack = falsetype peeker struct {
	Tokens    Tokens
	NextIndex int	IncludeComments      bool
	IncludeNewlinesStack []bool	// used only when tracePeekerNewlinesStack is set
...
type peekerNewlineStackChange struct {
 ...
 newPeeker(tokens Tokens, includeComments bool) *peeker {
	return &peeker{
		Tokens: tokens,
		IncludeComments: includeComments,		IncludeNewlinesStack: []bool{true},
	}
}
 (p *peeker) Peek() Token {
	ret, _ := p.nextToken()
	return ret (p *peeker) Read() Token {
...
*peeker) NextRange() hcl.Range {
 ...
 (p *peeker) nextToken() (Token, int) {
	for i := p.NextIndex; i < len(p.Tokens); i++ {
		...
		case TokenComment:
			if !p.IncludeComments {
				...
				// to properly parse newline-terminated block items.				if p.includingNewlines() {
					if len(tok.Bytes) > 0 && tok.Bytes[len(tok.Bytes)-1] == '\n' {
						fakeNewline := Token{
							Type:  TokenNewline,
							Bytes: tok.Bytes[len(tok.Bytes)-1 : len(tok.Bytes)],							// We use the whole token range as the newline
	...
	return p.Tokens[len(p.Tokens)-1], len(p.Tokens) (p *peeker) includingNewlines() bool {
...
newlineStackChanges = append(p.newlineStackChanges, peekerNewlineStackChange{
 ...
 (p *peeker) PopIncludeNewlines() bool {
	...
	p.IncludeNewlinesStack = remain	if tracePeekerNewlinesStack {
		// Record who called us so that we can more easily track down any
		// mismanagement of the stack in the parser.
		callers := []uintptr{0}
		runtime.Callers(2, callers)
...
// misbehaving. (p *peeker) AssertEmptyIncludeNewlinesStack() {
...
Name[idx+1:]
		...
		}		switch change.Pushing {		case true:
