...
// other configurationg syntaxes, such as JSON. ParseConfig(src []byte, filename string, start hcl.Pos) (*hcl.File, hcl.Diagnostics) {
	tokens, diags := LexConfig(src, filename, start)
	peeker := newPeeker(tokens, false)
	parser := &parser{peeker: peeker}
	body, parseDiags := parser.ParseBody(TokenEOF)
	diags = append(diags, parseDiags...)	// Panic if the parser uses incorrect stack discipline with the peeker's
	// newlines stack, since otherwise it will produce confusing downstream
	// errors.
	peeker.AssertEmptyIncludeNewlinesStack()	return &hcl.File{
		Body:  body,
		Bytes: src,		Nav: navigation{
			root: body,
		},
	}, diags
}// ParseExpression parses the given buffer as a standalone HCL expression,
eturning it as an instance of Expression. ParseExpression(src []byte, filename string, start hcl.Pos) (Expression, hcl.Diagnostics) {
	tokens, diags := LexExpression(src, filename, start)
	peeker := newPeeker(tokens, false)
	parser := &parser{peeker: peeker}	// Bare expressions are always parsed in  "ignore newlines" mode, as if
	// they were wrapped in parentheses.
	parser.PushIncludeNewlines(false)	expr, parseDiags := parser.ParseExpression()
	diags = append(diags, parseDiags...)	next := parser.Peek()
	...
	}	parser.PopIncludeNewlines()	// Panic if the parser uses incorrect stack discipline with the peeker's
	// newlines stack, since otherwise it will produce confusing downstream
	// errors.
	peeker.AssertEmptyIncludeNewlinesStack()	return expr, diags
}arseTemplate parses the given buffer as a standalone HCL template,
// returning it as an instance of Expression. ParseTemplate(src []byte, filename string, start hcl.Pos) (Expression, hcl.Diagnostics) {
...
// evaluating them. ParseTraversalAbs(src []byte, filename string, start hcl.Pos) (hcl.Traversal, hcl.Diagnostics) {
	tokens, diags := LexExpression(src, filename, start)
	peeker := newPeeker(tokens, false)
	parser := &parser{peeker: peeker}	// Bare traverals are always parsed in  "ignore newlines" mode, as if
	// they were wrapped in parentheses.
	parser.PushIncludeNewlines(false)	expr, parseDiags := parser.ParseTraversalAbs()
	diags = append(diags, parseDiags...)	parser.PopIncludeNewlines()	// Panic if the parser uses incorrect stack discipline with the peeker's
	// newlines stack, since otherwise it will produce confusing downstream
	// errors.
	peeker.AssertEmptyIncludeNewlinesStack()	return expr, diags
...
// detect _all_ syntax errors. LexConfig(src []byte, filename string, start hcl.Pos) (Tokens, hcl.Diagnostics) {
...
// a standalone HCL expression, and returns the resulting tokens.// Only minimal validation is done during lexical analysis, so the returned
// diagnostics may include errors about lexical issues such as bad character
// encodings or unrecognized characters, but full parsing is required to
// detect _all_ syntax errors. LexExpression(src []byte, filename string, start hcl.Pos) (Tokens, hcl.Diagnostics) {
...
// diagnostics may include errors about lexical issues such as bad character
// encodings or unrecognized characters, but full parsing is required to
// detect _all_ syntax errors. LexTemplate(src []byte, filename string, start hcl.Pos) (Tokens, hcl.Diagnostics) {
...
// will be traversable using the variable or attribute traversal syntax. ValidIdentifier(s string) bool {
