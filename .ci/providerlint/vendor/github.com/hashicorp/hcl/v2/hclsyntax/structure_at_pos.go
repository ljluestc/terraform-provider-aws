...
// is backed by a *Body. (b *Body) BlocksAtPos(pos hcl.Pos) []*hcl.Block {
	list, _ := b.blocksAtPos(pos, true)
	return list
}// InnermostBlockAtPos implements the method of the same name for an *hcl.File
hat is backed by a *Body. (b *Body) InnermostBlockAtPos(pos hcl.Pos) *hcl.Block {
...
// that is backed by a *Body. (b *Body) OutermostBlockAtPos(pos hcl.Pos) *hcl.Block {
...
// final element from that list. (b *Body) blocksAtPos(pos hcl.Pos, makeList bool) (list []*hcl.Block, innermost *Block) {
	current := bBlocks:
	for current != nil {
...
// analysis if necessary. (b *Body) outermostBlockAtPos(pos hcl.Pos) *Block {
	// This is similar to blocksAtPos, but simpler because we know it only
	// ever needs to search the first level of nested blocks.	for _, block := range b.Blocks {
		wholeRange := hcl.RangeBetween(block.TypeRange, block.CloseBraceRange)
		if wholeRange.ContainsPos(pos) {
			return block
		}
...
// that is backed by a *Body. (b *Body) AttributeAtPos(pos hcl.Pos) *hcl.Attribute {
...
// necessary. (b *Body) attributeAtPos(pos hcl.Pos) *Attribute {
	searchBody := b
...
// that is backed by a *Body. (b *Body) OutermostExprAtPos(pos hcl.Pos) hcl.Expression {
