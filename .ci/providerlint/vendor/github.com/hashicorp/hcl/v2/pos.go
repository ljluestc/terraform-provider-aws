...
// SPDX-License-Identifier: MPL-2.0package hclimport "fmt"// Pos represents a single position in a source file, by addressing the
...
type Pos struct {
	...
	// encountered.
	Line int	// Column is the source code column where this position points, in
	...
	Column int	// Byte is the byte offset into the file where the indicated character
...
var InitialPos = Pos{Byte: 0, Line: 1, Column: 1}// Range represents a span of characters between two positions in a source
// file.
//
...
type Range struct {
	...
	// point.
	Filename string	// Start and End represent the bounds of this range. Start is inclusive
...
// source file or if the end range appears before the start range. RangeBetween(start, end Range) Range {
	return Range{
...
ource file. RangeOver(a, b Range) Range {
	if a.Empty() {
		return b
	}
	if b.Empty() {
...
// offsets are given priority. (r Range) ContainsPos(pos Pos) bool {
	return r.ContainsOffset(pos.Byte)
}// ContainsOffset returns true if and only if the given byte offset is within
// the receiving Range. (r Range) ContainsOffset(offset int) bool {
...
// e.g. via markers directly on the relevant portion of source code. (r Range) String() string {
...
// trusted to produce an accurate start and end position within that slice. (r Range) CanSliceBytes(b []byte) bool {
	switch {
...
// be an accurate span of the requested range. (r Range) SliceBytes(b []byte) []byte {
	start := r.Start.Byte
	end := r.End.Byte
	if start < 0 {
		start = 0
...
// characters in common. (r Range) Overlaps(other Range) bool {
	switch {
...
//
// A non-empty result is either identical to or a subset of the receiver. (r Range) Overlap(other Range) Range {
	if !r.Overlaps(other) {
	...
	if r.End.Byte < other.End.Byte {
		...
		Filename: r.Filename,
		Start:    start,
		End:      end,}// PartitionAround finds the portion of the given range that overlaps with
...
// reciever then the corresponding outer range will be empty. (r Range) PartitionAround(other Range) (before, overlap, after Range) {
