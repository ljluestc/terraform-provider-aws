...
type File struct {
Body  Body
Bytes []byte// Nav is used to integrate with the "hcled" editor integration package,
...
Type   string
Labels []string
Body   BodyDefRange    Range   // Range that can be considered the "definition" for seeking in an editor
...
type Blocks []*Block// Attributes is a set of attributes keyed by their names.
type Attributes map[string]*Attribute// Body is a container for attributes and blocks. It serves as the primary
...
// the user.
Content(schema *BodySchema) (*BodyContent, Diagnostics)// PartialContent is like Content except that it permits the configuration
...
PartialContent(schema *BodySchema) (*BodyContent, Body, Diagnostics)// JustAttributes attempts to interpret all of the contents of the body
...
JustAttributes() (Attributes, Diagnostics)// MissingItemRange returns a range that represents where a missing item
...
Blocks     BlocksMissingItemRange Range
}// Attribute represents an attribute from within a body.
type Attribute struct {
Name string
Expr ExpressionRange     Range
NameRange Range
...
Value(ctx *EvalContext) (cty.Value, Diagnostics)// Variables returns a list of variables referenced in the receiving
...
Variables() []TraversalRange() Range
StartRange() Range
...
// requested type. (els Blocks) OfType(typeName string) Blocks {
ret := make(Blocks, 0)
for _, el := range els {
if el.Type == typeName {
ret = append(ret, el)
...
// name to block sequences of only that type. (els Blocks) ByType() map[string]Blocks {
