...
// SPDX-License-Identifier: MPL-2.0package hcl// -----------------------------------------------------------------------------
...
//
// The result is nil if no blocks at all contain the given position. (f *File) BlocksAtPos(pos Pos) []*Block {
...
//
he result is nil if no single block could be selected for any reason. (f *File) OutermostBlockAtPos(pos Pos) *Block {
	// The root body of the file must implement this interface in order
...
// all HCL syntaxes.// The result is nil if no single block could be selected for any reason. (f *File) InnermostBlockAtPos(pos Pos) *Block {
	// The root body of the file must implement this interface in order
...
//
// The result is nil if no single expression could be selected for any reason. (f *File) OutermostExprAtPos(pos Pos) Expression {
	// The root body of the file must implement this interface in order
...
//
// The result is nil if no single attribute could be selected for any reason. (f *File) AttributeAtPos(pos Pos) *Attribute {
	// The root body of the file must implement this interface in order
	// to support OutermostExprAtPos.
	type Interface interface {
