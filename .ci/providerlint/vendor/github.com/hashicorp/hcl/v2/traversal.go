...
// SPDX-License-Identifier: MPL-2.0package hclimport (
	"fmt"	"github.com/zclconf/go-cty/cty"
...
type Traversal []Traverser// TraversalJoin appends a relative traversal to an absolute traversal to
// produce a new absolute traversal. TraversalJoin(abs Traversal, rel Traversal) Traversal {
	if abs.IsRelative() {
...
nd will panic if applied to an absolute traversal. (t Traversal) TraverseRel(val cty.Value) (cty.Value, Diagnostics) {
	if !t.IsRelative() {
...
// traversals, and will panic if applied to a relative traversal. (t Traversal) TraverseAbs(ctx *EvalContext) (cty.Value, Diagnostics) {
	if t.IsRelative() {
		panic("can't use TraverseAbs on a relative traversal")
	}	split := t.SimpleSplit()
	root := split.Abs[0].(TraverseRoot)
	name := root.Name	thisCtx := ctx
	hasNonNil := false
	for thisCtx != nil {
...
// otherwise. (t Traversal) IsRelative() bool {
	if len(t) == 0 {
		return true
	}
...
// absolute traversals, and will panic if applied to a relative traversal.// This can be used by applications that have a relatively-simple variable
// namespace where only the top-level is directly populated in the scope, with
// everything else handled by relative lookups from those initial values. (t Traversal) SimpleSplit() TraversalSplit {
	if t.IsRelative() {
		panic("can't use SimpleSplit on a relative traversal")
	}
	return TraversalSplit{
		Abs: t[0:1],
		Rel: t[1:],}// RootName returns the root name for a absolute traversal. Will panic if
// called on a relative traversal. (t Traversal) RootName() string {
	if t.IsRelative() {
		panic("can't use RootName on a relative traversal")	}
	return t[0].(TraverseRoot).Name
}// SourceRange returns the source range for the traversal. (t Traversal) SourceRange() Range {
	if len(t) == 0 {
...
bsolute traversal. (t TraversalSplit) TraverseAbs(ctx *EvalContext) (cty.Value, Diagnostics) {
	return t.Abs.TraverseAbs(ctx)
}raverseRel traverses from a given value, assumed to be the result of
// TraverseAbs on some scope, to a final result for the entire split traversal. (t TraversalSplit) TraverseRel(val cty.Value) (cty.Value, Diagnostics) {
	return t.Rel.TraverseRel(val)
}// Traverse is a convenience 
tion to apply TraverseAbs followed by
// TraverseRel. (t TraversalSplit) Traverse(ctx *EvalContext) (cty.Value, Diagnostics) {
...
// absolute traversal. (t TraversalSplit) Join() Traversal {
	return TraversalJoin(t.Abs, t.Rel)
}// RootName returns the root name for the absolute part of the split. (t TraversalSplit) RootName() string {
...
// traversals cannot be directly traversed. TraverseRoot) TraversalStep(cty.Value) (cty.Value, Diagnostics) {
...
type TraverseAttr struct {
 ...
 (tn TraverseAttr) TraversalStep(val cty.Value) (cty.Value, Diagnostics) {
	return GetAttr(val, tn.Name, &tn.SrcRange) (tn TraverseAttr) SourceRange() Range {
	return tn.SrcRange
