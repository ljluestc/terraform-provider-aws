...
// no traversal is possible. Alternatively, an imentation can support
...
(expr Expression) bool {
	...
	return nil, Diagnostics{
		&Diagnostic{
			...
			Detail:   "A single static variable reference is required: only attribute access and indexing with constant keys. No calculations, 
tion calls, template expressions, etc are allowed here.",
...
//
// This 
tion is a variant of AbsTraversalForExpr, which uses the same
// interface on the given expression. This helper constrains the result
// further by requiring only a single root identifier.
//
// This 
tion is intended to be used with the following idiom, to recognize
...
// cannot not be accepted as keywords by this 
tion.
//
