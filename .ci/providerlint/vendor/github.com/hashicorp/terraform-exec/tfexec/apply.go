...
type ApplyOption interface {
 ...
 (opt *BackupOption) configureApply(conf *applyConfig) {
	conf.backup = opt.path (opt *TargetOption) configureApply(conf *applyConfig) {
...
t *LockTimeoutOption) configureApply(conf *applyConfig) {
 ...
 (opt *StateOutOption) configureApply(conf *applyConfig) {
	conf.stateOut = opt.path (opt *VarFileOption) configureApply(conf *applyConfig) {
...
t *LockOption) configureApply(conf *applyConfig) {
 ...
 (opt *RefreshOnlyOption) configureApply(conf *applyConfig) {
	conf.refreshOnly = opt.refreshOnly (opt *ReplaceOption) configureApply(conf *applyConfig) {
...
}// Apply represents the terraform apply subcommand. (tf *Terraform) Apply(ctx context.Context, opts ...ApplyOption) error {
...
// removed in a future major version in favour of Apply returning JSON by default. (tf *Terraform) ApplyJSON(ctx context.Context, w io.Writer, opts ...ApplyOption) error {
	err := tf.compatible(ctx, tf0_15_3, nil)
	if err != nil {
		return fmt.Errorf("terraform apply -json was added in 0.15.3: %w", err)
	}	tf.SetStdout(w)	cmd, err := tf.applyJSONCmd(ctx, opts...)
	if err != nil {
		return err
	}	return tf.runTerraformCmd(ctx, cmd) (tf *Terraform) applyCmd(ctx context.Context, opts ...ApplyOption) (*exec.Cmd, error) {
	c := defaultApplyOptions	for _, o := range opts {
		o.configureApply(&c)
 ...
 (tf *Terraform) applyJSONCmd(ctx context.Context, opts ...ApplyOption) (*exec.Cmd, error) {
	...
	if err != nil {
		return nil, err
	}	args = append(args, "-json")	return tf.buildApplyCmd(ctx, c, args)
}
 (tf *Terraform) buildApplyArgs(ctx context.Context, c applyConfig) ([]string, error) {
	args := []string{"apply", "-no-color", "-auto-approve", "-input=false"}	// string opts: only pass if set
	if c.backup != "" {
