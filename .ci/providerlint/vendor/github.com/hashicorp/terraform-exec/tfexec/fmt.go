...
// SPDX-License-Identifier: MPL-2.0package tfexecimport (
 ...
 (opt *RecursiveOption) configureFormat(conf *formatConfig) {
	conf.recursive = opt.recursive
}
 (opt *DirOption) configureFormat(conf *formatConfig) {
	conf.dir = opt.path
}// FormatString formats a passed string, given a path to Terraform. FormatString(ctx context.Context, execPath string, content string) (string, error) {
	tf, err := NewTerraform(filepath.Dir(execPath), execPath)
	if err != nil {
		return "", err
	}	return tf.FormatString(ctx, content)
// FormatString formats a passed string. (tf *Terraform) FormatString(ctx context.Context, content string) (string, error) {
...
// the formatted result on the formatted io.Writer. (tf *Terraform) Format(ctx context.Context, unformatted io.Reader, formatted io.Writer) error {
	cmd, err := tf.formatCmd(ctx, nil, Dir("-"))
	if err != nil {
		return err
	}	cmd.Stdin = unformatted
	cmd.Stdout = mergeWriters(cmd.Stdout, formatted)	return tf.runTerraformCmd(ctx, cmd)
}// FormatWrite attempts to format and modify all config files in the working or selected (via DirOption) directory. (tf *Terraform) FormatWrite(ctx context.Context, opts ...FormatOption) error {
	for _, o := range opts {
		switch o := o.(type) {
		case *DirOption:
			if o.path == "-" {
...
}// FormatCheck returns true if the config files in the working or selected (via DirOption) directory are already formatted. (tf *Terraform) FormatCheck(ctx context.Context, opts ...FormatOption) (bool, []string, error) {
	for _, o := range opts {
		switch o := o.(type) {
		case *DirOption:
			if o.path == "-" {
	...
	if cmd.ProcessState.ExitCode() == 3 {
		// unformatted, parse the file list		files := []string{}
 ...
 (tf *Terraform) formatCmd(ctx context.Context, args []string, opts ...FormatOption) (*exec.Cmd, error) {
	err := tf.compatible(ctx, tf0_7_7, nil)
	if err != nil {
		return nil, fmt.Errorf("fmt was first introduced in Terraform 0.7.7: %w", err)
	}	c := defaultFormatConfig	for _, o := range opts {
		switch o.(type) {
	...
	}	args = append([]string{"fmt", "-no-color"}, args...)	if c.recursive {
