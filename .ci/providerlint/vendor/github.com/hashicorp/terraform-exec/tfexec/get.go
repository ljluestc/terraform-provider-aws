...
}// Get represents the terraform get subcommand. (tf *Terraform) Get(ctx context.Context, opts ...GetCmdOption) error {
...
urn tf.runTerraformCmd(ctx, cmd)
 ...
 (tf *Terraform) getCmd(ctx context.Context, opts ...GetCmdOption) (*exec.Cmd, error) {
	c := getCmdConfig{}	for _, o := range opts {
		o.configureGet(&c)
	}	args := []string{"get", "-no-color"}	args = append(args, "-update="+fmt.Sprint(c.update))	if c.dir != "" {
