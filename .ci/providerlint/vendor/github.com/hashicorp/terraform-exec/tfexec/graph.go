...
// SPDX-License-Identifier: MPL-2.0package tfexecimport (
	...
	drawCycles bool
	graphType  string
}var defaultGraphOptions = graphConfig{}type GraphOption interface {
 ...
 (opt *DrawCyclesOption) configureGraph(conf *graphConfig) {
	conf.drawCycles = opt.drawCycles (opt *GraphTypeOption) configureGraph(conf *graphConfig) {
f.graphType = opt.graphType
 ...
 (tf *Terraform) graphCmd(ctx context.Context, opts ...GraphOption) (*exec.Cmd, error) {
	c := defaultGraphOptions	for _, o := range opts {
		o.configureGraph(&c)
	}	args := []string{"graph"}	if c.plan != "" {
