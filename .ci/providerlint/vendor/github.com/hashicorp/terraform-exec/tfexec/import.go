...
type ImportOption interface {
 ...
 (opt *ConfigOption) configureImport(conf *importConfig) {
	conf.config = opt.path (opt *AllowMissingConfigOption) configureImport(conf *importConfig) {
...
t *LockOption) configureImport(conf *importConfig) {
 ...
 (opt *ReattachOption) configureImport(conf *importConfig) {
	conf.reattachInfo = opt.info (opt *StateOption) configureImport(conf *importConfig) {
f.state = opt.path
}
...
t *VarOption) configureImport(conf *importConfig) {
...
}// Import represents the terraform import subcommand. (tf *Terraform) Import(ctx context.Context, address, id string, opts ...ImportOption) error {
 ...
 (tf *Terraform) importCmd(ctx context.Context, address, id string, opts ...ImportOption) (*exec.Cmd, error) {
	c := defaultImportOptions	for _, o := range opts {
		o.configureImport(&c)
	}	args := []string{"import", "-no-color", "-input=false"}	// string opts: only pass if set
	if c.backup != "" {
	...
	args = append(args, address, id)	mergeEnv := map[string]string{}
