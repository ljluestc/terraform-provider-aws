...
type StatePushCmdOption interface {
 ...
 (opt *LockOption) configureStatePush(conf *statePushConfig) {
	conf.lock = opt.lock (opt *LockTimeoutOption) configureStatePush(conf *statePushConfig) {
f.lockTimeout = opt.timeout
 ...
 (tf *Terraform) statePushCmd(ctx context.Context, path string, opts ...StatePushCmdOption) (*exec.Cmd, error) {
	c := defaultStatePushOptions	for _, o := range opts {
		o.configureStatePush(&c)
	}	args := []string{"state", "push"}	if c.force {
		args = append(args, "-force")
	}	args = append(args, "-lock="+strconv.FormatBool(c.lock))	if c.lockTimeout != "" {
		args = append(args, "-lock-timeout="+c.lockTimeout)
	}	args = append(args, path)	return tf.buildTerraformCmd(ctx, nil, args...), nil
}
