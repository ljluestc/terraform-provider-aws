...
type StateRmCmdOption interface {
 ...
 (opt *BackupOutOption) configureStateRm(conf *stateRmConfig) {
	conf.backupOut = opt.path (opt *DryRunOption) configureStateRm(conf *stateRmConfig) {
f.dryRun = opt.dryRun
}
t *LockOption) configureStateRm(conf *stateRmConfig) {
...
}// StateRm represents the terraform state rm subcommand. *Terraform) StateRm(ctx context.Context, address string, opts ...StateRmCmdOption) error {
 ...
 (tf *Terraform) stateRmCmd(ctx context.Context, address string, opts ...StateRmCmdOption) (*exec.Cmd, error) {
	c := defaultStateRmOptions	for _, o := range opts {
		o.configureStateRm(&c)
	}	args := []string{"state", "rm", "-no-color"}	// string opts: only pass if set
	if c.backup != "" {
	...
	args = append(args, address)	return tf.buildTerraformCmd(ctx, nil, args...), nil
}
