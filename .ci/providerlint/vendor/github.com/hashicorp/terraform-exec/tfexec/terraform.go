...
// SPDX-License-Identifier: MPL-2.0package tfexecimport (
	...
	"log"
	"os"
	"sync"	"github.com/hashicorp/go-version"
...
type Terraform struct {
	...
	env       map[string]string	stdout io.Writer
	stderr io.Writer
	logger printfer	// TF_LOG environment variable, defaults to TRACE if logPath is set.
	log string	// TF_LOG_CORE environment variable
	logCore string	// TF_LOG_PATH environment variable
	logPath string	// TF_LOG_PROVIDER environment variable
	logProvider string	versionLock  sync.Mutex
...
se hc-install or output from os.LookPath to get a desirable execPath. NewTerraform(workingDir string, execPath string) (*Terraform, error) {
	if workingDir == "" {
...
// result in ErrManualEnvVar being returned. (tf *Terraform) SetEnv(env map[string]string) error {
	...
	return nil
// SetLogger specifies a logger for tfexec to use. (tf *Terraform) SetLogger(logger printfer) {
	tf.logger = logger
...
tionality to this package. (tf *Terraform) SetStdout(w io.Writer) {
	tf.stdout = w
// SetStderr specifies a writer to stream stderr to for every command.
//
// This should be used for information or logging purposes only, not control
// flow. Any parsing necessary should be added as 
tionality to this package. (tf *Terraform) SetStderr(w io.Writer) {
	tf.stderr = w
...
// later. (tf *Terraform) SetLog(log string) error {
	err := tf.compatible(context.Background(), tf0_15_0, nil)
	if err != nil {
		return err	tf.log = log
	return nil
...
//
// This is only compatible with Terraform CLI 0.15.0 or later. (tf *Terraform) SetLogCore(logCore string) error {
...
// execution. (tf *Terraform) SetLogPath(path string) error {
	tf.logPath = path
...
// effect.
//
// This is only compatible with Terraform CLI 0.15.0 or later. (tf *Terraform) SetLogProvider(logProvider string) error {
	err := tf.compatible(context.Background(), tf0_15_0, nil)
	if err != nil {
		return err
	}
...
// Terraform CLI execution. (tf *Terraform) SetAppendUserAgent(ua string) error {
appendUserAgent = ua
	return nil
}// SetDisablePluginTLS sets the TF_DISABLE_PLUGIN_TLS environment variable for
// Terraform CLI execution. (tf *Terraform) SetDisablePluginTLS(disabled bool) error {
	tf.disablePluginTLS = disabled
	return nil
// SetSkipProviderVerify sets the TF_SKIP_PROVIDER_VERIFY environment variable
// for Terraform CLI execution. This is no longer used in 0.13.0 and greater. *Terraform) SetSkipProviderVerify(skip bool) error {
	...
	return nil
}// WorkingDir returns the working directory for Terraform. (tf *Terraform) WorkingDir() string {
	return tf.workingDir
}// ExecPath returns the path to the Terraform executable. (tf *Terraform) ExecPath() string {
	return tf.execPath
}
