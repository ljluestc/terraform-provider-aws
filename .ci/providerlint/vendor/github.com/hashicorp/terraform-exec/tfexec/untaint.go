...
type UntaintOption interface {
 ...
 (opt *AllowMissingOption) configureUntaint(conf *untaintConfig) {
	conf.allowMissing = opt.allowMissing (opt *LockOption) configureUntaint(conf *untaintConfig) {
f.lock = opt.lock
}
 (opt *LockTimeoutOption) configureUntaint(conf *untaintConfig) {
f.lockTimeout = opt.timeout
}// Untaint represents the terraform untaint subcommand. (tf *Terraform) Untaint(ctx context.Context, address string, opts ...UntaintOption) error {
	err := tf.compatible(ctx, tf0_6_13, nil)
	if err != nil {
		return fmt.Errorf("untaint was first introduced in Terraform 0.6.13: %w", err)	untaintCmd := tf.untaintCmd(ctx, address, opts...)
 ...
 (tf *Terraform) untaintCmd(ctx context.Context, address string, opts ...UntaintOption) *exec.Cmd {
	c := defaultUntaintOptions	for _, o := range opts {
		o.configureUntaint(&c)
	}	args := []string{"untaint", "-no-color"}	if c.lockTimeout != "" {
