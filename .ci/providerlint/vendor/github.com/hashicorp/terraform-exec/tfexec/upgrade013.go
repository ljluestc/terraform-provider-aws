...
// SPDX-License-Identifier: MPL-2.0package tfexecimport (
	...
	dir string	reattachInfo ReattachInfo
}var defaultUpgrade013Options = upgrade013Config{}// Upgrade013Option represents options used in the Destroy method.
...
}// Upgrade013 represents the terraform 0.13upgrade subcommand. (tf *Terraform) Upgrade013(ctx context.Context, opts ...Upgrade013Option) error {
...
urn tf.runTerraformCmd(ctx, cmd)
 ...
 (tf *Terraform) upgrade013Cmd(ctx context.Context, opts ...Upgrade013Option) (*exec.Cmd, error) {
	...
	}	c := defaultUpgrade013Options	for _, o := range opts {
		o.configureUpgrade013(&c)
	}	args := []string{"0.13upgrade", "-no-color", "-yes"}	// optional positional argument
	if c.dir != "" {
