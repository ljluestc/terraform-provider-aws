...
// SPDX-License-Identifier: MPL-2.0package tfexecimport (
	...
	"regexp"
	"strconv"
	"strings"	"github.com/hashicorp/go-version"
...
// is set to true. (tf *Terraform) Version(ctx context.Context, skipCache bool) (tfVersion *version.Version, providerVersions map[string]*version.Version, err error) {
	tf.versionLock.Lock()
	defer tf.versionLock.Unlock()	if tf.execVersion == nil || skipCache {
		tf.execVersion, tf.provVersions, err = tf.version(ctx)
		if err != nil {
			return nil, nil, err
		}
	}	return tf.execVersion, tf.provVersions, nil
}ersion does not use the locking on the Terraform instance and should probably not be used directly, prefer Version. (tf *Terraform) version(ctx context.Context) (*version.Version, map[string]*version.Version, error) {
	versionCmd := tf.buildTerraformCmd(ctx, nil, "version", "-json")	var outBuf bytes.Buffer
	versionCmd.Stdout = &outBuf	err := tf.runTerraformCmd(ctx, versionCmd)
	if err != nil {
		return nil, nil, err
	...
	}	return tfVersion, providerVersions, err parseJsonVersionOutput(stdout []byte) (*version.Version, map[string]*version.Version, error) {
 ...
 (tf *Terraform) versionFromPlaintext(ctx context.Context) (*version.Version, map[string]*version.Version, error) {
	versionCmd := tf.buildTerraformCmd(ctx, nil, "version")	var outBuf strings.Builder
	versionCmd.Stdout = &outBuf	err := tf.runTerraformCmd(ctx, versionCmd)
	if err != nil {
		return nil, nil, err
	...
	simpleVersionRe = `v?(?P<version>[0-9]+(?:\.[0-9]+)*(?:-[A-Za-z0-9\.]+)?)`sionOutputRe= regexp.MustCompile(`Terraform ` + simpleVersionRe)
 ...
 parsePlaintextVersionOutput(stdout string) (*version.Version, map[string]*version.Version, error) {
	stdout = strings.TrimSpace(stdout)	submatches := versionOutputRe.FindStringSubmatch(stdout)
	if len(submatches) != 2 {
	...
	provV := map[string]*version.Version{}	for _, submatches := range allSubmatches {
		if len(submatches) != 4 {
...
turn "-"
	...
	return v.String()
}// compatible asserts compatibility of the cached terraform version with the executable, and returns a well known error if not. (tf *Terraform) compatible(ctx context.Context, minInclusive *version.Version, maxExclusive *version.Version) error {
...
//
// Pre-release information is ignored for comparison. versionInRange(tfv *version.Version, minInclusive *version.Version, maxExclusive *version.Version) bool {
