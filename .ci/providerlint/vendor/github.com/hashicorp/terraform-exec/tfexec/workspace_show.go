...
// SPDX-License-Identifier: MPL-2.0package tfexecimport (
	...
	"os/exec"
	"strings"
)// WorkspaceShow represents the workspace show subcommand to the Terraform CLI. (tf *Terraform) WorkspaceShow(ctx context.Context) (string, error) {
	workspaceShowCmd, err := tf.workspaceShowCmd(ctx)
	if err != nil {
		return "", err
	}	var outBuffer strings.Builder
	workspaceShowCmd.Stdout = &outBuffer	err = tf.runTerraformCmd(ctx, workspaceShowCmd)
	if err != nil {
		return "", err
