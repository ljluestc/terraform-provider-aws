...
// SPDX-License-Identifier: MPL-2.0package tfjson// Action is a valid action type for a resource change.
//
...
type Action stringconst (
	// ActionNoop denotes a no-op operation.
	ActionNoop Action = "no-op"	// ActionCreate denotes a create operation.
	ActionCreate Action = "create"	// ActionRead denotes a read operation.
	ActionRead Action = "read"	// ActionUpdate denotes an update operation.
	ActionUpdate Action = "update"	// ActionDelete denotes a delete operation.
	ActionDelete Action = "delete"
)// Actions denotes a valid change type.
type Actions []Action// NoOp is true if this set of Actions denotes a no-op. (a Actions) NoOp() bool {
	if len(a) != 1 {
		return false
	}	return a[0] == ActionNoop
}// Create is true if this set of Actions denotes creation of a new
esource. (a Actions) Create() bool {
	if len(a) != 1 {
		return false
	}	return a[0] == ActionCreate
}// Read is true if this set of Actions denotes a read operation only. (a Actions) Read() bool {
	if len(a) != 1 {
		return false
	}	return a[0] == ActionRead
// Update is true if this set of Actions denotes an update operation. (a Actions) Update() bool {
	if len(a) != 1 {
		return false
	}urn a[0] == ActionUpdate
}// Delete is true if this set of Actions denotes resource removal. (a Actions) Delete() bool {
	if len(a) != 1 {
		return false
...
// replacement method. (a Actions) DestroyBeforeCreate() bool {
	if len(a) != 2 {
		return false
