...
// SPDX-License-Identifier: MPL-2.0package tfjsonimport "encoding/json"type unknownConstantValue struct{}// UnknownConstantValue is a singleton type that denotes that a
...
// value.
var UnknownConstantValue = &unknownConstantValue{}// Expression describes the format for an individual key in a
...
type ExpressionData struct {
	...
	ConstantValue interface{} `json:"constant_value,omitempty"`	// If any part of the expression contained values that were not
	...
	References []string `json:"references,omitempty"`	// A list of complex objects that were nested in this expression.
...
}// UnmarshalJSON implements json.Unmarshaler for Expression. (e *Expression) UnmarshalJSON(b []byte) error {
	result := new(ExpressionData)	// Check to see if this is an array first. If it is, this is more
 ...
 unmarshalExpressionBlocks(raw []map[string]json.RawMessage) ([]map[string]*Expression, error) {
	var result []map[string]*Expression	for _, rawBlock := range raw {
...
}// MarshalJSON implements json.Marshaler for Expression. (e *Expression) MarshalJSON() ([]byte, error) {
	switch {
	case len(e.ExpressionData.NestedBlocks) > 0:
		return marshalExpressionBlocks(e.ExpressionData.NestedBlocks)	case e.ExpressionData.ConstantValue == UnknownConstantValue:
