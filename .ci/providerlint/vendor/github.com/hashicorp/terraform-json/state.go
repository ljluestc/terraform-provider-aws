...
var StateFormatVersionConstraints = ">= 0.1, < 2.0"// State is the top-level representation of a Terraform state.
type State struct {
	...
	// State.UseJSONNumber.
	useJSONNumber bool	// The version of the state format. This should always match the
	// StateFormatVersion constant in this package, or else am
	// unmarshal will be unstable.
	FormatVersion string `json:"format_version,omitempty"`	// The Terraform version used to make the state.
	TerraformVersion string `json:"terraform_version,omitempty"`	// The values that make up the state.
	Values *StateValues `json:"values,omitempty"`	// Checks contains the results of any conditional checks when Values was
	// last updated.
...
// State will represent numbers in StateOutputs as float64s. (s *State) UseJSONNumber(b bool) {
	s.useJSONNumber = b
}// Validate checks to ensure that the state is present, and the
ersion matches the version supported by this library. (s *State) Validate() error {
	if s == nil {
	...
	if err != nil {
		...
		return fmt.Errorf("unsupported state format version: %q does not satisfy %q",
			version, constraint)
	}	return nil (s *State) UnmarshalJSON(b []byte) error {
	type rawState State
	var state rawState	dec := json.NewDecoder(bytes.NewReader(b))
	if s.useJSONNumber {
		dec.UseNumber()
	}
	...
	if err != nil {
		return err
	}	*s = *(*State)(&state)	return s.Validate()
...
type StateValues struct {
	...
	Outputs map[string]*StateOutput `json:"outputs,omitempty"`	// The root module in this state representation.
...
type StateModule struct {
	...
	Resources []*StateResource `json:"resources,omitempty"`	// The absolute module address, omitted for the root module.
	Address string `json:"address,omitempty"`	// Any child modules within this module.
...
type StateResource struct {
	...
	ProviderName string `json:"provider_name,omitempty"`	// The version of the resource type schema the "values" property
	// conforms to.
	SchemaVersion uint64 `json:"schema_version,"`	// The JSON representation of the attribute values of the resource,
	...
	// from absent values.
	AttributeValues map[string]interface{} `json:"values,omitempty"`	// The JSON representation of the sensitivity of the resource's
	// attribute values. Only attributes which are sensitive
	// are included in this structure.
	SensitiveValues json.RawMessage `json:"sensitive_values,omitempty"`	// The addresses of the resources that this resource depends on.
	DependsOn []string `json:"depends_on,omitempty"`	// If true, the resource has been marked as tainted and will be
	// re-created on the next update.
	Tainted bool `json:"tainted,omitempty"`	// DeposedKey is set if the resource instance has been marked Deposed and
...
type StateOutput struct {
	...
	Sensitive bool `json:"sensitive"`	// The value of the output.
	Value interface{} `json:"value,omitempty"`	// The type of the output.
