...
// DataSourceContext injects the data source type into logger contexts.
 ...
 DataSourceContext(ctx context.Context, dataSource string) context.Context {
...
nitContext creates SDK and provider logger contexts.

 InitContext(ctx context.Context, sdkOpts tfsdklog.Options, providerOpts tflog.Options) context.Context {
...
// ProtoSubsystemContext adds the proto subsystem to the SDK logger context.
 ...
 ProtoSubsystemContext(ctx context.Context, sdkOpts tfsdklog.Options) context.Context {
	ctx = tfsdklog.NewSubsystem(ctx, SubsystemProto, append(tfsdklog.Options{
		// All calls are through the Protocol* helper 
tions
...
// ProtocolVersionContext injects the protocol version into logger contexts.
 ...
 ProtocolVersionContext(ctx context.Context, protocolVersion string) context.Context {
...
// ProviderAddressContext injects the provider address into logger contexts.
 ...
 ProviderAddressContext(ctx context.Context, providerAddress string) context.Context {
 ...
 = tflog.SetField(ctx, KeyProviderAddress, providerAddress)
...
// RequestIdContext injects a unique request ID into logger contexts.
 ...
 RequestIdContext(ctx context.Context) context.Context {
 ...
 = tfsdklog.SubsystemSetField(ctx, SubsystemProto, KeyRequestID, reqID)
...
// ResourceContext injects the resource type into logger contexts.
 ...
 ResourceContext(ctx context.Context, resource string) context.Context {
 = tfsdklog.SetField(ctx, KeyResourceType, resource)
...
// RpcContext injects the RPC name into logger contexts.
 ...
 RpcContext(ctx context.Context, rpc string) context.Context {
