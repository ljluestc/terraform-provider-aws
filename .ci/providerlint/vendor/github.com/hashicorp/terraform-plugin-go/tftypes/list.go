...
// will be returned.
 ...
 (l List) ApplyTerraform5AttributePathStep(step AttributePathStep) (interface{}, error) {
	switch s := step.(type) {
	case ElementKeyInt:
		if int64(s) < 0 {
...
// Equal returns true if the two Lists are exactly equal. Unlike Is, passing in
 List with no ElementType will always return false.

 (l List) Equal(o Type) bool {
	v, ok := o.(List)
	if !ok {
		return false
	}
...
f the other List does not have a type compatible ElementType, it will
// return false.

 (l List) UsableAs(o Type) bool {
...
urn "tftypes.List[" + l.ElementType.String() + "]"
}
...
List) private() {}
 ...
 (l List) supportedGoTypes() []string {
	return []string{"[]tftypes.Value"}
}


 valueFromList(typ Type, in interface{}) (Value, error) {
	...
	default:
turn Value{}, fmt.Errorf("tftypes.NewValue can't use %T as a tftypes.List; expected types are: %s", in, formattedSupportedGoTypes(List{}))
	}
}
