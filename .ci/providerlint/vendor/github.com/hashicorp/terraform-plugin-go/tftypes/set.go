...
// will be returned.
 ...
 (s Set) ApplyTerraform5AttributePathStep(step AttributePathStep) (interface{}, error) {
	switch step.(type) {
...
// Equal returns true if the two Sets are exactly equal. Unlike Is, passing in
 Set with no ElementType will always return false.

 (s Set) Equal(o Type) bool {
	v, ok := o.(Set)
	if !ok {
		return false
	}
...
f the other Set does not have a type compatible ElementType, it will
// return false.

 (s Set) UsableAs(o Type) bool {
...
ok := t.(Set)
 ...
 (s Set) String() string {
urn "tftypes.Set[" + s.ElementType.String() + "]"
}
...
Set) private() {}
 ...
 (s Set) supportedGoTypes() []string {
	return []string{"[]tftypes.Value"}
}


 valueFromSet(typ Type, in interface{}) (Value, error) {
	...
	default:
turn Value{}, fmt.Errorf("tftypes.NewValue can't use %T as a tftypes.Set; expected types are: %s", in, formattedSupportedGoTypes(Set{}))
	}
}
