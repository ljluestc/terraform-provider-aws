...
// arguments (i.e. `[k1, v1, k2, v2, ...]`) expected by hc-log.Logger methods. FieldMapsToArgs(maps ...map[string]interface{}) []interface{} {
	switch len(maps) {
	case 0:
		return nil
	case 1:
		result := make([]interface{}, 0, len(maps[0])*2)		for k, v := range maps[0] {
