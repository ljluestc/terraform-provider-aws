package loggingimport (
	"strings"	"github.com/hashicorp/terraform-plugin-log/internal/fieldutils"
	"github.com/hashicorp/terraform-plugin-log/internal/hclogutils"
)const logMaskingReplacementString = "***"// ShouldOmit takes a log's *string message and slices of fields,
// and determines, based on the LoggerOpts configuration, if the
// log should be omitted (i.e. prevent it to be printed on the final writer). (lo LoggerOpts) ShouldOmit(msg *string, fieldMaps ...map[string]interface{}) bool {
...
//
ote that the given input is changed-in-place by this method. (lo LoggerOpts) ApplyMask(msg *string, fieldMaps ...map[string]interface{}) {
	...
	additionalFieldsMap := fieldutils.MergeFieldMaps(additionalFields...)	// Apply the provider root LoggerOpts to determine if this log should be omitted
	...
	tfLoggerOpts.ApplyMask(msg, tfLoggerOpts.Fields, additionalFieldsMap)urn hclogutils.FieldMapsToArgs(tfLoggerOpts.Fields, additionalFieldsMap), false
}
