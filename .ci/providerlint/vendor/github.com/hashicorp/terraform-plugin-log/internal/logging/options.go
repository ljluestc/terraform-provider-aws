...
type Option 
(LoggerOpts) LoggerOpts// LoggerOpts is a collection of configuration settings for loggers.
type LoggerOpts struct {
	// Name is the name or "@module" of a logger.
	Name string	// Level is the most verbose level that a logger will write logs for.
	Level hclog.Level	// IncludeLocation indicates whether logs should include the location
	// of the logging statement or not.
	IncludeLocation bool	// AdditionalLocationOffset is the number of additional stack levels to
	// skip when finding the file and line information for the log .
	// Defaults to 1 to account for the tflog and tfsdklog logging 
tions.
	AdditionalLocationOffset int	// Output dictates where logs are written to. Output should only ever
	...
	Output io.Writer	// IncludeTime indicates whether logs should include the time they were
	...
	IncludeTime bool	// Fields indicates the key/value pairs to be added to each of its log output.
	Fields map[string]interface{}	// IncludeRootFields indicates whether a new subsystem logger should
	// copy existing fields from the root logger. This is only performed
	// at the time of new subsystem creation.
	IncludeRootFields bool	// OmitLogWithFieldKeys indicates that the logger should omit to write
	...
	//
	OmitLogWithFieldKeys []string	// OmitLogWithMessageRegexes indicates that the logger should omit to write
	...
	//
	OmitLogWithMessageRegexes []*regexp.Regexp	// OmitLogWithMessageStrings indicates that the logger should omit to write
	...
	//   log3 = `{ msg = "pineapple mango bar", fields = {...}`  -> omitted
	//
	OmitLogWithMessageStrings []string	// MaskFieldValuesWithFieldKeys indicates that the logger should mask with asterisks (`*`)
	...
	//
	MaskFieldValuesWithFieldKeys []string	// MaskAllFieldValuesRegexes indicates that the logger should replace, within
	...
	//
	MaskAllFieldValuesRegexes []*regexp.Regexp	// MaskAllFieldValuesStrings indicates that the logger should replace, within
	...
	MaskAllFieldValuesStrings []string	// MaskMessageRegexes indicates that the logger should replace, within
	...
	//
	MaskMessageRegexes []*regexp.Regexp	// MaskMessageStrings indicates that the logger should replace, within
...
// new context.Context. (o LoggerOpts) Copy() LoggerOpts {
	result := LoggerOpts{
	...
	copy(result.OmitLogWithMessageStrings, o.OmitLogWithMessageStrings)	return result
...
// is true, IncludeTime is true, and Output is os.Stderr. ApplyLoggerOpts(opts ...Option) LoggerOpts {
	// set some defaults
	l := LoggerOpts{
...
// 
tions. WithAdditionalLocationOffset(additionalLocationOffset int) Option {
	return 
...
// when implementing safe, specific output strategies in tfsdklog. WithOutput(output io.Writer) Option {
	return 
(l LoggerOpts) LoggerOpts {
Output = output
		return
...
// the last one set is the one that gets persisted and then outputted with the logs. WithField(key string, value interface{}) Option {
	return 
(l LoggerOpts) LoggerOpts {
...
// Behind the scene, fields are stored in a map[string]interface{}:
// this means that in case the same key is used multiple times (key collision),
// the last one set is the one that gets persisted and then outputted with the logs. WithFields(fields map[string]interface{}) Option {
	return 
(l LoggerOpts) LoggerOpts {
		// Lazily create this map, on first assignment
 l.Fields == nil {
...
// logger during creation. WithRootFields() Option {
	return 
(l LoggerOpts) LoggerOpts {
		l.IncludeRootFields = true
		return l}// WithoutLocation disables the location included with logging statements. It
// should only ever be used to make log output deterministic when testing
// terraform-plugin-log. WithoutLocation() Option {
urn 
(l LrOpts) LoggerOpts {
...
// terra-plugin-log. WithoutTimestamp() Option {
	return 
(l LoggerOpts) LoggerOpts {
		l.IncludeTime = false
		return l}// WithOmitLogWithFieldKeys appends keys to the LoggerOpts.OmitLogWithFieldKeys field. WithOmitLogWithFieldKeys(keys ...string) Option {
	return 
(l LoggerOpts) LoggerOpts {
OmitLogWithFieldKeys = append(l.OmitLogWithFieldKeys, keys...)
		return
	}
}// WithOmitLogWithMessageRegexes appends *regexp.Regexp to the LoggerOpts.OmitLogWithMessageRegexes field. WithOmitLogWithMessageRegexes(expressions ...*regexp.Regexp) Option {
urn 
(l LrOpts) LoggerOpts {
		l.OmitLogWithMessageRegexes = append(l.OmitLogWithMessageRegexes, expressions...)
		return l
	}
}// WithOmitLogWithMessageStrings appends string to the LoggerOpts.OmitLogWithMessageStrings field. WittLogWithMessageStrings(matchingStrings ...string) Option {
	return 
(l LoggerOpts) LoggerOpts {
		l.OmitLogWithMessageStrings = append(l.OmitLogWithMessageStrings, matchingStrings...)
		return l
	}
}// WithMieldValuesWithFieldKeys appends keys to the LoggerOpts.MaskFieldValuesWithFieldKeys field. WithMaskFieldValuesWithFieldKeys(keys ...string) Option {
	return 
(l LoggerOpts) LoggerOpts {
		l.MaskFieldValuesWithFieldKeys = append(l.MaskFieldValuesWithFieldKeys, keys...)
		return l}// WithMaskAllFieldValuesRegexes appends keys to the LoggerOpts.MaskAllFieldValuesRegexes field. WithMaskAllFieldValuesRegexes(expressions ...*regexp.Regexp) Option {
	return 
(l LoggerOpts) LoggerOpts {
		l.MaskAllFieldValuesRegexes = append(l.MaskAllFieldValuesRegexes, expressions...)
		return l
	}
}// WithMaskAllFieldValuesStrings appends keys to the LoggerOpts.MaskAllFieldValuesStrings field. WithMaskAllFieldValuesStrings(matchingStrings ...string) Option {
	return 
(l LoggerOpts) LoggerOpts {
		l.MaskAllFieldValuesStrings = append(l.MaskAllFieldValuesStrings, matchingStrings...)
		return l
	}
}// WithMaskMessageRegexes appends *regexp.Regexp to the LoggerOpts.MaskMessageRegexes field. WithMaskMessageRegexes(expressions ...*regexp.Regexp) Option {
	return 
(l LoggerOpts) LoggerOpts {
		l.MaskMessageRegexes = append(l.MaskMessageRegexes, expressions...)
		return l
	}
}// WithMaskMessageStrings appends string to the LoggerOpts.MaskMessageStrings field. WithMaskMessageStrings(matchingStrings ...string) Option {
	return 
(l LoggerOpts) LoggerOpts {
