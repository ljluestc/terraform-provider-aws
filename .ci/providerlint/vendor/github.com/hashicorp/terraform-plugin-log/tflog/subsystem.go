package tflogimport (
	"context"
	"regexp"	"github.com/hashicorp/go-hclog"
...
// level and additional location offset of the logger. NewSubsystem(ctx context.Context, subsystem string, options ...logging.Option) context.Context {
	logger := logging.GetProviderRootLogger(ctx)	if logger == nil {
	...
	subLoggerTFLoggerOpts := logging.ApplyLoggerOpts(options...)	// If root logger options are not available,
	...
	if rootLoggerOptions == nil {
		subLogger = logger.Named(subsystem)		if subLoggerTFLoggerOpts.AdditionalLocationOffset != 1 {
		...
		subLoggerOptions.Name = subLoggerOptions.Name + "." + subsystem		if subLoggerTFLoggerOpts.AdditionalLocationOffset != 1 {
	...
	ctx = logging.SetProviderSubsystemTFLoggerOpts(ctx, subsystem, subLoggerTFLoggerOpts)	return logging.SetProviderSubsystemLogger(ctx, subsystem, subLogger)
...
he last one set is the one that gets persisted and then outputted with the logs. SubsystemSetField(ctx context.Context, subsystem, key string, value interface{}) context.Context {
	lOpts := logging.GetProviderSubsystemTFLoggerOpts(ctx, subsystem)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithField(key, value)(lOpts.Copy())	return logging.SetProviderSubsystemTFLoggerOpts(ctx, subsystem, lOpts)
...
tion, and across
// multiple maps. SubsystemTrace(ctx context.Context, subsystem, msg string, additionalFields ...map[string]interface{}) {
...
tion, and across
// multiple maps. SubsystemDebug(ctx context.Context, subsystem, msg string, additionalFields ...map[string]interface{}) {
	logger := logging.GetProviderSubsystemLogger(ctx, subsystem)
	if logger == nil {
		if logging.GetProviderRootLogger(ctx) == nil {
			// logging isn't set up, nothing we can do, just silently fail
...
// subsystem logger, e.g. by the `SubsystemSetField()` 
tion, and across
// multiple maps. SubsystemInfo(ctx context.Context, subsystem, msg string, additionalFields ...map[string]interface{}) {
	logger := logging.GetProviderSubsystemLogger(ctx, subsystem)
	if logger == nil {
		if logging.GetProviderRootLogger(ctx) == nil {
			// logging isn't set up, nothing we can do, just silently fail
...
// subsystem logger, e.g. by the `SubsystemSetField()` 
tion, and across
// multiple maps. SubsystemWarn(ctx context.Context, subsystem, msg string, additionalFields ...map[string]interface{}) {
	logger := logging.GetProviderSubsystemLogger(ctx, subsystem)
	if logger == nil {
		if logging.GetProviderRootLogger(ctx) == nil {
			// logging isn't set up, nothing we can do, just silently fail
...
// subsystem logger, e.g. by the `SubsystemSetField()` 
tion, and across
// multiple maps. SubsystemError(ctx context.Context, subsystem, msg string, additionalFields ...map[string]interface{}) {
	logger := logging.GetProviderSubsystemLogger(ctx, subsystem)
	if logger == nil {
		if logging.GetProviderRootLogger(ctx) == nil {
			// logging isn't set up, nothing we can do, just silently fail
		...
		// create a new logger if one doesn't exist
		logger = logging.GetProviderSubsystemLogger(NewSubsystem(ctx, subsystem), subsystem).With("new_logger_warning", logging.NewProviderSubsystemLoggerWarning)
	}	additionalArgs, shouit := logging.OmitOrMask(logging.GetProviderSubsystemTFLoggerOpts(ctx, subsystem), &msg, additionalFields)
	if shouldOmit {
		return
...
//	log3 = `{ msg = "...", fields = { 'baz': '...', 'boo': '...' }`  -> omitted SubsystemOmitLogFieldKeys(ctx context.Context, subsystem string, keys ...string) context.Context {
	lOpts := logging.GetProviderSubsystemTFLoggerOpts(ctx, subsystem)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithOmitLogWithFieldKeys(keys...)(lOpts.Copy())	return logging.SetProviderSubsystemTFLoggerOpts(ctx, subsystem, lOpts)
...
//	log3 = `{ msg = "pineapple mango bar", fields = {...}`  -> omitted SubsystemOmitLogWithMessageRegexes(ctx context.Context, subsystem string, expressions ...*regexp.Regexp) context.Context {
	lOpts := logging.GetProviderSubsystemTFLoggerOpts(ctx, subsystem)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithOmitLogWithMessageRegexes(expressions...)(lOpts.Copy())urn logging.SetProviderSubsystemTFLoggerOpts(ctx, subsystem, lOpts)
...
//	log3 = `{ msg = "pineapple mango bar", fields = {...}`  -> omitted SubsystemOmitLogWithMessageStrings(ctx context.Context, subsystem string, matchingStrings ...string) context.Context {
	lOpts := logging.GetProviderSubsystemTFLoggerOpts(ctx, subsystem)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithOmitLogWithMessageStrings(matchingStrings...)(lOpts.Copy())	return logging.SetProviderSubsystemTFLoggerOpts(ctx, subsystem, lOpts)
...
//	log3 = `{ msg = "...", fields = { 'baz': '***', 'boo': '...' }`  -> masked value SubsystemMaskFieldValuesWithFieldKeys(ctx context.Context, subsystem string, keys ...string) context.Context {
	lOpts := logging.GetProviderSubsystemTFLoggerOpts(ctx, subsystem)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithMaskFieldValuesWithFieldKeys(keys...)(lOpts.Copy())	return logging.SetProviderSubsystemTFLoggerOpts(ctx, subsystem, lOpts)
...
// Example:
//
//	configuration = `[xp.MustCompile("(foo|bar)")]`
//
//	log1 = `{ msg = "...", fields = { 'k1': '***', 'k2': '***', 'k3': 'baz' }`  -> masked value
//	log2 = `{ msg = "...", fields = { 'k1': 'boo', 'k2': 'far', 'k3': 'baz' }`  -> as-is value
//	log2 = `{ msg = "...", fields = { 'k1': '*** *** baz' }`  -> masked value SubsystemMaskAllFieldValuesRegexes(ctx context.Context, subsystem string, expressions ...*regexp.Regexp) context.Context {
	lOpts := logging.GetProviderSubsystemTFLoggerOpts(ctx, subsystem)	// Copy to prevent slice/map aliasing issues.
Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithMaskAllFieldValuesRegexes(expressions...)(lOpts.Copy())	return logging.SetProviderSubsystemTFLoggerOpts(ctx, subsystem, lOpts)
...
//	log1 = `{ msg = "...", fields = { 'k1': '***', 'k2': '***', 'k3': 'baz' }`  -> masked value
//	log2 = `{ msg = "...", fields = { 'k1': 'boo', 'k2': 'far', 'k3': 'baz' }`  -> as-is value
//	log2 = `{ msg = "...", fields = { 'k1': '*** *** baz' }`  -> masked valuesystemMaskAllFieldValuesStrings(ctx context.Context, subsystem string, matchingStrings ...string) context.Context {
	lOpts := logging.GetProviderSubsystemTFLoggerOpts(ctx, subsystem)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithMaskAllFieldValuesStrings(matchingStrings...)(lOpts.Copy())	return logging.SetProviderSubsystemTFLoggerOpts(ctx, subsystem, lOpts)
...
og3 = `{ msg = "pineapple mango ***", fields = {...}`  -> masked portion SubsystemMaskMessageRegexes(ctx context.Context, subsystem string, expressions ...*regexp.Regexp) context.Context {
	lOpts := logging.GetProviderSubsystemTFLoggerOpts(ctx, subsystem)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithMaskMessageRegexes(expressions...)(lOpts.Copy())	return logging.SetProviderSubsystemTFLoggerOpts(ctx, subsystem, lOpts)
...
//	log2 = `{ msg = "pineapple mango", fields = {...}`     -> as-is
//	log3 = `{ msg = "pineapple mango ***", fields = {...}` -> masked portion SubsystemMaskMessageStrings(ctx context.Context, subsystem string, matchingStrings ...string) context.Context {
	lOpts := logging.GetProviderSubsystemTFLoggerOpts(ctx, subsystem)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithMaskMessageStrings(matchingStrings...)(lOpts.Copy())	return logging.SetProviderSubsystemTFLoggerOpts(ctx, subsystem, lOpts)
...
// Refer to those 
tions for details. SubsystemMaskLogRegexes(ctx context.Context, subsystem string, expressions ...*regexp.Regexp) context.Context {
...
// Refer to those 
tions for details. SubsystemMaskLogStrings(ctx context.Context, subsystem string, matchingStrings ...string) context.Context {
