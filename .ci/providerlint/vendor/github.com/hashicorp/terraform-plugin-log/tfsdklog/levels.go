package tfsdklogimport (
	"sync"	"github.com/hashicorp/go-hclog"
)var (
	...
	// request is over.
	rootLevel hclog.Level = hclog.NoLevel	// rootLevelMutex is a read-write mutex that protects rootLevel from
	// triggering the data race detector.
	rootLevelMutex = sync.RWMutex{}	// subsystemLevels stores the effective level of all subsystem SDK loggers
	...
	// concern in the case of this level causing more context checks until the
	// request is over.
	subsystemLevels map[string]hclog.Level = make(map[string]hclog.Level)	// subsystemLevelsMutex is a read-write mutex that protects the
...
// performance. subsystemWouldLog(subsystem string, level hclog.Level) bool {
	subsystemLevelsMutex.RLock()	setLevel, ok := subsystemLevels[subsystem]	subsystemLevelsMutex.RUnlock()	if !ok {
		return false
...
erformance. rootWouldLog(level hclog.Level) bool {
	rootLevelMutex.RLock()	setLevel := rootLevel	rootLevelMutex.RUnlock()	return wouldLog(setLevel, level)
}ouldLog returns true if the set level would emit a log at the given
// level. This is performed outside the context-based logger for performance. wouldLog(setLevel, checkLevel hclog.Level) bool {
