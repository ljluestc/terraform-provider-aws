...
// configured with the passed options. NewRootSDKLogger(ctx context.Context, options ...logging.Option) context.Context {
	...
	if sink := logging.GetSink(ctx); sink != nil {
		...
		sdkLoggerOptions.Name = opts.Name		if opts.Level != hclog.NoLevel {
			logger.SetLevel(opts.Level)
			sdkLoggerOptions.Level = opts.Level
		}		ctx = logging.SetSDKRootLogger(ctx, logger)
		ctx = logging.SetSDKRootLoggerOptions(ctx, sdkLoggerOptions)		return ctx
	}
	if opts.Level == hclog.NoLevel {
		opts.Level = hclog.Trace
	}	// Cache root logger level outside context for performance reasons.
	rootLevelMutex.Lock()	rootLevel = opts.Level	rootLevelMutex.Unlock()	loggerOptions := &hclog.LoggerOptions{
		Name:   opts.Name,
		Level:  opts.Level,
		JSONFormat:      true,
	...
	ctx = logging.SetSDKRootLoggerOptions(ctx, loggerOptions)	return ctx
}// NewRootProviderLogger returns a new context.Context that contains a provider
ogger configured with the passed options. NewRootProviderLogger(ctx context.Context, options ...logging.Option) context.Context {
	opts := logging.ApplyLoggerOpts(options...)
	if opts.Name == "" {
	...
	if sink := logging.GetSink(ctx); sink != nil {
		...
		providerLoggerOptions.Name = opts.Name		if opts.Level != hclog.NoLevel {
			logger.SetLevel(opts.Level)
			providerLoggerOptions.Level = opts.Level
		}		ctx = logging.SetProviderRootLogger(ctx, logger)
		ctx = logging.SetProviderRootLoggerOptions(ctx, providerLoggerOptions)		return ctx
	}
	if opts.Level == hclog.NoLevel {
		opts.Level = hclog.Trace
	}
	...
	ctx = logging.SetProviderRootLoggerOptions(ctx, loggerOptions)	return ctx
...
//
n case of the same key is used multiple times (i.e. key collision),
// the last one set is the one that gets persisted and then outputted with the logs. SetField(ctx context.Context, key string, value interface{}) context.Context {
	lOpts := logging.GetSDKRootTFLoggerOpts(ctx)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithField(key, value)(lOpts.Copy())	return logging.SetSDKRootTFLoggerOpts(ctx, lOpts)
...
// shallow merged with any defined on the logger, e.g. by the `SetField()` 
tion,
// and across multiple maps. Trace(ctx context.Context, msg string, additionalFields ...map[string]interface{}) {
...
// shallow merged with any defined on the logger, e.g. by the `SetField()` 
tion,
// and across multiple maps. Debug(ctx context.Context, msg string, additionalFields ...map[string]interface{}) {
	logger := logging.GetSDKRootLogger(ctx)
	if logger == nil {
		// this essentially should never happen in production the root
		// logger for  code should be injected by the  in
	...
	if shouldOmit {
		return
	}ger.Debug(msg, additionalArgs...)
}// Info logs `msg` at the info level to the logger in `ctx`, with optional
// `additionalFields` structured key-value fields in the log output. Fields are
// shallow merged with any defined on the logger, e.g. by the `SetField()` 
tion,
// and across multiple maps. Info(ctx context.Context, msg string, additionalFields ...map[string]interface{}) {
	logger := logging.GetSDKRootLogger(ctx)
	if logger == nil {
		// this essentially should never happen in production the root
		// logger for  code should be injected by the  in
...
// shallow merged with any defined on the logger, e.g. by the `SetField()` 
tion,
// and across multiple maps. Warn(ctx context.Context, msg string, additionalFields ...map[string]interface{}) {
	logger := logging.GetSDKRootLogger(ctx)
	if logger == nil {
		// this essentially should never happen in production the root
		// logger for  code should be injected by the  in
...
// shallow merged with any defined on the logger, e.g. by the `SetField()` 
tion,
// and across multiple maps. Error(ctx context.Context, msg string, additionalFields ...map[string]interface{}) {
	logger := logging.GetSDKRootLogger(ctx)
	if logger == nil {
		// this essentially should never happen in production the root
		// logger for  code should be injected by the  in
...
hat will omit to write any log when any of the given keys is found
// within its fields.
//
// Each call to this 
tion is additive:
...
//	log3 = `{ msg = "...", fields = { 'baz': '...', 'boo': '...' }`  -> omitted OmitLogWithFieldKeys(ctx context.Context, keys ...string) context.Context {
	lOpts := logging.GetSDKRootTFLoggerOpts(ctx)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithOmitLogWithFieldKeys(keys...)(lOpts.Copy())	return logging.SetSDKRootTFLoggerOpts(ctx, lOpts)
...
//	log3 = `{ msg = "pineapple mango bar", fields = {...}`  -> omitted OmitLogWithMessageRegexes(ctx context.Context, expressions ...*regexp.Regexp) context.Context {
	lOpts := logging.GetSDKRootTFLoggerOpts(ctx)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithOmitLogWithMessageRegexes(expressions...)(lOpts.Copy())	return logging.SetSDKRootTFLoggerOpts(ctx, lOpts)
...
//	log2 = `{ msg = "pineapple mango", fields = {...}`      -> printed
//	log3 = `{ msg = "pineapple mango bar", fields = {...}`  -> omitted OmitLogWithMessageStrings(ctx context.Context, matchingStrings ...string) context.Context {
	lOpts := logging.GetSDKRootTFLoggerOpts(ctx)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
ts = logging.WithOmitLogWithMessageStrings(matchingStrings...)(lOpts.Copy())	return logging.SetSDKRootTFLoggerOpts(ctx, lOpts)
...
//	log3 = `{ msg = "...", fields = { 'baz': '***', 'boo': '...' }`  -> masked value MaskFieldValuesWithFieldKeys(ctx context.Context, keys ...string) context.Context {
	lOpts := logging.GetSDKRootTFLoggerOpts(ctx)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
ts = logging.WithMaskFieldValuesWithFieldKeys(keys...)(lOpts.Copy())	return logging.SetSDKRootTFLoggerOpts(ctx, lOpts)
...
//	log2 = `{ msg = "...", fields = { 'k1': '*** *** baz' }`  -> masked value MaskAllFieldValuesRegexes(ctx context.Context, expressions ...*regexp.Regexp) context.Context {
	lOpts := logging.GetSDKRootTFLoggerOpts(ctx)Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithMaskAllFieldValuesRegexes(expressions...)(lOpts.Copy())	return logging.SetSDKRootTFLoggerOpts(ctx, lOpts)
...
//	log1 = `{ msg = "...", fields = { 'k1': '***', 'k2': '***', 'k3': 'baz' }`  -> masked value
//	log2 = `{ msg = "...", fields = { 'k1': 'boo', 'k2': 'far', 'k3': 'baz' }`  -> as-is value
//	log2 = `{ msg = "...", fields = { 'k1': '*** *** baz' }`  -> masked value MaskAllFieldValuesStrings(ctx context.Context, matchingStrings ...string) context.Context {
	lOpts := logging.GetSDKRootTFLoggerOpts(ctx)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithMaskAllFieldValuesStrings(matchingStrings...)(lOpts.Copy())	return logging.SetSDKRootTFLoggerOpts(ctx, lOpts)
...
og2 = `{ msg = "pineapple mango", fields = {...}`      -> as-is
//	log3 = `{ msg = "pineapple mango ***", fields = {...}`  -> masked portion MaskMessageRegexes(ctx context.Context, expressions ...*regexp.Regexp) context.Context {
	lOpts := logging.GetSDKRootTFLoggerOpts(ctx)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithMaskMessageRegexes(expressions...)(lOpts.Copy())	return logging.SetSDKRootTFLoggerOpts(ctx, lOpts)
...
//	log3 = `{ msg = "pineapple mango ***", fields = {...}` -> masked portion MaskMessageStrings(ctx context.Context, matchingStrings ...string) context.Context {
	lOpts := logging.GetSDKRootTFLoggerOpts(ctx)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithMaskMessageStrings(matchingStrings...)(lOpts.Copy())	return logging.SetSDKRootTFLoggerOpts(ctx, lOpts)
}// MaskLogRegexes is a shortcut to invoke MaskMessageRegexes and MaskAllFieldValuesRegexes using the same input.
// Refer to those 
tions for details. MaskLogRegexes(ctx context.Context, expressions ...*regexp.Regexp) context.Context {
...
// Refer to those 
tions for details. MaskLogStrings(ctx context.Context, matchingStrings ...string) context.Context {
