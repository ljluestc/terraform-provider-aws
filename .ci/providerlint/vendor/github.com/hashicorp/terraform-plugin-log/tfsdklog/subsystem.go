...
// level and additional location offset of the logger. NewSubsystem(ctx context.Context, subsystem string, options ...logging.Option) context.Context {
	logger := logging.GetSDKRootLogger(ctx)	if logger == nil {
	...
	subsystemLevelsMutex.Lock()	subsystemLevels[subsystem] = subLoggerTFLoggerOpts.Level	subsystemLevelsMutex.Unlock()	// Set the configured log level
	...
	ctx = logging.SetSDKSubsystemTFLoggerOpts(ctx, subsystem, subLoggerTFLoggerOpts)	return logging.SetSDKSubsystemLogger(ctx, subsystem, subLogger)
...
he last one set is the one that gets persisted and then outputted with the logs. SubsystemSetField(ctx context.Context, subsystem, key string, value interface{}) context.Context {
	lOpts := logging.GetSDKSubsystemTFLoggerOpts(ctx, subsystem)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithField(key, value)(lOpts.Copy())	return logging.SetSDKSubsystemTFLoggerOpts(ctx, subsystem, lOpts)
...
//	log3 = `{ msg = "...", fields = { 'baz': '...', 'boo': '...' }`  -> omitted SubsystemOmitLogFieldKeys(ctx context.Context, subsystem string, keys ...string) context.Context {
	lOpts := logging.GetSDKSubsystemTFLoggerOpts(ctx, subsystem)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithOmitLogWithFieldKeys(keys...)(lOpts.Copy())	return logging.SetSDKSubsystemTFLoggerOpts(ctx, subsystem, lOpts)
...
//	log3 = `{ msg = "pineapple mango bar", fields = {...}`  -> omitted SubsystemOmitLogWithMessageRegexes(ctx context.Context, subsystem string, expressions ...*regexp.Regexp) context.Context {
	lOpts := logging.GetSDKSubsystemTFLoggerOpts(ctx, subsystem)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithOmitLogWithMessageRegexes(expressions...)(lOpts.Copy())urn logging.SetSDKSubsystemTFLoggerOpts(ctx, subsystem, lOpts)
...
//	log3 = `{ msg = "pineapple mango bar", fields = {...}`  -> omitted SubsystemOmitLogWithMessageStrings(ctx context.Context, subsystem string, matchingStrings ...string) context.Context {
	lOpts := logging.GetSDKSubsystemTFLoggerOpts(ctx, subsystem)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithOmitLogWithMessageStrings(matchingStrings...)(lOpts.Copy())	return logging.SetSDKSubsystemTFLoggerOpts(ctx, subsystem, lOpts)
...
//	log3 = `{ msg = "...", fields = { 'baz': '***', 'boo': '...' }`  -> masked value SubsystemMaskFieldValuesWithFieldKeys(ctx context.Context, subsystem string, keys ...string) context.Context {
	lOpts := logging.GetSDKSubsystemTFLoggerOpts(ctx, subsystem)	// Copy to prevent slice/map aliasing issues.
Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithMaskFieldValuesWithFieldKeys(keys...)(lOpts.Copy())	return logging.SetSDKSubsystemTFLoggerOpts(ctx, subsystem, lOpts)
...
//	log2 = `{ msg = "...", fields = { 'k1': '*** *** baz' }`  -> masked value SubsystemMaskAllFieldValuesRegexes(ctx context.Context, subsystem string, expressions ...*regexp.Regexp) context.Context {
	lOpts := logging.GetSDKSubsystemTFLoggerOpts(ctx, subsystem)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithMaskAllFieldValuesRegexes(expressions...)(lOpts.Copy())	return logging.SetSDKSubsystemTFLoggerOpts(ctx, subsystem, lOpts)
...
//	log2 = `{ msg = "...", fields = { 'k1': '*** *** baz' }`  -> masked valuesystemMaskAllFieldValuesStrings(ctx context.Context, subsystem string, matchingStrings ...string) context.Context {
	lOpts := logging.GetSDKSubsystemTFLoggerOpts(ctx, subsystem)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithMaskAllFieldValuesStrings(matchingStrings...)(lOpts.Copy())	return logging.SetSDKSubsystemTFLoggerOpts(ctx, subsystem, lOpts)
...
og3 = `{ msg = "pineapple mango ***", fields = {...}`  -> masked portion SubsystemMaskMessageRegexes(ctx context.Context, subsystem string, expressions ...*regexp.Regexp) context.Context {
	lOpts := logging.GetSDKSubsystemTFLoggerOpts(ctx, subsystem)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithMaskMessageRegexes(expressions...)(lOpts.Copy())	return logging.SetSDKSubsystemTFLoggerOpts(ctx, subsystem, lOpts)
...
//	log3 = `{ msg = "pineapple mango ***", fields = {...}` -> masked portion SubsystemMaskMessageStrings(ctx context.Context, subsystem string, matchingStrings ...string) context.Context {
	lOpts := logging.GetSDKSubsystemTFLoggerOpts(ctx, subsystem)	// Copy to prevent slice/map aliasing issues.
	// Reference: https://github.com/hashicorp/terraform-plugin-log/issues/131
	lOpts = logging.WithMaskMessageStrings(matchingStrings...)(lOpts.Copy())	return logging.SetSDKSubsystemTFLoggerOpts(ctx, subsystem, lOpts)
