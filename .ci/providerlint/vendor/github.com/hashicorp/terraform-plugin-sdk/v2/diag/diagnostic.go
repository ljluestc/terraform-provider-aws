...
// SPDX-License-Identifier: MPL-2.0package diagimport (
	"errors"
...
// SDK.
type Diagnostics []Diagnostic// HasError returns true is Diagnostics contains an instance of
// Severity == Error.
//
...
// stack. (diags Diagnostics) HasError() bool {
	for i := range diags {
...
type Diagnostic struct {
	...
	Severity Severity	// Summary is a short description of the problem, rendered above location
	// information
	Summary string	// Detail is an optional second message rendered below location information
	// typically used to communicate a potential fix to the user.
	Detail string	// AttributePath is a representation of the path starting from the root of
...
}alidate ensures a valid Severity and a non-empty Summary are set. (d Diagnostic) Validate() error {
	var validSev bool
