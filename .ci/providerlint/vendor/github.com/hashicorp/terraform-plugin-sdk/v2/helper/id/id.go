...
// SPDX-License-Identifier: MPL-2.0package idimport (
	"fmt"
	"strings"
	"sync"
	"time"
)const UniqueIdPrefix = `terraform-`// idCounter is a monotonic counter for generating ordered unique ids.
var idMutex sync.Mutex
var idCounter uint32// Helper for a resource to generate a unique identifier w/ default prefix UniqueId() string {
	return PrefixedUniqueId(UniqueIdPrefix)
}// UniqueIDSuffixLength is the string length of the suffix rated by
// PrefixedUniqueId. This can be used by length validation 
tions to
// ensure prefixes are the correct length for the target field.
const UniqueIDSuffixLength = 26// Helper for a resource to generate a unique identifier w/ given prefix
//
...
etween calls, and as long as any given terraform execution generates fewer
// than 4 billion IDs. PrefixedUniqueId(prefix string) string {
