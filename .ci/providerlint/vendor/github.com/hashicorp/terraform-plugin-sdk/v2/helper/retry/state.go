...
// SPDX-License-Identifier: MPL-2.0package retryimport (
	...
	"log"
	"time"
)var refreshGracePeriod = 30 * time.Second// StateRefresh
 is a 
...
() (result interface{}, state string, err error)// StateChangeConf is the couration struct used for `WaitForState`.
type StateChangeConf struct {
 ...
 // Refreshes the current state
	...
	NotFoundChecks int     // Number of times to allow not found (nil result from Refresh)	// This is to work around inconsistent APIs
...
// listed in Pending, return immediately with an error.// If the Timeout is exceeded before reaching the Target state, return an
// error.
//
...
//
// Cancellation from the passed in context will cancel the refresh loop (conf *StateChangeConf) WaitForStateContext(ctx context.Context) (interface{}, error) {
	log.Printf("[DEBUG] Waiting for state to become: %s", conf.Target)	notfoundTick := 0
	targetOccurence := 0	// Set a default for times to check for not found
	...
	cancelCh := make(chan struct{})	result := Result{}	go 
() {
		defer close(resCh)		select {
		case <-time.After(conf.Delay):
		case <-cancelCh:
			return
		}		// start with 0 delay for the first loop
		var wait time.Duration		for {
			// store the last result
			resCh <- result			// wait and watch for cancellation
			select {
			case <-cancelCh:
				return
			...
			if res == nil && len(conf.Target) == 0 {
				...
				notfoundTick = 0
				found := false				for _, allowed := range conf.Target {
	...
	lastResult := Result{}	timeout := time.After(conf.Timeout)
	for {
		select {
		case r, ok := <-resCh:
		...
		case <-timeout:
			...
			log.Printf("[WARN] WaitForState starting %s refresh grace period", refreshGracePeriod)			// cancel the goroutine and start our grace period timer
			close(cancelCh)
			timeout := time.After(refreshGracePeriod)			// we need a for loop and a label to break on, because we may have
...
// waiting the number of seconds specified in the timeout configuration.
//
// Deprecated: Please use WaitForStateContext to ensure proper plugin shutdown (conf *StateChangeConf) WaitForState() (interface{}, error) {
