...
// SPDX-License-Identifier: MPL-2.0package schemaimport (
	...
	"strconv"
	"strings"
	"sync"	"github.com/mitchellh/mapstructure"	"github.com/hashicorp/terraform-plugin-sdk/v2/terraform"
...
type ConfigFieldReader struct {
	...
	Schema map[string]*Schema	indexMaps map[string]map[string]int
	once      sync.Once
}
 (r *CoFieldReader) ReadField(address []string) (FieldReadResult, error) {
	r.once.Do(
() { r.indexMaps = make(map[string]map[string]int) })
	return r.readField(address, false) (r *ConfigFieldReader) readField(
	...
	schema := schemaList[len(schemaList)-1]	switch schema.Type {
 ...
 (r *ConfigFieldReader) readMap(k string, schema *Schema) (FieldReadResult, error) {
	...
	case string:
		...
		// currently.
		v, _ := r.Config.Get(k)		// If this isn't a map[string]interface, it must be computed.
...
ists:   true,
 ...
 (r *ConfigFieldReader) readSet(
	...
	set := schema.ZeroValue().(*Set)	raw, err := readListField(&nestedConfigFieldReader{r}, address)
	if err != nil {
		return FieldReadResult{}, err
	}
	if !raw.Exists {
	...
	for i, v := range raw.Value.([]interface{}) {
		...
		computed := r.hasComputedSubKeys(
			fmt.Sprintf("%s.%d", strings.Join(address, "."), i), schema)		code := set.add(v, computed)
		indexMap[code] = i
	}	r.indexMaps[strings.Join(address, ".")] = indexMap	return FieldReadResult{
		Value:  set,
ists: true,
	}, nil
}// hasComputedSubKeys walks through a schema and returns whether or not the
// given key contains any subkeys that are computed. (r *ConfigFieldReader) hasComputedSubKeys(key string, schema *Schema) bool {
	prefix := key + "."	switch t := schema.Elem.(type) {
	case *Resource:
