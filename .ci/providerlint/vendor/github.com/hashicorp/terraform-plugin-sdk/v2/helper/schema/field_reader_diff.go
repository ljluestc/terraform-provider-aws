...
// SPDX-License-Identifier: MPL-2.0package schemaimport (
	"fmt"
	"strings"	"github.com/mitchellh/mapstructure"	"github.com/hashicorp/terraform-plugin-sdk/v2/terraform"
...
type DiffFieldReader struct {
	...
	Source FieldReader
	Schema map[string]*Schema	// cache for memoizing ReadField calls.
 ...
 (r *DiffFieldReader) ReadField(address []string) (FieldReadResult, error) {
	...
	var err error	schema := schemaList[len(schemaList)-1]
	switch schema.Type {
 ...
 (r *DiffFieldReader) readMap(
	...
	resultSet := false	// First read the map from the underlying source
	...
	if resultSet {
		...
		Value:  resultVal,
		Exists: resultSet,
	}, nil (r *DiffFieldReader) readPrimitive(
	address []string, schema *Schema) (FieldReadResult, error) {
 ...
 (r *DiffFieldReader) readSet(
	...
	address = append([]string(nil), address...)	prefix := strings.Join(address, ".") + "."	// Create the set that will be our result
	set := schema.ZeroValue().(*Set)	// Go through the map and find all the set items
	for k, d := range r.Diff.Attributes {
		...
		idx := parts[0]		raw, err := r.ReadField(append(address, idx))
		if err != nil {
			return FieldReadResult{}, err
		}
		if !raw.Exists {
