...
type MapFieldReader struct {
 ...
 (r *MapFieldReader) readMap(k string, schema *Schema) (FieldReadResult, error) {
	...
	resultSet := false	// If the name of the map field is directly in the map with an
...
(k, v string) bool {
		if strings.HasPrefix(k, prefix) {
			resultSet = true			key := k[len(prefix):]
			if key != "%" && key != "#" {
				result[key] = v
			}
		}		return true
...
ists: true,
 ...
 (r *MapFieldReader) readSet(
	...
	address = append([]string(nil), address...)	// Get the number of elements in the list
	...
	set := schema.ZeroValue().(*Set)	// If we have an empty list, then return an empty list
...
(k, _ string) bool {
		...
		idx := parts[0]		var raw FieldReadResult
		...
		}		set.Add(raw.Value)		// Due to the way multimap readers work, if we've seen the number
		...
		countActual[idx] = struct{}{}		return len(countActual) < countExpected
	})
