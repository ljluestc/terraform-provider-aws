...
// SPDX-License-Identifier: MPL-2.0package schemaimport (
	...
	"github.com/hashicorp/go-cty/cty/msgpack"	"github.com/hashicorp/terraform-plugin-go/tfprotov5"
...
e ctx is
// also passed to this 
tion and waited upon so no goroutine leak is caused. mergeStop(ctx context.Context, cancel context.Cancel
, stopCh chan struct{}) {
	select {
	case <-ctx.Done():
		return
...
// cancellation to the derived grpc context. (s *GRPCProviderServer) StopContext(ctx context.Context) context.Context {
	ctx = logging.InitContext(ctx)
	s.stopMu.Lock()
	defer s.stopMu.Unlock()	stoppable, cancel := context.WithCancel(ctx)
 ...
 (s *GRPCProviderServer) GetMetadata(ctx context.Context, req *tfprotov5.GetMetadataRequest) (*tfprotov5.GetMetadataResponse, error) {
	ctx = logging.InitContext(ctx)	logging.HelperSchemaTrace(ctx, "Getting provider metadata")	resp := &tfprotov5.GetMetadataResponse{
...
sp.Resources = append(resp.Resources, tfprotov5.ResourceMetadata{
 ...
 (s *GRPCProviderServer) GetProviderSchema(ctx context.Context, req *tfprotov5.GetProviderSchemaRequest) (*tfprotov5.GetProviderSchemaResponse, error) {
	ctx = logging.InitContext(ctx)	logging.HelperSchemaTrace(ctx, "Getting provider schema")	resp := &tfprotov5.GetProviderSchemaResponse{
		...
		logging.HelperSchemaTrace(ctx, "Found resource type", map[string]interface{}{logging.KeyResourceType: typ})		resp.ResourceSchemas[typ] = &tfprotov5.Schema{
		...
		logging.HelperSchemaTrace(ctx, "Found data source type", map[string]interface{}{logging.KeyDataSourceType: typ})sp.DataSourceSchemas[typ] = &tfprotov5.Schema{
 ...
 (s *GRPCProviderServer) PrepareProviderConfig(ctx context.Context, req *tfprotov5.PrepareProviderConfigRequest) (*tfprotov5.PrepareProviderConfigResponse, error) {
	ctx = logging.InitContext(ctx)
	resp := &tfprotov5.PrepareProviderConfigRese{}	logging.HelperSchemaTrace(ctx, "Preparing provider configuration")	schemaBlock := s.getProviderSchemaBlock()	configVal, err := msgpack.Unmarshal(req.Config.MsgPack, schemaBlock.ImpliedType())
	if err != nil {
...
(path cty.Path, val cty.Value) (cty.Value, error) {
		...
		tmpVal := hcl2shim.HCL2ValueFromConfigValue(def)		// helper/schema used to allow setting "" to a bool
	...
	if err := validateConfigNulls(ctx, configVal, nil); err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	}	config := terraform.NewResourceConfigShimmed(configVal, schemaBlock)ging.HelperSchemaTrace(ctx, "Calling downstream")
	resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, s.provider.Validate(config))
	logging.HelperSchemaTrace(ctx, "Called downstream")	preparedConfigMP, err := msgpack.Marshal(configVal, schemaBlock.ImpliedType())
	if err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	}	resp.PreparedConfig = &tfprotov5.DynamicValue{MsgPack: preparedConfigMP}	return resp, nil
}
 (s *GRPCProviderServer) ValidateResourceTypeConfig(ctx context.Context, req *tfprotov5.ValidateResourceTypeConfigRequest) (*tfprotov5.ValidateResourceTypeConfigResponse, error) {
	ctx = logging.InitContext(ctx)
	resp := &tfprotov5.ValidateResourceTypeConfigResponse{}	schemaBlock := s.getResourceSchemaBlock(req.TypeName)	configVal, err := msgpack.Unmarshal(req.Config.MsgPack, schemaBlock.ImpliedType())
	if err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	}	config := terraform.NewResourceConfigShimmed(configVal, schemaBlock)	logging.HelperSchemaTrace(ctx, "Calling downstream")
	resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, s.provider.ValidateResource(req.TypeName, config))
	logging.HelperSchemaTrace(ctx, "Called downstream")	return resp, nil
}
 (s *GRPCProviderServer) ValidateDataSourceConfig(ctx context.Context, req *tfprotov5.ValidateDataSourceConfigRequest) (*tfprotov5.ValidateDataSourceConfigResponse, error) {
	ctx = logging.InitContext(ctx)
	resp := &tfprotov5.ValidateDataSourceConfigResponse{}	schemaBlock := s.getDatasourceSchemaBlock(req.TypeName)	configVal, err := msgpack.Unmarshal(req.Config.MsgPack, schemaBlock.ImpliedType())
	if err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	// Ensure there are no nulls that will cause helper/schema to panic.
 ...
 (s *GRPCProviderServer) UpgradeResourceState(ctx context.Context, req *tfprotov5.UpgradeResourceStateRequest) (*tfprotov5.UpgradeResourceStateResponse, error) {
	ctx = logging.InitContext(ctx)
	resp := &tfprotov5.UpgradeResourceStateResponse{}	res, ok := s.provider.ResourcesMap[req.TypeName]
	if !ok {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, fmt.Errorf("unknown resource type: %s", req.TypeName))
		return resp, nil
	}
	schemaBlock := s.getResourceSchemaBlock(req.TypeName)	version := int(req.Version)	jsonMap := map[string]interface{}{}
	...
	case len(req.RawState.Flatmap) > 0:
		logging.HelperSchemaTrace(ctx, "Upgrading flatmap state")		jsonMap, version, err = s.upgradeFlatmapState(ctx, version, req.RawState.Flatmap, res)
		if err != nil {
			resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
			return resp, nil
		}
	...
	logging.HelperSchemaTrace(ctx, "Upgrading JSON state")	jsonMap, err = s.upgradeJSONState(ctx, version, jsonMap, res)
	if err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	}	// The provider isn't required to clean out removed fields
	s.removeAttributes(ctx, jsonMap, schemaBlock.ImpliedType())	// now we need to turn the state into the default json representation, so
 ...
 = objchange.NormalizeObjectFromLegacySDK(val, schemaBlock)	// encode the final state to the expected msgpack format
...
// version. (s *GRPCProviderServer) upgradeFlatmapState(_ context.Context, version int, m map[string]string, res *Resource) (map[string]interface{}, int, error) {
	// this will be the version we've upgraded so, defaulting to the given
	// version in case no migration was called.
	upgradedVersion := version	// first determine if we need to call the legacy MigrateState 	requiresMigrate := version < res.SchemaVersion	schemaType := res.CoreConfigSchema().ImpliedType()	// if there are any StateUpgraders, then we need to only compare
	...
	if len(res.StateUpgraders) > 0 {
		...
		// the ID
		m := is.Attributes
		m["id"] = is.ID		// if there are further upgraders, then we've only updated that far
		if len(res.StateUpgraders) > 0 {
			schemaType = res.StateUpgraders[0].Type
			upgradedVersion = res.StateUpgraders[0].Version
		}
...
tions
 ...
 (s *GRPCProviderServer) upgradeJSONState(ctx context.Context, version int, m map[string]interface{}, res *Resource) (map[string]interface{}, error) {
	var err error	for _, upgrader := range res.StateUpgraders {
...
// be correctly decoded. (s *GRPCProviderServer) removeAttributes(ctx context.Context, v interface{}, ty cty.Type) {
 ...
 (s *GRPCProviderServer) StopProvider(ctx context.Context, _ *tfprotov5.StopProviderRequest) (*tfprotov5.StopProviderResponse, error) {
	ctx = logging.InitContext(ctx)	logging.HelperSchemaTrace(ctx, "Stopping provider")	s.stopMu.Lock()
	defer s.stopMu.Unlock()	// stop
	close(s.stopCh)
	// reset the stop signal
	s.stopCh = make(chan struct{})	logging.HelperSchemaTrace(ctx, "Stopped provider")	return &tfprotov5.StopProviderResponse{}, nil
}
 (s *GRPCProviderServer) ConfigureProvider(ctx context.Context, req *tfprotov5.ConfigureProviderRequest) (*tfprotov5.ConfigureProviderResponse, error) {
	ctx = logging.InitContext(ctx)
	resp := &tfprotov5.ConfigureProviderResponse{}	schemaBlock := s.getProviderSchemaBlock()	configVal, err := msgpack.Unmarshal(req.Config.MsgPack, schemaBlock.ImpliedType())
	if err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	}rovider.TerraformVersion = req.TerraformVersion	// Ensure there are no nulls that will cause helper/schema to panic.
	if err := validateConfigNulls(ctx, configVal, nil); err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
...
tion. Ideally a provider should migrate to the context aware API that receives
	...
	ctxHack := context.WithValue(ctx, StopContextKey, s.StopContext(context.Background()))	logging.HelperSchemaTrace(ctx, "Calling downstream")
	diags := s.provider.Configure(ctxHack, config)
	logging.HelperSchemaTrace(ctx, "Called downstream")	resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, diags)	return resp, nil
}
 (s *GRPCProviderServer) ReadResource(ctx context.Context, req *tfprotov5.ReadResourceRequest) (*tfprotov5.ReadResourceResponse, error) {
	...
	schemaBlock := s.getResourceSchemaBlock(req.TypeName)	stateVal, err := msgpack.Unmarshal(req.CurrentState.MsgPack, schemaBlock.ImpliedType())
	if err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	...
	instanceState.RawState = stateVal	private := make(map[string]interface{})
	...
	instanceState.Meta = private	pmSchemaBlock := s.getProviderMetaSchemaBlock()
	...
	newInstanceState.Attributes["id"] = newInstanceState.ID	newStateVal, err := hcl2shim.HCL2ValueFromFlatmap(newInstanceState.Attributes, schemaBlock.ImpliedType())
	if err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	}	newStateVal = normalizeNullValues(newStateVal, stateVal, false)
	newStateVal = copyTimeoutValues(newStateVal, stateVal)	newStateMP, err := msgpack.Marshal(newStateVal, schemaBlock.ImpliedType())
	if err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	}	resp.NewState = &tfprotov5.DynamicValue{
 ...
 (s *GRPCProviderServer) PlanResourceChange(ctx context.Context, req *tfprotov5.PlanResourceChangeRequest) (*tfprotov5.PlanResourceChangeResponse, error) {
	ctx = logging.InitContext(ctx)
	resp := &tfprotov5.PlanResourceChangeResponse{}	res, ok := s.provider.ResourcesMap[req.TypeName]
	if !ok {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, fmt.Errorf("unknown resource type: %s", req.TypeName))
		return resp, nil
	}
	schemaBlock := s.getResourceSchemaBlock(req.TypeName)	// This is a signal to Terraform Core that we're doing the best we can to
	...
	if err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	}	create := priorStateVal.IsNull()	proposedNewStateVal, err := msgpack.Unmarshal(req.ProposedNewState.MsgPack, schemaBlock.ImpliedType())
	if err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	...
	}	priorState.Meta = priorPrivate	pmSchemaBlock := s.getProviderMetaSchemaBlock()
	if pmSchemaBlock != nil && req.ProviderMeta != nil {
		providerSchemaVal, err := msgpack.Unmarshal(req.ProviderMeta.MsgPack, pmSchemaBlock.ImpliedType())
		if err != nil {
			resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
	...
	cfg := terraform.NewResourceConfigShimmed(proposedNewStateVal, schemaBlock)	diff, err := res.SimpleDiff(ctx, priorState, cfg, s.provider.Meta())
	if err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	...
	plannedAttrs, err := diff.Apply(priorState.Attributes, schemaBlock)	if err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	...
	if err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	}	plannedStateVal = normalizeNullValues(plannedStateVal, proposedNewStateVal, false)	if err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	}	plannedStateVal = copyTimeoutValues(plannedStateVal, proposedNewStateVal)	// The old SDK code has some imprecisions that cause it to sometimes
 ...
 modified config fields are hidden.
	...
	}	newExtra := map[string]interface{}{}	for k, v := range diff.Attributes {
		if v.NewExtra != nil {
			newExtra[k] = v.NewExtra
		}
	}
	privateMap[newExtraKey] = newExtra	// the Meta field gets encoded into PlannedPrivate
	plannedPrivate, err := json.Marshal(privateMap)
	if err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	}
	resp.PlannedPrivate = plannedPrivate	// collect the attributes that require instance replacement, and convert
	// them to cty.Paths.
...
f d.RequiresNew {
 ...
 (s *GRPCProviderServer) ApplyResourceChange(ctx context.Context, req *tfprotov5.ApplyResourceChangeRequest) (*tfprotov5.ApplyResourceChangeResponse, error) {
	...
	schemaBlock := s.getResourceSchemaBlock(req.TypeName)	priorStateVal, err := msgpack.Unmarshal(req.PriorState.MsgPack, schemaBlock.ImpliedType())
	if err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	...
	destroy := false	// a null state means we are destroying the instance
	...
	if newExtra := private[newExtraKey]; newExtra != nil {
		...
		d.RequiresNew = false		// Check that any "removed" attributes that don't actually exist in the
	...
	resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, diags)	newStateVal := cty.NullVal(schemaBlock.ImpliedType())	// Always return a null value for destroy.
	...
	if err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	}	newStateVal = normalizeNullValues(newStateVal, plannedStateVal, true)	newStateVal = copyTimeoutValues(newStateVal, plannedStateVal)StateMP, err := msgpack.Marshal(newStateVal, schemaBlock.ImpliedType())
	if err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	}
	...
	resp.Private = meta	// This is a signal to Terraform Core that we're doing the best we can to
	// shim the legacy type system of the SDK onto the Terraform type system
	// but we need it to cut us some slack. This setting should not be taken
	// forward to any new SDK implementations, since setting it prevents us
	// from catching certain classes of provider bug that can lead to
 ...
 (s *GRPCProviderServer) ImportResourceState(ctx context.Context, req *tfprotov5.ImportResourceStateRequest) (*tfprotov5.ImportResourceStateResponse, error) {
	ctx = logging.InitContext(ctx)
	resp := &tfprotov5.ImportResourceStateResponse{}	info := &terraform.InstanceInfo{
		Type: req.TypeName,
	...
	if err != nil {
		...
		is.Attributes["id"] = is.ID		resourceType := is.Ephemeral.Type
		if resourceType == "" {
		...
		newStateVal = objchange.NormalizeObjectFromLegacySDK(newStateVal, schemaBlock)		// Ensure any timeouts block is null in the imported state. There is no
		...
		newStateType := newStateVal.Type()		if newStateVal != cty.NilVal && !newStateVal.IsNull() && newStateType.IsObjectType() && newStateType.HasAttribute(TimeoutsConfigKey) {
ewStateValueMap := newStateVal.AsValueMap()
 ...
 (s *GRPCProviderServer) ReadDataSource(ctx context.Context, req *tfprotov5.ReadDataSourceRequest) (*tfprotov5.ReadDataSourceResponse, error) {
	ctx = logging.InitContext(ctx)
	resp := &tfprotov5.ReadDataSourceResponse{}	schemaBlock := s.getDatasourceSchemaBlock(req.TypeName)	configVal, err := msgpack.Unmarshal(req.Config.MsgPack, schemaBlock.ImpliedType())
	if err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	}	// Ensure there are no nulls that will cause helper/schema to panic.
	if err := validateConfigNulls(ctx, configVal, nil); err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	}	config := terraform.NewResourceConfigShimmed(configVal, schemaBlock)	// we need to still build the diff separately with the Read method to match
	// the old behavior
	...
	if err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	}	newStateVal = copyTimeoutValues(newStateVal, configVal)	newStateMP, err := msgpack.Marshal(newStateVal, schemaBlock.ImpliedType())
	if err != nil {
		resp.Diagnostics = convert.AppendProtoDiag(ctx, resp.Diagnostics, err)
		return resp, nil
	}
 ...
 steps []tftypes.AttributePathStep	for _, step := range path {
		switch s := step.(type) {
...
// so that core doesn't see a perpetual diff with the timeout block. copyTimeoutValues(to cty.Value, from cty.Value) cty.Value {
	// if `is null we are planning to remove it altogether.
	if to.IsNull() {
		return to	toAttrs := to.AsValueMap()
	...
	timeouts, ok = fromAttrs[TimeoutsConfigKey]	// timeouts shouldn't be unknown, but don't copy possibly invalid values either
	if !ok || timeouts.IsNull() || !timeouts.IsWhollyKnown() {
		// no timeouts block to copy
		return cty.ObjectVal(toAttrs)
	}	toAttrs[TimeoutsConfigKey] = timeouts	return cty.ObjectVal(toAttrs)
...
// been applied. stripResourceModifiers(r *Resource) *Resource {
	if r == nil {
		return nil
	}
	// start with a shallow copy
	newResource := new(Resource)
	*newResource = *r	newResource.CustomizeDiff = nil
	newResource.Schema = map[string]*Schema{}	for k, s := range r.SchemaMap() {
 ...
 stripSchema(s *Schema) *Schema {
	...
	*newSchema = *s	newSchema.State
 = nil	switch e := newSchema.Elem.(type) {
	case *Schema:
...
// very uncommon nor was it reliable before 0.12 either. normalizeNullValues(dst, src cty.Value, apply bool) cty.Value {
	ty := dst.Type()
	...
	if ty.IsCollectionType() && apply {
		...
		srcEmpty := !src.IsNull() && src.IsKnown() && src.LengthInt() == 0		if (src.IsNull() && dstEmpty) || (srcEmpty && dst.IsNull()) {
			return src
		}
	...
	case ty.IsMapType(), ty.IsObjectType():
		...
		}		return cty.ObjectVal(dstMap)	case ty.IsSetType():
		...
		if srcLen == dstLen && srcLen > 0 {
			...
			dsts := dst.AsValueSlice()			for i := 0; i < srcLen; i++ {
...
// indicating the attribute with the null value. validateConfigNulls(ctx context.Context, v cty.Value, path cty.Path) []*tfprotov5.Diagnostic {
