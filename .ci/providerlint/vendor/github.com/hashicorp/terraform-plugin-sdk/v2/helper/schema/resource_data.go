...
type ResourceData struct {
	...
	providerMeta cty.Value	// Don't set
	multiReader *MultiLevelFieldReader
	setWriter   *MapFieldWriter
	newState    *terraform.InstanceState
	partial     bool
	once        sync.Once
	isNew       bool	panicOnError bool
...
// use GetOk. (d *ResourceData) Get(key string) interface{} {
	v, _ := d.GetOk(key)
	return v
...
ields which have a zero value of false. (d *ResourceData) GetChange(key string) (interface{}, interface{}) {
...
// The second result should be checked to determine this information. (d *ResourceData) GetOk(key string) (interface{}, bool) {
	r := d.getRaw(key, getSourceSet)
	exists := r.Exists && !r.Computed
	if exists {
		// If it exists, we also want to verify it is not the zero-value.
		value := r.Value
		zero := r.Schema.Type.Zero()		if eq, ok := value.(Equal); ok {
...
// no Default value have been set.// Deprecated: usage is discouraged due to undefined behaviors and may be
// removed in a future version of the SDK (d *ResourceData) GetOkExists(key string) (interface{}, bool) {
	r := d.getRaw(key, getSourceSet)
...
}// HasChanges returns whether or not any of the given keys has been changed. (d *ResourceData) HasChanges(keys ...string) bool {
...
//
// This 
tion only works with root attribute keys. (d *ResourceData) HasChangesExcept(keys ...string) bool {
	if d == nil || d.diff == nil {
		return false
	}
	for attr := range d.diff.Attributes {
		rootAttr := strings.Split(attr, ".")[0]
		var skipAttr bool		for _, key := range keys {
			if rootAttr == key {
				skipAttr = true
				break
			}
...
}// HasChange returns whether or not the given key has been changed.*ResourceData) HasChange(key string) bool {
	o, n := d.GetChange(key)	return !cmp.Equal(n, o)
}// HasChangeExcept returns whether any keys outside the given key have been changed.
//
// This 
tion only works with root attribute keys. (d *ResourceData) HasChangeExcept(key string) bool {
	if d == nil || d.diff == nil {
		return false
	}
	for attr := range d.diff.Attributes {
		rootAttr := strings.Split(attr, ".")[0]		if rootAttr == key {
			continue
...
// 
tion has been preserved for the specific usecase. (d *ResourceData) Partial(on bool) {
	d.partial = on
}// Set sets the value for the given key.
//
// If the key is invalid or the value is not a correct type, an error
// will be returned. (d *ResourceData) Set(key string, value interface{}) error {
	d.once.Do(d.init)	// If the value is a pointer to a non-struct, get its value and
...
og.Printf("[ERROR] setting state: %s", err)
 ...
 (d *ResourceData) IsNewResource() bool {
	return d.isNew
}// Id returns the ID of the resource. (d *ResourceData) Id() string {
	var result stringd.state != nil {
		result = d.state.ID
		if result == "" {
...
}// ConnInfo returns the connection info for this resource. (d *ResourceData) ConnInfo() map[string]string {
	if d.newState != nil {
...
esource is destroyed. (d *ResourceData) SetId(v string) {
	d.once.Do(d.init)
	d.newState.ID = v	// once we transition away from the legacy state types, "id" will no longer
be a special field, and will become a normal attribute.
	// set the attribute normally
	d.setWriter.unsafeWriteField("id", v)	// Make sure the newState is also set, otherwise the old value
	// may get precedence.
	if d.newState.Attributes == nil {
newState.Attributes = map[string]string{}
	}
	d.newState.Attributes["id"] = v
}// SetConnInfo sets the connection info for a resource. (d *ResourceData) SetConnInfo(v map[string]string) {
	d.once.Do(d.init)
	d.newState.Ephemeral.ConnInfo = v
}// SetType sets the ephemeral type for the data. This is only required
// for importing. (d *ResourceData) SetType(t string) {
	d.once.Do(d.init)
	d.newState.Ephemeral.Type = t
}// State returns the new InstanceState after the diff and any Set
// calls. (d *ResourceData) State() *terraform.InstanceState {
	var result terraform.InstanceState
	result.ID = d.Id()
	result.Meta = d.meta	// If we have no ID, then this resource doesn't exist and we just
	// return nil.
	if result.ID == "" {
		return nil
	}	if d.timeouts != nil {
	...
	if err := mapW.WriteField(nil, rawMap); err != nil {
		...
		return nil
	}	result.Attributes = mapW.Map()	if hasDynamicAttributes {
...
// Returns a duration of 20 minutes for any key not found, or not found and no default. (d *ResourceData) Timeout(key string) time.Duration {
	key = strings.ToLower(key)	// System default of 20 minutes
	defaultTimeout := 20 * time.Minute	if d.timeouts == nil {
 ...
 (d *ResourceData) init() {
	...
	d.newState = &copyState	// Initialize the map for storing set data
	d.setWriter = &MapFieldWriter{Schema: d.schema}	// Initialize the reader for getting data from the
...
!o.Exists {
 ...
 (d *ResourceData) get(addr []string, source getSource) getResult {
	d.once.Do(d.init)	var level string
...
tionality, and
// familiarity with the Terraform protocol is suggested when using it. (d *ResourceData) GetRawConfig() cty.Value {
...
tionality, and
// familiarity with the Terraform protocol is suggested when using it. (d *ResourceData) GetRawState() cty.Value {
...
tionality, and
// familiarity with the Terraform protocol is suggested when using it. (d *ResourceData) GetRawPlan() cty.Value {
