...
// SPDX-License-Identifier: MPL-2.0package schemaimport (
	"context"
	"encoding/json"	"github.com/hashicorp/go-cty/cty"
	ctyjson "github.com/hashicorp/go-cty/cty/json"	"github.com/hashicorp/terraform-plugin-sdk/v2/internal/configs/configschema"
...
// convert them to a state+diff required for the legacy Apply method. DiffFromValues(ctx context.Context, prior, planned, config cty.Value, res *Resource) (*terraform.InstanceDiff, error) {
...
// have already been done. diffFromValues(ctx context.Context, prior, planned, config cty.Value, res *Resource, cust CustomizeDiff
	...
	instanceState.RawState = prior	configSchema := res.CoreConfigSchema()	cfg := terraform.NewResourceConfigShimmed(planned, configSchema)
	removeConfigUnknowns(cfg.Config)
	removeConfigUnknowns(cfg.Raw)	diff, err := schemaMap(res.SchemaMap()).Diff(ctx, instanceState, cfg, cust, nil, false)
	if err != nil {
		return nil, err
	}	return diff, err
...
// need to be removed to prevent the UnknownVariableValue from appearing the diff. removeConfigUnknowns(cfg map[string]interface{}) {
	for k, v := range cfg {
		switch v := v.(type) {
		case string:
...
// PlanResourceChange method.lyDiff(base cty.Value, d *terraform.InstanceDiff, schema *configschema.Block) (cty.Value, error) {
	return d.ApplyToValue(base, schema)
}tateValueToJSONMap converts a cty.Value to generic JSON map via the cty JSON
// encoding. StateValueToJSONMap(val cty.Value, ty cty.Type) (map[string]interface{}, error) {
...
// to the specific type, ensuring that the values conform to the schema. JSONMapToStateValue(m map[string]interface{}, block *configschema.Block) (cty.Value, error) {
	var val cty.Value	js, err := json.Marshal(m)
	if err != nil {
		return val, err
...
// ID as the "id" attribute. StateValueFromInstanceState(is *terraform.InstanceState, ty cty.Type) (cty.Value, error) {
