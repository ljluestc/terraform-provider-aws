...
// SPDX-License-Identifier: MPL-2.0package hcl2shimimport (
	"fmt"
	"math/big"	"github.com/hashicorp/go-cty/cty"	"github.com/hashicorp/terraform-plugin-sdk/v2/internal/configs/configschema"
)// UnknownVariableValue is a sentinel value that can be used
// to denote that the value of a variable is unknown at this time.
// RawConfig uses this information to build up data about
// unknown keys.
const UnknownVariableValue = "74D93920-ED26-11E3-AC10-0800200C9A66"// ConfigValueFromHCL2Block is like ConfigValueFromHCL2 but it works only for
...
ubtle differences within its validation code. ConfigValueFromHCL2Block(v cty.Value, schema *configschema.Block) map[string]interface{} {
	if v.IsNull() {
		return nil
	}
	if !v.IsKnown() {
	...
	ret := make(map[string]interface{})	for name := range schema.Attributes {
		...
		if bv.IsNull() {
			continue
		}		switch blockS.Nesting {		case configschema.NestingSingle, configschema.NestingGroup:
			ret[name] = ConfigValueFromHCL2Block(bv, &blockS.Block)		case configschema.NestingList, configschema.NestingSet:
			l := bv.LengthInt()
			if l == 0 {
			...
			ret[name] = elems		case configschema.NestingMap:
			if bv.LengthInt() == 0 {
				// skip empty collections to better mimic how HCL1 would behave
				continue
...
// need to detect and reject any null values. ConfigValueFromHCL2(v cty.Value) interface{} {
	if !v.IsKnown() {
...
// a cty.Value so it can be used within, for example, an HCL2 EvalContext. HCL2ValueFromConfigValue(v interface{}) cty.Value {
	if v == nil {
