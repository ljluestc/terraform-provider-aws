...
// SPDX-License-Identifier: MPL-2.0package objchangeimport (
	"github.com/hashicorp/go-cty/cty"	"github.com/hashicorp/terraform-plugin-sdk/v2/internal/configs/configschema"
...
// returned from the provider over the RPC wire protocol anyway. NormalizeObjectFromLegacySDK(val cty.Value, schema *configschema.Block) cty.Value {
	...
	for name, blockS := range schema.BlockTypes {
		lv := val.GetAttr(name)		// Legacy SDK never generates dynamically-typed attributes and so our
...
// as a special case. unknownBlockStub(schema *configschema.Block) cty.Value {
	vals := make(map[string]cty.Value)
