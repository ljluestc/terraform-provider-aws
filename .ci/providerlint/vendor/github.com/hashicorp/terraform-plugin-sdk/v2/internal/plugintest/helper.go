...
// for use in other tests. AutoInitProviderHelper(ctx context.Context, sourceDir string) *Helper {
...
type Helper struct {
	baseDir string	// sourceDir is the dir containing the provider source code, needed
	// for tests that use fixture files.
	sourceDir     string
	terraformExec string	// execTempDir is created during DiscoverConfig to store any downloaded
	// binaries
	execTempDir string
...
or constructing a config in a different way. AutoInitHelper(ctx context.Context, sourceDir string) (*Helper, error) {
...
// automatically clean those up. InitHelper(ctx context.Context, config *Config) (*Helper, error) {
...
// left behind in the filesystem after the tests complete. (h *Helper) Close() error {
	if h.execTempDir != "" {
...
// delete it. (h *Helper) NewWorkingDir(ctx context.Context, t TestControl) (*WorkingDir, error) {
	dir, err := os.MkdirTemp(h.baseDir, "work")
	if err != nil {
		return nil, err
	}	ctx = logging.TestWorkingDirectoryContext(ctx, dir)	// symlink the provider source files into the config directory
	// e.g. testdata
	...
	if err != nil {
		return nil, err
	}	tf, err := tfexec.NewTerraform(dir, h.terraformExec)	if err != nil {
		return nil, fmt.Errorf("unable to create terraform-exec instance: %w", err)
	}	err = tf.SetDisablePluginTLS(true)	if err != nil {
		return nil, fmt.Errorf("unable to disable terraform-exec plugin TLS: %w", err)
	}	err = tf.SetSkipProviderVerify(true) // Only required for Terraform CLI 0.12.x	var mismatch *tfexec.ErrVersionMismatch
	...
	tfLogProvider := os.Getenv(EnvTfLogProvider)	if tfAccLog != "" && tfLogCore != "" {
		err = fmt.Errorf(
		...
		)		err := tf.SetLog(tfAccLog)		if err != nil {
		...
		)		err := tf.SetLogCore(tfLogCore)		if err != nil {
			logging.HelperResourceError(
				ctx,
		...
		)		err := tf.SetLogProvider(tfLogProvider)		if err != nil {
			logging.HelperResourceError(
				ctx,
	...
	}	var logPath, logPathEnvVar string	if tfAccLogPath != "" {
...
// working directory fails. (h *Helper) RequireNewWorkingDir(ctx context.Context, t TestControl) *WorkingDir {
	t.Helper()	wd, err := h.NewWorkingDir(ctx, t)
	if err != nil {
		t := testingT{t}
Fatalf("failed to create new working directory: %s", err)
	...
	return wd
}orkingDirectory returns the working directory being used when running tests. (h *Helper) WorkingDirectory() string {
	return h.baseDir
}// TerraformExecPath returns the location of the Terraform CLI executable that
// should be used when running tests. (h *Helper) TerraformExecPath() string {
