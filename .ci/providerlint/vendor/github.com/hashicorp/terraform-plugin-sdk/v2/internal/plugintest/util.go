...
// SPDX-License-Identifier: MPL-2.0package plugintestimport (
 ...
 symlinkFile(src string, dest string) error {
	err := os.Symlink(src, dest)	if err != nil {
		return fmt.Errorf("unable to symlink %q to %q: %w", src, dest, err)
	}	srcInfo, err := os.Stat(src)	if err != nil {
		return fmt.Errorf("unable to stat %q: %w", src, err)
	}	err = os.Chmod(dest, srcInfo.Mode())	if err != nil {
...
f file descriptors used. symlinkDirectoriesOnly(srcDir string, destDir string) error {
	...
	if err != nil {
	...
	}	dirEntries, err := os.ReadDir(srcDir)	if err != nil {
		...
		err := symlinkFile(srcPath, destPath)		if err != nil {
