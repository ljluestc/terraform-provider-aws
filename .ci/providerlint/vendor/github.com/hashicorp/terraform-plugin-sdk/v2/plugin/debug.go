...
// SPDX-License-Identifier: MPL-2.0package pluginimport (
	"context"
	"errors"
	"time"	"github.com/hashicorp/go-plugin"
...
// normal usage; Serve is the correct 
 for that. DebugServe(ctx context.Context, opts *ServeOpts) (ReattachConfig, <-chan struct{}, error) {
	reattachCh := make(chan *plugin.ReattachConfig)
	closeCh := make(chan struct{})	if opts == nil {
...
")
		...
		CloseCh: closeCh,
	}	go Serve(opts)	var config *plugin.ReattachConfig
	select {
...
tion with the ServeOpts Debug field instead. Debug(ctx context.Context, providerAddr string, opts *ServeOpts) error {
	if opts == nil {
		return errors.New("ServeOpts must be passed in with at least GRPCProvider
, GRPCProviderV6
, or Provider
")
	}	opts.Debug = true	if opts.ProviderAddr == "" {
		opts.ProviderAddr = providerAddr
	}	Serve(opts)	return nil
}
