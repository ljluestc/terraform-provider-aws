...
// SPDX-License-Identifier: MPL-2.0package pluginimport (
	"errors"
	"log"	hclog "github.com/hashicorp/go-hclog"
	"github.com/hashicorp/go-plugin"
	testing "github.com/mitchellh/go-testing-interface"	"github.com/hashicorp/terraform-plugin-go/tfprotov5"
...
() tfprotov6.ProviderServer// ServeOpts the configuras to serve a plugin.
...
tions when possible.
 ...
 GRPCProviderV6
	...
	Logger hclog.Logger	// Debug starts a debug server and controls its lifecycle, printing the
	...
	Debug bool	// TestConfig should only be set when the provider is being tested; it
	...
	TestConfig *plugin.ServeTestConfig	// Set NoLogOutputOverride to not override the log output with an hclog
	// adapter. This should only be used when running the plugin in
	// acceptance tests.
	NoLogOutputOverride bool	//TFLogSink is the testingor a test 
 that will turn on
	// the terraform-plugin-log logging sink.
	UseTFLogSink testing.T	// ProviderAddr is the address of the provider under test or debugging,
...
tion of the plugin. Serve(opts *ServeOpts) {
...
())
 ...
 tf5serverServe(opts *ServeOpts) error {
	var erveOpts []tf5server.ServeOpt	if opts.Debug {
		...
		reattachConfigCh := make(chan *plugin.ReattachConfig)		go 
() {
...
, tf5serveOpts...)
 ...
 tf6serverServe(opts *ServeOpts) error {
	var tf6serveOpts []tf6server.ServeOpt	if opts.Debug {
