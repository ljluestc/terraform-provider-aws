...
// SPDX-License-Identifier: MPL-2.0package terraformimport (
	...
	"strconv"
	"strings"
	"sync"	"github.com/hashicorp/go-cty/cty"	"github.com/hashicorp/terraform-plugin-sdk/v2/internal/configs/configschema"
	"github.com/hashicorp/terraform-plugin-sdk/v2/internal/configs/hcl2shim"
)// diffChangeType is an enum with the kind of changes a diff has planned.
type diffChangeType byteconst (
...
var multiVal = regexp.MustCompile(`\.(#|%)$`)// InstanceDiff is the diff of a resource from some state to another.
type InstanceDiff struct {
	mu    sync.Mutex
	Attributes     map[string]*ResourceAttrDiff
	Destroy        bool
	DestroyDeposed bool
	DestroyTainted bool	RawConfig cty.Value
	RawState  cty.Value
	RawPlan   cty.Value	// Meta is a simple K/V map that is stored in a diff and persisted to
...
*InstanceDiff) Lock()   { d.mu.Lock() } (d *InstanceDiff) Unlock() { d.mu.Unlock() }// ApplyToValue merges the receiver into the given base value, returning a
...
// legacy diff type to work with the new-style types. (d *InstanceDiff) ApplyToValue(base cty.Value, schema *configschema.Block) (cty.Value, error) {
...
// returning the new instance attributes.// This method is intended for shimming old subsystems that still use this
// legacy diff type to work with the new-style types. (d *InstanceDiff) Apply(attrs map[string]string, schema *configschema.Block) (map[string]string, error) {
	...
	result := map[string]string{}	if d.Destroy || d.DestroyDeposed || d.DestroyTainted {
		return result, nil
 ...
 (d *InstanceDiff) applyBlockDiff(path []string, attrs map[string]string, schema *configschema.Block) (map[string]string, error) {
	...
	for n, attrSchema := range schema.Attributes {
		var err error
		newAttrs, err := d.applyAttrDiff(append(path, n), attrs, attrSchema)		if err != nil {
			return result, err
	...
	for n, block := range schema.BlockTypes {
		...
		localBlockPrefix := localPrefix + n + "."		// we can only trust the diff for sets, since the path changes, so don't
		...
		if countDiff, ok := d.Attributes[countAddr]; ok {
			if countDiff.NewComputed {
				...
				result[localBlockPrefix+"#"] = countDiff.New				// While sets are complete, list are not, and we may not have all the
...
turn d.applySetDiff(path, attrs, attrSchema)
 ...
 (d *InstanceDiff) applySingleAttrDiff(path []string, attrs map[string]string, attrSchema *configschema.Attribute) (map[string]string, error) {
	currentKey := strings.Join(path, ".")	attr := path[len(path)-1]	result := map[string]string{}
	diff := d.Attributes[currentKey]
	old, exists := attrs[currentKey]	if diff != nil && diff.NewComputed {
	...
	if exists &&
		...
		result[attr] = hcl2shim.UnknownVariableValue
		return result, nil
	result[attr] = diff.New	return result, nil
}
 (d *InstanceDiff) applyCollectionDiff(path []string, attrs map[string]string, attrSchema *configschema.Attribute) (map[string]string, error) {
	result := map[string]string{}	prefix := ""
	if len(path) > 1 {
		prefix = strings.Join(path[:len(path)-1], ".") + "."
	}	name := ""
	if len(path) > 0 {
		name = path[len(path)-1]
	}	currentKey := prefix + name	// check the index first for special handling
	for k, diff := range d.Attributes {
	...
	length, _ := strconv.Atoi(count)	if count != "" && count != hcl2shim.UnknownVariableValue &&
	...
	if attrSchema.Type.IsListType() {
		for key := range result {
			if key == countKey {
				...
				continue
			}			index := key[len(name)+1:]			// It is possible to have nested sets or maps, so look for another dot
...
// trailing ".#", or ".%". countFlatmapContainerValues(key string, attrs map[string]string) string {
	if len(key) < 3 || !(strings.HasSuffix(key, ".#") || strings.HasSuffix(key, ".%")) {
		panic(fmt.Sprintf("invalid index value %q", key))
	}	prefix := key[:len(key)-1]
	items := map[string]int{}	for k := range attrs {
		if k == key {
			continue
		}
...
// for this single instance. (d *InstanceDiff) ChangeType() diffChangeType {
	if d.Empty() {
		return diffNone
...
}// Empty returns true if this diff encapsulates no changes. (d *InstanceDiff) Empty() bool {
	if d == nil {
		return true
	}u.Lock()
	defer d.mu.Unlock()
...
// TODO: investigate why removing this unused method causes panic in tests (d *InstanceDiff) Equal(d2 *InstanceDiff) bool {
 ...
 (d *InstanceDiff) GoString() string {
	return fmt.Sprintf("*%#v", InstanceDiff{
		...
		DestroyDeposed: d.DestroyDeposed,}// RequiresNew returns true if the diff requires the creation of a new
// resource (implying the destruction of the old). (d *InstanceDiff) RequiresNew() bool {
	if d == nil {
		return false
	}	d.mu.Lock()
	defer d.mu.Unlock()	return d.requiresNew()
}
*InstanceDiff) requiresNew() bool {
 ...
 (d *InstanceDiff) GetDestroyDeposed() bool {
	d.mu.Lock()
er d.mu.Unlock()	return d.DestroyDeposed
}
 (d *InstanceDiff) GetDestroyTainted() bool {
	d.mu.Lock()
	defer d.mu.Unlock()	return d.DestroyTainted
}
 (d *InstanceDiff) GetDestroy() bool {
	d.mu.Lock()
	defer d.mu.Unlock()	return d.Destroy
}
 (d *InstanceDiff) GetAttribute(key string) (*ResourceAttrDiff, bool) {
	d.mu.Lock()
	defer d.mu.Unlock()	attr, ok := d.Attributes[key]
	return attr, ok
}// Safely copies the Attributes map (d *InstanceDiff) CopyAttributes() map[string]*ResourceAttrDiff {
	d.mu.Lock()
	defer d.mu.Unlock()	attrs := make(map[string]*ResourceAttrDiff)
...
// isn't suddenly happening, etc. (d *InstanceDiff) Same(d2 *InstanceDiff) (bool, string) {
	...
	defer d.mu.Unlock()	// If we're going from requiring new to NOT requiring new, then we have
	...
	if oldNew && !newNew {
		oldNew = false		// This section builds a list of ignorable attributes for requiresNew
	...
	sort.StringSlice(keys).Sort()	for _, k := range keys {
		diffOld := d.Attributes[k]		if _, ok := checkOld[k]; !ok {
			// We're not checking this key for whatever reason (see where
			// check is modified).
			continue
		}		// Remove this key since we'll never hit it again
		delete(checkOld, k)
		delete(checkNew, k)		_, ok := d2.GetAttribute(k)
		if !ok {
			...
			if strings.Contains(k, "~") {
				...
				parts2 := append([]string(nil), parts...)				re := regexp.MustCompile(`^~\d+$`)
		...
		match := multiVal.FindStringSubmatch(k)		if diffOld.NewComputed && len(match) == 2 {
			matchLen := len(match[1])			// This is a computed list, set, or map, so remove any keys with
