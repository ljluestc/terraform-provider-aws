...
// SPDX-License-Identifier: MPL-2.0package terraformimport (
	...
	"github.com/mitchellh/reflectwalk"	"github.com/hashicorp/terraform-plugin-sdk/v2/internal/configs/configschema"
...
type InstanceInfo struct {
	...
	Id string	// ModulePath is the complete path of the module containing this
	// instance.
	ModulePath []string	// Type is the resource type of this instance
	Type string
...
// sequences as we might've seen in Terraform v0.11 and prior. NewResourceConfigRaw(raw map[string]interface{}) *ResourceConfig {
	v := hcl2shim.HCL2ValueFromConfigValue(raw)	// This is a little weird but we round-trip the value through the hcl2shim
	...
	cfg := hcl2shim.ConfigValueFromHCL2(v).(map[string]interface{})	return &ResourceConfig{
		Raw:    raw,
		Config: cfg,		ComputedKeys: newResourceConfigShimmedComputedKeys(v, ""),
	}
...
// schema then this 
tion will panic. NewResourceConfigShimmed(val cty.Value, schema *configschema.Block) *ResourceConfig {
	if !val.Type().IsObjectType() {
		panic(fmt.Errorf("NewResourceConfigShimmed given %#v; an object type is required", val.Type()))
	}
	ret := &ResourceConfig{}	legacyVal := hcl2shim.ConfigValueFromHCL2Block(val, schema)
	if legacyVal != nil {
		ret.Config = legacyVal		// Now we need to walk through our structure and find any unknown values,
...
// between the config and diff. newResourceConfigShimmedComputedKeys(val cty.Value, path string) []string {
	var ret []string
	ty := val.Type()	if val.IsNull() {
		return ret
	}	if !val.IsKnown() {
...
// affecting the original configuration. (c *ResourceConfig) DeepCopy() *ResourceConfig {
...
ult := copiedConfig.(*ResourceConfig)	return result
}// Equal checks the equality of two resource configs. (c *ResourceConfig) Equal(c2 *ResourceConfig) bool {
	...
	sort.Strings(c2.ComputedKeys)	// Two resource configs if their exported properties are equal.
...
// with IsComputed. (c *ResourceConfig) Get(k string) (interface{}, bool) {
...
// from the raw, uninterpolated config.// The second return value is true if the get was successful. Get will
// not succeed if the value is being computed. (c *ResourceConfig) GetRaw(k string) (interface{}, bool) {
	return c.get(k, c.Raw)
}// IsComputed returns whether the given key is computed or not. (c *ResourceConfig) IsComputed(k string) bool {
...
// must be some kind of interface implementation (w *unknownCheckWalker) Primitive(v reflect.Value) error {
