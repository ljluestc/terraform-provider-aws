...
// parseResourceAddress. (f *stateFilter) filter(fs ...string) ([]*stateFilterResult, error) {
	// Parse all the addresses
	var as []*resourceAddress	if len(fs) == 0 {
 ...
 (f *stateFilter) filterSingle(a *resourceAddress) []*stateFilterResult {
	...
	var results []*stateFilterResult	// Go through modules first.
	modules := make([]*ModuleState, 0, len(f.State.Modules))
	for _, m := range f.State.Modules {
		if f.relevant(a, m) {
			modules = append(modules, m)			// Only add the module to the results if we haven't specified a type.
...
}// relevant checks for relevance of this address against the given value. (f *stateFilter) relevant(addr *resourceAddress, raw interface{}) bool {
	switch v := raw.(type) {
	case *ModuleState:
		path := v.Path[1:]		if len(addr.Path) > len(path) {
...
type stateFilterResult struct {
	...
	Path []string	// Address is the address that can be used to reference this exact result.
	Address string	// Parent, if non-nil, is a parent of this result. For instances, the
	// parent would be a resource. For resources, the parent would be
	// a module. For modules, this is currently nil.
	Parent *stateFilterResult	// Value is the actual value. This must be type switched on. It can be
...
type stateFilterResultSlice []*stateFilterResult
 (s stateFilterResultSlice) Len() int      { return len(s) } (s stateFilterResultSlice) Swap(i, j int) { s[i], s[j] = s[j], s[i] } (s stateFilterResultSlice) Less(i, j int) bool {
	a, b := s[i], s[j]	// if these address contain an index, we want to sort by index rather than name
