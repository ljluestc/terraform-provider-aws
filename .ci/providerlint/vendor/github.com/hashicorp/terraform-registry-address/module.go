...
type Module struct {
	...
	Package ModulePackage	// If Subdir is non-empty then it represents a sub-directory within the
...
const DefaultModuleRegistryHost = svchost.Hostname("registry.terraform.io")var moduleRegistryNamePattern = regexp.MustCompile("^[0-9A-Za-z](?:[0-9A-Za-z-_]{0,62}[0-9A-Za-z])?$")
var moduleRegistryTargetSystemPattern = regexp.MustCompile("^[0-9a-z]{1,64}$")// ParseModuleSource only accepts module registry addresses, and
// will reject any other address type. ParseModuleSource(raw string) (Module, error) {
	var err error	var subDir string
...
t returns an error. MustParseModuleSource(raw string) (Module) {
...
// "namespace" or "name" position of a module registry source address. parseModuleRegistryName(given string) (string, error) {
...
// provider addresses have three slash-separated components of their own. parseModuleRegistryTargetSystem(given string) (string, error) {
	// Similar to the names in provider source addresses, we defined these
	// to be compatible with what filesystems and typical remote systems
	// like GitHub allow in names. Unfortunately we didn't end up defining
	...
	// as we do in ParseProviderPart.	if !moduleRegistryTargetSystemPattern.MatchString(given) {
...
// user-written addresses.// We typically use this longer representation in error message, in case
// the inclusion of normally-omitted components is helpful in debugging
// unexpected behavior. (s Module) String() string {
	if s.Subdir != "" {
...
// such as the note that we're about to start downloading a package. (s Module) ForDisplay() string {
	if s.Subdir != "" {
...
// just be returned verbatim in packageAddr, with an empty subDir value. splitPackageSubdir(given string) (packageAddr, subDir string) {
...
//   proto://dom.com/path//path2?q=p => proto://dom.com/path?q=p, "path2" sourceDirSubdir(src string) (string, string) {
	...
	subdir := src[idx+2:]
	src = src[:idx]	// Next, check if we have query parameters and push them onto the
	// URL.
