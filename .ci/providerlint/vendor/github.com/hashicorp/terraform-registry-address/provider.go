...
// SPDX-License-Identifier: MPL-2.0package tfaddrimport (
	"fmt"
	"strings"	svchost "github.com/hashicorp/terraform-svchost"
...
const DefaultProviderRegistryHost = svchost.Hostname("registry.terraform.io")// BuiltInProviderHost is the pseudo-hostname used for the "built-in" provider
...
const BuiltInProviderHost = svchost.Hostname("terraform.io")// BuiltInProviderNamespace is the provider namespace used for "built-in"
...
const BuiltInProviderNamespace = "builtin"// UnknownProviderNamespace is the special string used to indicate
...
const UnknownProviderNamespace = "?"// LegacyProviderNamespace is the special string used in the Namespace field
...
const LegacyProviderNamespace = "-"// String returns an FQN string, indended for use in machine-readable output. (pt Provider) String() string {
	if pt.IsZero() {
...
he provider is using the default hostname, the hostname is omitted. (pt Provider) ForDisplay() string {
	if pt.IsZero() {
...
// ParseProviderPart first to check that the given value is valid. NewProvider(hostname svchost.Hostname, namespace, typeName string) Provider {
...
// refactoring, this 
tion panics if the Provider is not a legacy provider. (pt Provider) LegacyString() string {
	if pt.IsZero() {
...
uch a value is likely to either panic or otherwise misbehave. (pt Provider) IsZero() bool {
	return pt == Provider{}
}// HasKnownNamespace returns true if the provider namespace is known
// (also if it is legacy namespace) (pt Provider) HasKnownNamespace() bool {
...
// ady be available without any further installation. Provider) IsBuiltIn() bool {
...
// to change in future. Provider) LessThan(other Provider) bool {
	switch {
...
}// IsLegacy returns true if the provider is a legacy-style provider (pt Provider) IsLegacy() bool {
	if pt.IsZero() {
		panic("called IsLegacy() on zero-value addrs.Provider")
	}	return pt.Hostname == DefaultProviderRegistryHost && pt.Namespace == LegacyProviderNamespace}// Equals returns true if the receiver and other provider have the same attributes. (pt Provider) Equals(other Provider) bool {
urn pt == other
...
// requiring further identification of the namespace via Registry API ParseProviderSource(str string) (Provider, error) {
	var ret Provider
	...
	ret.Type = name
	ret.Hostname = DefaultProviderRegistryHost	if len(parts) == 1 {
		return Provider{
...
// it returns an error. MustParseProviderSource(raw string) (Provider) {
...
// hostname/namespace/name. ValidateProviderAddress(raw string) error {
 ...
 parseSourceStringParts(str string) ([]string, error) {
	...
	parts[len(parts)-1] = name	return parts, nil
...
// Additionally, dashes may not appear at the start or end of the string.// These restrictions are intended to allow these names to appear in fussy
...
// subsequent call, in which case the result will be identical. ParseProviderPart(given string) (string, error) {
	if len(given) == 0 {
...
// it returns an error. MustParseProviderPart(given string) string {
