...
odeCustomValue(e *Encoder, v reflect.Value) error {switch v.Kind() {case reflect.Chan, reflect.
flect.Interface, reflect.Map, reflect.Ptr, reflect.Slice:if v.IsNil() {return e.EncodeNil()}}encoder := v.Interface().(CustomEncoder)return encoder.EncodeMsgpack(e)}
...
shalValue(e *Encoder, v reflect.Value) error {switch v.Kind() {case reflect.Chan, reflect.
flect.Interface, reflect.Map, reflect.Ptr, reflect.Slice:if v.IsNil() {return e.EncodeNil()}}marshaler := v.Interface().(Marshaler)b, err := marshaler.MarshalMsgpack()if err != nil {return err}_, err = e.w.Write(b)return err}
