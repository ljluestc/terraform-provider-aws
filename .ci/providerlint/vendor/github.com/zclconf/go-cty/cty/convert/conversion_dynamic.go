...
// dealing with any inconsistencies during final evaluation.
 ...
 dynFixup(wantType cty.Type) conversion {
	return 
...
arget type is DynamicPseudoType, indicating that the caller doesn't care
// which type is returned.

 dynamicPassthrough(in cty.Value, path cty.Path) (cty.Value, error) {
	return in, nil
}

// dynamicReplace aims to return the out type unchanged, but if it finds a
// dynamype either directly or in any descendent elements it replaces them
// with the equivalent type from in.
//
// This 
 assumes that in and out are compatible from a Convert
// perspective, and will panic if it finds that they are not. For example if
// in is an object and out is a map, this 
tion will still attempt to iterate
// through both as if they were the same.

 dynamicReplace(in, out cty.Type) cty.Type {
