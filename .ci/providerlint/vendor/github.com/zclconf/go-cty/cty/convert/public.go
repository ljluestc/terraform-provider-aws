...
// Conversion is a named 
tion type representing a conversion from a
// value of one type to a value of another type.
//
// The ce type for a conversion is always the source type given to
// the 
tion that returned the Conversion, but there is no way to recover
// that from a Conversion value itself.a Conversion is given a value
// that is not of its expected type (with the exception of DynamicPseudoType,
// which is alwaupported) then the 
tion may panic or produce undefined
// results.
type Conversion 
cty.Value) (out cty.Value, err error)

// GetConversion returns a Conversion between the given in and out Types if
// a safe one is available, or returns nil otherwise.

 GetConversion(in cty.Type, out cty.Type) Conversion {
urn retConversion(getConversion(in, out, false))
}
...
onvert returns the result of converting the given value to the given type
...
// immediately passing the given value to the resulting 
tion.

 Convert(in cty.Value, want cty.Type) (cty.Value, error) {
...
//
// If no common supertype can be found, this 
tion returns cty.NilType and
// a nil slice.
//
...
ompatible. If *all* given types are DynamicPseudoType, or in the
...
// cty.DynamicPseudoType and no conversions are attempted.
 ...
 Unify(types []cty.Type) (cty.Type, []Conversion) {
...
// conversions in situations where a safe conversion isn't also available.
 ...
 UnifyUnsafe(types []cty.Type) (cty.Type, []Conversion) {
