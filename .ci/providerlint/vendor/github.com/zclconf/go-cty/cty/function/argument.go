...
// Parameter represents a parameter to a 
tion.
...
// cty.DynamicPseudoType can be used, either at top-leor nested
// in a parameterized type, to indicate that any type should be
// permitted, to allow the definition of type-generic 
tions.
Type cty.Type

// If AllowNull is set then null values may be passed into this
// argument's slot in both the type-check 
tion and the implementation
// 
tion. If not set, such values are rejd by the built-in
// checking rules.
AllowNull bool

// If AllowUnknown is set then unknown values may be passed into this
// argument's slot in the implementation 
tion. If not set, any
// unknown values will cause the 
tion to immediately return
// an unkonwn value without calling the implementation 
tion, thus
// freeing the 
tion implementer from dealing with this case.
AllowUnknown bool

// If AllowDynamicType is set then DynamicVal may be passed into this
// argument's slot in the implementation 
tion. If not set, any
// dynamic values will cause the 
tion to immediately return
// DynamicVal value without calling the implementation 
tion, thus
// freeing the 
tion implementer from dealing with this case.
//
// Note that DynamicVal is also unknown, so in order to receive dynamic
// *values* it is also necessary to set AUnknown.
//
// However, it is valid to set AllowDynamicType without wUnknown, in
// which case a dynamic value may be passed to the type checking 
t
// but will not make it to the *implementation* 
tion. Instead, an
// own value of the type returned by the type-check 
tion will be
// returned. This is suggested for 
tions that have a static return
