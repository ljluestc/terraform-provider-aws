...
// Bytes is a capsule type that can be used with the binary 
tions to
...
// BytesLe a 
tion that returns the length of the buffer encapsulated
// in a Bytes value.
var BytesLen
 = 
tion.New(&
tion.Spec{
Description: `rns the total number of bytes in the given buffer.`,
Params: []
tiarameter{
{
Name:             "buf",
...
// BytesSlice is a 
tion that returns a slice of the given Bytes value.
var BytesSlice
 = 
tion.New(&
tion.Spec{
...
set %d is greater than total buffer length %d",
offset, len(*bu),
)
}
...
return BytesVal((*bufPtr)[offset:end]), nil
 ...
 BytesLen(buf cty.Value) (cty.Value, error) {
return BytesLen
.Call([]cty.Value{buf})
}


 BytesSlice(buf cty.Value, offset cty.Value, length cty.Value) (cty.Value, error) {
return BytesSlice
.Call([]cty.Value{buf, offset, length})
}
