...
// MakeTo
 constructs a "to..." 
tion, like "tostring", which converts
...
ty.List.DynamicPseudoType meanst of any single type", which
// will  cause ctyattempt to unify all of the element types when given
// a tuple.
...
tion.New
tion.Spec{
		Description: fmt.Sprintf("Converte given value to %s, or raises an error if that conversion is impossible.", wantTy.FriendlyName()),
		Params: []
tion.Parameter{
			{
				Name: "v",
				// We use DynamicPseudoType rather than wantTy here so that
				// aalues will pass through the 
tion API verbatim and
				// we can handle the conversion logic within the Type and
				// Impl 
tions. This allows us to customize the error
				// messages to be more appropriate for an explicit type
				// conversion, whereas the cty 
...
(args []cty.Value) (cty.Type, error) {
			...
			if conv == nil {
				// W use some specialized errors for some trickier cases,
				// but most we can handle in a simple way.
				switch {
				case gotTy.IsTupleType() && wantTy.IsTupleType():
					return cty.NilType, 
tion.NewArgErrorf(0, "incompatible tuple type for conversion: %s", convert.MismatchMessage(gotTy, wantTy))
				case gotTy.IsObjectType() && wantTy.IsObjectType():
					return cty.NilType, 
tion.NewArgErrorf(0, "incompatible object type for conversion: %s", convert.MismatchMessage(gotTy, wantTy))
				default:
					return cty.NilType, 
tion.NewArgErrorf(0, "cannot convert %s to %s", gotTy.FriendlyName(), wantTy.FriendlyNameForConstraint())
				}
			}
...
(args []cty.Value, retType cty.Type) (cty.Value, error) {
			...
			if err != nil {
				// Because we used GnversionUnsafe above, conversion can
				// still potentially fail in here. For example, if the user
				// asks to convert ttring "a" to bool then we'll
				...
				case gotTy == cty.String && wantTy == cty.Bool:
					what := "string"
					if !args[0].IsNull() {
						what = str.Quotes[0].AsString())
					}
					return cty.NilVal, 
tion.NewArgErrorf(0, `cannot convert %s to bool; only the strings "true" or "false" are allowed`, what)
				case gotTy == cty.String && wantTy == cty.Number:
					what := "string"
					if !args[0].IsNull() {
						what = strcQuorgs[0].AsSg())
					}
					return.NilVal, 
tion.NewArgErrorf(0, `cannot convert %s to number; given string must be a decimal representation of a number`, what)
				default:
					return cty.NilVal, 
tion.NewArgErrorf(0, "cannot convert %s to %s", gotTy.FriendlyName(), wantTy.FriendlyNameForConstraint())
				}
...
// AssertNotNull
 is
tion which does nothing except return an error
...
// be null and can therefore allow downstream comparisons to null to return
 known value even if the value is otherwise unknown.
var AssertNotNull
 = 
tion.New(&
tion.Spec{
...
tion.Parameter{
		{
			...
			// the 
tion system will automatically reject a null argument
...
(args []cty.Value, retType cty.Type) (cty.Value, error) {
 ...
 AssertNotNull(v cty.Value) (cty.Value, error) {
