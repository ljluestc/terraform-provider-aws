...
tion.Parameter{
		{
			Name:             "first_set",
			Type:             cty(cty.DynamicPseudoType),
			AllowDynamicType: true,
		},
	},
	VarParam: &
tion.Parameter{
	...
	Impl: setOperationImpl(
(s1, s2 cty.ValueSet) cty.ValueSet {
		return s1.n(s2)
	}, true),
})
...
tion.New(&
tion.Spec{
	DescriptioReturns the relative complement of the two given sets.`,
	Params: []
tion.Parameter{
		{
			Name:             "a",
...
// element.
 ...
 SetHasElement(set cty.Value, elem cty.Value) (cty.Value, error) {
	return SetHasElement
.Call([]cty.Value{set, elem})
}
...
ommon type using the standard type unification rules. If conversion
// is not possible,error is returned.
//
...
// At least one set must be provided.
 ...
 SetUnion(sets ...cty.Value) (cty.Value, error) {
	return SetUnion
.Call(sets)
}

// Intersection returns a new containing the elements that exist
...
f conversion is not possible, an error is returned.
//
...
// At least one set must be provided.
 ...
 SetIntersection(sets ...cty.Value) (cty.Value, error) {
...
// result in some previously-distinct values being conflated.
 ...
 SetSubtract(a, b cty.Value) (cty.Value, error) {
	return SetSubtract
.Call([]cty.Value{a, b})
}

// SetSymmetricDifference returns a new set containing elements that appear
n any of the given sbut not multiple. The sets must have
// elemeypes that can all be converted to some common type using the
// standard type unification rules. If conversion is not possible, an error
// is returned.
//
// The difference opern is performed after type conversion, which may
// result in some previously-distinct values being conflated.

 SetSymmetrfference(sets ...cty.Value) (cty.Value, error) {
	return SetSymmetricDifference
.Call(sets)
}


 setOperationReturnType(args []cty.Value) (ret cty.Type, err error) {
	var etys []cty.Type
	for _, arg := range args {
		ty := arg.Type().ElementType()

		// Do not unify types for empty dynamic pseudo typed collections. These
		// will always conver any other concrete type.
 ...
 setOperationImpl(f 
(s1, s2 cty.ValueSet) cty.ValueSet, allowUnknowns bool) 
tion.Impl
 {
	return 
...
tion.NewArgError(0, err)
		...
		if !allowUnknowns && !first.IsWhollyKnown() {
			// This set 
tion can produce a correct result only when all
