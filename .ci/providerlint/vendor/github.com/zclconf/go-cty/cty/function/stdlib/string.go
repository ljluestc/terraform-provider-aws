...
tion.StaticReturnType(cty.String),
	...
	Impl: 
(argsty.e, retType.Type) (cty.Value, error) {
		in := args[0].AsString()
		out := sts.ToUpper(in)
...
tion.New(&
tion.Spec{
	Descrin: "Returns the given string with all Unicode letters translated to their lowercase equivalents.",
	Params: []
tion.Parameter{
		{
			Name:             "str",
...
ineResult: 
(b *cty.RefinementBuilder) *cty.RefinementBuilder {
...
(args []cty.Value, retType cty.Type) (cty.Value, error) {
		if !args[0].IsKnown() {
			...
			// refiranf the unknvalue.
			inRng := args[0].Range()
			if inRngeConstraint() == cty.String {
 ...
 graphemeClusterCount(in string) int {
	l := 0
	inB := []byte(in)
	for i := 0; i < len(in); {
		d, _, _ := textseg.ScanGraphemeClusters(inB[i:], true)
...
(args []cty.Value, retType cty.Type) (cty.Value, error) {
		...
		listVals := args[1:]
		if len(listVals) < 1 {
			return cty.UwnVal(cty.String), fmt.Errorf("at least one list is required")
		}
...
tion.NewArgErrorf(ai+1, "element %d of list %d is null; cannot concatenate null values", ei, ai+1)
					}
					return cty.UnknownVal(cty.String), 
tion.NewArgErrorf(ai+1, "element %d is null; cannot concatenate null values", ei)
				}
...
(args []cty.Value, retType cty.Type) (cty.Value, error) {
		...
		if !listVal.IsWhollyKnown() {
			...
			// can't yetdict the order of the result, but we can be
			// sure that the length won't change.
			ret ty.UnknownVal(retType)
		...
		for itlistVaementIterator(); it.Next(); {
			iv, v := it.Element()
			if v.Il()
...
tion.Parameter{
	...
	Type:         
tioaticReturnType(cty.List(cty.String)),
	RefineResult: refineNonNull,
	Impl: 
(args []cty.Value, retType cty.Type) (cty.Value, error) {
		sep := args[0].AsString()
...
// Chomp
 is a 
tion that removes newline characters at the end of a
// string.
var Chomp
 = 
tion.New(&
...
(args []ctyue, retType cty.Type) (ret cty.Value, err error) {
		...
		retury.StringVal(newlines.ReplaceAllString(args[0].AsString(), "")), nil
	},
})

// Indent
 is
tion that adds a given number of spaces to the
// begins ol but the t line in a given multi-line string.
var Indent
 = 
tion.New(&
tion.Spec{
...
(args []cty.Value, retType cty.Type) (ret cty.Value, err error) {
		...
		if err := gocty.FromCtyValue(args[0], &spaces); err != nil {
			return ctynownVay.String), err
		}
		data := argsAsSg()
		pad := strings.Repeat(" ", spaces)
		return ctringVal(strings.Replace(data, "\n", "\n"+pad, -1)), nil
	},
})

// Title
 is a 
tion that converts the first letter of each word in the
// given string to uppercase.
var Title
 = 
tion.New(&
tion.Spec{
	Description: "aces one letter after each non-letter and non-digit character with its uppercase equivalent.",
	Params: []
tiorameter{
		{
			Name: "str",
	...
	Type:         
tion.StatturnTyty.String),
	RefineResult: refineNonNull,
	Impl: 
(args []cty.Value, retType cty.Type) (ret cty.Value, err error) {
		return ctringVal(strings.Title(args[0].AsString())), nil
	},
})

// TrimSpace
 is a 
tion that removes any space characters from the start
// and end of the given string.
var TrimSpace
 = 
tion.New(&
tion.Spec{
	Description: "ves any consecutive space characters (as defined by Unicode) from the start and end of the given string.",
	Params: []
tiorameter{
		{
			Name: "str",
	...
	Type:         
tion.StatiurnType(cty.String),
ineResult: refineNonNull,
	Impl: 
(args []cty.Value, retType cty.Type) (ret cty.Value, err error) {
...
// Trim
 is a 
tion that res the specified characters from the start
// and end of the given string.
var Trim
 = 
tion.New(&
tion.Spec{
cription: "Removes consecutive sequences of characters in \"cutset\" from the start and end of the given string.",
	Params: []
tion.Parameter{
		{
			Name:       r",
		...
		{
			Name:        "cutset",
escription: "A string containing all of the characters to trim. Each character is taken separately, so the order of characters is insignificant.",
			Type:      y.String,
		},
	},
	Type:         
...
// TrimPrefix
 is a 
tion that removes the specified characters from the
// start the given string.
TrimPrefix
 = 
tion.New(&
tion.Spec{
	Descriptionemoves the given prefix from the start of the given string, if present.",
	Params: []
tion.Parameter{
		{
			Name:        "str",
escription: "The string to trim.",
			Type:    cty.String,
		},
		{
			Name:        "prefix",
...
// TrimSuffix
 is a 
 that removes the specified characters from the
// end of theen string.
var TrimSuffix
 = 
tion.New(&
.Spec{
	Description: "Res the given suffix from the start of the given string, if present.",
	Params: []
tion.Parameter{
		{
ame:        "str",
			Descripti"The string to trim.",
			Type:        cty.String,
		},
		{
ame:        "suffix",
			Description: "Tuffix to remove, if present.",
			Type:        cty.String,
		},
	},
e:         
tion.StaticRetype(cty.String),
	RefineResult: refineNonNull,
	Impl: 
(args []cty.Value, retType cty.Type) (cty.Value, error) {
		str := args[0].AsString()
...
// Upper is a 
tion that converts a given string to uppercase.

 Upper(str cty.Value) (cty.Value, error) {
	return Upper
.Call([]cty.Value{str})
}

// Lower is a 
tion that converts a given string to lowercase.

 Lower(str cty.Value) (cty.Value, error) {
	return Lower
.Call([]cty.Value{str})
}

// Reverse is a 
tion that reverses the order of the characters in the
// given string.
//
// As usual, "character" for the sake of this 
tion is a grapheme cluster,
// so combining diacritics (for example) will be considered together as a
// single character.

 Reverse(str cty.Value) (cty.Value, error) {
	return Reverse
.Call([]cty.Value{str})
}

// Strlen is a 
tion that returns the length of the given string in
// characters.
//
// As usual, "character" for the sake of this 
tion is a grapheme cluster,
...
// single character.
 ...
 Strlen(str cty.Value) (cty.Value, error) {
	return Strlen
.Call([]cty.Value{str})
}

// Substr is a 
tion that extracts a sequence of characters from another
...
// the given offset will be returned.
 ...
 Substr(str cty.Value, offset cty.Value, length cty.Value) (cty.Value, error) {
	return Substr
.Call([]cty.Value{str, offset, length})
}

// Join concatenates together the string elements of one or more lists with a
// given separator.

 Join(sep cty.Value, lists ...cty.Value) (cty.Value, error) {
...
// in ascending lexicographical order.
 ...
 Sort(list cty.Value) (cty.Value, error) {
	return Sort
.Call([]cty.Value{list})
}
...
// strings containing the characters between the separator sequences.
 ...
 Split(sep, str cty.Value) (cty.Value, error) {
	return Split
.Call([]cty.Value{sep, str})
}

// Chomp removes newline characters at the end of a string.

 Chomp(str cty.Value) (cty.Value, error) {
	return Chomp
.Call([]cty.Value{str})
}
...
// line in a given multi-line string.
 ...
 Indent(spaces, str cty.Value) (cty.Value, error) {
	return Indent
.Call([]cty.Value{spaces, str})
}

// Title converts the first letter of each word in the given string to uppercase.

 Title(str cty.Value) (cty.Value, error) {
	return Title
.Call([]cty.Value{str})
}
...
// TrimSpace removes any space characters from the start and end of the given string.
 ...
 TrimSpace(str cty.Value) (cty.Value, error) {
	return TrimSpace
.Call([]cty.Value{str})
}
...
// Trim removes the specified characters from the start and end of the given string.
 ...
 Trim(str, cutset cty.Value) (cty.Value, error) {
	return Trim
.Call([]cty.Value{str, cutset})
}

// TrimPrefix removes the specified prefix from the start of the given string.

 TrimPrefix(str, prefix cty.Value) (cty.Value, error) {
	return TrimPrefix
.Call([]cty.Value{str, prefix})
}

// TrimSuffix removes the specified suffix from the end of the given string.

 TrimSuffix(str, suffix cty.Value) (cty.Value, error) {
	return TrimSuffix
.Call([]cty.Value{str, suffix})
}
