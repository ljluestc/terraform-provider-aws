...
// program.
 ...
 ImpliedType(gv interface{}) (cty.Type, error) {
...
return impliedType(rt, path)
 ...
 impliedType(rt reflect.Type, path cty.Path) (cty.Type, error) {
switch rt.Kind() {
...
return cty.NilType, path.NewErrorf("no cty.Type for %s", rt)
 ...
 impliedStructType(rt reflect.Type, path cty.Path) (cty.Type, error) {
