...
n error is returned. Otherwise (finally!) the result is nil, nil.

 typeCheck(required Type, ret Type, values ...Value) (shortCircuit *Value, err error) {
hasDynamic := false
hasUnknown := false
...
ustTypeCheck is a wrapper around typeCheck that immediately panics if
// any error is returned.

 mustTypeCheck(required Type, ret Type, values ...Value) *Value {
...
// then that subsequent operat won't run if the operation panics.

// If the given short-circuit value is *not* DynamicVal then it must be
// of the given type, or this 
tion will panic.

 forceShortCircuitType(shortCircuit *Value, ty Type) *Value {
if shortCircuit == nil {
return nil
}
