...
tion the caller must no longer access it,
ince ownership is transferred to this library.

 Object(attrTypes map[string]Type) Type {
...
//
// This 
tion is EXPERIMENTAL. The behavior of the 
tion or of any other
// 
tions working either directly or indirectly with a type created by
// this 
tion is not currently considered as a compatibility constraint, and
...
//
// Optioattributes are significant only when an object type is being used
// as a target type for conversion in the "convert" package. A value of an
// object type always has a value for eachthe attributes in the attribute
...
tion will panic.
//
// After a map or array is passed to this 
tion the caller must no longer
// access it, since ownership is transferred to this library.

 ObjectWithOptionalAttrs(attrTypes map[string]Type, optional []string) Type {
...
AttrOptional: optionalSet,
 ...
 (t typeObject) Equals(other Type) bool {
...
// or if the object type has no such attribute (use HasAttribute to confirm).
 ...
 (t Type) AttributeType(name string) Type {
name = NormalizeString(name)
if ot, ok := t.typeImpl.(typeObject); ok {
...
ap. For many purposes the attribute-related methods of Value are more
// appropriate and more convenient to use.

 (t Type) AttributeTypes() map[string]Type {
if ot, ok := t.typeImpl.(typeObject); ok {
return ot.AttrTypes
}
...
// map.
 ...
 (t Type) OptionalAttributes() map[string]struct{} {
if ot, ok := t.typeImpl.(typeObject); ok {
...
// confirm).
 ...
 (t Type) AttributeOptional(name string) bool {
name = NormalizeString(name)
if ot, ok := t.typeImpl.(typeObject); ok {
