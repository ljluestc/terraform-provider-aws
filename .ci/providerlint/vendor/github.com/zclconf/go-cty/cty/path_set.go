...
f a Path is mutated while in a set, behavior is undefined.

 (s PathSet) Add(path Path) {
s.set.Add(path)
}
...
// the given path.
//
or example, if given a path representing "foo.bar", it will add both
// "foo" and "bar".

 (s PathSet) AddAllSteps(path Path) {
...
// receiver and the given other set.
 ...
 (s PathSet) Intersection(other PathSet) PathSet {
return PathSet{
...
ny elements of the other given set subtracted.

 (s PathSet) Subtract(other PathSet) PathSet {
return PathSet{
set: s.set.Subtract(other.set),
}
}

ymmetricDifference returns a new set whose contents are the symmetric
// difference of the receiver and the given other set.

 (s PathSet) SymmetricDifference(other PathSet) PathSet {
return PathSet{
...
// set contain exactly the same paths.
 ...
 (s PathSet) Equal(other PathSet) bool {
...
type pathSetRules struct {
 ...
 (r pathSetRules) Hash(path Path) int {
...
default:
or any other step type we just append a predefined value,
...
return int(hash.Sum64())
 ...
 (r pathSetRules) Equivalent(aPath, bPath Path) bool {
...
// SameRules is true if both Rules instances are pathSetRules structs.
 ...
 (r pathSetRules) SameRules(other set.Rules[Path]) bool {
