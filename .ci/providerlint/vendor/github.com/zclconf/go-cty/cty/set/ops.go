...
here in the first place. If the value is not present, this is a no-op.

 (s Set[T]) Remove(val T) {
hv := s.rules.Hash(val)
bucket, ok := s.vals[hv]
if !ok {
return
...
as returns true if the given value is in the receiving set, or false if
// it is not.

 (s Set[T]) Has(val T) bool {
hv := s.rules.Hash(val)
bucket, ok := s.vals[hv]
if !ok {
return false
...
// or else this 
 ...
 (s1 Set[T]) Intersection(s2 Set[T]) Set[T] {
mustHaveSameR(s1, s2)
rs := NewSet(s1.rules)
s1.EachValue(
(v T) {
...
hat are not also in the given set. Both sets must have the same rules,
// or else this 
tion will panic.

 (s1 Set[T]) Subtract(s2 Set[T]) Set[T] {
mustHaveSameRules(s1, s2)
rs := NewSet(s1.rules)
s1.EachValue(
(v T) {
...
// panic.
 ...
 (s1 Set[T]) SymmetricDifference(s2 Set[T]) Set[T] {
mustHaveSameRules(s1, s2)
rs := NewSet(s1.rules)
s1.EachValue(
(v T) {
