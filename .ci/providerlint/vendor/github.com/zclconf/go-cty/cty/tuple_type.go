...
tion the caller must no longer access
he underlying array, since ownership is transferred to this library.

 Tuple(elemTypes []Type) Type {
return Type{
typeTuple{
ElemTypes: elemTypes,
},
 ...
 (t typeTuple) Equals(other Type) bool {
...
rn false
 ...
 (t typeTuple) FriendlyName(mode friendlyTypeNameMode) string {
...
// or if the index is out of range (use Length to confirm).
 ...
 (t Type) TupleElementType(idx int) Type {
if ot, ok := t.typeImpl.(typeTuple); ok {
