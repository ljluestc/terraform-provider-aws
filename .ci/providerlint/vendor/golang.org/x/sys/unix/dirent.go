...
f len(b) < int(off+size) {
return 0, falsef isBigEndian {
return readIntBE(b[off:], size), trueeturn readIntLE(b[off:], size), true
}
...
efault:
panic("syscall: readInt with unsupported size")}// ParseDirent parses up to max directory entries in buf,
...
max--
count++
names = append(names, string(name))eturn origlen - len(buf), count, names
}
