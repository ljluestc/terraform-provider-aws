...
// +build aix,ppcpackage unix//sysnbtrlimit(resource int, rlim *Rlimit) (err error) = getrlimit64
//sysek(fd int, offset int64, whence int) (off int64, err error) = lseek64//sysma(addr uintptr, length uintptr, prot int, flags int, fd int, offset int64) (xaddr uintptr, err error)
 setTimespec(sec, nsec int64) Timespec {
turn Timespec{Sec: int32(sec), Nsec: int32(nsec)}
}
 setTimeval(sec, usec int64) Timeval {
turn Timeval{Sec: int32(sec), Usec: int32(usec)}
}
...
turn fstatat(dirfd, path, stat, flags)
}
 Lstat(path string, stat *Stat_t) error {
turn lstat(path, stat)
}
 Stat(path string, statptr *Stat_t) error {
turn stat(path, statptr)
}
