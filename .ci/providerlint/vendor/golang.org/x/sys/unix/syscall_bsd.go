...
// license that can be found in the LICENSE file.//go:build darwin || dragonfly || freebsd || netbsd || openbsd
// +build darwin dragonfly freebsd netbsd openbsd// BSD system call wrappers shared by *BSD based systems
// including OS X (Darwin) and FreeBSD.  Like the other
// syscall_*.go files it is compiled as Go code but also
// used as input to mksyscall which parses the //sys
// lines and generates system call stubs.package uniximport (
"runtime"
"syscall"
"unsafe"
 ...
 * Wrapped
 *///sysnbgetgroups(ngid int, gid *_Gid_t) (n int, err error)
//sysnbsetgroups(ngid int, gid *_Gid_t) (err error)
...
return sig
 ...
 (w WaitStatus) Killed() bool { return w&mask == killed && syscall.Signal(w>>shift) != SIGKILL }
...
return syscall.Signal(w>>shift) & 0xFF
 ...
 (w WaitStatus) TrapCause() int { return -1 }//syswait4(pid int, wstatus *_C_int, options int, rusage *Rusage) (wpid int, err error)
...
return
}//sysaccept(s int, rsa *RawSockaddrAny, addrlen *_Socklen) (fd int, err error)
...
//sysShutdown(s int, how int) (err error)
...
return unsafe.Pointer(&sa.raw), _Socklen(sa.raw.Len), nil
 ...
 (sa *SockaddrDatalink) sockaddr() (unsafe.Pointer, _Socklen, error) {
if sa.Index == 0 {
return nil, 0, EINVAL
}
sa.raw.Len = sa.Len
...
sa.Slen = pp.Slen
sa.Data = pp.Data
return sa, nilcase AF_UNIX:
pp := (*RawSockaddrUnix)(unsafe.Pointer(rsa))
if pp.Len < 2 || pp.Len > SizeofSockaddrUnix {
return nil, EINVAL
}
sa := new(SockaddrUnix)// Some BSDs include the trailing NUL in the length, whereas
...
return sa, nilcase AF_INET:
pp := (*RawSockaddrInet4)(unsafe.Pointer(rsa))
sa := new(SockaddrInet4)
p := (*[2]byte)(unsafe.Pointer(&pp.Port))
sa.Port = int(p[0])<<8 + int(p[1])
sa.Addr = pp.Addr
return sa, nilcase AF_INET6:
...
}//sysnbsocketpair(domain int, typ int, proto int, fd *[2]int32) (err error)// GetsockoptString returns the string value of the socket option opt for the
...
return n, nil
}//syskevent(kq int, change unsafe.Pointer, nchange int, event unsafe.Pointer, nevent int, timeout *Timespec) (n int, err error)
...
}// sysctlmib translates name to mib number and appends any additional args. sysctlmib(name string, args ...int) ([]_C_int, error) {
...
return SysctlArgs(name)
 ...
 SysctlArgs(name string, args ...int) (string, error) {
buf, err := SysctlRaw(name, args...)
if err != nil {
return "", err
}
n := len(buf)// Throw away terminating NUL.
if n > 0 && buf[n-1] == '\x00' {
n--
}
...
return SysctlUint32Args(name)
 ...
 SysctlUint32Args(name string, args ...int) (uint32, error) {
...
return *(*uint32)(unsafe.Pointer(&buf[0])), nil
 ...
 SysctlUint64(name string, args ...int) (uint64, error) {
mib, err := sysctlmib(name, args...)
if err != nil {
return 0, err
}n := uintptr(8)
...
return buf[:n], nil
 ...
 SysctlClockinfo(name string) (*Clockinfo, error) {
...
return &tv, nil
}//sysutimes(path string, timeval *[2]Timeval) (err error)
...
}//sysfutimes(fd int, timeval *[2]Timeval) (err error)
 Futimes(fd int, tv []Timeval) error {
if tv == nil {
...
}//syspoll(fds *PollFd, nfds int, timeout int) (n int, err error)
...
//Ptrace(req int, pid int, addr uintptr, data int) (ret uintptr, err error)//sysMadvise(b []byte, behav int) (err error)
//sysMlock(b []byte) (err error)
//sysMlockall(flags int) (err error)
//sysMprotect(b []byte, prot int) (err error)
//sysMsync(b []byte, flags int) (err error)
//sysMunlock(b []byte) (err error)
//sysMunlockall() (err error)
