...
// is in the high bits.type WaitStatus uint32const (
 ...
 (w WaitStatus) TrapCause() int { return -1 }//sys	wait4(pid int, wstatus *_C_int, options int, rusage *Rusage) (wpid int, err error)
...
}//sysnb	socketpair(domain int, typ int, proto int, fd *[2]int32) (err error)// GetsockoptString returns the string value of the socket option opt for the
...
//	Ptrace(req int, pid int, addr uintptr, data int) (ret uintptr, err error)//sys	Madvise(b []byte, behav int) (err error)
