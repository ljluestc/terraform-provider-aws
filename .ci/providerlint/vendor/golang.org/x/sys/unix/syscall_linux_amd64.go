...
// license that can be found in the LICENSE file.//go:build amd64 && linux
// +build amd64,linuxpackage unix//sys	EpollWait(epfd int, events []EpollEvent, msec int) (n int, err error)
...
//sys	mmap(addr uintptr, length uintptr, prot int, flags int, fd int, offset int64) (xaddr uintptr, err error)//sys	futimesat(dirfd int, path string, times *[2]Timeval) (err error)
...
//sys	utimes(path string, times *[2]Timeval) (err error)
 ...
 (r *PtraceRegs) PC() uint64 { return r.Rip }
 (r *PtraceRegs) SetPC(pc uint64) { r.Rip = pc }
