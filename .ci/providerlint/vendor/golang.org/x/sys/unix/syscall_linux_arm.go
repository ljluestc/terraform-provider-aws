...
// license that can be found in the LICENSE file.//go:build arm && linux
// +build arm,linuxpackage uniximport (
	"unsafe"
)
...
//sys	sendmsg(s int, msg *Msghdr, flags int) (n int, err error)// 64-bit file system and 32-bit uid calls
...
//sys	Ustat(dev int, ubuf *Ustat_t) (err error)//sys	futimesat(dirfd int, path string, times *[2]Timeval) (err error)
...
}//sys	utimes(path string, times *[2]Timeval) (err error)//sys	pread(fd int, p []byte, offset int64) (n int, err error) = SYS_PREAD64
...
//sys	Ftruncate(fd int, length int64) (err error) = SYS_FTRUNCATE64
 ...
 mmap(addr uintptr, length uintptr, prot int, flags int, fd int, offset int64) (xaddr uintptr, err error) {
	...
	Cur uint32
	Max uint32
}//sysnb	getrlimit(resource int, rlim *rlimit32) (err error) = SYS_UGETRLIMITconst rlimInf32 = ^uint32(0)
const rlimInf64 = ^uint64(0)
 ...
 (r *PtraceRegs) PC() uint64 { return uint64(r.Uregs[15]) }
 (r *PtraceRegs) SetPC(pc uint64) { r.Uregs[15] = uint32(pc) }
