...
// license that can be found in the LICENSE file.//go:build loong64 && linux
// +build loong64,linuxpackage uniximport "unsafe"//sysEpollWait(epfd int, events []EpollEvent, msec int) (n int, err error) = SYS_EPOLL_PWAIT
...
//sysSplice(rfd int, roff *int64, wfd int, woff *int64, len int, flags int) (n int64, err error)
 timespecFromStatxTimestamp(x StatxTimestamp) Timespec {
return Timespec{
Sec:  x.Sec,
Nsec: int64(x.Nsec),
}
}
 Fstatat(fd int, path string, stat *Stat_t, flags int) error {
var r Statx_t
...
stat.Ctim = timespecFromStatxTimestamp(r.Ctime)return nil
}
...
//sysmmap(addr uintptr, length uintptr, prot int, flags int, fd int, offset int64) (xaddr uintptr, err error)//sysnbGettimeofday(tv *Timeval) (err error)
...
return utimensat(AT_FDCWD, path, (*[2]Timespec)(unsafe.Pointer(&ts[0])), 0)
 ...
 (r *PtraceRegs) PC() uint64 { return r.Era }
 (r *PtraceRegs) SetPC(era uint64) { r.Era = era }
