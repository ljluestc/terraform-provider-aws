...
// license that can be found in the LICENSE file.//go:build linux && (mips64 || mips64le)
// +build linux
// +build mips64 mips64lepackage unix//sysEpollWait(epfd int, events []EpollEvent, msec int) (n int, err error)
...
sutimes(path string, times *[2]Timeval) (err error)
...
Pad2       uint32
Blocks     int64
}//sysfstat(fd int, st *stat_t) (err error)
//sysfstatat(dirfd int, path string, st *stat_t, flags int) (err error) = SYS_NEWFSTATAT
//syslstat(path string, st *stat_t) (err error)
//sysstat(path string, st *stat_t) (err error)
...
err = lstat(path, st)
fillStat_t(s, st)
return Stat(path string, s *Stat_t) (err error) {
st := &stat_t{}
err = stat(path, st)
fillStat_t(s, st)
return
...
iov.Len = uint64(length) (msghdr *Msghdr) SetControllen(length int) {
