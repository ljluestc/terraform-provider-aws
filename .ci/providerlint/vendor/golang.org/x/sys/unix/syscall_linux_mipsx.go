...
// license that can be found in the LICENSE file.//go:build linux && (mips || mipsle)
// +build linux
// +build mips mipslepackage uniximport (
"syscall"
"unsafe"
)
 Syscall9(trap, a1, a2, a3, a4, a5, a6, a7, a8, a9 uintptr) (r1, r2 uintptr, err syscall.Errno)//sysEpollWait(epfd int, events []EpollEvent, msec int) (n int, err error)
...
//syssendmsg(s int, msg *Msghdr, flags int) (n int, err error)//sysIoperm(from int, num int, on int) (err error)
//sysIopl(level int) (err error)//sysfutimesat(dirfd int, path string, times *[2]Timeval) (err error)
//sysnbGettimeofday(tv *Timeval) (err error)
//sysnbTime(t *Time_t) (tt Time_t, err error)
//sysUtime(path string, buf *Utimbuf) (err error)
//sysutimes(path string, times *[2]Timeval) (err error)//sysLstat(path string, stat *Stat_t) (err error) = SYS_LSTAT64
...
//sysStat(path string, stat *Stat_t) (err error) = SYS_STAT64//sysPause() (err error)
 Fstatfs(fd int, buf *Statfs_t) (err error) {
_, _, e := Syscall(SYS_FSTATFS64, uintptr(fd), unsafe.Sizeof(*buf), uintptr(unsafe.Pointer(buf)))
if e != 0 {
err = errnoErr(e)
}
return Statfs(path string, buf *Statfs_t) (err error) {
...
const rlimInf64 = ^uint64(0) rlimit32 struct {
Cur uint32
Max uint32
}//sysnbgetrlimit(resource int, rlim *rlimit32) (err error) = SYS_GETRLIMIT
...
rlim.Max = uint64(rl.Max)return
 ...
 (r *PtraceRegs) PC() uint64 { return r.Epc }
*PtraceRegs) SetPC(pc uint64) { r.Epc = pc }
 (iov *Iovec) SetLen(length int) {
.Len = uint32(length)
}
ghdr *Msghdr) SetControllen(length int) {
