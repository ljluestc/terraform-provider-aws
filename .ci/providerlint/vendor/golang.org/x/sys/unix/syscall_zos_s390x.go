...
// +build zos,s390xpackage uniximport (
 ...
 syscall_syscall(trap, a1, a2, a3 uintptr) (r1, r2 uintptr, err Errno) syscall_rawsyscall(trap, a1, a2, a3 uintptr) (r1, r2 uintptr, err Errno) syscall_syscall6(trap, a1, a2, a3, a4, a5, a6 uintptr) (r1, r2 uintptr, err Errno) syscall_rawsyscall6(trap, a1, a2, a3, a4, a5, a6 uintptr) (r1, r2 uintptr, err Errno) syscall_syscall9(trap, a1, a2, a3, a4, a5, a6, a7, a8, a9 uintptr) (r1, r2 uintptr, err Errno) syscall_rawsyscall9(trap, a1, a2, a3, a4, a5, a6, a7, a8, a9 uintptr) (r1, r2 uintptr, err Errno)
 copyStat(stat *Stat_t, statLE *Stat_LE_t) {
 ...
 svcCall(fnptr unsafe.Pointer, argv *unsafe.Pointer, dsa *uint64) svcLoad(name *byte) unsafe.Pointer svcUnload(name *byte, fnptr unsafe.Pointer) int64
 (d *Dirent) NameString() string {
	if d == nil {
		return ""
	}
	s := string(d.Name[:])
 ...
 anyToSockaddr(_ int, rsa *RawSockaddrAny) (Sockaddr, error) {
...
//sys	write(fd int, p []byte) (n int, err error)//sys	accept(s int, rsa *RawSockaddrAny, addrlen *_Socklen) (fd int, err error) = SYS___ACCEPT_A
...
//sys   ioctlPtr(fd int, req int, arg unsafe.Pointer) (err error) = SYS_IOCTL//sys   Access(path string, mode uint32) (err error) = SYS___ACCESS_A
...
//sys   W_Getmntent_A(buff *byte, size int) (lastsys int, err error) = SYS___W_GETMNTENT_A//sys   mount_LE(path string, filesystem string, fstype string, mtm uint32, parmlen int32, parm string) (err error) = SYS___MOUNT_A
//sys   unmount(filesystem string, mtm int) (err error) = SYS___UMOUNT_A
//sys   Chroot(path string) (err error) = SYS___CHROOT_A
//sys   Select(nmsgsfds int, r *FdSet, w *FdSet, e *FdSet, timeout *Timeval) (ret int, err error) = SYS_SELECT
//sysnb Uname(buf *Utsname) (err error) = SYS___UNAME_A
 Ptsname(fd int) (name string, err error) {
 ...
 u2s(cstr unsafe.Pointer) string {
	str := (*[1024]uint8)(cstr)
	i := 0
	for str[i] != 0 {
		i++
 ...
 Close(fd int) (err error) {
	...
	return
}// Dummy function: there are no semantics for Madvise on z/OS Madvise(b []byte, advice int) (err error) {
	return
...
//sysnb	Getrlimit(resource int, rlim *Rlimit) (err error) = SYS_GETRLIMIT//sysnb getrusage(who int, rusage *rusage_zos) (err error) = SYS_GETRUSAGE
...
//sys	stat(path string, statLE *Stat_LE_t) (err error) = SYS___STAT_A
 Stat(path string, sta *Stat_t) (err error) {
	var statLE Stat_LE_t
...
//sys	Utime(path string, utim *Utimbuf) (err error) = SYS___UTIME_A//sys	open(path string, mode int, perm uint32) (fd int, err error) = SYS___OPEN_A
 ...
 Mkfifoat(dirfd int, path string, mode uint32) (err error) {
	...
	defer Chdir(wd)	return Mkfifo(path, mode)
...
// from stopped via the core dump bit.const (
 ...
 (w WaitStatus) StopSignal() Signal {
	if !w.Stopped() {
		return -1
	}
	return Signal(w>>shift) & 0xFF
}
 (w WaitStatus) TrapCause() int { return -1 }//sys	waitpid(pid int, wstatus *_C_int, options int) (wpid int, err error)
 ...
 setTimeval(sec, usec int64) Timeval { //fix
 ...
 Getsockname(fd int) (sa Sockaddr, err error) {
	...
	nwmICMPTStatsEntry     = 0xd5e6d4c9c3d4d7e3	// nwmHeader constants
	nwmVersion1   = 1
	nwmVersion2   = 2
	nwmCurrentVer = 2	nwmTCPConnType     = 1
	nwmGlobalStatsType = 14	// nwmFilter constants
	...
	nwmFilterSrcPortMask = 0x10000000 // Source port	// nwmConnEntry constants
	nwmTCPStateClosed   = 1
	nwmTCPStateListen   = 2
	...
	nwmTCPStateDeletTCB = 12	// Existing constants on linux
	BPF_TCP_CLOSE        = 1
	BPF_TCP_LISTEN       = 2
	BPF_TCP_SYN_SENT     = 3
	BPF_TCP_SYN_RECV     = 4
 ...
 GetsockoptTCPInfo(fd, level, opt int) (*TCPInfo, error) {
	...
	var bufferLen, returnValue, returnCode int32 = 4096, 0, 0	dsa := [18]uint64{0}
	...
	argv[6] = unsafe.Pointer(&reasonCode)	request := (*struct {
		header nwmHeader
		filter nwmFilter
	})(unsafe.Pointer(&responseBuffer[0]))	EZBNMIF4 := svcLoad(&svcNameTable[svc_EZBNMIF4][0])
	if EZBNMIF4 == nil {
	...
	request.header.options = 0x80000000	svcCall(EZBNMIF4, &argv[0], &dsa[0])	// outputDesc field is filled by EZBNMIF4 on success
	...
	argv[0] = unsafe.Pointer(uintptr(*nameptr))	request.header.ident = nwmHeaderIdentifier
	request.header.length = uint32(unsafe.Sizeof(request.header))
	request.header.version = nwmCurrentVer
	request.header.nwmType = nwmTCPConnType
	request.header.options = 0x80000000	request.filter.ident = nwmFilterIdentifier	var localSockaddr RawSockaddrAny
	...
	}	svcCall(EZBNMIF4, &argv[0], &dsa[0])	// outputDesc field is filled by EZBNMIF4 on success
	if returnCode != 0 || request.header.outputDesc.offset == 0 {
		return nil, errnoErr(EINVAL)
	...
	tcpinfo.Total_retrans = conn.reXmtCount	svcUnload(&svcNameTable[svc_EZBNMIF4][0], EZBNMIF4)	return &tcpinfo, nil
...
// socket associated with fd at the given socket level. GetsockoptString(fd, level, opt int) (string, error) {
 ...
 Opendir(name string) (uintptr, error) {
	...
	return dir, err
}// clearsyscall.Errno resets the errno value to 0. clearErrno()
 Readdir(dir uintptr) (*Dirent, error) {
	var ent Dirent
	var res uintptr
	// __readdir_r_a returns errno at the end of the directory stream, rather than 0.
	// Therefore to avoid false positives we clear errno before calling it.	// TODO(neeilan): Commented this out to get sys/unix compiling on z/OS. Uncomment and fix. Error: "undefined: clearsyscall"
	//clearsyscall.Errno() // TODO(mundaym): check pre-emption rules.	e, _, _ := syscall_syscall(SYS___READDIR_R_A, dir, uintptr(unsafe.Pointer(&ent)), uintptr(unsafe.Pointer(&res)))
	var err error
	if e != 0 {
 ...
 readdir_r(dirp uintptr, entry *direntLE, result **direntLE) (err error) {
 ...
 Seekdir(dir uintptr, pos int) {
	_, _, _ = syscall_syscall(SYS_SEEKDIR, dir, uintptr(pos), 0)
}
 Telldir(dir uintptr) (int, error) {
 ...
 Flock(fd int, how int) error {	var flock_type int16
	var fcntl_cmd int	switch how {
 ...
 Mlock2(b []byte, flags int) (err error) {
	_, _, e1 := syscall_syscall(SYS___MLOCKALL, _BPX_NONSWAP, 0, 0)
	if e1 != 0 {
		err = errnoErr(e1)
	}
 ...
 ClockGettime(clockid int32, ts *Timespec) error {	var ticks_per_sec uint32 = 100 //TODO(kenan): value is currently hardcoded; need sysconf() call otherwise
	var nsec_per_sec int64 = 1000000000	if ts == nil {
		return EFAULT
	}
...
// allocations at runtime. errnoErr(e Errno) error {
	switch e {
	case 0:
		return nil
	case EAGAIN:
	...
	return e
}// ErrnoName returns the error name for error number e. ErrnoName(e Errno) string {
	...
	return ""
}// SignalName returns the signal name for signal number s. SignalName(s syscall.Signal) string {
...
// The signal name should start with "SIG". SignalNum(s string) syscall.Signal {
...
}// clen returns the index of the first NULL byte in n or len(n) if n contains no NULL byte. clen(n []byte) int {
	i := bytes.IndexByte(n, 0)
	if i == -1 {
		i = len(n)
	}
	return i
}// Mmap manager, for use by operating system-specific implementations.type mmapper struct {
	sync.Mutex
	active map[*byte][]byte // active mappings; key is last byte in mapping
	mmap   func(addr, length uintptr, prot, flags, fd int, offset int64) (uintptr, error)
	munmap func(addr uintptr, length uintptr) error
}
 (m *mmapper) Mmap(fd int, offset int64, length int, prot int, flags int) (data []byte, err error) {
	if length <= 0 {
		return nil, EINVAL
	...
	var sl = struct {
		...
		len  int
		cap  int
	}{addr, length, length}	// Use unsafe to turn sl into a []byte.
	b := *(*[]byte)(unsafe.Pointer(&sl))	// Register mapping in m and return it.
	p := &b[cap(b)-1]
	m.Lock()
	defer m.Unlock()
	m.active[p] = b
 ...
 (m *mmapper) Munmap(data []byte) (err error) {
...
var SocketDisableIPv6 bool// Sockaddr represents a socket address.
...
type SockaddrUnix struct {
 ...
 GetsockoptLinger(fd, level, opt int) (*Linger, error) {
	var linger Linger
	vallen := _Socklen(SizeofLinger)
	err := getsockopt(fd, level, opt, unsafe.Pointer(&linger), &vallen)
	return &linger, err
}
 GetsockoptTimeval(fd, level, opt int) (*Timeval, error) {
	var tv Timeval
	vallen := _Socklen(unsafe.Sizeof(tv))
	err := getsockopt(fd, level, opt, unsafe.Pointer(&tv), &vallen)
	return &tv, err
}
 GetsockoptUint64(fd, level, opt int) (value uint64, err error) {
	var n uint64
	vallen := _Socklen(8)
	err = getsockopt(fd, level, opt, unsafe.Pointer(&n), &vallen)
	return n, err
 ...
 SetsockoptUint64(fd, level, opt int, value uint64) (err error) {
 ...
 CloseOnExec(fd int) { fcntl(fd, F_SETFD, FD_CLOEXEC) }
...
// process (["USER=go", "PWD=/tmp"]). Exec(argv0 string, argv []string, envv []string) error {
 ...
 Unmount(name string, mtm int) (err error) {
 ...
 fdToPath(dirfd int) (path string, err error) {
 ...
 direntLeToDirentUnix(dirent *direntLE, dir uintptr, path string) (Dirent, error) {
	var d Dirent	d.Ino = uint64(dirent.Ino)
	offset, err := Telldir(dir)
	if err != nil {
		return d, err
	}	d.Off = int64(offset)
	s := string(bytes.Split(dirent.Name[:], []byte{0})[0])
	copy(d.Name[:], s)	d.Reclen = uint16(24 + len(d.NameString()))
	var st Stat_t
	path = path + "/" + s
	err = Lstat(path, &st)
	if err != nil {
 ...
 Getdirentries(fd int, buf []byte, basep *uintptr) (n int, err error) {
	...
	// the directory being edited underfoot.	skip, err := Seek(fd, 0, 1 /* SEEK_CUR */)
	if err != nil {
		return 0, err
