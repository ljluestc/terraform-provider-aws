...
// +build linuxpackage unixconst (
	...
	_C_short int16
	_C_int   int32	_C_long_long int64
	...
	Cur uint64
	Max uint64
}type _Gid_t uint32type StatxTimestamp struct {
	Sec  int64
	Nsec uint32
	_    int32
}type Statx_t struct {
...
}type RawSockaddrPPPoX [0x1e]bytetype RawSockaddrTIPC struct {
	Family   uint16
	Addrtype uint8
	Scope    int8
	Addr     [12]byte
	...
	Target_idx   uint32
	Nfc_protocol uint32
}type _Socklen uint32type Linger struct {
	Onoff  int32
	Linger int32
}type IPMreq struct {
	...
	ICMP_FILTER = 0x1	ICMPV6_FILTER             = 0x1
	...
	Cookie uint32
	Len    uint32
}const SizeofInotifyEvent = 0x10const SI_LOAD_SHIFT = 0x10type Utsname struct {
	Sysname    [65]byte
	Nodename   [65]byte
	Release    [65]byte
	Version    [65]byte
	...
	AT_REMOVEDIR    = 0x200	AT_STATX_SYNC_AS_STAT = 0x0
	AT_STATX_FORCE_SYNC   = 0x2000
	AT_STATX_DONT_SYNC    = 0x4000	AT_RECURSIVE = 0x8000	AT_SYMLINK_FOLLOW   = 0x400
	AT_SYMLINK_NOFOLLOW = 0x100	AT_EACCESS = 0x200	OPEN_TREE_CLONE = 0x1	MOVE_MOUNT_F_SYMLINKS   = 0x1
	MOVE_MOUNT_F_AUTOMOUNTS = 0x2
	MOVE_MOUNT_F_EMPTY_PATH = 0x4
	MOVE_MOUNT_T_SYMLINKS   = 0x10
	MOVE_MOUNT_T_AUTOMOUNTS = 0x20
	MOVE_MOUNT_T_EMPTY_PATH = 0x40
	MOVE_MOUNT_SET_GROUP    = 0x100	FSOPEN_CLOEXEC = 0x1	FSPICK_CLOEXEC          = 0x1
	FSPICK_SYMLINK_NOFOLLOW = 0x2
	FSPICK_NO_AUTOMOUNT     = 0x4
	FSPICK_EMPTY_PATH       = 0x8	FSMOUNT_CLOEXEC = 0x1
)type OpenHow struct {
	Flags   uint64
	Mode    uint64
	Resolve uint64
}const SizeofOpenHow = 0x18const (
	RESOLVE_BENEATH       = 0x8
	...
	SizeofTpacket3Hdr = 0x30	SizeofTpacketStats   = 0x8
...
)const SO_ORIGINAL_DST = 0x50type Nfgenmsg struct {
	Nfgen_family uint8
	Version      uint8
	Res_id       uint16
}const (
	...
	SOF_TIMESTAMPING_OPT_ID_TCP   = 0x10000	SOF_TIMESTAMPING_LAST = 0x10000
	SOF_TIMESTAMPING_MASK = 0x1ffff	SCM_TSTAMP_SND   = 0x0
	...
	LWTUNNEL_ENCAP_MAX        = 0xa	MPLS_IPTUNNEL_UNSPEC = 0x0
	...
	ETHTOOL_LINK_MODE_100baseFX_Full_BIT                                    = 0x5b	ETHTOOL_MSG_USER_NONE                     = 0x0
...
)const SPEED_UNKNOWN = -0x1type EthtoolDrvinfo struct {
	Cmd          uint32
	Driver       [32]byte
	Version      [32]byte
	Fw_version   [32]byte
	...
	NL_ATTR_TYPE_BITFIELD32   = 0xf	NL_POLICY_TYPE_ATTR_UNSPEC          = 0x0
