...
// license that can be found in the LICENSE file.// Package analysistest provides utilities for testing analyzers.
package analysistestimport (
	"bytes"
	"fmt"
	"go/format"
	"go/token"
	...
	"strings"
	"testing"
	"text/scanner"	"golang.org/x/tools/go/analysis"
...
tion to delete it. WriteFiles(filemap map[string]string) (dir string, cleanup 
(), err error) {
	...
	cleanup = 
() { os.RemoveAll(gopath) }	for name, content := range filemap {
...
//	} RunWithSuggestedFixes(t Testing, dir string, a *analysis.Analyzer, patterns ...string) []*Result {
	r := Run(t, dir, a, patterns...)	// Process each result (package) separately, matching up the suggested
	...
	for _, act := range r {
		...
		fileContents := make(map[*token.File][]byte)		// Validate edits, prepare the fileEdits map and read the file contents.
		for _, diag := range act.Diagnostics {
			...
			if err != nil {
				...
				// one virtual file per kind of suggested fix				if len(ar.Comment) != 0 {
				...
				// all suggested fixes are represented by a single file				var catchallEdits []diff.Edit
				...
				want := string(ar.Comment)				formatted, err := format.Source(out)
				if err != nil {
...
// the results, but a test may use it to perform additional checks. Run(t Testing, dir string, a *analysis.Analyzer, patterns ...string) []*Result {
...
type Result = checker.TestAnalyzerResult// loadPackages uses go/packages to load a specified packages (from source, with
...
// matched no packages. loadPackages(a *analysis.Analyzer, dir string, patterns ...string) ([]*packages.Package, error) {
	...
	// typechecking, though this feature seems to be a recurring need.	mode := packages.NeedName | packages.NeedFiles | packages.NeedCompiledGoFiles | packages.NeedImports |
...
// source files, which must have been parsed with comments enabled. check(t Testing, gopath string, pass *analysis.Pass, diagnostics []analysis.Diagnostic, facts map[types.Object][]analysis.Fact) {
	type key struct {
		file string
		line int
	}	want := make(map[key][]expectation)	// processComment parses expectations out of comments.
	processComment := 
(filename string, linenum int, text string) {
		text = strings.TrimSpace(text)		// Any comment starting with "want" is treated
	...
	for _, f := range pass.Files {
		for _, cgroup := range f.Comments {
			for _, c := range cgroup.List {				text := strings.TrimPrefix(c.Text, "//")
	...
	for _, filename := range pass.OtherFiles {
		...
		for _, line := range strings.Split(string(data), "\n") {
			linenum++			// Hack: treat a comment of the form "//...// want..."
			// or "/*...//t... */
			// as if it starts at 'want'.
			// This allows us to add comments on comments,
			// as required when testing the buildtag analyzer.
...
// facts (name:"rx"). parseExpectations(text string) (lineDelta int, expects []expectation, err error) {
	var scanErr string
...
(s *scanner.Scanner, msg string) {
	...
	sc.Mode = scanner.ScanIdents | scanner.ScanStrings | scanner.ScanRawStrings | scanner.ScanInts	scanRegexp := 
(tok rune) (*regexp.Regexp, error) {
		...
		case scanner.String, scanner.RawString:
			...
			expects = append(expects, expectation{"diagnostic", "", rx})		case scanner.Ident:
			name := sc.TokenText()
			tok = sc.Scan()
			if tok != ':' {
...
ok = sc.Scan()
			...
			expects = append(expects, expectation{"fact", name, rx})		case scanner.EOF:
			if scanErr != "" {
				return 0, nil, fmt.Errorf("%s", scanErr)
			}
			return lineDelta, expects, nil		default:
			return 0, nil, fmt.Errorf("unexpected %s", scanner.TokenString(tok))
		}
	}
}// sanitize removes the GOPATH portion of the filename,
// typically a gnarly /tmp directory, and returns the rest. sanitize(gopath, filename string) string {
