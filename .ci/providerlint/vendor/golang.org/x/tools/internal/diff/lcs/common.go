...
type lcs []diag// A diag is a piece of the edit graph where A[X+i] == B[Y+i], for 0<=i<Len.
// All computed diagonals are parts of a longest common subsequence.
type diag struct {
	X, Y int
	Len  int
}// sort sorts in place, by lowest X, and if tied, inversely by Len (l lcs) so lcs {
	sort.Slice(l, 
(i, j int) bool {
		if l[i].X != l[j].X {
			return l[i].X < l[j].X
...
// expects the lcs to be sorted (l lcs) valid() bool {
	for i := 1; i < len(l); i++ {
...
// only called if two-sided stops early (l lcs) fix() lcs {
...
// the existing diag that has already been added to the lcs. overlap(exist, prop diag) (direction, diag) {
	...
	return bad, prop
}// manipulating Diag and lcs// prepend a diagonal (x,y)-(x+1,y+1) segment either to an empty lcs
// or to its first Diag. prepend is only called to extend diagonals
// the backward direction. (lcs lcs) prepend(x, y int) lcs {
	if len(lcs) > 0 {
		d := &lcs[0]
...
// to extend diagonals in the forward direction. (lcs lcs) append(x, y int) lcs {
	if len(lcs) > 0 {
		last := &lcs[len(lcs)-1]
...
}// enforce constraint on d, k ok(d, k int) bool {
	return d >= 0 && -d <= k && k <= d
}
