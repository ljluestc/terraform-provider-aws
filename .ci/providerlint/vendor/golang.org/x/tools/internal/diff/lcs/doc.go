...
// license that can be found in the LICENSE file.// package lcs contains code to find longest-common-subsequences
// (and diffs)
package lcs/*
...
that changes A into B.The key concept is the edit graph of A and B.
...
When A[i] == B[j] there is a diagonal edge from v[i][j] to v[i+1][j+1].A path between in the graph between (0,0) and (N,M) determines a sequence
...
element of B.A vertex (x,y) is on (forward) diagonal k if x-y=k. A path in the graph
...
with the origin in the lower-left corner.)Here is the edit graph for A:"aabbaa", B:"aacaba". (I know the diagonals look weird.)
          ⊙   -------   ⊙   -------   ⊙   -------   ⊙   -------   ⊙   -------   ⊙   -------   ⊙
   a      |   ___/‾‾‾   |   ___/‾‾‾   |             |             |   ___/‾‾‾   |   ___/‾‾‾   |
          ⊙   -------   ⊙   -------   ⊙   -------   ⊙   -------   ⊙   -------   ⊙   -------   ⊙
   b      |             |             |   ___/‾‾‾   |   ___/‾‾‾   |             |             |
...
only the x coordinate of the vertex.)The forward algorithm: Find the longest diagonal starting at (0,0) and
...
diagonal, in which case take the one with the larger x.Eventually the path gets to (N,M), and the diagonals on it are the LCS.Here is the edit graph with the ends of D-paths labelled. (So, for instance,
...
A:"aabbaa", B:"aacaba"
   ...
   a      |   ___/‾‾‾   |   ___/‾‾‾   |             |             |   ___/‾‾‾   |   ___/‾‾‾   |
          ⊙   -------   ⊙   -------   ⊙   -------   ⊙   -------   ⊙   -------   ⊙   -------   ⊙
                 a             a             b             b             a             aThe 4-path is reconstructed starting at (4/6,6), horizontal to (3/5,6), diagonal to (3,4), vertical
to (2/3,3), horizontal to (1/2,3), vertical to (0/2,2), and diagonal to (0,0). As expected,
there are 4 non-diagonal steps, and the diagonals form an LCS.There is a symmetric backward algorithm, which gives (backwards labels are prefixed with a colon):
A:"aabbaa", B:"aacaba"
            ⊙   --------    ⊙   --------    ⊙   --------    ⊙   --------    ⊙   --------    ⊙   --------    ⊙
    ...
    a       |   ____/‾‾‾    |   ____/‾‾‾    |               |               |   ____/‾‾‾    |   ____/‾‾‾    |
        (:4/0,0)--------    ⊙   --------    ⊙   --------    ⊙   --------(:4/4,0)--------    ⊙   --------    ⊙
                    a               a               b               b               a               aNeither of these is ideal for use in an editor, where it is undesirable to send very long diffs to the
...
asymmetry.Fortunately there is a two-sided algorithm, implied by results in Myers' paper. Here's what the labels in
...
A:"aabbaa", B:"aacaba"
    ...
    a        |    ____/‾‾‾‾    |    ____/‾‾‾‾    |                 |                 |    ____/‾‾‾‾    |    ____/‾‾‾‾    |
             ⊙    ---------    ⊙    ---------    ⊙    ---------    ⊙    ---------    ⊙    ---------    ⊙    ---------    ⊙
                      a                 a                 b                 b                 a                 aThe algorithm stopped when it saw the backwards 2-path ending at (1,3) and the forwards 2-path ending at (3,5). The criterion
...
from the edgegraph. The implementation looks for a number of special cases to try to avoid computing an extra forward path.If the two-sided algorithm has stop early (because D has become too large) it will have found a forward LCS and a
...
and can be found at
http://www.xmailserver.org/diff2.pdf(There is a generic implementation of the algorithm the the repository with git hash
