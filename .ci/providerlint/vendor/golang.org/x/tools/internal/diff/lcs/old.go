...
// license that can be found in the LICENSE file.package lcs// TODO(adonovan): remove unclear references to "old" in this package.import (
	"fmt"
...
// It does not respect rune boundaries. DiffStrings(a, b string) []Diff { return diff(stringSeqs{a, b}) }// DiffBytes returns the differences between two byte sequences.
t does not respect rune boundaries. DiffBytes(a, b []byte) []Diff { return diff(bytesSeqs{a, b}) }// DiffRunes returns the differences between two rune sequences. DiffRunes(a, b []rune) []Diff { return diff(runesSeqs{a, b}) }
...
// The algorithm is one of {forward, backward, twosided}. compute(seqs sequences, algo 
...
type editGraph struct {
	seqs   sequences
	vf, vb label // forward and backward labels	limit int // maximal value of D
the bounding rectangle of the current edit graph
	lx, ly, ux, uy int
	delta          int // common subexpression: (ux-lx)-(uy-ly)
}// toDiffs converts an LCS to a list of edits. (lcs lcs) toDiffs(alen, blen int) []Diff {
	var diffs []Diff
...
urn diffs
}// --- FORWARD ---// fdone decides if the forwward path has reached the upper right
// corner of the rectangle. If so, it also returns the computed lcs. (e *editGraph) fdone(D, k int) (bool, lcs) {
...
= x - k
	...
	return false, nil
}// run the forward algorithm, until success or up to the limit on D. forward(e *editGraph) lcs {
...
}// recover the lcs by backtracking from the farthest point reached (e *editGraph) forwardlcs(D, k int) lcs {
	var ans lcs
...
nd label the result with d (e *editGraph) lookForward(k, relx int) int {
	rely := relx - k
...
= e.lookForward(k, relx)
 ...
 (e *editGraph) getForward(d, k int) int {
	x := e.vf.get(d, k)
	return x
}// --- BACKWARD ---// bdone decides if the backward path has reached the lower left corner (e *editGraph) bdone(D, k int) (bool, lcs) {
	// x, y, k are relative to the rectangle
	x := e.vb.get(D, k)
	y := x - (k + e.delta)
	if x == 0 && y == 0 {
	...
	return false, nil
}// run the backward algorithm, until success or up to the limit on D. backward(e *editGraph) lcs {
...
}// recover the lcs by backtracking (e *editGraph) backwardlcs(D, k int) lcs {
	var ans lcs
...
+
	...
	return ans
}tart at (x,y), go down the diagonal as far as possible, (e *editGraph) lookBackward(k, relx int) int {
	rely := relx - (k + e.delta) // forward k = k + e.delta
	x, y := relx+e.lx, rely+e.ly
	if x > 0 && y > 0 {
		x -= e.seqs.commonSuffixLen(0, x, 0, y)	return x
}// convert to rectangle, and label the result with d (e *editGraph) setBackward(d, k, relx int) {
 ...
 twosided(e *editGraph) lcs {
	...
	e.setForward(0, 0, e.lx)
	e.setBackward(0, 0, e.ux)	// from D to D+1
	for D := 0; D < e.limit; D++ {
...
find the D path with minimal x+y inside the rectangle and
	...
	ans := lcs.fix()
	return ans
}// Does Myers' Lemma apply? (e *editGraph) twoDone(df, db int) (int, bool) {
 ...
 (e *editGraph) twolcs(df, db, kf int) lcs {
	...
	kb := kf - e.delta
	u := e.vb.get(db, kb)
	v := u - kf	// Myers proved there is a df-path from (0,0) to (u,v)
	...
	// followed by the forward path to (x,y) from (0,0).	// Look for some special cases to avoid computing either of these paths.
	if x == u {
		// "babaab" "cccaba"
