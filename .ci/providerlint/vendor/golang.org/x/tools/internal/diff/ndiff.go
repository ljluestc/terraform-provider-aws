...
// license that can be found in the LICENSE file.package diffimport (
	"bytes"
	"unicode/utf8"	"golang.org/x/tools/internal/diff/lcs"
)// Strings computes the differences between two strings.
// The resulting edits respect rune boundaries. Strings(before, after string) []Edit {
	if before == after {
...
he resulting edits respect rune boundaries. Bytes(before, after []byte) []Edit {
	if bytes.Equal(before, after) {
		return nil // common case
	}	if bytesIsASCII(before) && bytesIsASCII(after) {
		return diffASCII(before, after)
	}
	return diffRunes(runes(before), runes(after)) diffASCII(before, after []byte) []Edit {
	diffs := lcs.DiffBytes(before, after)	// Convert from LCS diffs.
...
urn res
 ...
 diffRunes(before, after []rune) []Edit {
	diffs := lcs.DiffRunes(before, after)	// The diffs returned by the lcs package use indexes
...
urn res
}// runes is like []rune(string(bytes)) without the duplicate allocation. runes(bytes []byte) []rune {
	n := utf8.RuneCount(bytes)
	runes := make([]rune, n)
	for i := 0; i < n; i++ {
		r, sz := utf8.DecodeRune(bytes)
		bytes = bytes[sz:]
		runes[i] = r	return runes
}// runesLen returns the length in bytes of the UTF-8 encoding of runes. runesLen(runes []rune) (len int) {
	for _, r := range runes {
		len += utf8.RuneLen(r)	return len
}// stringIsASCII reports whether s contains only ASCII.
// TODO(adonovan): combine when x/tools allows generics. stringIsASCII(s string) bool {
	for i := 0; i < len(s); i++ {
 s[i] >= utf8.RuneSelf {
			return false
		}
