...
// license that can be found in the LICENSE file.package testenvimport (
	"context"
	"os"
	"os/exec"
	"reflect"
...
// using os.StartProcess or (more commonly) exec.Command. HasExec() bool {
	switch runtime.GOOS {
	case "js", "ios":
		return false
	}
...
f not, NeedsExec calls t.Skip with an explanation. NeedsExec(t testing.TB) {
	if !HasExec() {
...
//     leak a subprocess. CommandContext(t testin, ctx context.Context, name string, args ...string) *exec.Cmd {
	t.Helper()
	NeedsExec(t)	var (
		cancelCtx   context.Cancel		gracePeriod time.Duration // unlimited unless the test has a deadline (to allow for interactive debugging)
		...
		cmdTimeout := testTimeout - 2*gracePeriod		if cd, ok := ctx.Deadline(); !ok || time.Until(cd) > cmdTimeout {
			// Either ctx doesn't have a deadline, or its deadline would expire
			// after (or too close before) the test has already timed out.
			// Add a shorter timeout so that the test will produce useful output.
			ctx, cancelCtx = context.WithTimeout(ctx, cmdTimeout)
		}
	}	cmd := exec.CommandContext(ctx, name, args...)	// Use reflection to set the el and WaitDelay fields, if present.
	...
	rc := reflect.ValueOf(cmd).Elem()	if rCancel := rc.FieldByName("Cancel"); rCancel.IsValid() {
...
// testenv.CommandContext (with a default Context). Command(t testing.TB, name string, args ...string) *exec.Cmd {
	t.Helper()
