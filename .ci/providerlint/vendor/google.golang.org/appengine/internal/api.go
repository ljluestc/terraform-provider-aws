...
c.req = rstopFlushing := make(chan int)// Patch up RemoteAddr so it looks reasonable.
...
c.outHeader = nil // make sure header changes aren't respected any morestopFlushing <- 1 // any logging beyond this point will be dropped// Flush any pending logs asynchronously.
c.pendingLogs.Lock()
...
type context struct {
req *http.RequestoutCode   int
outHeader http.Header
outBody   []bytependingLogs struct {
sync.Mutex
