...
//
// Notice: This API is EXPERIMENTAL and may be changed or removed in a
// later release. NewSharedBufferPool() SharedBufferPool {
...
type simpleSharedBufferPool struct {
 ...
 (p *simpleSharedBufferPool) Get(size int) []byte {
	return p.pools[p.poolIdx(size)].Get(size)
}
 (p *simpleSharedBufferPool) Put(bs *[]byte) {
	p.pools[p.poolIdx(cap(*bs))].Put(bs)
}
 (p *simpleSharedBufferPool) poolIdx(size int) int {
	switch {
...
type bufferPool struct {
 ...
 (p *bufferPool) Get(size int) []byte {
	bs := p.Pool.Get().(*[]byte)
 ...
 newBytesPool(size int) simpleSharedBufferChildPool {
	return &bufferPool{
		Pool: sync.Pool{
			New: func() interface{} {
				bs := make([]byte, size)
...
type nopBufferPool struct {
 ...
 (nopBufferPool) Get(length int) []byte {
	return make([]byte, length)
}
 (nopBufferPool) Put(*[]byte) {
}
