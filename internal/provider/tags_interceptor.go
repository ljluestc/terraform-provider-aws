...
functagsUpdateFunc(ctxcontext.Context,dschemaResourceData,spconns.ServicePackage,spt*types.ServicePackageResourceTags,serviceName,resourceNamestring,metaany,diagsdiag.Diagnostics)(context.Context,diag.Diagnostics){
...
varidentifierstring
ifidentifierAttribute:=spt.IdentifierAttribute;identifierAttribute=="id"{
identifier=d.Id()
}else{
...
ifconfig:=d.GetRawConfig();!config.IsNull()&&config.IsKnown(){
c:=config.GetAttr(names.AttrTags)
if!c.IsNull(){
fork,v:=rangec.AsValueMap(){
if!v.IsNull(){
...
ifstate:=d.GetRawState();!state.IsNull()&&state.IsKnown(){
s:=state.GetAttr(names.AttrTagsAll)
fork,v:=ranges.AsValueMap(){
stateTags[k]=v.AsString()
}
}

oldTags:=tftags.New(ctx,stateTags)
//iftags_allwascomputedbecausenotwhollyknown
...
functagsReadFunc(ctxcontext.Context,dschemaResourceData,spconns.ServicePackage,spt*types.ServicePackageResourceTags,serviceName,resourceNamestring,metaany,diagsdiag.Diagnostics)(context.Context,diag.Diagnostics){
inContext,ok:=conns.FromContext(ctx)
if!ok{
returnctx,diags
}
...
err=v.ListTags(ctx,meta,identifier,spt.ResourceType)//SetstagsinContext
}
...
iferr:=d.Set(names.AttrTags,toAdd.ResolveDuplicates(ctx,tagsInContext.DefaultConfig,tagsInContext.IgnoreConfig,d).Map());err!=nil{
returnctx,sdkdiag.AppendErrorf(diags,"setting%s:%s",names.AttrTags,err)
}

//Computedtags_alldo.
iferr:=d.Set(names.AttrTagsAll,toAdd.Map());err!=nil{
