...
func TestRetry(t *testing.T) {
	t.Parallel()	ctx, cf := context.WithDeadline(context.Background(), time.Now().Add(time.Second))
	defer cf()
...
func TestSleepCancellation(t *testing.T) {
	t.Parallel()	const cancelDelay = time.Millisecond * 10
