...
// SPDX-License-Identifier: MPL-2.0package retryimport (
	...
	"errors"
	"fmt"
	"time"	"github.com/hashicorp/terraform-provider-aws/internal/tfresource"
...
}type PredicateFunc[T any] 
func(T, error) (bool, error)
func (f PredicateFunc[T]) Invoke(t T, err error) (bool, error) {
	...
	transformRunError 
func(error) error
}// Operation returns a new wrapper on top of the specified 
function.
...
func (o operation[T]) withTransformRunError(f 
func(error) error) operation[T] {
...
continuousTargetOccurence = 1
	}	targetOccurence := 0	predicate := 
func(_ T, err error) (bool, error) {
if err == nil {
	targetOccurence++	if continuousTargetOccurence == targetOccurence {
return false, nil
	}	return true, nil
}if tfresource.NotFound(err) {
	targetOccurence = 0	return true, err
}return false, err
...
func (o operation[T]) Run(ctx context.Context, timeout time.Duration) (T, error) {
	...
	defer cancel()	for r := Begin(); r.Continue(ctx); {
t, err := o.op.Invoke(ctx)if retry, err := o.predicate.Invoke(t, err); !retry {
	return t, err
}
	}	var t T
