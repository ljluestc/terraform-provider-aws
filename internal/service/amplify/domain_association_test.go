...
// SPDX-License-Identifier: MPL-2.0package amplify_testimport (
	...
	resourceName := "aws_amplify_domain_association.test"	resource.Test(t, resource.TestCase{
	...
	return func(s *terraform.State) error {
		...
		}		appID, domainName, err := tfamplify.DomainAssociationParseResourceID(rs.Primary.ID)		if err != nil {
			return err
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).AmplifyConn(ctx)		domainAssociation, err := tfamplify.FindDomainAssociationByAppIDAndDomainName(ctx, conn, appID, domainName)		if err != nil {
			return err
		}		*v = *domainAssociation		return nil
	}
	...
	return func(s *terraform.State) error {
		conn := acctest.Provider.Meta().(*conns.AWSClient).AmplifyConn(ctx)		for _, rs := range s.RootModule().Resources {
			if rs.Type != "aws_amplify_domain_association" {
				...
				return err
			}			_, err = tfamplify.FindDomainAssociationByAppIDAndDomainName(ctx, conn, appID, domainName)			if tfresource.NotFound(err) {
				continue
			}			if err != nil {
				return err
