...
// SPDX-License-Identifier: MPL-2.0package amplifyimport (
	...
	input := &amplify.GetAppInput{
		AppId: aws.String(id),
	}	output, err := conn.GetAppWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, amplify.ErrCodeNotFoundException) {
	...
	}	output, err := conn.GetBackendEnvironmentWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, amplify.ErrCodeNotFoundException) {
		return nil, &retry.NotFoundError{
			LastError:   err,
			LastRequest: input,
		}
	...
	}	output, err := conn.GetBranchWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, amplify.ErrCodeNotFoundException) {
		return nil, &retry.NotFoundError{
			LastError:   err,
			LastRequest: input,
		}
	...
	}	output, err := conn.GetDomainAssociationWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, amplify.ErrCodeNotFoundException) {
		return nil, &retry.NotFoundError{
			LastError:   err,
			LastRequest: input,
		}
	...
	}	output, err := conn.GetWebhookWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, amplify.ErrCodeNotFoundException) {
		return nil, &retry.NotFoundError{
			LastError:   err,
			LastRequest: input,
		}
