...
// +build sweeppackage amplifyimport (
	"fmt"
	...
	sweepResources := make([]sweep.Sweepable, 0)	input := &amplify.ListAppsInput{}
	err = listAppsPages(ctx, conn, input, func(page *amplify.ListAppsOutput, lastPage bool) bool {
		if page == nil {
			...
			r := ResourceApp()
			d := r.Data(nil)
			d.SetId(aws.StringValue(app.AppId))			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
