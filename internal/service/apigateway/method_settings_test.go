...
func stage1, stage2 apigateway.Stage
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_api_gateway_method_settings.test"
...
funcourceName := "aws_api_gateway_method_settings.test"
...
func TestAccAPIGatewayMethodSettings_Settings_throttlingBurstLimit(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		PreCheck:nc() { acctest.PreCheck(ctx, t); acctest.PreCheckAPIGatewayTypeEDGE(t) },
		ErrorCheck:  acctest.ErrorCheck(t, apigateway.EndpointsID),
funceckDestroy:testAccCheckMethodSettingsDestroy(ctx),
		Steps: []resource.TestStep{
			{
...
func TestAccAPIGatewayMethodSettings_Settings_throttlingRateLimitDisabledByDefault(t *testing.T) {
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		CheckDestroy:testAccCheckMethodSettingsDestroy(ctx),
		Steps: []resource.TestStep{
funcConfig: testAccMethodSettingsConfig_loggingLevel(rName, "INFO"),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckStageExists(ctx, resourceName, &stage1),
					resource.TestCheckResourceAttr(resourceName, "settings.#", "1"),
...
func stage apigateway.Stage
	rName :funcourceName := "aws_api_gateway_method_settings.test"
...
pplication/json" = "Error"
  ...
  request_parameters = {
funcd.request.querystring.page" = e
  }
}
...
resource "aws_api_gateway_method_settings" "test" {
  ...
  settings {
che_ttl_in_seconds = %d
  }
funcacheTtlInSeconds)
}
...
funcurn testAccMethodSettingsBaseConfig(rName) + fmt.Sprintf(`
resource "aws_api_gateway_method_settings" "test" {
  rest_api_id = aws_api_gateway_rest_api.test.id
  stage_name  = aws_api_gateway_deployment.test.stage_name
  method_path = "${aws_api_gateway_resource.test.path_part}/${aws_api_gateway_method.test.http_method}"
...
resource "aws_api_gateway_method_settings" "test" {
  ...
  settings {
quire_authorization_for_cache_control = %t
  }
}
...
funcurce "aws_api_gateway_method_settings" "test" {
  method_path = "${aws_api_gateway_resource.test.path_part}/${aws_api_gateway_method.test.http_method}"
  rest_api_id = aws_api_gateway_rest_api.test.id
  stage_name  = aws_api_gateway_deployment.test.stage_name

  settings {
rottling_burst_limit = %d
  }
}
...
resource "aws_api_gateway_method_settings" "test" {
  method_path = "${aws_api_gateway_resource.test.path_part}/${aws_api_gateway_method.test.http_method}"
funcage_name  = aws_api_gateway_deployment.test.stage_name

  settings {
authorized_cache_control_header_strategy = %q
  }
}
