...
resourceName := "aws_appconfig_application.test"resource.ParallelTest(t, resource.TestCase{
...
}output, err := conn.GetApplicationWithContext(ctx, input)if tfawserr.ErrCodeEquals(err, appconfig.ErrCodeResourceNotFoundException) {
continue
}if err != nil {
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).AppConfigConn(ctx)input := &appconfig.GetApplicationInput{
ApplicationId: aws.String(rs.Primary.ID),
