...
resourceName := "aws_appconfig_extension.test"resource.ParallelTest(t, resource.TestCase{
...
pRequiredFalse := "false"resource.ParallelTest(t, resource.TestCase{
PreCheck:  func() { acctest.PreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, appconfig.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckExtensionDestroy(ctx),
...
rDescription2 := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)resourceName := "aws_appconfig_extension.test"resource.ParallelTest(t, resource.TestCase{
PreCheck:  func() { acctest.PreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, appconfig.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckExtensionDestroy(ctx),
...
}output, err := conn.GetExtensionWithContext(ctx, input)if tfawserr.ErrCodeEquals(err, appconfig.ErrCodeResourceNotFoundException) {
continue
}if err != nil {
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).AppConfigConn(ctx)in := &appconfig.GetExtensionInput{
ExtensionIdentifier: aws.String(rs.Primary.ID),
}output, err := conn.GetExtensionWithContext(ctx, in)if err != nil {
return fmt.Errorf("error reading AppConfig Extension (%s): %w", rs.Primary.ID, err)
}if output == nil {
