...
resourceName := "aws_appconfig_extension_association.test"resource.ParallelTest(t, resource.TestCase{
...
pValue2 := "ParameterValue2"resource.ParallelTest(t, resource.TestCase{
PreCheck:func() { acctest.PreCheck(ctx, t) },
ErrorCheck:  acctest.ErrorCheck(t, appconfig.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckExtensionAssociationDestroy(ctx),
...
}output, err := conn.GetExtensionAssociationWithContext(ctx, input)if tfawserr.ErrCodeEquals(err, appconfig.ErrCodeResourceNotFoundException) {
continue
}if err != nil {
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).AppConfigConn(ctx)in := &appconfig.GetExtensionAssociationInput{
ExtensionAssociationId: aws.String(rs.Primary.ID),
}output, err := conn.GetExtensionAssociationWithContext(ctx, in)if err != nil {
return fmt.Errorf("error reading AppConfig ExtensionAssociation (%s): %w", rs.Primary.ID, err)
}if output == nil {
