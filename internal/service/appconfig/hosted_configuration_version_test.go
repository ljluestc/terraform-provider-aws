...
resourceName := "aws_appconfig_hosted_configuration_version.test"resource.ParallelTest(t, resource.TestCase{
...
continue
}appID, confProfID, versionNumber, err := tfappconfig.HostedConfigurationVersionParseID(rs.Primary.ID)if err != nil {
return err
...
}output, err := conn.GetHostedConfigurationVersionWithContext(ctx, input)if tfawserr.ErrCodeEquals(err, appconfig.ErrCodeResourceNotFoundException) {
continue
}if err != nil {
...
return err
}conn := acctest.Provider.Meta().(*conns.AWSClient).AppConfigConn(ctx)output, err := conn.GetHostedConfigurationVersionWithContext(ctx, &appconfig.GetHostedConfigurationVersionInput{
ApplicationId: aws.String(appID),
ConfigurationProfileId: aws.String(confProfID),
VersionNumber: aws.Int64(int64(versionNumber)),
})if err != nil {
