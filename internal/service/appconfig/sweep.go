...
// +build sweeppackage appconfigimport (
"fmt"
...
var errs *multierror.Errorinput := &appconfig.ListApplicationsInput{}err = conn.ListApplicationsPagesWithContext(ctx, input, func(page *appconfig.ListApplicationsOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, item := range page.Items {
if item == nil {
continue
}id := aws.StringValue(item.Id)log.Printf("[INFO] Deleting AppConfig Application (%s)", id)
...
if item == nil {
continue
}appId := aws.StringValue(item.Id)profilesInput := &appconfig.ListConfigurationProfilesInput{
...
if item == nil {
continue
}id := fmt.Sprintf("%s:%s", aws.StringValue(item.Id), appId)log.Printf("[INFO] Deleting AppConfig Configuration Profile (%s)", id)
...
sweepResources := make([]sweep.Sweepable, 0)
var errs *multierror.Errorinput := &appconfig.ListDeploymentStrategiesInput{}err = conn.ListDeploymentStrategiesPagesWithContext(ctx, input, func(page *appconfig.ListDeploymentStrategiesOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, item := range page.Items {
if item == nil {
continue
}id := aws.StringValue(item.Id)// Deleting AppConfig Predefined Strategies is not supported; returns BadRequestException
...
if item == nil {
continue
}appId := aws.StringValue(item.Id)envInput := &appconfig.ListEnvironmentsInput{
ApplicationId: item.Id,
...
if item == nil {
continue
}profId := aws.StringValue(item.Id)versionInput := &appconfig.ListHostedConfigurationVersionsInput{
...
if item == nil {
continue
}id := fmt.Sprintf("%s/%s/%d", appId, profId, aws.Int64Value(item.VersionNumber))log.Printf("[INFO] Deleting AppConfig Hosted Configuration Version (%s)", id)
