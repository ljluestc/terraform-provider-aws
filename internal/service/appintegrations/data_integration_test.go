...
// SPDX-License-Identifier: MPL-2.0package appintegrations_testimport (
	...
	var dataIntegration appintegrationsservice.GetDataIntegrationOutput	rName := sdkacctest.RandomWithPrefix("resource-test-terraform")
	description := "example description"
	firstExecutionFrom := "1439788442681"	resourceName := "aws_appintegrations_data_integration.test"	key := "DATA_INTEGRATION_SOURCE_URI"
	...
	return func(s *terraform.State) error {
		conn := acctest.Provider.Meta().(*conns.AWSClient).AppIntegrationsConn(ctx)		for _, rs := range s.RootModule().Resources {
			if rs.Type != "aws_appintegrations_data_integration" {
				continue
			}			input := &appintegrationsservice.GetDataIntegrationInput{
				Identifier: aws.String(rs.Primary.ID),
			}			resp, err := conn.GetDataIntegrationWithContext(ctx, input)			if err == nil {
	...
	return func(s *terraform.State) error {
		...
		resp, err := conn.GetDataIntegrationWithContext(ctx, input)		if err != nil {
			return err
		}		*dataIntegration = *resp		return nil
	}
...
resource "aws_appintegrations_data_integration" "test" {
  ...
  source_uri  = %[3]q  schedule_config {
