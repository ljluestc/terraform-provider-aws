...
// SPDX-License-Identifier: MPL-2.0package appintegrations_testimport (
	...
	var eventIntegration appintegrationsservice.GetEventIntegrationOutput	rName := sdkacctest.RandomWithPrefix("resource-test-terraform")
	originalDescription := "original description"
	updatedDescription := "updated description"
	resourceName := "aws_appintegrations_event_integration.test"	key := "EVENT_BRIDGE_PARTNER_EVENT_SOURCE_NAME"
...
acctest.PreCheckPartitionHasService(t, appintegrationsservice.EndpointsID)rorCheck: acctest.ErrorCheck(t, appintegrationsservice.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckEventIntegrationDestroy(ctx),
...
resource.TestCheckResourceAttr(resourceName, "tags.Name", "Test Event Integration"),
	...
	sourceName = os.Getenv(key)
	if sourceName == "" {
urceName = "aws.partner/examplepartner.com"
	}	resource.ParallelTest(t, resource.TestCase{
eCheck: func() {
acctest.PreCheck(ctx, t)
acctest.PreCheckPartitionHasService(t, appintegrationsservice.EndpointsID)rorCheck: acctest.ErrorCheck(t, appintegrationsservice.EndpointsID),
...
nn := acctest.Provider.Meta().(*conns.AWSClient).AppIntegrationsConn(ctx)
...
}resp, err := conn.GetEventIntegrationWithContext(ctx, input)if err == nil {
...
ern nil
	...
	return func(s *terraform.State) error {
, ok := s.RootModule().Resources[name]ifok {
return fmt.Errorf("Not found: %s", name)
o := acctest.Provider.Meta().(*conns.AWSClient).AppIntegrationsConn(ctx)
put := &appintegrationsservice.GetEventIntegrationInput{
Name: aws.String(rs.Primary.ID),sp, err := conn.GetEventIntegrationWithContext(ctx, input)ifrr != nil {
return err
entIntegration = *respretu nil
	}
