...
orgUnitDN := orgUnitFromDomain("Test", domain)resource.ParallelTest(t, resource.TestCase{
PreCheck:
func() { acctest.PreCheck(ctx, t) },
ProtoV5Prov
funckDestroy:testAccCheckDirectoryConfigDestroy(ctx),
...
orgUnitDN2 := orgUnitFromDomain("Two", domain)resource.ParallelTest(t, resource.TestCase{
PreCheck:
func() { acctest.PreCheck(ctx, t) },
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckDirectoryConfigDestroy(ctx),
...
}*appStreamDirectoryConfig = *resp.DirectoryConfigs[0]return nil
}
}
...
sb.Writ
func _, dc := range strings.Split(domainName, ".") {
...
directory_name= %[1]q
organizational_unit_distinguished_names = [%[4]q]service_account_credentials {
 account_name= %[2]q
...
directory_name= %[1]q
organizational_unit_distinguished_names = [%[4]q, %[5]q]service_account_credentials {
 account_name= %[2]q
 account_password = %[3]q
}depends_on = [
func
