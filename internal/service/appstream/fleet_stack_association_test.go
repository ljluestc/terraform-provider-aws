...
func() {
acctest.P
funccctest.PreCheckHasIAMRole(ctx, t, "AmazonAppStreamServiceAccess")
},
...
if !ok {
func
funcnn := acctest.Provider.Meta().(*conns.AWSClient).AppStreamConn(ctx)fleetName, stackName, err := tfappstream.DecodeStackFleetID(rs.Primary.ID)
if err != nil {
return fmt.Errorf("error decoding AppStream Fleet Stack Association ID (%s): %w", rs.Primary.ID, err)
}err = tfappstream.FindFleetStackAssociation(ctx, conn, fleetName, stackName)if tfresource.NotFound(err) {
