...
// SPDX-License-Identifier: MPL-2.0package appstream_testimport (
"context"
"fmt"
...
CheckDestr
funcrCheck:acctest.ErrorCheck(t, appstream.EndpointsID),
Steps: []resource.TestStep{
{
Config: testAccFleetConfig_basic(rName, instanceType),
Check: resource.ComposeTestCheckFunc(
...
instanceType := "stream.standard.small"
instanceTypeUpdate := "stream.standard.medium"resource.ParallelTest(t, resource.TestCase{
PreCheck: 
func() {
acctest.PreCheck(ctx, t)
acctest.PreCheckHasIAMRole(ctx, t, "AmazonAppStreamServiceAccess")
...
fleetType := "ON_DEMAND"
instanceType := "stream.standard.small"
funcplayNameUpdated := "display name of a test updated"resource.ParallelTest(t, resource.TestCase{
PreCheck: 
func() {
acctest.PreCheck(ctx, t)
acctest.PreCheckHasIAMRole(ctx, t, "AmazonAppStreamServiceAccess")
...
resource.TestCheckResourceAttr(resourceName, "name", rName),
resource.
funcource.TestCheckResourceAttr(resourceName, "state", appstream.FleetStateRunning),
resource.TestCheckResourceAttr(resourceName, "stream_view", appstream.StreamViewApp),
acctest.CheckResourceAttrRFC3339(resourceName, "created_time"),
),
},
...
resp, err := conn.DescribeFleetsWithContext(ctx, &appstream.DescribeFleetsInput{Names: []*string{aws.String(rs.Primary.ID)}})if err != nil {
return err
}
funcurn fmt.Errorf("appstream fleet %q does not exist", rs.Primary.ID)
}
func
*appStreamFleet = *resp.Fleets[0]return nil
}
}
func testAccCheckFleetDestroy(ctx context.Context) resource.TestCheckFunc {
return 
func(s *terraform.State) error {
conn := acctest.Provider.Meta().(*conns.AWSClient).AppStreamConn(ctx)for _, rs := range s.RootModule().Resources {
if rs.Type != "aws_appstream_fleet" {
continue
}resp, err := conn.DescribeFleetsWithContext(ctx, &appstream.DescribeFleetsInput{Names: []*string{aws.String(rs.Primary.ID)}})if tfawserr.ErrCodeEquals(err, appstream.ErrCodeResourceNotFoundException) {
continue
}if err != nil {
func
...
instance_type = %[2]qcompute_capacity {
 desired_instances = 1
}
}
`, name, instanceType)
...
funcage_arn = "arn:${data.aws_partition.current.partition}:appstream:${data.aws_region.current.name}::image/Amazon-AppStream2-Sample-Image-03-11-2023"compute_capacity {
 desired_instances = 1
}description= %[2]q
...
stream_view= "DESKTOP"vpc_config {
...
name = %[1]q
image_name = "Amazon-AppStream2-Sample-Image-03-11-2023"compute_capacity {
funcdescription= %[2]q
display_name= %[5]q
idle_disconnect_timeout_in_seconds = 70
enable_default_internet_access= false
fleet_type = %[3]q
instance_type = %[4]q
max_user_duration_in_seconds = 1000vpc_config {
 subnet_ids = aws_subnet.test[*].id
}
}
...
fleet_type = %[3]q
instance_type = %[4]q
max_user_duration_in_seconds = 1000tags = {
 Key = "value"
}vpc_config {
 subnet_ids = aws_subnet.test[*].id
}
