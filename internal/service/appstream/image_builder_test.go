...
func() { acctest.PreCheck(ctx, t) },
		ProtoV5Provid
funceckDestroy:testAccCheckImageBuilderDestroy(ctx),
...
functanceType := "stream.standard.medium"
...
func() { acctest.PreCheck(ctx, t) },
		...
		Steps: []resource.TestStep{
			{
funcConfig: testAccImageBuilderConfig_basic(instanceType, rName),
				Check: resource.ComposeTestCheckFunc(
					testAccCheckImageBuilderExists(ctx, resourceName),
...
functanceTypeUpdate := "stream.standard.medium"	resource.ParallelTest(t, resource.TestCase{
		PreCheck: 
func() { acctest.PreCheck(ctx, t) },
		ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
		CheckDestroy:testAccCheckImageBuilderDestroy(ctx),
		...
		Steps: []resource.TestStep{
			{
				...
				Check: reso
func	testAccCheckImageBuilderExists(ctx, resourceName),
					resource.TestCheckResourceAttr(resourceName, "name", rName),
					resource.TestCheckResourceAttr(resourceName, "state", appstream.ImageBuilderStateRunning),
...
func TestAccAppStreamImageBuilder_tags(t *testing.T) {
	...
	resourceName := "aws_appstream_image_builder.test"
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	instanceType := "stream.standard.small"	resource.ParallelTest(t, resource.TestCase{
		PreCheck: 
func() { acctest.PreCheck(ctx, t) },
funceckDestroy:testAccCheckImageBuilderDestroy(ctx),
		ErrorCheck:acctest.ErrorCheck(t, appstream.EndpointsID),
...
func err := tfappstream.FindImageBuilderByName(ctx, conn, rs.Primary.ID)
functurn err
	}
}
...
func(s *terraform.State) error {
		conn := acctest.Provider.Meta().(*conns.AWSClient).AppStreamConn(ctx)		for _, rs := range s.RootModule().Resources {
			if rs.Type != "aws_appstream_image_builder" {
				continue
			}			_, err := tfappstream.FindImageBuilderByName(ctx, conn, rs.Primary.ID)			if tfresource.NotFound(err) {
				continue
			}
funcf err != nil {
				retu
func			return fmt.Errorf("AppStream ImageBuilder %s still exists", rs.Primary.ID)
		}		return nil
	}
}
...
func testAccImageBuilderConfig_tags1(instanceType, rName, key, value string) string {
	return fmt.Sprintf(`
funcage_name = "AppStream-WinServer2019-06-12-2023"
instance_type = %[1]q
name = %[2]qtags = {
 %[3]q = %[4]q
}
...
funcage_arn= "arn:${data.aws_partition.current.partition}:appstream:%[1]s::image/%[2]s"
instance_type = %[3]q
name = %[4]q
}
