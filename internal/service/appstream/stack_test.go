...
"context"
"fmt"
"testing""github.com/aws/aws-sdk-go/service/appstream"
...
ProtoV5Prov
funckDestroy:testAccCheckStackDestroy(ctx),
...
descriptionUpdated := "Updated Description of a test"resource.ParallelTest(t, resource.TestCase{
PreCheck:
func() { acctest.PreCheck(ctx, t) },
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckStackDestroy(ctx),
...
Check: reso
functAccCheckStackExists(ctx, resourceName, &stackOutput),
resource.TestCheckResourceAttr(resourceName, "application_settings.#", "1"),
resource.TestCheckResourceAttr(resourceName, "application_settings.0.enabled", "true"),
resource.TestCheckResourceAttr(resourceName, "application_settings.0.settings_group", settingsGroup),
),
...
resourceName := "aws_appstream_stack.test"
rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
settingsGroup := "group"resource.ParallelTest(t, resource.TestCase{
...
testAccCheckStackExists(ctx, resourceName, &stackOutput),
resource.T
funcource.TestCheckResourceAttr(resourceName, "application_settings.0.enabled", "true"),
resource.TestCheckResourceAttr(resourceName, "application_settings.0.settings_group", settingsGroup),
),
},
{
...
newPreferredProtocol := "UDP"resource.ParallelTest(t, resource.TestCase{
PreCheck:
func() { acctest.PreCheck(ctx, t) },
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckStackDestroy(ctx),
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).AppStreamConn(ctx)output, err := tfappstream.FindStackByName(ctx, conn, rs.Primary.ID)if err != nil {
func
func *outputreturn nil
}
}
...
continue
}_, err := tfappstream.FindStackByName(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
}
...
resource "aws_appstream_stack" "test" {
name= %[1]q
description = %[2]qembed_host_domains = ["example.com", "subdomain.example.com"]storage_connectors {
...
return fmt.Sprintf(`
resource "aws_appstream_stack" "test" {
funcscription = %[2]qstorage_connectors {
 connector_type = "HOMEFOLDERS"
}user_settings {
...
return fmt.Sprintf(`
resource "aws_appstream_stack" "test" {
name = %[1]qstreaming_experience_settings {
