...
// +build sweeppackage appstreamimport (
"fmt"
...
funcource.AddTestSweepers("aws_appstream_directory_config", &resource.Sweeper{
...
sweepResources := make([]sweep.Sweepable, 0)err = describeDirectoryConfigsPages(ctx, conn, input, 
func(page *appstream.DescribeDirectoryConfigsOutput, lastPage bool) bool {
if page == nil {
return !lastPage
funcfor _, v := range page.DirectoryConfigs {
r := ResourceDirectoryConfig()
d := r.Data(nil)
d.SetId(aws.StringValue(v.DirectoryName))sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
...
functurn fmt.Errorf("error getting client: %w", err)
}
conn := client.AppStreamConn(ctx)
input := &appstream.DescribeFleetsInput{}
sweepResources := make([]sweep.Sweepable, 0)err = describeFleetsPages(ctx, conn, input, 
func(page *appstream.DescribeFleetsOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}
funcr _, v := range page.Fleets {
r := ResourceFleet()
d := r.Data(nil)
...
if err != nil {
return fmt.Errorf("error getting client: %w", err)
funcn := client.AppStreamConn(ctx)
input := &appstream.DescribeImageBuildersInput{}
sweepResources := make([]sweep.Sweepable, 0)err = describeImageBuildersPages(ctx, conn, input, 
func(page *appstream.DescribeImageBuildersOutput, lastPage bool) bool {
if page == nil {
return !lastPage
...
conn := client.AppStreamConn(ctx)
funcepResources := make([]sweep.Sweepable, 0)err = describeStacksPages(ctx, conn, input, 
func(page *appstream.DescribeStacksOutput, lastPage bool) bool {
if page == nil {
return !lastPage
