...
func := acctest.Context(t)
	...
	rEmail := acctest.RandomEmailAddress(domain)	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
func() {
	acctest.P
func
...
ProtoV5Pro
funckDestroy:testAccCheckUserDestroy(ctx),
ErrorCheck:acctest.ErrorCheck(t, appstream.EndpointsID),
Steps: []resource.TestStep{
	{
Config: testAccUserConfig_basic(authType, rEmail),
...
funcrName, authType, err := tfappstream.DecodeUserID(rs.Primary.ID)
	if err 
funcrn err
	}	resp, err := conn.DescribeUsersWithContext(ctx, &appstream.DescribeUsersInput{AuthenticationType: aws.String(authType)})	if tfawserr.ErrCodeEquals(err, appstream.ErrCodeResourceNotFoundException) {
continue
	}	if err != nil {
return err
	}	found := false	for _, out := range resp.Users {
