...
functeConf := &retry.StateChangeConf{
	...
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if output, ok := outputRaw.(*appstream.Fleet); ok {
		if errors := output.FleetErrors; len(errors) > 0 {
			var errs *multierror.Error			for _, err := range errors {
...
funcrget:[]string{appstream.ImageBuilderStateRunning},
	...
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if output, ok := outputRaw.(*appstream.ImageBuilder); ok {
...
func	outputRaw, err := stateConf.WaitForStateContext(ctx)	if output, ok := outputRaw.(*appstream.User); ok {
		return output, err
	}	return nil, err
}
