...
func StatusAPICache(ctx context.Context, conn *appsync.AppSync, name string) retry.StateRefreshFunc {
	return func() (interface{}, string, error) {
		output, err := FindAPICacheByID(ctx, conn, name)		if tfresource.NotFound(err) {
			return nil, "", nil
		}		if err != nil {
			return nil, "", err
...
func statusDomainNameAPIAssociation(ctx context.Context, conn *appsync.AppSync, id string) retry.StateRefreshFunc {
	return func() (interface{}, string, error) {
		output, err := FindDomainNameAPIAssociationByID(ctx, conn, id)		if tfresource.NotFound(err) {
			return nil, "", nil
		}		if err != nil {
			return nil, "", err
