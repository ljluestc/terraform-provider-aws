...
// +build sweeppackage appsyncimport (
	"fmt"
...
func sweepGraphQLAPIs(region string) error {
	...
	var errs *multierror.Error	input := &appsync.ListGraphqlApisInput{}	for {
		...
		if sweep.SkipSweepError(err) {
			...
			break
		}		for _, graphAPI := range output.GraphqlApis {			r := ResourceGraphQLAPI()
			d := r.Data(nil)			id := aws.StringValue(graphAPI.ApiId)
...
func sweepDomainNames(region string) error {
	...
	conn := client.AppSyncConn(ctx)
	sweepResources := make([]sweep.Sweepable, 0)
	var errs *multierror.Error	input := &appsync.ListDomainNamesInput{}	for {
		...
		if sweep.SkipSweepError(err) {
			...
			errs = multierror.Append(errs, err)
			break
		}		for _, dm := range output.DomainNameConfigs {			r := ResourceDomainName()
			d := r.Data(nil)			id := aws.StringValue(dm.DomainName)
			d.SetId(id)			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		if aws.StringValue(output.NextToken) == "" {
			break
		}		input.NextToken = output.NextToken
...
func sweepDomainNameAssociations(region string) error {
	...
	var errs *multierror.Error	input := &appsync.ListDomainNamesInput{}	for {
		...
		if sweep.SkipSweepError(err) {
			...
			errs = multierror.Append(errs, err)
			break
		}		for _, dm := range output.DomainNameConfigs {			r := ResourceDomainNameAPIAssociation()
			d := r.Data(nil)			id := aws.StringValue(dm.DomainName)
			d.SetId(id)			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		if aws.StringValue(output.NextToken) == "" {
			break
