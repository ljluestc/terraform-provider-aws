...
func TestAccAutoScalingAttachment_elb(t *testing.T) {
	...
	resourceName := "aws_autoscaling_attachment.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck:  
...
func TestAccAutoScalingAttachment_multipleELBs(t *testing.T) {
	...
	resource11Name := "aws_autoscaling_attachment.test.10"	resource.ParallelTest(t, resource.TestCase{
PreCheck:  
func() { acctest.PreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, autoscaling.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
continue
	}	var err error	if lbName := rs.Primary.Attributes["elb"]; lbName != "" {
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).AutoScalingConn(ctx)return tfautoscaling.FindAttachmentByLoadBalancerName(ctx, conn, rs.Primary.Attributes["autoscaling_group_name"], rs.Primary.Attributes["elb"])
	}
}
...
if !ok {
	return fmt.Errorf("Not found: %s", n)
}conn := acctest.Provider.Meta().(*conns.AWSClient).AutoScalingConn(ctx)return tfautoscaling.FindAttachmentByTargetGroupARN(ctx, conn, rs.Primary.Attributes["autoscaling_group_name"], rs.Primary.Attributes["lb_target_group_arn"])
	}
}
...
resource "aws_elb" "test" {
  ...
  availability_zones = data.aws_availability_zones.available.names  listener {
 instance_port= 8000
...
resource "aws_autoscaling_attachment" "test" {
  count = %[1]d  autoscaling_group_name = aws_autoscaling_group.test.id
