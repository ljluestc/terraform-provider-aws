...
groupResourceName := "aws_autoscaling_group.test"resource.ParallelTest(t, resource.TestCase{
PreCheck:  
func() { acctest.PreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, autoscaling.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
continue
}_, err := tfautoscaling.FindLaunchConfigurationByName(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).AutoScalingConn(ctx)output, err := tfautoscaling.FindLaunchConfigurationByName(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*v = *outputreturn nil
}
}
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).EC2Conn(ctx)output, err := tfec2.FindImageByID(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*v = *outputreturn nil
}
}
...
return 
func(s *terraform.State) error {
conn := acctest.Provider.Meta().(*conns.AWSClient).EC2Conn(ctx)instanceID := aws.StringValue(group.Instances[idx].InstanceId)
instance, err := tfec2.FindInstanceByID(ctx, conn, instanceID)if err != nil {
return err
}hasPublicIPAddress := aws.StringValue(instance.PublicIpAddress) != ""if hasPublicIPAddress != expected {
...
resource "aws_launch_configuration" "test" {
  name = %[1]q
  image_id= data.aws_ami.amzn-ami-minimal-hvm-ebs.id
  instance_type = "m1.small"  root_block_device {
 volume_type = "gp2"
 volume_size = 11
  }  ebs_block_device {
 device_name = "/dev/sdb"
...
resource "aws_launch_configuration" "test" {
  name= %[1]q
  image_id = data.aws_ami.amzn-ami-minimal-pv-instance-store.id  # When the instance type is updated, the new type must support ephemeral storage.
...
return acctest.ConfigCompose(acctest.ConfigLatestAmazonLinuxHVMEBSAMI(), fmt.Sprintf(`
data "aws_region" "current" {}resource "aws_ami_copy" "test" {
  name= %[1]q
  ...
  instance_type = "t3.micro"  root_block_device {
 volume_size = 10
  }
}
`, rName))
...
resource "aws_launch_configuration" "test" {
  name = %[1]q
  image_id= data.aws_ami.amzn-ami-minimal-hvm-ebs.id
  instance_type = "t3.nano"  root_block_device {
 encrypted= true
 volume_type = "gp2"
 volume_size = 11
  }
...
resource "aws_vpc" "test" {
 ...
 Name = %[1]q
  ...
  map_public_ip_on_launch = %[2]t  tags = {
 Name = %[1]q
  }
}resource "aws_launch_configuration" "test" {
  name= %[1]q
