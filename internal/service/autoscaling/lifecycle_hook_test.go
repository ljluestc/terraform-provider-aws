...
}conn := acctest.Provider.Meta().(*conns.AWSClient).AutoScalingConn(ctx)_, err := tfautoscaling.FindLifecycleHook(ctx, conn, rs.Primary.Attributes["autoscaling_group_name"], rs.Primary.ID)return err
	}
}
...
continue
	}	_, err := tfautoscaling.FindLifecycleHook(ctx, conn, rs.Primary.Attributes["autoscaling_group_name"], rs.Primary.ID)	if tfresource.NotFound(err) {
continue
	}	if err != nil {
return err
...
EOF
  ...
  lifecycle_transition= "autoscaling:EC2_INSTANCE_LAUNCHING"  notification_metadata = <<EOF
{
  "foo": "bar"
}
EOF  notification_target_arn = aws_sqs_queue.test.arn
