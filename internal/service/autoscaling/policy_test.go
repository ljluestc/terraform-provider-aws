...
func TestAccAutoScalingPolicy_basic(t *testing.T) {
	...
	resourceTargetTrackingName := "aws_autoscaling_policy.test_tracking"	resource.ParallelTest(t, resource.TestCase{
		PreCheck:  
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceSimpleName, "scaling_adjustment", "2"),					testAccCheckScalingPolicyExists(ctx, resourceStepName, &v),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(resourceSimpleName, "enabled", "true"),
					resource.TestCheckResourceAttr(resourceSimpleName, "policy_type", "SimpleScaling"),					testAccCheckScalingPolicyExists(ctx, resourceStepName, &v),
...
func TestAccAutoScalingPolicy_disappears(t *testing.T) {
	...
	var v autoscaling.ScalingPolicy
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_autoscaling_policy.test_simple"	resource.ParallelTest(t, resource.TestCase{
		PreCheck:  
func() { acctest.PreCheck(ctx, t) },
		ErrorCheck:acctest.ErrorCheck(t, autoscaling.EndpointsID),
		ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
func TestAccAutoScalingPolicy_zeroValue(t *testing.T) {
	...
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceSimpleName := "aws_autoscaling_policy.test_simple"
	resourceStepName := "aws_autoscaling_policy.test_step"	resource.ParallelTest(t, resource.TestCase{
		PreCheck:  
func() { acctest.PreCheck(ctx, t) },
		ErrorCheck:acctest.ErrorCheck(t, autoscaling.EndpointsID),
		ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
func(s *terraform.State) error {
		...
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).AutoScalingConn(ctx)		output, err := tfautoscaling.FindScalingPolicy(ctx, conn, rs.Primary.Attributes["autoscaling_group_name"], rs.Primary.ID)		if err != nil {
			return err
		}		*v = *output		return nil
	}
}
...
func(s *terraform.State) error {
		conn := acctest.Provider.Meta().(*conns.AWSClient).AutoScalingConn(ctx)		for _, rs := range s.RootModule().Resources {
			if rs.Type != "aws_autoscaling_policy" {
				continue
			}			_, err := tfautoscaling.FindScalingPolicy(ctx, conn, rs.Primary.Attributes["autoscaling_group_name"], rs.Primary.ID)			if tfresource.NotFound(err) {
				continue
			}			if err != nil {
				return err
...
resource "aws_autoscaling_policy" "test_simple" {
 ...
 metric_interval_lower_bound = 2.0
  ...
  autoscaling_group_name = aws_autoscaling_group.test.name
  enabled = true  target_tracking_configuration {
...
resource "aws_autoscaling_policy" "test_simple" {
 ...
 metric_interval_lower_bound = 2.0
  ...
  policy_type= "TargetTrackingScaling"
  autoscaling_group_name = aws_autoscaling_group.test.name
  enabled = false  target_tracking_configuration {
...
resource "aws_autoscaling_policy" "test" {
  name = "%[1]s-tracking"
  policy_type= "TargetTrackingScaling"
  autoscaling_group_name = aws_autoscaling_group.test.name  target_tracking_configuration {
 predefined_metric_specification {
predefined_metric_type = "ASGAverageCPUUtilization"
 } target_value = 40.0
  }
...
resource "aws_autoscaling_policy" "test_simple" {
  ...
  metric_aggregation_type= "Minimum"  step_adjustment {
 scaling_adjustment = 1
 metric_interval_lower_bound = 2.0
