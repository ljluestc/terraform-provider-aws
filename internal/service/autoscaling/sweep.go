...
// +build sweeppackage autoscalingimport (
"fmt"
...
sweepResources := make([]sweep.Sweepable, 0)err = conn.DescribeAutoScalingGroupsPagesWithContext(ctx, input, 
...
d.Set("force_delete", true)sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
...
sweepResources := make([]sweep.Sweepable, 0)err = conn.DescribeLaunchConfigurationsPagesWithContext(ctx, input, 
...
d := r.Data(nil)
d.SetId(aws.StringValue(v.LaunchConfigurationName))sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
})if sweep.SkipSweepError(err) {
