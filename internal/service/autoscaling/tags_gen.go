...
Values: []*string{aws.String(key)},
	...
	}	output, err := conn.DescribeTagsWithContext(ctx, input)	if err != nil {
return nil, err
	}	listTags := KeyValueTags(ctx, output.Tags, identifier, resourceType)	if !listTags.KeyExists(key) {
...
func (p *servicePackage) ListTags(ctx context.Context, meta any, identifier, resourceType string) error {
	tags, err := listTags(ctx, meta.(*conns.AWSClient).AutoScalingConn(ctx), identifier, resourceType)	if err != nil {
return err
	}	if inContext, ok := tftags.FromContext(ctx); ok {
inContext.TagsOut = types.Some(tags)
	}	return nil
}// []*SERVICE.Tag handling// ListOfMap returns a list of autoscaling in flattened map.
//
// Compatible with setting Terraform state for strongly typed configuration blocks.
//
// This 
function strips tag resource identifier and type. Generally, this is
...
func ListOfMap(tags tftags.KeyValueTags) []any {
	var result []any	for _, key := range tags.Keys() {
m := map[string]any{
	"key":key,
	"value": aws.StringValue(tags.KeyValue(key)),	"propagate_at_launch": aws.BoolValue(tags.KeyAdditionalBoolValue(key, "PropagateAtLaunch")),
}result = append(result, m)
	}	return result
}// Tags returns autoscaling service tags.
func Tags(tags tftags.KeyValueTags) []*autoscaling.Tag {
	var result []*autoscaling.Tag	for _, key := range tags.Keys() {
tag := &autoscaling.Tag{
	Key:aws.String(key),
...
m := make(map[string]*tftags.TagData, len(tags))for _, tag := range tags {
...
Value: tag.Value,
	...
	tagData.AdditionalStringFields["ResourceType"] = &resourceType	m[aws.StringValue(tag.Key)] = tagData
...
return KeyValueTags(ctx, tags.List(), identifier, resourceType)
	case []any:
result := make(map[string]*tftags.TagData)for _, tfMapRaw := range tags {
	tfMap, ok := tfMapRaw.(map[string]any)	if !ok {
continue
	}	key, ok := tfMap["key"].(string)	if !ok {
continue
	}	tagData := &tftags.TagData{}	if v, ok := tfMap["value"].(string); ok {
tagData.Value = &v
	}	tagData.AdditionalBoolFields = make(map[string]*bool)
...
tagData.AdditionalBoolFields["PropagateAtLaunch"] = &v
	...
	tagData.AdditionalStringFields["ResourceId"] = &identifier
	tagData.AdditionalStringFields["ResourceType"] = &resourceType	result[key] = tagData
...
func updateTags(ctx context.Context, conn autoscalingiface.AutoScalingAPI, identifier, resourceType string, oldTagsSet, newTagsSet any) error {
	...
	newTags := KeyValueTags(ctx, newTagsSet, identifier, resourceType)	ctx = tflog.SetField(ctx, logging.KeyResourceId, identifier)	removedTags := oldTags.Removed(newTags)
	removedTags = removedTags.IgnoreSystem(names.AutoScaling)
	if len(removedTags) > 0 {
input := &autoscaling.DeleteTagsInput{
	Tags: Tags(removedTags),
}_, err := conn.DeleteTagsWithContext(ctx, input)if err != nil {
...
}_, err := conn.CreateOrUpdateTagsWithContext(ctx, input)if err != nil {
