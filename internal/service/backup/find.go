...
func FindJobByID(ctx context.Context, conn *backup.Backup, id string) (*backup.DescribeBackupJobOutput, error) {
	...
	}	output, err := conn.DescribeBackupJobWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, backup.ErrCodeResourceNotFoundException) {
...
func FindRecoveryPointByTwoPartKey(ctx context.Context, conn *backup.Backup, backupVaultName, recoveryPointARN string) (*backup.DescribeRecoveryPointOutput, error) {
	...
	}	output, err := conn.DescribeRecoveryPointWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, backup.ErrCodeResourceNotFoundException, errCodeAccessDeniedException) {
		return nil, &retry.NotFoundError{
			LastError:err,
			LastRequest: input,
		}
...
func FindVaultAccessPolicyByName(ctx context.Context, conn *backup.Backup, name string) (*backup.GetBackupVaultAccessPolicyOutput, error) {
	...
	}	output, err := conn.GetBackupVaultAccessPolicyWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, backup.ErrCodeResourceNotFoundException, errCodeAccessDeniedException) {
		return nil, &retry.NotFoundError{
			LastError:err,
			LastRequest: input,
		}
...
func FindVaultByName(ctx context.Context, conn *backup.Backup, name string) (*backup.DescribeBackupVaultOutput, error) {
	input := &backup.DescribeBackupVaultInput{
		BackupVaultName: aws.String(name),
	}	output, err := conn.DescribeBackupVaultWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, backup.ErrCodeResourceNotFoundException, errCodeAccessDeniedException) {
		return nil, &retry.NotFoundError{
			LastError:err,
			LastRequest: input,
		}
