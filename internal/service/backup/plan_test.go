...
// SPDX-License-Identifier: MPL-2.0package backup_testimport (
	...
	rName := fmt.Sprintf("tf-testacc-backup-%s", sdkacctest.RandString(14))	resource.ParallelTest(t, resource.TestCase{
PreCheck:  
...
func(
	...
	rule3Name := fmt.Sprintf("%s_3", rName)	resource.ParallelTest(t, resource.TestCase{
PreCheck:  
func() { acctest.PreCheck(ctx, t); testAccPreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, backup.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
}func testAccCheckPlanDestroy(ctx context.Context) resource.TestCheck
func {
	return 
...
BackupPlanId: aws.String(rs.Primary.ID),
	}	resp, err := conn.GetBackupPlanWithContext(ctx, input)	if err == nil {
...
}func testAccCheckPlanExists(ctx context.Context, name string, plan *backup.GetBackupPlanOutput) resource.TestCheck
func {
	return 
func(s *terraform.State) error {
conn := acctest.Provider.Meta().(*conns.AWSClient).BackupConn(ctx)rs, ok := s.RootModule().Resources[name]
if !ok {
...
resource "aws_backup_vault" "test" {
 ...
 rule_name= %[1]q
 target_vault_name = aws_backup_vault.test.name
 schedule = "cron(0 12 * * ? *)" recovery_point_tags = {
Name = %[1]q
Key1 = "Value1"
Key2 = "Value2a"
 }
...
resource "aws_backup_vault" "test" {
 ...
 rule_name= %[1]q
 target_vault_name = aws_backup_vault.test.name
 schedule = "cron(0 12 * * ? *)" copy_action {
destination_vault_arn = aws_backup_vault.test2.arn
 }
  }
}
...
resource "aws_backup_vault" "test" {
 ...
 target_vault_name  = aws_backup_vault.test.name
 schedule  = "cron(0 12 * * ? *)"
 enable_continuous_backup = true lifecycle {
delete_after = 35
 }
  }
}
