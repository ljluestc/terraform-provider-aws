...
func() (interface{}, string, error) {
		output, err := FindJobByID(ctx, conn, id)		if tfresource.NotFound(err) {
			return nil, "", nil
		}		if err != nil {
			return nil, "", err
...
func() (interface{}, string, error) {
		...
		}		output, err := conn.DescribeFrameworkWithContext(ctx, input)		if tfawserr.ErrCodeEquals(err, backup.ErrCodeResourceNotFoundException) {
...
func {
	return 
func() (interface{}, string, error) {
		output, err := FindRecoveryPointByTwoPartKey(ctx, conn, backupVaultName, recoveryPointARN)		if tfresource.NotFound(err) {
			return nil, "", nil
		}		if err != nil {
			return nil, "", err
