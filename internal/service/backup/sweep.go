...
// +build sweeppackage backupimport (
	"fmt"
...
func sweepFramework(region string) error {
	...
	sweepResources := make([]sweep.Sweepable, 0)	err = conn.ListFrameworksPagesWithContext(ctx, input,
...
func(page *backup.ListFrameworksOutput, lastPage bool) bool {
			if page == nil {
				...
				r := ResourceFramework()
				d := r.Data(nil)
				d.SetId(aws.StringValue(framework.FrameworkName))				sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
			}			return !lastPage
...
func sweepReportPlan(region string) error {
	...
	var sweeperErrs *multierror.Error
	sweepResources := make([]sweep.Sweepable, 0)	err = conn.ListReportPlansPagesWithContext(ctx, input,
...
func(page *backup.ListReportPlansOutput, lastPage bool) bool {
			if page == nil {
				...
				d := r.Data(nil)
				d.SetId(aws.StringValue(reportPlan.ReportPlanName))				sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
			}			return !lastPage
		})	if sweep.SkipSweepError(err) {
...
func sweepVaultLockConfiguration(region string) error {
	...
	var errs *multierror.Error	input := &backup.ListBackupVaultsInput{}	err = conn.ListBackupVaultsPagesWithContext(ctx, input,
...
func(page *backup.ListBackupVaultsOutput, lastPage bool) bool {
			if page == nil {
				...
				d := r.Data(nil)
				d.SetId(aws.StringValue(vault.BackupVaultName))				sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
			}			return !lastPage
		})	if err != nil {
...
func sweepVaultPolicies(region string) error {
	...
	var sweeperErrs *multierror.Error
	sweepResources := make([]sweep.Sweepable, 0)	err = conn.ListBackupVaultsPagesWithContext(ctx, input,
...
func(page *backup.ListBackupVaultsOutput, lastPage bool) bool {
			if page == nil {
				...
				name := aws.StringValue(vault.BackupVaultName)				// Ignore Default and Automatic EFS Backup Vaults in region (cannot be deleted)
				...
				d := r.Data(nil)
				d.SetId(name)
				d.Set("force_destroy", true)				sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
			}			return !lastPage
		})	if sweep.SkipSweepError(err) {
