...
// SPDX-License-Identifier: MPL-2.0package backup_testimport (
	...
	var vault backup.GetBackupVaultNotificationsOutput	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
...
BackupVaultName: aws.String(rs.Primary.ID),
	}	resp, err := conn.GetBackupVaultNotificationsWithContext(ctx, input)	if err == nil {
...
resource "aws_backup_vault" "test" {
  ...
  policy_id = "__default_policy_ID"  statement {
 actions = [
"SNS:Publish",
