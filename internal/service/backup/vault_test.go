...
// SPDX-License-Identifier: MPL-2.0package backup_testimport (
	...
	resourceName := "aws_backup_vault.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck:  
...
continue
	}	_, err := tfbackup.FindVaultByName(ctx, conn, rs.Primary.ID)	if tfresource.NotFound(err) {
continue
	}	if err != nil {
return err
...
}func testAccCheckVaultExists(ctx context.Context, name string, v *backup.DescribeBackupVaultOutput) resource.TestCheck
func {
	return 
func(s *terraform.State) error {
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).BackupConn(ctx)output, err := tfbackup.FindVaultByName(ctx, conn, rs.Primary.ID)if err != nil {
	return err
}*v = *outputreturn nil
	}
}func testAccCheckRunDynamoDBTableBackupJob(ctx context.Context, rName string) resource.TestCheck
func { // nosemgrep:ci.backup-in-
func-name
	return 
func(s *terraform.State) error {
client := acctest.Provider.Meta().(*conns.AWSClient)
conn := client.BackupConn(ctx)iamRoleARN := arn.ARN{
...
}jobID := aws.StringValue(output.BackupJobId)_, err = tfbackup.WaitJobCompleted(ctx, conn, jobID, 10*time.Minute)if err != nil {
	return fmt.Errorf("error waiting for Backup Job (%s) complete: %w", jobID, err)
}return nil
	}
}func testAccPreCheck(ctx context.Context, t *testing.T) {
	conn := acctest.Provider.Meta().(*conns.AWSClient).BackupConn(ctx)	input := &backup.ListBackupVaultsInput{}	_, err := conn.ListBackupVaultsWithContext(ctx, input)	if acctest.PreCheckSkipError(err) {
