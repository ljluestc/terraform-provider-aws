...
funcTestAccBatchJobQueueDataSource_basic(t*testing.T){
ctx:=acctest.Context(t)
rName:=sdkacctest.RandomWithPrefix("tf_acc_test_")
resourceName:="aws_batch_job_queue.test"
datasourceName:="data.aws_batch_job_queue.by_name"resource.ParallelTest(t,resource.TestCase{
...
funcTestAccBatchJobQueueDataSource_schedulingPolicy(t*testing.T){
ctx:=acctest.Context(t)
rName:=sdkacctest.RandomWithPrefix("tf_acc_test_")
resourceName:="aws_batch_job_queue.test"
datasourceName:="data.aws_batch_job_queue.test"resource.ParallelTest(t,resource.TestCase{
PreCheck:func(){acctest.PreCheck(ctx,t);testAccPreCheck(ctx,t)},
ErrorCheck:acctest.ErrorCheck(t,batch.EndpointsID),
ProtoV5ProviderFactories:acctest.ProtoV5ProviderFactories,
Steps:[]resource.TestStep{
...
functestAccJobQueueDataSourceConfigBase(rNamestring)string{
returnfmt.Sprintf(`
data"aws_partition""current"{}resource"aws_iam_role""ecs_instance_role"{
...
instance_role=aws_iam_instance_profile.ecs_instance_role.arninstance_type=[
"c4.large",
]max_vcpus=16
min_vcpus=0security_group_ids=[
...
functestAccJobQueueDataSourceConfig_basic(rNamestring)string{
...
functestAccJobQueueDataSourceConfig_schedulingPolicy(rNamestring)string{
returnacctest.ConfigCompose(
testAccJobQueueDataSourceConfigBase(rName),
fmt.Sprintf(`
