...
func monitor costexplorer.AnomalyMonitor
	resourceName := "aws_ce_anomaly_monitor.test"
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
func() { acctest.PreCheck(ctx, t) },
...
funcourceName := "aws_ce_anomaly_monitor.test"
...
func TestAccCEAnomalyMonitor_Dimensional(t *testing.T) {
	...
	var monitor costexplorer.AnomalyMonitor
	resourceName := "aws_ce_anomaly_monitor.test"
	rName := sdkacctest.RandomWithPre
func
...
}resp, err := tfce.FindAnomalyMonitorByARN(ctx, conn, rs.Primary.ID)if err != nil {
	return err
}if resp == nil {
	return fmt.Errorf("Cost Explorer %q does not exist", rs.Primary.ID)
}*anomalyMonitor = *respreturn nil
	}
}
...
func	_, err := tfce.FindAnomalyMonitorByARN(ctx, conn, rs.Primary.ID)	if tfresource.NotFound(err) {
continue
	}	if err != nil {
return err
