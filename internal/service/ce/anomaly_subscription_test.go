...
func := acctest.Context(t)
	...
	address := acctest.RandomEmailAddress(domain)	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
...
func subscription costexplorer.AnomalySubscription
	resourceName := "aws_ce_anomaly_subscription.test"
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	domain := acctest.RandomDomainName()
	address := acctest.RandomEmailAddress(domain)	resource.ParallelTest(t, resource.TestCase{
...
funcourceName := "aws_ce_anomaly_subscription.test"
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	domain := acctest.RandomDomainName()
	address := acctest.RandomEmailAddress(domain)	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
...
funcain := acctest.RandomDomainName()
	...
	address2 := acctest.RandomEmailAddress(domain)	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
func() { acctest.PreCheck(ctx, t) },
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckAnomalySubscriptionDestroy(ctx),
...
func TestAccCEAnomalySubscription_Tags(t *testing.T) {
	...
	resourceName := "aws_ce_anomaly_subscription.test"
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
funcress := acctest.RandomEmailAddress(domain)	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
func() { acctest.PreCheck(ctx, t) },
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckAnomalySubscriptionDestroy(ctx),
...
conn := acctest.Provider.Meta().(*conns.AWSClient).CEConn(ctx)rs, ok := s.RootModule().Resources[n]
if !ok {
funcif rs.Primary.ID == "" {
	return fmt.Errorf("No Cost Explorer Anomaly Subscription is set")
}resp, err := tfce.FindAnomalySubscriptionByARN(ctx, conn, rs.Primary.ID)if err != nil {
	return err
}if resp == nil {
	return fmt.Errorf("Cost Explorer %q does not exist", rs.Primary.ID)
}*anomalySubscription = *respreturn nil
	}
}
...
continue
func
	_, err := tfce.FindAnomalySubscriptionByARN(ctx, conn, rs.Primary.ID)	if tfresource.NotFound(err) {
continue
	}	if err != nil {
return err
...
name= %[1]q
monitor_type = "CUSTOM"monitor_specification = <<JSON
{
funcstCategories": null,
	"Dimensions": null,
	"Not": null,
	"Or": null,
	"Tags": {
...
name= %[1]q
frequency = "WEEKLY"monitor_arn_list = [
...
name = %[1]q
}data "aws_caller_identity" "current" {}data "aws_iam_policy_document" "test" {
policy_id = %[1]q
funcsid = "AWSAnomalyDetectionSNSPublishingPermissions" actions = [
"SNS:Publish",
...
test= "StringEquals"
variable = "AWS:SourceOwner"values = [
...
arn = aws_sns_topic.test.arnpolicy = data.aws_iam_policy_document.test.json
}resource "aws_ce_anomaly_subscription" "test" {
name= %[1]q
frequency = "IMMEDIATE"monitor_arn_list = [
 aws_ce_anomaly_monitor.test.arn,
]subscriber {
 type = "SNS"
