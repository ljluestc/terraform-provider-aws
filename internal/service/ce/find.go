...
MaxResults:aws.Int64(1),
	}	out, err := conn.GetAnomalyMonitorsWithContext(ctx, in)	if tfawserr.ErrCodeEquals(err, costexplorer.ErrCodeUnknownMonitorException) {
...
MaxResults: aws.Int64(1),
	}	out, err := conn.GetAnomalySubscriptionsWithContext(ctx, in)	if tfawserr.ErrCodeEquals(err, costexplorer.ErrCodeUnknownMonitorException) {
return nil, &retry.NotFoundError{
	LastError:err,
	LastRequest: in,
}
...
funcxResults: aws.Int64(1),
	}	out, err := conn.ListCostAllocationTagsWithContext(ctx, in)	if tfawserr.ErrCodeEquals(err, costexplorer.ErrCodeUnknownMonitorException) {
return nil, &retry.NotFoundError{
	LastError:err,
	LastRequest: in,
}
...
func
	out, err := conn.DescribeCostCategoryDefinitionWithContext(ctx, in)	if tfawserr.ErrCodeEquals(err, costexplorer.ErrCodeResourceNotFoundException) {
return nil, &retry.NotFoundError{
	LastError:err,
	LastRequest: in,
}
