...
resourceName := "aws_chime_voice_connector_termination_credentials.test"resource.ParallelTest(t, resource.TestCase{
PreCheck: func() {
...
resp, err := conn.ListVoiceConnectorTerminationCredentialsWithContext(ctx, input)if tfawserr.ErrCodeEquals(err, chime.ErrCodeNotFoundException) {
continue
}if err != nil {
return err
}if resp != nil && resp.Usernames != nil {
...
resource "aws_chime_voice_connector" "chime" {
  ...
  voice_connector_id = aws_chime_voice_connector.chime.id  calling_regions = ["US"]
