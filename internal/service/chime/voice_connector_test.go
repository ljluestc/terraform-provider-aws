...
var voiceConnector *chime.VoiceConnectorvcName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName := "aws_chime_voice_connector.test"resource.ParallelTest(t, resource.TestCase{
PreCheck: func() {
...
if err != nil {
return err
}vc = resp.VoiceConnectorreturn nil
}
...
conn := acctest.Provider.Meta().(*conns.AWSClient).ChimeConn(ctx)input := &chime.ListVoiceConnectorsInput{}_, err := conn.ListVoiceConnectorsWithContext(ctx, input)if acctest.PreCheckSkipError(err) {
