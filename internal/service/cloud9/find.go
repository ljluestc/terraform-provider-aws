...
// SPDX-License-Identifier: MPL-2.0package cloud9import (
	...
	output, err := findEnvironments(ctx, conn, input)	if err != nil {
		return nil, err
	...
	output, err := conn.DescribeEnvironmentsWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, cloud9.ErrCodeNotFoundException) {
	...
	}	output, err := findEnvironment(ctx, conn, input)	if err != nil {
		return nil, err
	...
	out, err := conn.DescribeEnvironmentMembershipsWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, cloud9.ErrCodeNotFoundException) {
		return nil, &retry.NotFoundError{
			LastError:   err,
			LastRequest: input,
		}
		...
		return nil, err
	}	envs := out.Memberships	if len(envs) == 0 {
		return nil, tfresource.NewEmptyResultError(input)
	}	env := envs[0]	if env == nil {
		return nil, tfresource.NewEmptyResultError(input)
	}	return env, nil
}
