...
ARNSeparator = "/"
ARNService= "cloudformation"ResourcePrefixType = "type"
...
// returns arn:aws:cloudformation:us-west-2:123456789012:type/resource/HashiCorp-TerraformAwsProvider-TfAccTestzwv6r2i7 and 00000001.func TypeVersionARNToTypeARNAndVersionID(inputARN string) (string, string, error) {
...
}resourceParts := strings.Split(parsedARN.Resource, ARNSeparator)if actual, expected := len(resourceParts), 4; actual != expected {
...
}.String()return outputTypeARN, resourceParts[len(resourceParts)-1], nil
}
