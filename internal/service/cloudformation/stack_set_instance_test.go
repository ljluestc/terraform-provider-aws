...
// SPDX-License-Identifier: MPL-2.0package cloudformation_testimport (
	"context"
	"fmt"
	"strings"
...
func TestAccCloudFormationStackSetInstance_basic(t *testing.T) {
	...
	resourceName := "aws_cloudformation_stack_set_instance.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
...
//- Destroy for real outside Terraformfunc TestAccCloudFormationStackSetInstance_retainStack(t *testing.T) {
	ctx := acctest.Context(t)
...
if !ok {
	return fmt.Errorf("Not found: %s", resourceName)
}callAs := rs.Primary.Attributes["call_as"]conn := acctest.Provider.Meta().(*conns.AWSClient).CloudFormationConn(ctx)stackSetName, accountID, region, err := tfcloudformation.StackSetInstanceParseResourceID(rs.Primary.ID)if err != nil {
	return err
}output, err := tfcloudformation.FindStackInstanceByName(ctx, conn, stackSetName, accountID, region, callAs)if err != nil {
	return err
}*v = *outputreturn nil
	}
...
// unit IDs rather than an account IDfunc testAccCheckStackSetInstanceForOrganizationalUnitExists(ctx context.Context, resourceName string, v []*cloudformation.StackInstanceSummary) resource.TestCheckFunc {
	return 
func(s *terraform.State) error {
rs, ok := s.RootModule().Resources[resourceName]
if !ok {
	return fmt.Errorf("Not found: %s", resourceName)
}callAs := rs.Primary.Attributes["call_as"]conn := acctest.Provider.Meta().(*conns.AWSClient).CloudFormationConn(ctx)stackSetName, accountOrOrgID, region, err := tfcloudformation.StackSetInstanceParseResourceID(rs.Primary.ID)
if err != nil {
	return err
}
orgIDs := strings.Split(accountOrOrgID, "/")output, err := tfcloudformation.FindStackInstanceSummariesByOrgIDs(ctx, conn, stackSetName, region, callAs, orgIDs)if err != nil {
	return err
}v = outputreturn nil
	}
...
// unit IDs rather than an account IDfunc testAccCheckStackSetInstanceForOrganizationalUnitDestroy(ctx context.Context) resource.TestCheckFunc {
	return 
func(s *terraform.State) error {
...
continue
	}	callAs := rs.Primary.Attributes["call_as"]	stackSetName, accountOrOrgID, region, err := tfcloudformation.StackSetInstanceParseResourceID(rs.Primary.ID)
	if err != nil {
return err
	}
	orgIDs := strings.Split(accountOrOrgID, "/")	output, err := tfcloudformation.FindStackInstanceSummariesByOrgIDs(ctx, conn, stackSetName, region, callAs, orgIDs)	if tfresource.NotFound(err) {
continue
	}
	if len(output) == 0 {
continue
...
func testAccCheckStackSetInstanceStackExists(ctx context.Context, stackInstance *cloudformation.StackInstance, v *cloudformation.Stack) resource.TestCheckFunc {
	return 
func(s *terraform.State) error {
conn := acctest.Provider.Meta().(*conns.AWSClient).CloudFormationConn(ctx)output, err := tfcloudformation.FindStackByName(ctx, conn, aws.StringValue(stackInstance.StackId))if err != nil {
	return err
}*v = *outputreturn nil
	}
}
...
conn := acctest.Provider.Meta().(*conns.AWSClient).CloudFormationConn(ctx)for _, rs := range s.RootModule().Resources {
	if rs.Type != "aws_cloudformation_stack_set_instance" {
continue
	}	callAs := rs.Primary.Attributes["call_as"]	stackSetName, accountID, region, err := tfcloudformation.StackSetInstanceParseResourceID(rs.Primary.ID)	if err != nil {
return err
	}	_, err = tfcloudformation.FindStackInstanceByName(ctx, conn, stackSetName, accountID, region, callAs)	if tfresource.NotFound(err) {
continue
	}	if err != nil {
return err
	}	return fmt.Errorf("CloudFormation StackSet Instance %s still exists", rs.Primary.ID)
...
EOF  role = aws_iam_role.Administration.name
...
EOF  role = aws_iam_role.Execution.name
}resource "aws_cloudformation_stack_set" "test" {
  depends_on = [aws_iam_role_policy.Execution]  administration_role_arn = aws_iam_role.Administration.arn
...
resource "aws_cloudformation_stack_set_instance" "test" {
  depends_on = [aws_iam_role_policy.Administration, aws_iam_role_policy.Execution]  stack_set_name = aws_cloudformation_stack_set.test.name
}
`)
}
...
resource "aws_cloudformation_stack_set_instance" "test" {
  depends_on = [aws_iam_role_policy.Administration, aws_iam_role_policy.Execution]  parameter_overrides = {
 Parameter1 = %[1]q
...
func testAccStackSetInstanceConfig_retain(rName string, retainStack bool) string {
	return acctest.ConfigCompose(testAccStackSetInstanceBaseConfig(rName), fmt.Sprintf(`
resource "aws_cloudformation_stack_set_instance" "test" {
  depends_on = [aws_iam_role_policy.Administration, aws_iam_role_policy.Execution]  retain_stack= %[1]t
  stack_set_name = aws_cloudformation_stack_set.test.name
}
`, retainStack))
}
func testAccStackSetInstanceBaseConfig_ServiceManagedStackSet(rName string) string {
	return fmt.Sprintf(`
data "aws_partition" "current" {}resource "aws_iam_role" "Administration" {
  assume_role_policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
...
EOF  role = aws_iam_role.Execution.name
}data "aws_organizations_organization" "test" {}resource "aws_cloudformation_stack_set" "test" {
  depends_on = [data.aws_organizations_organization.test]  name= %[1]q
  permission_model = "SERVICE_MANAGED"  auto_deployment {
 enabled= true
...
resource "aws_cloudformation_stack_set_instance" "test" {
  depends_on = [aws_iam_role_policy.Administration, aws_iam_role_policy.Execution]  deployment_targets {
...
func testAccStackSetInstanceConfig_operationPreferences(rName string) string {
	return acctest.ConfigCompose(testAccStackSetInstanceBaseConfig_ServiceManagedStackSet(rName), `
resource "aws_cloudformation_stack_set_instance" "test" {
  depends_on = [aws_iam_role_policy.Administration, aws_iam_role_policy.Execution]  operation_preferences {
