...
func TestAccCloudFormationStackSet_basic(t *testing.T) {
	...
	resourceName := "aws_cloudformation_stack_set.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
...
func TestAccCloudFormationStackSet_Parameters_default(t *testing.T) {
	...
	//  * https://github.com/hashicorp/terraform/issues/18863	ctx := acctest.Context(t)
	var stackSet1, stackSet2 cloudformation.StackSet
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_cloudformation_stack_set.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
...
func TestAccCloudFormationStackSet_Parameters_noEcho(t *testing.T) {
	...
	// Additional references:
	//  * https://github.com/hashicorp/terraform-provider-aws/issues/55	ctx := acctest.Context(t)
	var stackSet1, stackSet2 cloudformation.StackSet
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_cloudformation_stack_set.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
...
func TestAccCloudFormationStackSet_PermissionModel_serviceManaged(t *testing.T) {
	acctest.Skip(t, "API does not support enabling Organizations access (in particular, creating the Stack Sets IAM Service-Linked Role)")	ctx := acctest.Context(t)
	var stackSet1 cloudformation.StackSet
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_cloudformation_stack_set.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
...
}// https://github.com/hashicorp/terraform-provider-aws/issues/19015.func TestAccCloudFormationStackSet_autoDeploymentEnabled(t *testing.T) {
	ctx := acctest.Context(t)
	var stackSet cloudformation.StackSet
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_cloudformation_stack_set.test"	resource.ParallelTest(t, resource.TestCase{
...
}// https://github.com/hashicorp/terraform-provider-aws/issues/19015.func TestAccCloudFormationStackSet_autoDeploymentDisabled(t *testing.T) {
	ctx := acctest.Context(t)
	var stackSet cloudformation.StackSet
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_cloudformation_stack_set.test"	resource.ParallelTest(t, resource.TestCase{
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).CloudFormationConn(ctx)output, err := tfcloudformation.FindStackSetByName(ctx, conn, rs.Primary.ID, rs.Primary.Attributes["call_as"])if err != nil {
	return err
}*v = *outputreturn nil
	}
}
...
continue
	}	_, err := tfcloudformation.FindStackSetByName(ctx, conn, rs.Primary.ID, rs.Primary.Attributes["call_as"])	if tfresource.NotFound(err) {
continue
	}	if err != nil {
return err
...
func testAccPreCheckStackSet(ctx context.Context, t *testing.T) {
	conn := acctest.Provider.Meta().(*conns.AWSClient).CloudFormationConn(ctx)	input := &cloudformation.ListStackSetsInput{}
	_, err := conn.ListStackSetsWithContext(ctx, input)	if acctest.PreCheckSkipError(err) || tfawserr.ErrMessageContains(err, "ValidationError", "AWS CloudFormation StackSets is not supported") {
...
resource "aws_iam_role" "test" {
  count = %[2]d  assume_role_policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
 {
...
resource "aws_cloudformation_stack_set" "test" {
  ...
  name = %[1]q  template_body = <<TEMPLATE
%[2]s
TEMPLATE
}
...
resource "aws_s3_bucket" "test" {
  ...
  bucket = aws_s3_bucket.test.id  block_public_acls = false
...
resource "aws_s3_object" "test" {
  acl = "public-read"
  bucket = aws_s3_bucket.test.bucket  content = <<CONTENT
%[2]s
CONTENT  key = "%[1]s-template1.yml"  depends_on = [aws_s3_bucket_acl.test]
}resource "aws_cloudformation_stack_set" "test" {
  administration_role_arn = aws_iam_role.test[0].arn
  name = %[1]q
...
resource "aws_s3_object" "test" {
  ...
  bucket = aws_s3_bucket.test.bucket  content = <<CONTENT
%[2]s
CONTENT  key = "%[1]s-template2.yml"  depends_on = [aws_s3_bucket_acl.test]
}resource "aws_cloudformation_stack_set" "test" {
  administration_role_arn = aws_iam_role.test[0].arn
  name = %[1]q
  template_url= "https://${aws_s3_bucket.test.bucket_regional_domain_name}/${aws_s3_object.test.key}"
...
resource "aws_cloudformation_stack_set" "test" {
  administration_role_arn = aws_iam_role.test[0].arn
  name = %[1]q  operation_preferences {
