...
func TestAccCloudFormationStack_basic(t *testing.T) {
	...
	resourceName := "aws_cloudformation_stack.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
...
func TestAccCloudFormationStack_updateFailure(t *testing.T) {
	...
	var stack cloudformation.Stack
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_cloudformation_stack.test"	vpcCidrInitial := "10.0.0.0/16"
	vpcCidrInvalid := "1000.0.0.0/16"	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
func() { acctest.PreCheck(ctx, t) },
ErrorCheck:  acctest.ErrorCheck(t, cloudformation.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
func TestAccCloudFormationStack_allAttributes(t *testing.T) {
	...
	expectedPolicyBody := "{\"Statement\":[{\"Action\":\"Update:*\",\"Effect\":\"Deny\",\"Principal\":\"*\",\"Resource\":\"LogicalResourceId/StaticVPC\"},{\"Action\":\"Update:*\",\"Effect\":\"Allow\",\"Principal\":\"*\",\"Resource\":\"*\"}]}"	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
func() { acctest.PreCheck(ctx, t) },
ErrorCheck:  acctest.ErrorCheck(t, cloudformation.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
}// Regression for https://github.com/hashicorp/terraform/issues/4332func TestAccCloudFormationStack_withParams(t *testing.T) {
	ctx := acctest.Context(t)
	var stack cloudformation.Stack
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_cloudformation_stack.test"	vpcCidrInitial := "10.0.0.0/16"
	vpcCidrUpdated := "12.0.0.0/16"	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
func() { acctest.PreCheck(ctx, t) },
ErrorCheck:  acctest.ErrorCheck(t, cloudformation.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
}// Regression for https://github.com/hashicorp/terraform/issues/4534func TestAccCloudFormationStack_WithURL_withParams(t *testing.T) {
	ctx := acctest.Context(t)
	var stack cloudformation.Stack
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_cloudformation_stack.test"	resource.ParallelTest(t, resource.TestCase{
...
}// Test for https://github.com/hashicorp/terraform/issues/5653func TestAccCloudFormationStack_WithURLWithParams_noUpdate(t *testing.T) {
	ctx := acctest.Context(t)
	var stack cloudformation.Stack
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_cloudformation_stack.test"	resource.ParallelTest(t, resource.TestCase{
...
}// TestAccCloudFormationStack_onFailure verifies https://github.com/hashicorp/terraform-provider-aws/issues/5204func TestAccCloudFormationStack_onFailure(t *testing.T) {
	ctx := acctest.Context(t)
	var stack cloudformation.Stack
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_cloudformation_stack.test"	resource.ParallelTest(t, resource.TestCase{
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).CloudFormationConn(ctx)output, err := tfcloudformation.FindStackByName(ctx, conn, rs.Primary.ID)if err != nil {
	return err
}*v = *outputreturn nil
	}
}
...
continue
	}	_, err := tfcloudformation.FindStackByName(ctx, conn, rs.Primary.ID)	if tfresource.NotFound(err) {
continue
	}	if err != nil {
return err
...
func testAccCheckStackDisappears(ctx context.Context, stack *cloudformation.Stack) resource.TestCheckFunc {
	return 
func(s *terraform.State) error {
conn := acctest.Provider.Meta().(*conns.AWSClient).CloudFormationConn(ctx)input := &cloudformation.DeleteStackInput{
	StackName: stack.StackName,
}_, err := conn.DeleteStackWithContext(ctx, input)if err != nil {
	return err
...
resource "aws_cloudformation_stack" "test" {
  name = %[1]q
  on_failure = %[2]q  template_body = <<STACK
{
  "Resources" : {
 "MyVPC": {
"Type" : "AWS::EC2::VPC",
...
data "aws_partition" "current" {}resource "aws_cloudformation_stack" "test" {
  name = %[1]q
  template_body = <<STACK
{
  "Parameters" : {
...
func testAccStackConfig_baseTemplateURL(rName string) string {
	return fmt.Sprintf(`
data "aws_partition" "current" {}data "aws_region" "current" {}resource "aws_s3_bucket" "b" {
  bucket = %[1]q
}resource "aws_s3_bucket_public_access_block" "b" {
  bucket = aws_s3_bucket.b.id  block_public_acls = false
...
resource "aws_s3_bucket" "test" {
  ...
  name = %[1]q
  on_failure = "DO_NOTHING"  template_body = jsonencode({
 AWSTemplateFormatVersion = "2010-09-09"
 Resources = {
S3Bucket = {
  Type = "AWS::S3::Bucket"
