...
// +build sweeppackage cloudformationimport (
	"fmt"
...
Status: aws.String(cloudformation.StackSetStatusActive),
	...
	sweepResources := make([]sweep.Sweepable, 0)	err = conn.ListStackSetsPagesWithContext(ctx, input, 
...
if page == nil {
	...
	r := ResourceStackSet()
	d := r.Data(nil)
	d.SetId(aws.StringValue(summary.StackSetName))	sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
	})	if sweep.SkipSweepError(err) {
...
StackStatusFilter: aws.StringSlice([]string{
	...
	var sweeperErrs *multierror.Error	err = conn.ListStacksPagesWithContext(ctx, input, 
func(page *cloudformation.ListStacksOutput, lastPage bool) bool {
for _, stack := range page.StackSummaries {
	name := aws.StringValue(stack.StackName)	updateTerminationProtectionInput := &cloudformation.UpdateTerminationProtectionInput{
EnableTerminationProtection: aws.Bool(false),
StackName:stack.StackName,
	}	log.Printf("[INFO] Disabling termination protection for CloudFormation Stack: %s", name)
	_, err := conn.UpdateTerminationProtectionWithContext(ctx, updateTerminationProtectionInput)	if err != nil {
