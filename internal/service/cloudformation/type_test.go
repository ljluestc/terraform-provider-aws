...
func TestAccCloudFormationType_basic(t *testing.T) {
	...
	resourceName := "aws_cloudformation_type.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).CloudFormationConn(ctx)_, err := tfcloudformation.FindTypeByARN(ctx, conn, rs.Primary.ID)return err
	}
}
...
continue
	}	_, err := tfcloudformation.FindTypeByARN(ctx, conn, rs.Primary.ID)	if tfresource.NotFound(err) {
continue
	}	if err != nil {
return err
...
// The test skip messaging includes directions for how to locally build the binary.func testAccTypeZipGenerator(t *testing.T, typeName string) string {
	t.Helper()
	tempDir := t.TempDir()
	_, currentFilePath, _, _ := runtime.Caller(0)	sourceDirectoryPath := filepath.Join(filepath.Dir(currentFilePath), "testdata", "examplecompany-exampleservice-exampleresource")
	targetDirectoryPath := filepath.Join(tempDir, "examplecompany-exampleservice-exampleresource")
	targetZipFilePath := filepath.Join(tempDir, "examplecompany-exampleservice-exampleresource.zip")	err := os.Mkdir(targetDirectoryPath, 0777)	if err != nil {
t.Fatalf("error making directory %s: %s", targetDirectoryPath, err)
	...
	targetRpdkConfigFilePath := filepath.Join(targetDirectoryPath, ".rpdk-config")	err = testAccTypeCopyFileWithTypeNameReplacement(sourceRpdkConfigFilePath, targetRpdkConfigFilePath, typeName)	if err != nil {
t.Fatal(err)
	...
	targetHandlerZipFilePath := filepath.Join(targetDirectoryPath, "handler.zip")	err = testAccTypeBuildHandlerZip(sourceBinHandlerFilePath, targetHandlerZipFilePath)	if errors.Is(err, os.ErrNotExist) {
...
t.Fatal(err)
	...
	targetSchemaFilePath := filepath.Join(targetDirectoryPath, "schema.json")	err = testAccTypeCopyFileWithTypeNameReplacement(sourceSchemaFilePath, targetSchemaFilePath, typeName)	if err != nil {
t.Fatal(err)
	}	err = testAccTypeBuildDirectoryZip(targetDirectoryPath, targetZipFilePath)	if err != nil {
t.Fatal(err)
	}	return targetZipFilePath
}
func testAccTypeBuildDirectoryZip(sourceDirectoryPath string, targetZipFilePath string) error {
	targetZipFile, err := os.Create(targetZipFilePath)	if err != nil {
return fmt.Errorf("error creating file %s: %w", targetZipFilePath, err)
	}	targetZipFileWriter := zip.NewWriter(targetZipFile)	targetDirectoryFiles, err := os.ReadDir(sourceDirectoryPath)	if err != nil {
...
sourceFilePath := filepath.Join(sourceDirectoryPath, targetDirectoryFile.Name())sourceFile, err := os.Open(sourceFilePath)if err != nil {
	return fmt.Errorf("error reading file %s: %w", sourceFilePath, err)
}targetFile, err := targetZipFileWriter.Create(sourceFilePath)if err != nil {
	return fmt.Errorf("error creating file %s in zip: %w", sourceFilePath, err)
}_, err = io.Copy(targetFile, sourceFile)if err != nil {
	return fmt.Errorf("error copying file %s into zip: %w", sourceFilePath, err)
}
	}	err = targetZipFileWriter.Close()	if err != nil {
...
func testAccTypeBuildHandlerZip(sourceBinHandlerFilePath string, targetZipFilePath string) error {
	sourceBinHandlerFile, err := os.Open(sourceBinHandlerFilePath)	if err != nil {
return fmt.Errorf("error reading file %s: %w", sourceBinHandlerFilePath, err)
	}	targetZipFile, err := os.Create(targetZipFilePath)	if err != nil {
return fmt.Errorf("error creating file %s: %w", targetZipFilePath, err)
	}	targetZipWriter := zip.NewWriter(targetZipFile)	targetHandlerFile, err := targetZipWriter.Create("handler")	if err != nil {
return fmt.Errorf("error creating file %s in zip: %w", "handler", err)
	}	_, err = io.Copy(targetHandlerFile, sourceBinHandlerFile)	if err != nil {
return fmt.Errorf("error copying file %s into zip: %w", sourceBinHandlerFilePath, err)
	}	err = targetZipWriter.Close()	if err != nil {
return fmt.Errorf("error writing zip %s: %w", targetZipFilePath, err)
	}	err = targetZipFile.Close()	if err != nil {
return fmt.Errorf("error writing file %s: %w", targetZipFilePath, err)
	}	return nil
}
func testAccTypeCopyFileWithTypeNameReplacement(sourceFilePath string, targetFilePath string, targetTypeName string) error {
	...
	sourceFile, err := os.ReadFile(sourceFilePath)	if err != nil {
return fmt.Errorf("error reading file %s: %w", sourceFilePath, err)
	}	err = os.WriteFile(targetFilePath, []byte(strings.ReplaceAll(string(sourceFile), sourceTypeName, targetTypeName)), 0644)	if err != nil {
