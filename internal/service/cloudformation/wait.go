...
Refresh: StatusChangeSet(ctx, conn, stackID, changeSetName),
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if output, ok := outputRaw.(*cloudformation.DescribeChangeSetOutput); ok {
if status := aws.StringValue(output.Status); status == cloudformation.ChangeSetStatusFailed {
	...
	StackSetInstanceCreatedDefaultTimeout = 30 * time.Minute	// Default maximum amount of time to wait for a StackSetInstance to be Updated
	StackSetInstanceUpdatedDefaultTimeout = 30 * time.Minute	// Default maximum amount of time to wait for a StackSetInstance to be Deleted
	StackSetInstanceDeletedDefaultTimeout = 30 * time.Minute	stackSetOperationDelay = 5 * time.Second
)
...
Delay:stackSetOperationDelay,
	}	outputRaw, waitErr := stateConf.WaitForStateContext(ctx)	if output, ok := outputRaw.(*cloudformation.StackSetOperation); ok {
...
CallAs: aws.String(callAs),
	...
	var summaries []*cloudformation.StackSetOperationResultSummary	listErr := conn.ListStackSetOperationResultsPagesWithContext(ctx, input, 
func(page *cloudformation.ListStackSetOperationResultsOutput, lastPage bool) bool {
if page == nil {
	return !lastPage
}summaries = append(summaries, page.Summaries...)return !lastPage
	})	if listErr == nil {
...
Timeout: TypeRegistrationTimeout,
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if output, ok := outputRaw.(*cloudformation.DescribeTypeRegistrationOutput); ok {
return output, err
	}	return nil, err
}
