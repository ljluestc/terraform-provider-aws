...
// SPDX-License-Identifier: MPL-2.0package cloudsearch_testimport (
	"context"
	"fmt"
	"testing"	"github.com/aws/aws-sdk-go/service/cloudsearch"
	...
	rName := acctest.ResourcePrefix + "-" + sdkacctest.RandString(28-(len(acctest.ResourcePrefix)+1))	resource.ParallelTest(t, resource.TestCase{
	...
	return func(s *terraform.State) error {
		...
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).CloudSearchConn(ctx)		_, err := tfcloudsearch.FindAccessPolicyByName(ctx, conn, rs.Primary.ID)		return err
	}
	...
	return func(s *terraform.State) error {
		for _, rs := range s.RootModule().Resources {
			if rs.Type != "aws_cloudsearch_domain_service_access_policy" {
				continue
			}			conn := acctest.Provider.Meta().(*conns.AWSClient).CloudSearchConn(ctx)			_, err := tfcloudsearch.FindAccessPolicyByName(ctx, conn, rs.Primary.ID)			if tfresource.NotFound(err) {
				continue
			}			if err != nil {
				return err
...
resource "aws_cloudsearch_domain" "test" {
  ...
  domain_name = aws_cloudsearch_domain.test.id  access_policy = <<POLICY
{
  "Version":"2012-10-17",
  "Statement":[{
