...
rName := testAccDomainName()resource.ParallelTest(t, resource.TestCase{
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).CloudSearchConn(ctx)output, err := tfcloudsearch.FindDomainStatusByName(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*v = *outputreturn nil
}
...
continue
}conn := acctest.Provider.Meta().(*conns.AWSClient).CloudSearchConn(ctx)_, err := tfcloudsearch.FindDomainStatusByName(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
