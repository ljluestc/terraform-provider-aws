...
// SPDX-License-Identifier: MPL-2.0package cloudwatch_testimport (
	...
	resourceName := "aws_cloudwatch_composite_alarm.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck:k(ctx, t) },
rorCheck: acctest.ErrorCheck(t, cloudwatch.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckCompositeAlarmDestroy(ctx),
...
nn := acctest.Provider.Meta().(*conns.AWSClient).CloudWatchConn(ctx)fo_, rs := range s.RootModule().Resources {
if rs.Type != "aws_cloudwatch_composite_alarm" {
continue
}_, err := tfcloudwatch.FindCompositeAlarmByName(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
o := acctest.Provider.Meta().(*conns.AWSClient).CloudWatchConn(ctx)_, e := tfcloudwatch.FindCompositeAlarmByName(ctx, conn, rs.Primary.ID)returnrr
	}
}func testAccCompositeAlarmConfig_base(rName string) string {
	return fmt.Sprintf(`
resource "aws_cloudwatch_metric_alarm" "test" {
  count = 2  alarm_name = "%[1]s-${count.index}"
...
resource "aws_cloudwatch_composite_alarm" "test" {
  alarm_name = %[1]q
  alarm_rule = join(" OR ", formatlist("ALARM(%%s)", aws_cloudwatch_metric_alarm.test[*].alarm_name))  tags = {
%[2]q = %[3]q
  }
}
...
resource "aws_cloudwatch_composite_alarm" "test" {
  ...
  alarm_name
  alarm_rulet("ALARM(%%s)", aws_cloudwatch_metric_alarm.test[*].alarm_name))
}
`, rName, description))
...
resource "aws_sns_topic" "test" {
  ...
  alarm_rule = "ALARM(${aws_cloudwatch_metric_alarm.test[0].alarm_name})"
  insufficient_data_actions = [aws_sns_topic.test[1].arn]
  ok_actions = [aws_sns_topic.test[2].arn]  actions_suppressor {
