...
eckDestroy:testAccCheckDashboardDestroy(ctx),
...
o := acctest.Provider.Meta().(*conns.AWSClient).CloudWatchConn(ctx)
rams := cloudwatch.GetDashboardInput{
DashboardName: aws.String(rs.Primary.ID),
e, err := conn.GetDashboardWithContext(ctx, &params)
 err != nil {
return err
dhboard = *respretu nil
	}
...
ern nil
	...
	return func(s *terraform.State) error {
nn := acctest.Provider.Meta().(*conns.AWSClient).CloudWatchConn(ctx)pams := cloudwatch.GetDashboardInput{
DashboardName: aws.String(dashboardName),
,rr := conn.GetDashboardWithContext(ctx, &params)if e == nil {
return fmt.Errorf("Dashboard still exists: %s", dashboardName)
ftfcloudwatch.IsDashboardNotFoundErr(err) {
return err
ern nil
	}
}const (
  ...
  "widgets": [
{
 ...
 "height": 6,
 "properties": {
om Terraform: CloudWatch - updated"
 }
}
  ]
}`
...
frr := json.NewDecoder(bodyReader).Decode(&body); err != nil {
...
freflect.DeepEqual(body, expectedBody) {
