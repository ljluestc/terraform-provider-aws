...
// +build sweeppackage cloudwatchimport (
"fmt"
...
sweepResources := make([]sweep.Sweepable, 0)err = conn.DescribeAlarmsPagesWithContext(ctx, input, func(page *cloudwatch.DescribeAlarmsOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, v := range page.CompositeAlarms {
r := ResourceCompositeAlarm()
d := r.Data(nil)
d.SetId(aws.StringValue(v.AlarmName))sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
