...
returnnames.CodeArtifact
}

//NewConnreturnsanewAWSSDKforGov1clientforthisservicepackage'sAWSAPI.
func(p*servicePackage)NewConn(ctxcontext.Context,configmap[string]any)(*codeartifact_sdkv1.CodeArtifact,error){
sess:=config["session"].(*session_sdkv1.Session)

returncodeartifact_sdkv1.New(sess.Copy(&aws_sdkv1.Config{Endpoint:aws_sdkv1.String(config["endpoint"].(string))})),nil
}
