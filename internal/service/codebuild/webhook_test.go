...
funcTestAccCodeBuildWebhook_bitbucket(t*testing.T){
ctx:=acctest.Context(t)
varwebhookcodebuild.Webhook
...
funcTestAccCodeBuildWebhook_gitHub(t*testing.T){
ctx:=acctest.Context(t)
varwebhookcodebuild.Webhook
rName:=sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName:="aws_codebuild_webhook.test"
...
funcTestAccCodeBuildWebhook_gitHubEnterprise(t*testing.T){
ctx:=acctest.Context(t)
varwebhookcodebuild.Webhook
rName:=sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName:="aws_codebuild_webhook.test"
...
funcTestAccCodeBuildWebhook_buildType(t*testing.T){
ctx:=acctest.Context(t)
varwebhookcodebuild.Webhook
rName:=sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName:="aws_codebuild_webhook.test"
...
funcTestAccCodeBuildWebhook_branchFilter(t*testing.T){
ctx:=acctest.Context(t)
varwebhookcodebuild.Webhook
rName:=sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName:="aws_codebuild_webhook.test"
...
funcTestAccCodeBuildWebhook_filterGroup(t*testing.T){
ctx:=acctest.Context(t)
varwebhookcodebuild.Webhook
rName:=sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName:="aws_codebuild_webhook.test"
...
functestAccCheckWebhookFilter(webhook*codebuild.Webhook,expectedFilters[][]*codebuild.WebhookFilter)resource.TestCheck
func{
return
func(s*terraform.State)error{
ifwebhook==nil{
returnfmt.Errorf("webhookmissing")
}

if!reflect.DeepEqual(webhook.FilterGroups,expectedFilters){
returnfmt.Errorf("expectedwebhookfilterconfiguration(%v),got:%v",expectedFilters,webhook.FilterGroups)
}
...
functestAccCheckWebhookDestroy(ctxcontext.Context)resource.TestCheck
func{
return
func(s*terraform.State)error{
...
ifrs.Type!="aws_codebuild_webhook"{
continue
}
...
iflen(resp.Projects)==0{
returnnil
}

project:=resp.Projects[0]
ifproject.Webhook!=nil&&project.Webhook.Url!=nil{
returnfmt.Errorf("FoundCodeBuildProject%qWebhook:%s",rs.Primary.ID,project.Webhook)
}
}
returnnil
}
...
functestAccCheckWebhookExists(ctxcontext.Context,namestring,webhook*codebuild.Webhook)resource.TestCheck
func{
return
func(s*terraform.State)error{
...
iflen(resp.Projects)==0{
returnfmt.Errorf("CodeBuildProject%qnotfound",rs.Primary.ID)
}

project:=resp.Projects[0]
ifproject.Webhook==nil||aws.StringValue(project.Webhook.PayloadUrl)==""{
returnfmt.Errorf("CodeBuildProject%qWebhooknotfound",rs.Primary.ID)
}
...
functestAccWebhookConfig_bitbucket(rName,sourceLocationstring)string{
...
functestAccWebhookConfig_gitHub(rNamestring)string{
returnacctest.ConfigCompose(
...
functestAccWebhookConfig_gitHubEnterprise(rNamestring,branchFilterstring)string{
...
functestAccWebhookConfig_buildType(rName,branchFilterstring)string{
returnacctest.ConfigCompose(testAccProjectConfig_basic(rName),fmt.Sprintf(`
resource"aws_codebuild_webhook""test"{
build_type=%[1]q
project_name=aws_codebuild_project.test.name
}
...
functestAccWebhookConfig_branchFilter(rName,branchFilterstring)string{
returnacctest.ConfigCompose(testAccProjectConfig_basic(rName),fmt.Sprintf(`
resource"aws_codebuild_webhook""test"{
branch_filter=%[1]q
project_name=aws_codebuild_project.test.name
...
functestAccWebhookConfig_filterGroup(rNamestring)string{
returnacctest.ConfigCompose(
testAccProjectConfig_basic(rName),
`
resource"aws_codebuild_webhook""test"{
