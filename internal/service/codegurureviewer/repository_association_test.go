...
// SPDX-License-Identifier: MPL-2.0package codegurureviewer_testimport (
	"context"
	"errors"
	"fmt"
...
func TestAccCodeGuruReviewerRepositoryAssociation_basic(t *testing.T) {
	...
	resourceName := "aws_codegurureviewer_repository_association.test"	resource.ParallelTest(t, resource.TestCase{
		PreCheck: 
func() {
...
func repositoryassociation codegurureviewer.DescribeRepositoryAssociationOutput
...
funcourceName := "aws_codegurureviewer_repository_association.test"
...
funcf rs.Type != "aws_codegurureviewer_repository_association" {
				continue
...
func
		...
		}		*repositoryassociation = *resp		return nil
	}
}
func testAccPreCheck(ctx context.Context, t *testing.T) {
	conn := acctest.Provider.Meta().(*conns.AWSClient).CodeGuruReviewerConn(ctx)	input := &codegurureviewer.ListRepositoryAssociationsInput{}
	_, err := conn.ListRepositoryAssociationsWithContext(ctx, input)	
funcSkipf("skipping acceptance testing: %s", err)
	}	if err != nil {
