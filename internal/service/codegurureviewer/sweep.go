...
// +build sweeppackage codegurureviewerimport (
	"fmt"
...
funcent, err := sweep.SharedRegionalSweepClient(ctx, region)
	...
	conn := client.CodeGuruReviewerConn(ctx)	sweepResources := make([]sweep.Sweepable, 0)	err = conn.ListRepositoryAssociationsPagesWithContext(ctx, input, 
