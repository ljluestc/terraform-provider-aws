...
// +build sweeppackage codepipelineimport (
	"fmt"
	...
	sweepResources := make([]sweep.Sweepable, 0)	err = conn.ListPipelinesPagesWithContext(ctx, input, func(page *codepipeline.ListPipelinesOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
		}		for _, v := range page.Pipelines {
			r := ResourcePipeline()
			d := r.Data(nil)			d.SetId(aws.StringValue(v.Name))			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
