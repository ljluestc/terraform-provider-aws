...
}conn := acctest.Provider.Meta().(*conns.AWSClient).CodeStarConnectionsClient(ctx)output, err := tfcodestarconnections.FindConnectionByARN(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*v = *outputreturn nil
}
}
...
continue
}_, err := tfcodestarconnections.FindConnectionByARN(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
