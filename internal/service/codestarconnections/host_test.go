...
// SPDX-License-Identifier: MPL-2.0package codestarconnections_testimport (
"context"
"fmt"
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).CodeStarConnectionsClient(ctx)output, err := tfcodestarconnections.FindHostByARN(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*v = *outputreturn nil
}
}
...
return func(s *terraform.State) error {
conn := acctest.Provider.Meta().(*conns.AWSClient).CodeStarConnectionsClient(ctx)for _, rs := range s.RootModule().Resources {
if rs.Type != "aws_codestarconnections_host" {
continue
}_, err := tfcodestarconnections.FindHostByARN(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
resource "aws_vpc" "test" {
 ...
 Name = %[1]q
  ...
  cidr_blockdrsubnet(aws_vpc.test.cidr_block, 8, count.index + 2)
  vpc_idst.id  tags = {
 Name = %[1]q
  }
