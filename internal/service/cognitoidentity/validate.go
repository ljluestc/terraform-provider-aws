...
t := v["type"].(string)
isRequired := t == cognitoidentity.RoleMappingTypeToken || t == cognitoidentity.RoleMappingTypeRulesif value, ok := v["ambiguous_role_resolution"]; (!ok || value == "") && isRequired {
...
value := v.(string)if !regexache.MustCompile(`^[\p{L}\p{M}\p{S}\p{N}\p{P}]+$`).MatchString(value) {
errors = append(errors, fmt.Errorf("%q must contain only alphanumeric characters, dots, underscores, colons, slashes and hyphens", k))
}return
...
}// Validates that either authenticated or unauthenticated is definedfunc validRoles(v map[string]interface{}) (errors []error) {
k := "roles"
_, hasAuthenticated := v["authenticated"].(string)
_, hasUnauthenticated := v["unauthenticated"].(string)if !hasAuthenticated && !hasUnauthenticated {
