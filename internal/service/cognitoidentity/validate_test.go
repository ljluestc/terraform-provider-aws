...
// SPDX-License-Identifier: MPL-2.0package cognitoidentityimport (
	"strings"
	"testing"	"github.com/aws/aws-sdk-go/service/cognitoidentity"
)func TestValidIdentityPoolName(t *testing.T) {
	t.Parallel()	validValues := []string{
"123",
"1 2 3",
"foo",
"foo bar",
...
m["type"] = tc.Typeerrors := validRoleMappingsAmbiguousRoleResolutionAgainstType(m)
...
m["type"] = tc.Typeerrors := validRoleMappingsRulesConfiguration(m)
if len(errors) != tc.ErrCount {
	t.Fatalf("Cognito Role Mappings validation failed: %v, expected err count %d, got %d, for config %#v", errors, tc.ErrCount, len(errors), m)
}
	}
}func TestValidRoles(t *testing.T) {
	t.Parallel()	validValues := []map[string]interface{}{
{"authenticated": "hoge"},
