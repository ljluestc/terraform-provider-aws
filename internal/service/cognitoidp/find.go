...
// SPDX-License-Identifier: MPL-2.0package cognitoidpimport (
	"context"
	"fmt"
	"reflect"	"github.com/aws/aws-sdk-go/aws"
...
UserPoolId: aws.String(userPoolId),
	}	output, err := conn.GetUICustomizationWithContext(ctx, input)	if err != nil {
return nil, err
...
Username:aws.String(username),
	}	found := false	err := conn.AdminListGroupsForUserPagesWithContext(ctx, input, func(page *cognitoidentityprovider.AdminListGroupsForUserOutput, lastPage bool) bool {
if page == nil {
return !lastPage
...
ClientId:aws.String(clientId),
UserPoolId: aws.String(userPoolId),
	}	output, err := conn.DescribeUserPoolClientWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, cognitoidentityprovider.ErrCodeResourceNotFoundException) {
...
}type cognitoUserPoolClientDescriptionNameFilter func(string) (bool, error)func listCognitoUserPoolClientDescriptions(ctx context.Context, conn *cognitoidentityprovider.CognitoIdentityProvider, userPoolId string, nameFilter cognitoUserPoolClientDescriptionNameFilter) ([]*cognitoidentityprovider.UserPoolClientDescription, error) {
	var errs *multierror.Error
	var descs []*cognitoidentityprovider.UserPoolClientDescription	input := &cognitoidentityprovider.ListUserPoolClientsInput{
UserPoolId: aws.String(userPoolId),
	}
...
input.ClientId = aws.String(clientId)
	}	output, err := conn.DescribeRiskConfigurationWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, cognitoidentityprovider.ErrCodeResourceNotFoundException) {
return nil, &retry.NotFoundError{
LastError:err,
LastRequest: input,
}
