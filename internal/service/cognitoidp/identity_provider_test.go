...
// SPDX-License-Identifier: MPL-2.0package cognitoidp_testimport (
	...
	userPoolName := fmt.Sprintf("tf-acc-cognito-user-pool-%s", sdkacctest.RandString(7))	resource.ParallelTest(t, resource.TestCase{
	...
	return func(s *terraform.State) error {
		...
		if err != nil {
			return err
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).CognitoIDPConn(ctx)		input := &cognitoidentityprovider.DescribeIdentityProviderInput{
			ProviderName: aws.String(providerName),
			UserPoolId:   aws.String(userPoolID),
		}		output, err := conn.DescribeIdentityProviderWithContext(ctx, input)		if err != nil {
			return err
		}		if output == nil || output.IdentityProvider == nil {
			return fmt.Errorf("Cognito Identity Provider %q does not exist", rs.Primary.ID)
		}		*identityProvider = *output.IdentityProvider		return nil
	}
...
resource "aws_cognito_user_pool" "test" {
  ...
  provider_type = "Google"  provider_details = {
...
resource "aws_cognito_user_pool" "test" {
  ...
  user_pool_id  = aws_cognito_user_pool.test.id
  provider_name = "Google"
  provider_type = "Google"  idp_identifiers = [%[2]q]  provider_details = {
    attributes_url = "https://people.googleapis.com/v1/people/me?personFields="
    attributes_url_add_attributes = "true"
    authorize_scopes     = "email"
    authorize_url  = "https://accounts.google.com/o/oauth2/v2/auth"
