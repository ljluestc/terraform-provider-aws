...
}conn := acctest.Provider.Meta().(*conns.AWSClient).CognitoIDPConn(ctx)_, err := tfcognitoidp.FindUserByTwoPartKey(ctx, conn, rs.Primary.Attributes["user_pool_id"], rs.Primary.Attributes["username"])return err
}
...
continue
}_, err := tfcognitoidp.FindUserByTwoPartKey(ctx, conn, rs.Primary.Attributes["user_pool_id"], rs.Primary.Attributes["username"])if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
clientId := clientRs.Primary.Attributes["id"]conn := acctest.Provider.Meta().(*conns.AWSClient).CognitoIDPConn(ctx)params := &cognitoidentityprovider.InitiateAuthInput{
