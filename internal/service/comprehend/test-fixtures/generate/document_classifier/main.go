...
func.SetFlags(0)	seed := int64(1) // Default rand seed
	rand.Seed(seed)
	faker.Seed(seed)	documentFile, err := os.OpenFile("./test-fixtures/document_classifier/documents.csv", os.O_TRUNC|os.O_CREATE|os.O_WRONLY, 0600)
	if err != nil {
	...
	documentsWriter := csv.NewWriter(documentFile)	for i := 0; i < 100; i++ {
		...
		doctype := doctypes[rand.Intn(len(doctypes))]		var line string
		if doctype == "PHISHING" {
