...
func.SetFlags(0)	seed := int64(1) // Default rand seed
	rand.Seed(seed)
	faker.Seed(seed)	// documentFile, err := os.OpenFile("./test-fixtures/document_classifier_multilabel/documents.csv", os.O_TRUNC|os.O_CREATE|os.O_WRONLY, 0600)
	...
	documentsWriter := csv.NewWriter(documentFile)	for i := 0; i < 100; i++ {
		f := rand.Intn(2)
		var doctype string
		if f == 0 {
		...
		}		title := faker.Lorem().Word()		var desc string
		if doctype == "DRAMA" {
		...
		}		line := fmt.Sprintf("%s is %s", title, desc)		if err := documentsWriter.Write([]string{doctype, line}); err != nil {
