...
"context"
"fmt"
"testing""github.com/aws/aws-sdk-go/service/configservice"
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).ConfigServiceConn(ctx)output, err := tfconfig.FindConfigurationRecorderByName(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*v = *outputreturn nil
}
...
continue
}_, err := tfconfig.FindConfigurationRecorderByName(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
