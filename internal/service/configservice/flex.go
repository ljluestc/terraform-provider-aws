...
recordingGroup := configservice.RecordingGroup{}if v, ok := group["all_supported"]; ok {
...
scope := &configservice.Scope{}if v, ok := configured["compliance_resource_id"].(string); ok && v != "" {
...
var results []*configservice.SourceDetailfor _, item := range configured.List() {
detail := item.(map[string]interface{})
src := configservice.SourceDetail{}if msgType, ok := detail["message_type"].(string); ok && msgType != "" {
...
var result []interface{}if len(sources) == 0 {
return result
}source := sources[0]
...
var result []interface{}if source == nil {
return result
}m := make(map[string]interface{})
m["all_regions"] = aws.BoolValue(source.AllAwsRegions)
m["regions"] = flex.FlattenStringList(source.AwsRegions)
...
m := make(map[string]interface{}, 1)if g.AllSupported != nil {
...
var items []interface{}m := make(map[string]interface{})
...
m["source_identifier"] = aws.StringValue(source.SourceIdentifier)if source.CustomPolicyDetails != nil {
...
m["enable_debug_log_delivery"] = aws.BoolValue(source.EnableDebugLogDelivery)result = append(result, m)
return result
...
m := make(map[string]interface{})if p.DeliveryFrequency != nil {
