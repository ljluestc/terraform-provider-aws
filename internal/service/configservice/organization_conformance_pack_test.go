...
// SPDX-License-Identifier: MPL-2.0package configservice_testimport (
	...
	resourceName := "aws_config_organization_conformance_pack.test"	resource.Test(t, resource.TestCase{
PreCheck:k(ctx, t); acctest.PreCheckOrganizationsAccount(ctx, t) },
rorCheck: acctest.ErrorCheck(t, configservice.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckOrganizationConformancePackDestroy(ctx),
...
ImportStateVerifyIgnore: []string{"template_body"},
	...
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_config_organization_conformance_pack.test"	resource.Test(t, resource.TestCase{
eCheck:k(ctx, t); acctest.PreCheckOrganizationsAccount(ctx, t) },
rorCheck: acctest.ErrorCheck(t, configservice.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckOrganizationConformancePackDestroy(ctx),
eps: []resource.TestStep{
...
nn := acctest.Provider.Meta().(*conns.AWSClient).ConfigServiceConn(ctx)fo_, rs := range s.RootModule().Resources {
if rs.Type != "aws_config_organization_conformance_pack" {
continue
}pack, err := tfconfig.DescribeOrganizationConformancePack(ctx, conn, rs.Primary.ID)if tfawserr.ErrCodeEquals(err, configservice.ErrCodeNoSuchOrganizationConformancePackException) {
continue
...
o := acctest.Provider.Meta().(*conns.AWSClient).ConfigServiceConn(ctx)packerr := tfconfig.DescribeOrganizationConformancePack(ctx, conn, rs.Primary.ID)if err= nil {
return fmt.Errorf("error describing Config Organization Conformance Pack (%s): %w", rs.Primary.ID, err)
fack == nil {
...
o = *packretu nil
	...
	return func(s *terraform.State) error {
 aws.StringValue(before.OrganizationConformancePackArn) == aws.StringValue(after.OrganizationConformancePackArn) {
return fmt.Errorf("AWS Config Organization Conformance Pack was not recreated")turn nil
	}
...
stAccOrganizationConformancePackBase(rName),
t.Sprintf(`
...
t.Sprintf(`
resource "aws_config_organization_conformance_pack" "test" {
  depends_on = [aws_config_configuration_recorder.test, aws_organizations_organization.test]  excluded_accounts = ["111111111111"]
  name= %q  template_body = <<EOT
Resources:
  IAMPasswordPolicy:
Properties:
 ConfigRuleName: IAMPasswordPolicy
...
t.Sprintf(`
resource "aws_config_organization_conformance_pack" "test" {
  depends_on = [aws_config_configuration_recorder.test, aws_organizations_organization.test]  excluded_accounts = ["111111111111", "222222222222"]
  name= %q  template_body = <<EOT
Resources:
  IAMPasswordPolicy:
Properties:
