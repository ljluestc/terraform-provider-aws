...
// SPDX-License-Identifier: MPL-2.0package configservice_testimport (
	...
	resourceName := "aws_config_organization_custom_policy_rule.test"	resource.Test(t, resource.TestCase{
		...
		ErrorCheck:      acctest.ErrorCheck(t, configservice.EndpointsID),
		ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
		CheckDestroy:    testAccCheckOrganizationCustomPolicyRuleDestroy(ctx),		Steps: []resource.TestStep{
			{
	...
	return func(s *terraform.State) error {
		...
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).ConfigServiceConn(ctx)		resp, err := tfconfigservice.FindOrganizationConfigRule(ctx, conn, rs.Primary.ID)		if err != nil {
