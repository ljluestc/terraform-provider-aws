...
t.Skip("skipping long-running test in short mode")
	...
	ctx := acctest.Context(t)	var rule configservice.OrganizationConfigRule
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_config_organization_custom_policy_rule.test"	resource.Test(t, resource.TestCase{
eCheck: func() {
acctest.PreCheck(ctx, t)
acctest.PreCheckPartitionHasService(t, configservice.EndpointsID)
acctest.PreCheckOrganizationsAccount(ctx, t)rorCheck: acctest.ErrorCheck(t, configservice.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckOrganizationCustomPolicyRuleDestroy(ctx),
...
Skip("skipping long-running test in short mode")
	...
	ctx := acctest.Context(t)	var organizationcustompolicy configservice.OrganizationConfigRule
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_config_organization_custom_policy_rule.test"	resource.Test(t, resource.TestCase{
eCheck: func() {
acctest.PreCheck(ctx, t)
...
acctest.PreCheckOrganizationsAccount(ctx, t)rorCheck: acctest.ErrorCheck(t, configservice.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckOrganizationCustomPolicyRuleDestroy(ctx),Sts: []resource.TestStep{
{
...
continue
}_, err := tfconfigservice.FindOrganizationConfigRule(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
...
o := acctest.Provider.Meta().(*conns.AWSClient).ConfigServiceConn(ctx)resperr := tfconfigservice.FindOrganizationConfigRule(ctx, conn, rs.Primary.ID)if err= nil {
...
data "aws_partition" "current" {}resource "aws_config_configuration_recorder" "test" {
  depends_on = [aws_iam_role_policy_attachment.config]  name= %[1]q
  role_arn = aws_iam_role.config.arn
}resource "aws_iam_role" "config" {
  name = "%[1]s-config"  assume_role_policy = <<POLICY
{
  "Version": "2012-10-17",
  "Statement": [
{
...
stAccOrganizationCustomPolicyRuleConfig_base(rName),
t.Sprintf(`
resource "aws_config_organization_custom_policy_rule" "test" {
  depends_on = [aws_config_configuration_recorder.test, aws_organizations_organization.test]  name = %[1]q  trigger_types  = ["ConfigurationItemChangeNotification"]
