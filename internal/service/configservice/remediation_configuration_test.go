...
eckDestroy:testAccCheckRemediationConfigurationDestroy(ctx),
...
o := acctest.Provider.Meta().(*conns.AWSClient).ConfigServiceConn(ctx)
t, err := conn.DescribeRemediationConfigurationsWithContext(ctx, &configservice.DescribeRemediationConfigurationsInput{
ConfigRuleNames: []*string{aws.String(rs.Primary.Attributes["config_rule_name"])}, err != nil {
return create.Error(names.ConfigService, create.ErrActionCheckingExistence, tfconfigservice.ResNameRemediationConfiguration, n, err) len(out.RemediationConfigurations) < 1 {
...
c= out.RemediationConfigurations[0]
bj = *rcrern nil
	}
...
ern nil
	...
	return func(s *terraform.State) error {
 aws.StringValue(before.Arn) != aws.StringValue(after.Arn) {
return create.Error(names.ConfigService, create.ErrActionCheckingNotRecreated, tfconfigservice.ResNameRemediationConfiguration, aws.StringValue(before.Arn), fmt.Errorf("ARNs changed, new: %s", aws.StringValue(after.Arn)))turn nil
	}
	...
	return func(s *terraform.State) error {
 aws.StringValue(before.Arn) == aws.StringValue(after.Arn) {
return create.Error(names.ConfigService, create.ErrActionCheckingRecreated, tfconfigservice.ResNameRemediationConfiguration, aws.StringValue(before.Arn), fmt.Errorf("wasn't recreated, new: %s", aws.StringValue(after.Arn)))turn nil
	}
