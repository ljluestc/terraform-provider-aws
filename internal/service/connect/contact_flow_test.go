...
// SPDX-License-Identifier: MPL-2.0package connect_testimport (
	...
	resourceName := "aws_connect_contact_flow.test"	resource.Test(t, resource.TestCase{
...
instanceID, contactFlowID, err := tfconnect.ContactFlowParseID(rs.Primary.ID)if err != nil {
	return err
}conn := acctest.Provider.Meta().(*conns.AWSClient).ConnectConn(ctx)params := &connect.DescribeContactFlowInput{
...
continue
	}	conn := acctest.Provider.Meta().(*conns.AWSClient).ConnectConn(ctx)	instanceID, contactFlowID, err := tfconnect.ContactFlowParseID(rs.Primary.ID)	if err != nil {
return err
	}	params := &connect.DescribeContactFlowInput{
ContactFlowId: aws.String(contactFlowID),
InstanceId: aws.String(instanceID),
	}	_, err = conn.DescribeContactFlowWithContext(ctx, params)	if tfawserr.ErrCodeEquals(err, connect.ErrCodeResourceNotFoundException) {
continue
	}	if err != nil {
return err
	}
