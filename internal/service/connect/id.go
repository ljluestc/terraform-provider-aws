...
func BotV1AssociationParseResourceID(id string) (string, string, string, error) {
	parts := strings.SplitN(id, botV1AssociationIDSeparator, 3)	if len(parts) != 3 || parts[0] == "" || parts[1] == "" || parts[2] == "" {
...
func BotV1AssociationCreateResourceID(instanceID string, botName string, region string) string {
	...
	id := strings.Join(parts, botV1AssociationIDSeparator)	return id
}
func Lambda
...
functionAssociationIDSeparator)	return id
}
