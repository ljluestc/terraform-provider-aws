...
}output, err := conn.DescribePhoneNumberWithContext(ctx, input)if tfawserr.ErrCodeEquals(err, connect.ErrCodeResourceNotFoundException) {
...
}output, err := conn.DescribeVocabularyWithContext(ctx, input)if tfawserr.ErrCodeEquals(err, connect.ErrCodeResourceNotFoundException) {
return output, connect.ErrCodeResourceNotFoundException, nil
}if err != nil {
return nil, "", err
