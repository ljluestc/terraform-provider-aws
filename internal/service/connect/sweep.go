...
// +build sweeppackage connectimport (
	"fmt"
...
func sweepInstance(region string) error {
	...
	}	conn := client.ConnectConn(ctx)	var errs *multierror.Error
	sweepResources := make([]sweep.Sweepable, 0)	// MaxResults:Maximum value of 10. https://docs.aws.amazon.com/connect/latest/APIReference/API_ListInstances.html
	input := &connect.ListInstancesInput{MaxResults: aws.Int64(ListInstancesMaxResults)}	err = conn.ListInstancesPagesWithContext(ctx, input,
...
func(page *connect.ListInstancesOutput, lastPage bool) bool {
			if page == nil {
				...
				if instanceSummary == nil {
					continue
				}				id := aws.StringValue(instanceSummary.Id)				log.Printf("[INFO] Deleting Connect Instance (%s)", id)
				r := ResourceInstance()
				d := r.Data(nil)
