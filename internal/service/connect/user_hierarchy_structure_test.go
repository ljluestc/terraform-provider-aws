...
// SPDX-License-Identifier: MPL-2.0package connect_testimport (
	...
	resourceName := "aws_connect_user_hierarchy_structure.test"	resource.Test(t, resource.TestCase{
...
Check: resource.ComposeTestCheckfunc(
	...
	levelOneName := sdkacctest.RandomWithPrefix("resource-test-terraform")	resource.Test(t, resource.TestCase{
PreCheck:func() { acctest.PreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, connect.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy: testAccCheckUserHierarchyStructureDestroy(ctx),
...
instanceID := rs.Primary.IDconn := acctest.Provider.Meta().(*conns.AWSClient).ConnectConn(ctx)params := &connect.DescribeUserHierarchyStructureInput{
...
continue
	}	conn := acctest.Provider.Meta().(*conns.AWSClient).ConnectConn(ctx)	instanceID := rs.Primary.ID	params := &connect.DescribeUserHierarchyStructureInput{
InstanceId: aws.String(instanceID),
	}	resp, err := conn.DescribeUserHierarchyStructureWithContext(ctx, params)	if tfawserr.ErrCodeEquals(err, connect.ErrCodeResourceNotFoundException) {
continue
	}	if err != nil {
return err
