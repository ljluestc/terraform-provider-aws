...
// SPDX-License-Identifier: MPL-2.0package connectimport (
	...
	botAssociationCreateTimeout = 5 * time.Minute	phoneNumberCreatedTimeout = 2 * time.Minute
	phoneNumberUpdatedTimeout = 2 * time.Minute
	phoneNumberDeletedTimeout = 2 * time.Minute	vocabularyCreatedTimeout = 5 * time.Minute
...
func waitPhoneNumberCreated(ctx context.Context, conn *connect.Connect, timeout time.Duration, phoneNumberId string) (*connect.DescribePhoneNumberOutput, error) {
	stateConf := &retry.StateChangeConf{
		...
		Timeout: timeout,
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if output, ok := outputRaw.(*connect.DescribePhoneNumberOutput); ok {
...
func waitPhoneNumberDeleted(ctx context.Context, conn *connect.Connect, timeout time.Duration, phoneNumberId string) (*connect.DescribePhoneNumberOutput, error) {
	stateConf := &retry.StateChangeConf{
		...
		Refresh: statusPhoneNumber(ctx, conn, phoneNumberId),
		Timeout: timeout,
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if v, ok := outputRaw.(*connect.DescribePhoneNumberOutput); ok {
		return v, err
	}	return nil, err
}
func waitVocabularyCreated(ctx context.Context, conn *connect.Connect, timeout time.Duration, instanceId, vocabularyId string) (*connect.DescribeVocabularyOutput, error) {
	stateConf := &retry.StateChangeConf{
		...
		Timeout: timeout,
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if v, ok := outputRaw.(*connect.DescribeVocabularyOutput); ok {
		return v, err
	}	return nil, err
}
