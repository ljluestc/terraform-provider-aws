...
// SPDX-License-Identifier: MPL-2.0package controltower_testimport (
	"context"
	"fmt"
	"testing"	"github.com/aws/aws-sdk-go/service/controltower"
...
"Control": {
	...
	ouName := "Security"	resource.Test(t, resource.TestCase{
PreCheck: func() {
...
}targetIdentifier, controlIdentifier, err := tfcontroltower.ControlParseResourceID(rs.Primary.ID)if err != nil {
	return err
}conn := acctest.Provider.Meta().(*conns.AWSClient).ControlTowerConn(ctx)output, err := tfcontroltower.FindEnabledControlByTwoPartKey(ctx, conn, targetIdentifier, controlIdentifier)if err != nil {
	return err
}*v = *outputreturn nil
	}
}func testAccCheckControlDestroy(ctx context.Context) resource.TestCheckFunc {
	return func(s *terraform.State) error {
conn := acctest.Provider.Meta().(*conns.AWSClient).ControlTowerConn(ctx)for _, rs := range s.RootModule().Resources {
	if rs.Type != "aws_controltower_control" {
continue
	}	targetIdentifier, controlIdentifier, err := tfcontroltower.ControlParseResourceID(rs.Primary.ID)	if err != nil {
return err
	}	_, err = tfcontroltower.FindEnabledControlByTwoPartKey(ctx, conn, targetIdentifier, controlIdentifier)	if tfresource.NotFound(err) {
continue
	}	if err != nil {
return err
	...
	return fmt.Sprintf(`
data "aws_region" "current" {}data "aws_partition" "current" {}data "aws_organizations_organization" "test" {}data "aws_organizations_organizational_units" "test" {
