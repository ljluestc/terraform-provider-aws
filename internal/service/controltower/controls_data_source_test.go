...
// SPDX-License-Identifier: MPL-2.0package controltower_testimport (
	"context"
	"fmt"
	"testing"	"github.com/aws/aws-sdk-go/service/cloudtrail"
	...
	ouName := "Security"	resource.ParallelTest(t, resource.TestCase{
PreCheck: func() {
...
Check: resource.ComposeTestCheckFunc(
	...
	var trails []string
	conn := acctest.Provider.Meta().(*conns.AWSClient).CloudTrailConn(ctx)	input := &cloudtrail.ListTrailsInput{}
...
t.Skip("skipping since Control Tower not found")
	...
	return fmt.Sprintf(`
data "aws_organizations_organization" "test" {}data "aws_organizations_organizational_units" "test" {
