...
funce := fmt.Sprintf("tf-datapipeline-%s", sdkacctest.RandString(5))
resourceName := "aws_datapipeline_pipeline.default"
...
CheckDestroy:testAccCheckPipelineDestroy(ctx),
Steps: []resource.TestStep{
{funcig: testAccPipelineConfig_basic(rName),
Check: resource.ComposeTestCheckFunc(
...
funcheck:    func() { acctest.PreCheck(ctx, t); testAccPreCheck(ctx, t) },
ErrorCheck:  acctest.ErrorCheck(t, datapipeline.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckPipelineDestroy(ctx),
Steps: []resource.TestStep{
...
Check: resourfuncAccCheckPipelineExists(ctx, resourceName, &conf),
...
funcrn err
}funcrn tfdatapipeline.WaitForDeletion(ctx, conn, *conf.PipelineId)
}
}
...
conn := acctest.Provider.Meta().(*conns.AWSClient).DataPipelineConn(ctx)
funclineDescription, err := tfdatapipeline.PipelineRetrieve(ctx, rs.Primary.ID, conn)
funcrr != nil {
return err
}
if pipelineDescription == nil {
