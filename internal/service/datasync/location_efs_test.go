...
eckDestroy:testAccCheckLocationEFSDestroy(ctx),
...
continue
}_, err := tfdatasync.FindLocationEFSByARN(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
o := acctest.Provider.Meta().(*conns.AWSClient).DataSyncConn(ctx)outp, err := tfdatasync.FindLocationEFSByARN(ctx, conn, rs.Primary.ID)if err= nil {
return err
v *outputretu nil
	}
...
resource "aws_datasync_location_efs" "test" {
  efs_file_system_arn = aws_efs_mount_target.test.file_system_arn  ec2_config {
...
resource "aws_efs_access_point" "test" {
  file_system_id = aws_efs_file_system.test.id  tags = {
Name = %[1]q
  }
}resource "aws_datasync_location_efs" "test" {
  efs_file_system_arn= aws_efs_mount_target.test.file_system_arn
  access_point_arn = aws_efs_access_point.test.arn
  in_transit_encryption = "TLS1_2"  ec2_config {
security_group_arns = [aws_security_group.test.arn]
subnet_arn = aws_subnet.test.arn
  }
}
