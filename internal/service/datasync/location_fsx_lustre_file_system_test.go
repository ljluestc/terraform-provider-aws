...
// SPDX-License-Identifier: MPL-2.0package datasync_testimport (
	...
	fsResourceName := "aws_fsx_lustre_file_system.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck: func() {
acctest.PreCheck(ctx, t)
acctest.PreCheckPartitionHasService(t, fsx.EndpointsID)
testAccPreCheck(ctx, t)rorCheck: acctest.ErrorCheck(t, datasync.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckLocationFSxLustreDestroy(ctx),
...
resource.TestMatchResourceAttr(resourceName, "uri", regexache.MustCompile(`^fsxl://.+/`)),
...
ImportStateIdFunc: testAccLocationFSxLustreImportStateID(resourceName),
	...
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	var locationFsxLustre1 datasync.DescribeLocationFsxLustreOutput
	resourceName := "aws_datasync_location_fsx_lustre_file_system.test"	resource.ParallelTest(t, resource.TestCase{
eCheck: func() {
acctest.PreCheck(ctx, t)
acctest.PreCheckPartitionHasService(t, fsx.EndpointsID)
testAccPreCheck(ctx, t)rorCheck: acctest.ErrorCheck(t, datasync.EndpointsID),
...
continue
}_, err := tfdatasync.FindLocationFSxLustreByARN(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
o := acctest.Provider.Meta().(*conns.AWSClient).DataSyncConn(ctx)outp, err := tfdatasync.FindLocationFSxLustreByARN(ctx, conn, rs.Primary.ID)if err= nil {
return err
v *outputretu nil
	}
...
ern fmt.Sprintf("%s#%s", rs.Primary.ID, rs.Primary.Attributes["fsx_filesystem_arn"]), nil
	}
}func testAccFSxLustreFileSystemConfig_base(rName string) string {
	return acctest.ConfigCompose(acctest.ConfigVPCWithSubnets(rName, 1), fmt.Sprintf(`
data "aws_partition" "current" {}resource "aws_security_group" "test" {
  name= %[1]q
...
resource "aws_datasync_location_fsx_lustre_file_system" "test" {
  fsx_filesystem_arn  = aws_fsx_lustre_file_system.test.arn
  security_group_arns = [aws_security_group.test.arn]  tags = {
%[1]q = %[2]q
  }
}
`, key1, value1))
