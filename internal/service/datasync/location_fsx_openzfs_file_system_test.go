...
// SPDX-License-Identifier: MPL-2.0package datasync_testimport (
	...
	fsResourceName := "aws_fsx_openzfs_file_system.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck: func() {
...
eckDestroy:testAccCheckLocationFSxOpenZFSDestroy(ctx),
...
ImportStateIdFunc: testAccLocationFSxOpenZFSImportStateID(resourceName),
	...
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	var v datasync.DescribeLocationFsxOpenZfsOutput
	resourceName := "aws_datasync_location_fsx_openzfs_file_system.test"	resource.ParallelTest(t, resource.TestCase{
eCheck: func() {
acctest.PreCheck(ctx, t)
acctest.PreCheckPartitionHasService(t, fsx.EndpointsID)
testAccPreCheck(ctx, t)rorCheck: acctest.ErrorCheck(t, datasync.EndpointsID),
...
continue
}_, err := tfdatasync.FindLocationFSxOpenZFSByARN(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
o := acctest.Provider.Meta().(*conns.AWSClient).DataSyncConn(ctx)outp, err := tfdatasync.FindLocationFSxOpenZFSByARN(ctx, conn, rs.Primary.ID)if err= nil {
return err
v *outputretu nil
	}
...
resource "aws_datasync_location_fsx_openzfs_file_system" "test" {
  ...
  security_group_arns = [aws_security_group.test.arn]  protocol {
nfs {
 mount_options {
