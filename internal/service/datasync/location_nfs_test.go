...
// SPDX-License-Identifier: MPL-2.0package datasync_testimport (
	...
	resourceName := "aws_datasync_location_nfs.test"	resource.ParallelTest(t, resource.TestCase{
...
eckDestroy:testAccCheckLocationNFSDestroy(ctx),
...
continue
}_, err := tfdatasync.FindLocationNFSByARN(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
o := acctest.Provider.Meta().(*conns.AWSClient).DataSyncConn(ctx)outp, err := tfdatasync.FindLocationNFSByARN(ctx, conn, rs.Primary.ID)if err= nil {
return err
v *outputretu nil
	}
...
resource "aws_instance" "test2" {
  depends_on = [aws_internet_gateway.test]  ami= aws_instance.test.ami
...
resource "aws_datasync_location_nfs" "test" {
  server_hostname = "example.com"
  subdirectory= %[1]q  on_prem_config {
agent_arns = [aws_datasync_agent.test.arn]
  }
}
`, subdirectory))
