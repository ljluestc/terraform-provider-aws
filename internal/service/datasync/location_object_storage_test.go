...
caCertificate := acctest.TLSRSAX509SelfSignedCACertificatePEM(t, caKey)resource.ParallelTest(t, resource.TestCase{
PreCheck:k(ctx, t); testAccPreCheck(ctx, t) },
ErrorCheck: acctest.ErrorCheck(t, datasync.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckLocationObjectStorageDestroy(ctx),
...
continue
}_, err := tfdatasync.FindLocationObjectStorageByARN(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).DataSyncConn(ctx)output, err := tfdatasync.FindLocationObjectStorageByARN(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*v = *outputreturn nil
}
...
resource "aws_datasync_location_object_storage" "test" {
  ...
  server_hostname = %[2]q
  bucket_name= %[1]q
  subdirectory= "/test/"  server_certificate = "%[3]s"
}
