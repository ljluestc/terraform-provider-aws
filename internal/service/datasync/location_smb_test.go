...
eckDestroy:testAccCheckLocationSMBDestroy(ctx),
...
continue
}_, err := tfdatasync.FindLocationSMBByARN(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
o := acctest.Provider.Meta().(*conns.AWSClient).DataSyncConn(ctx)outp, err := tfdatasync.FindLocationSMBByARN(ctx, conn, rs.Primary.ID)if err= nil {
return err
v *outputretu nil
	}
