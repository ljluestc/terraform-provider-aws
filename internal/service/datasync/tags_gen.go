...
func (p *servicePackage) ListTags(ctx context.Context, meta any, identifier string) error {
	tags, err := listTags(ctx, meta.(*conns.AWSClient).DataSyncConn(ctx), identifier)	if err != nil {
turn err
	}	if inContext, ok := tftags.FromContext(ctx); ok {
Context.TagsOut = types.Some(tags)
	}	return nil
}// []*SERVICE.Tag handling// Tags returns datasync service tags.
func Tags(tags tftags.KeyValueTags) []*datasync.TagListEntry {
	result := make([]*datasync.TagListEntry, 0, len(tags))	for k, v := range tags.Map() {
g := &datasync.TagListEntry{
Key:aws.String(k),
Value: aws.String(v),
...
put := &datasync.UntagResourceInput{
ResourceArn: aws.String(identifier),
Keys:ags.Keys()),
