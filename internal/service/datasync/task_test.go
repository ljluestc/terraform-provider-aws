...
// SPDX-License-Identifier: MPL-2.0package datasync_testimport (
	...
	resourceName := "aws_datasync_task.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck:k(ctx, t); testAccPreCheck(ctx, t) },
rorCheck: acctest.ErrorCheck(t, datasync.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckTaskDestroy(ctx),
...
ImportStateVerify: true,
	...
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_datasync_task.test"	resource.ParallelTest(t, resource.TestCase{
eCheck:k(ctx, t); testAccPreCheck(ctx, t) },
rorCheck: acctest.ErrorCheck(t, datasync.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckTaskDestroy(ctx),
eps: []resource.TestStep{
...
nn := acctest.Provider.Meta().(*conns.AWSClient).DataSyncConn(ctx)fo_, rs := range s.RootModule().Resources {
if rs.Type != "aws_datasync_task" {
continue
}_, err := tfdatasync.FindTaskByARN(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
o := acctest.Provider.Meta().(*conns.AWSClient).DataSyncConn(ctx)outp, err := tfdatasync.FindTaskByARN(ctx, conn, rs.Primary.ID)if err= nil {
return err
fws.StringValue(output.Status) != datasync.TaskStatusAvailable && aws.StringValue(output.Status) != datasync.TaskStatusRunning {
...
tk = *outputretu nil
	...
	return func(s *terraform.State) error {
 aws.StringValue(i.TaskArn) != aws.StringValue(j.TaskArn) {
return errors.New("DataSync Task was recreated")
ern nil
	}
}func testAccPreCheck(ctx context.Context, t *testing.T) {
	conn := acctest.Provider.Meta().(*conns.AWSClient).DataSyncConn(ctx)	input := &datasync.ListTasksInput{
xResults: aws.Int64(1),
	}	_, err := conn.ListTasksWithContext(ctx, input)	if acctest.PreCheckSkipError(err) {
Skipf("skipping acceptance testing: %s", err)
	}	if err != nil {
Fatalf("unexpected PreCheck error: %s", err)
	}
...
stAccTaskConfig_baseLocationS3(rName),
stAccTaskConfig_baseLocationNFS(rName),
t.Sprintf(`
...
resource "aws_datasync_task" "test" {
  destination_location_arn = aws_datasync_location_s3.test.arn
  name= %[1]q
  source_location_arn = aws_datasync_location_nfs.test.arn  schedule {
...
resource "aws_datasync_task" "test" {
  destination_location_arn = aws_datasync_location_s3.test.arn
  name= %[1]q
  source_location_arn = aws_datasync_location_nfs.test.arn  excludes {
...
resource "aws_datasync_task" "test" {
  destination_location_arn = aws_datasync_location_s3.test.arn
  name= %[1]q
  source_location_arn = aws_datasync_location_nfs.test.arn  includes {
filter_type = "SIMPLE_PATTERN"
value= %[2]q
  }
}
...
resource "aws_datasync_task" "test" {
  destination_location_arn = aws_datasync_location_s3.test.arn
  name= %[1]q
  source_location_arn = aws_datasync_location_nfs.test.arn  options {
atime = %[2]q
mtime = %[3]q
  }
}
...
func testAccTaskConfig_defaultSyncOptionsSecurityDescriptorCopyFlags(rName, domain, securityDescriptorCopyFlags string) string {
	return acctest.ConfigCompose(
ctest.ConfigVPCWithSubnets(rName, 2),
 Reference: https://docs.aws.amazon.com/datasync/latest/userguide/agent-requirements.html
ctest.AvailableEC2InstanceTypeForAvailabilityZone("data.aws_availability_zones.available.names[0]", "m5.2xlarge", "m5.4xlarge"),
t.Sprintf(`
data "aws_partition" "current" {}# Reference: https://docs.aws.amazon.com/datasync/latest/userguide/deploy-agents.html
...
Name = %[1]q
  ...
  depends_on = [aws_default_route_table.test]  ami= data.aws_ssm_parameter.test.value
...
Name = %[1]q
  ...
  name= %[1]q
  source_location_arn = aws_datasync_location_smb.test.arn  options {
gid = "NONE"
...
resource "aws_datasync_task" "test" {
  destination_location_arn = aws_datasync_location_s3.test.arn
  name= %[1]q
  source_location_arn = aws_datasync_location_nfs.test.arn  tags = {
%[2]q = %[3]q
  }
}
`, rName, key1, value1))
