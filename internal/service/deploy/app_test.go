...
// SPDX-License-Identifier: MPL-2.0package deploy_testimport (
	...
	resourceName := "aws_codedeploy_app.test"	resource.ParallelTest(t, resource.TestCase{
	...
	return func(s *terraform.State) error {
		...
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).DeployConn(ctx)		input := &codedeploy.GetApplicationInput{
			ApplicationName: aws.String(rs.Primary.Attributes["name"]),
		...
		}		*application = *output.Application		return nil
	}
