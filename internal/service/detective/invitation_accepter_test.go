...
// SPDX-License-Identifier: MPL-2.0package detective_testimport (
	"context"
	"fmt"
	"testing"	"github.com/aws/aws-sdk-go/service/detective"
...
func testAccInvitationAccepter_basic(t *testing.T) {
	...
	email := testAccMemberFromEnv(t)	resource.Test(t, resource.TestCase{
		PreCheck: func() {
...
func testAccCheckInvitationAccepterExists(ctx context.Context, resourceName string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).DetectiveConn(ctx)		result, err := tfdetective.FindInvitationByGraphARN(ctx, conn, rs.Primary.ID)		if err != nil {
			return err
		}		if result == nil {
	...
	return func(s *terraform.State) error {
		conn := acctest.Provider.Meta().(*conns.AWSClient).DetectiveConn(ctx)		for _, rs := range s.RootModule().Resources {
			if rs.Type != "aws_detective_invitation_accepter" {
				continue
			}			result, err := tfdetective.FindInvitationByGraphARN(ctx, conn, rs.Primary.ID)			if tfawserr.ErrCodeEquals(err, detective.ErrCodeResourceNotFoundException) ||
...
data "aws_caller_identity" "member" {
...
}resource "aws_detective_graph" "test" {}resource "aws_detective_member" "test" {
  ...
  provider  = "awsalternate"
  graph_arn = aws_detective_member.test.graph_arn  depends_on = [aws_detective_member.test]
}
`, email)
}
