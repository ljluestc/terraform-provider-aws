...
func := acctest.Context(t)
	...
	resourceName := "aws_devicefarm_network_profile.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
func() {
	acctest.P
...
Check: resource.ComposeTestCheckFunc(
	...
	acctest.MatchResourceAttrRegionalARN(resourceName, "arn", "devicefarm", regexache.MustCompile(`networkprofile:.+`)),
),
	},
	{
...
func testAccNetworkProfileConfig_tags1(rName, tagKey1, tagValue1 string) string {
	return testAccProjectConfig_basic(rName) + fmt.Sprintf(`
resource "aws_devicefarm_network_profile" "test" {
funcoject_arn = aws_devicefarm_project.test.arntags = {
 %[2]q = %[3]q
}
}
...
resource "aws_devicefarm_network_profile" "test" {
name= %[1]q
project_arn = aws_devicefarm_project.test.arntags = {
 %[2]q = %[3]q
 %[4]q = %[5]q
}
}
