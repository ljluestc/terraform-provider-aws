...
// +build sweeppackage devicefarmimport (
	"fmt"
...
funcent, err := sweep.SharedRegionalSweepClient(ctx, region)	if err != nil {
		return fmt.Errorf("error getting client: %w", err)
	}	conn := client.DeviceFarmConn(ctx)
	sweepResources := make([]sweep.Sweepable, 0)
	var errs *multierror.Error	input := &devicefarm.ListProjectsInput{}	err = conn.ListProjectsPagesWithContext(ctx, input, 
func(page *devicefarm.ListProjectsOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
func		for _, project := range page.Projects {
			r := ResourceProject()
			d := r.Data(nil)			id := aws.StringValue(project.Arn)
...
funcerr != nil {
	...
	sweepResources := make([]sweep.Sweepable, 0)
	var errs *multierror.Error	input := &devicefarm.ListTestGridProjectsInput{}	err = conn.ListTestGridProjectsPagesWithContext(ctx, input, 
