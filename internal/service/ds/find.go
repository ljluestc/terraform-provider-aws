...
DirectoryIds: aws.StringSlice([]string{id}),
	...
	var output []*directoryservice.DirectoryDescription	err := describeDirectoriesPages(ctx, conn, input, func(page *directoryservice.DescribeDirectoriesOutput, lastPage bool) bool {
if page == nil {
return !lastPage
...
return nil, tfresource.NewTooManyResultsError(count, input)
	}	directory := output[0]	if stage := aws.StringValue(directory.Stage); stage == directoryservice.DirectoryStageDeleted {
return nil, &retry.NotFoundError{
Message:stage,
LastRequest: input,
}
...
DomainControllerIds: aws.StringSlice([]string{domainControllerID}),
	}	output, err := FindDomainControllers(ctx, conn, input)	if err != nil {
return nil, err
	}	if len(output) == 0 {
return nil, tfresource.NewEmptyResultError(input)
	}	if count := len(output); count > 1 {
return nil, tfresource.NewTooManyResultsError(count, input)
	}	domainController := output[0]	if status := aws.StringValue(domainController.Status); status == directoryservice.DomainControllerStatusDeleted {
return nil, &retry.NotFoundError{
Message:status,
LastRequest: input,
}
	...
	var output []*directoryservice.DomainController	err := conn.DescribeDomainControllersPagesWithContext(ctx, input, func(page *directoryservice.DescribeDomainControllersOutput, lastPage bool) bool {
if page == nil {
return !lastPage
...
return nil, err
	...
	output, err := FindDirectoryByID(ctx, conn, directoryID)	if err != nil {
return nil, err
...
RegionName:  aws.String(regionName),
	...
	var output []*directoryservice.RegionDescription	err := describeRegionsPages(ctx, conn, input, func(page *directoryservice.DescribeRegionsOutput, lastPage bool) bool {
if page == nil {
return !lastPage
...
return nil, tfresource.NewTooManyResultsError(count, input)
	}	region := output[0]	if status := aws.StringValue(region.Status); status == directoryservice.DirectoryStageDeleted {
return nil, &retry.NotFoundError{
Message:status,
LastRequest: input,
}
...
SharedDirectoryIds: aws.StringSlice([]string{sharedDirectoryID}),
	}	var output []*directoryservice.SharedDirectory	err := conn.DescribeSharedDirectoriesPagesWithContext(ctx, input, func(page *directoryservice.DescribeSharedDirectoriesOutput, lastPage bool) bool {
if page == nil {
return !lastPage
...
return nil, tfresource.NewTooManyResultsError(count, input)
	}	sharedDirectory := output[0]	if status := aws.StringValue(sharedDirectory.ShareStatus); status == directoryservice.ShareStatusDeleted {
return nil, &retry.NotFoundError{
Message:status,
LastRequest: input,
}
