...
continue
}directoryID, regionName, err := tfds.RegionParseResourceID(rs.Primary.ID)if err != nil {
return err
}_, err = tfds.FindRegion(ctx, conn, directoryID, regionName)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
return err
}conn := acctest.Provider.Meta().(*conns.AWSClient).DSConn(ctx)output, err := tfds.FindRegion(ctx, conn, directoryID, regionName)if err != nil {
return err
}*v = *outputreturn nil
	}
...
Name = %[1]q
  ...
  provider = awsalternate  count = 2  vpc_id= aws_vpc.secondary.id
  availability_zone = data.aws_availability_zones.secondary.names[count.index]
  cidr_blockondary.cidr_block, 8, count.index)  tags = {
Name = %[1]q
  }
...
resource "aws_directory_service_region" "test" {
  ...
  region_name  = data.aws_region.secondary.name  vpc_settings {
