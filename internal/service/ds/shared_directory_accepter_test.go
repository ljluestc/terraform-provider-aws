...
}conn := acctest.Provider.Meta().(*conns.AWSClient).DSConn(ctx)_, err := tfds.FindSharedDirectory(ctx, conn, rs.Primary.Attributes["owner_directory_id"], rs.Primary.Attributes["shared_directory_id"])return err
	}
...
`
data "aws_caller_identity" "current" {}resource "aws_directory_service_shared_directory" "test" {
  directory_id = aws_directory_service_directory.test.id
  notesallegations"  target {
id = data.aws_caller_identity.consumer.account_id
  ...
  provider = "awsalternate"  shared_directory_id = aws_directory_service_shared_directory.test.shared_directory_id
}
`)
}
