...
//SPDX-License-Identifier:MPL-2.0packageds_testimport(
	"context"
	"fmt"
...
}conn:=acctest.Provider.Meta().(*conns.AWSClient).DSConn(ctx)output,err:=tfds.FindSharedDirectory(ctx,conn,rs.Primary.Attributes["directory_id"],rs.Primary.Attributes["shared_directory_id"])iferr!=nil{
	returnerr
}*v=*outputreturnnil
	}
}functestAccCheckSharedDirectoryDestroy(ctxcontext.Context)resource.TestCheckFunc{
	returnfunc(s*terraform.State)error{
conn:=acctest.Provider.Meta().(*conns.AWSClient).DSConn(ctx)for_,rs:=ranges.RootModule().Resources{
	ifrs.Type!="aws_directory_service_shared_directory"{
continue
	}	_,err:=tfds.FindSharedDirectory(ctx,conn,rs.Primary.Attributes["directory_id"],rs.Primary.Attributes["shared_directory_id"])	iftfresource.NotFound(err){
continue
	}	iferr!=nil{
returnerr
