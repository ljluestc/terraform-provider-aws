...
conn := client.DSConn(ctx)sweepResources := make([]sweep.Sweepable, 0)input := &directoryservice.DescribeDirectoriesInput{}
...
r := ResourceDirectory()
d := r.Data(nil)
d.SetId(aws.StringValue(directory.DirectoryId))sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
...
var errs *multierror.Errorinput := &directoryservice.DescribeDirectoriesInput{}err = describeDirectoriesPages(ctx, conn, input, func(page *directoryservice.DescribeDirectoriesOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, directory := range page.DirectoryDescriptions {
if directory == nil {
