...
// SPDX-License-Identifier: MPL-2.0package ds_testimport (
	...
	domainNameOther := acctest.RandomDomainName()	resource.ParallelTest(t, resource.TestCase{
PreCheck: func() {
...
resource.TestCheckNoResourceAttr(resourceName, "trust_state_reason"),testAccCheckTrustExists(ctx, resourceOtherName, &v2),
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).DSClient(ctx)output, err := tfds.FindTrustByID(ctx, conn, rs.Primary.Attributes["directory_id"], rs.Primary.ID)if err != nil {
return err
}*v = *outputreturn nil
	}
}func testAccCheckTrustDestroy(ctx context.Context) resource.TestCheckFunc {
	return func(s *terraform.State) error {
conn := acctest.Provider.Meta().(*conns.AWSClient).DSClient(ctx)for _, rs := range s.RootModule().Resources {
if rs.Type != "aws_directory_service_trust" {
continue
}_, err := tfds.FindTrustByID(ctx, conn, rs.Primary.Attributes["directory_id"], rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
resource "aws_directory_service_trust" "test" {
  directory_id = aws_directory_service_directory.test.id  remote_domain_name = aws_directory_service_directory.other.name
  trust_direction= "Two-Way"
  trust_password= "Some0therPassword"  conditional_forwarder_ip_addrs = aws_directory_service_directory.other.dns_ip_addresses
...
subnet_ids = aws_subnet.test[*].id
  ...
  security_group_id = aws_directory_service_directory.test.security_group_id  type = "egress"
  protocol  = "all"
  from_port = 0
  to_port= 65535
  source_security_group_id = aws_directory_service_directory.other.security_group_id
}resource "aws_security_group_rule" "other" {
  security_group_id = aws_directory_service_directory.other.security_group_id  type = "egress"
  protocol  = "all"
  from_port = 0
  to_port= 65535
...
resource "aws_directory_service_trust" "test" {
  ...
  directory_id = aws_directory_service_directory.other.id  remote_domain_name = aws_directory_service_directory.test.name
  trust_direction= "Two-Way"
  trust_password= "Some0therPassword"  conditional_forwarder_ip_addrs = aws_directory_service_directory.test.dns_ip_addresses
}resource "aws_directory_service_directory" "test" {
  name= %[1]q
  password = "SuperSecretPassw0rd"
  type= "MicrosoftAD"
...
resource "aws_directory_service_trust" "test" {
  directory_id = aws_directory_service_directory.test.id  remote_domain_name = aws_directory_service_directory.other.name
  trust_direction= "Two-Way"
  trust_password= "Some0therPassword"  conditional_forwarder_ip_addrs = aws_directory_service_directory.other.dns_ip_addresses  selective_auth = %[3]q
}resource "aws_directory_service_directory" "test" {
  name= %[1]q
  password = "SuperSecretPassw0rd"
  type= "MicrosoftAD"
...
resource "aws_directory_service_trust" "test" {
  directory_id = aws_directory_service_directory.test.id  remote_domain_name = aws_directory_service_directory.other.name
  trust_direction= "Two-Way"
  trust_password= "Some0therPassword"  conditional_forwarder_ip_addrs = aws_directory_service_directory.other.dns_ip_addresses  trust_type = %[3]q
}resource "aws_directory_service_directory" "test" {
  name= %[1]q
  password = "SuperSecretPassw0rd"
  type= "MicrosoftAD"
...
resource "aws_directory_service_trust" "test" {
  directory_id = aws_directory_service_directory.test.id  remote_domain_name = aws_directory_service_directory.other.name
  trust_direction= "Two-Way"
  trust_password= "Some0therPassword"  conditional_forwarder_ip_addrs = toset(slice(tolist(aws_directory_service_directory.other.dns_ip_addresses), 0, 1))
}resource "aws_directory_service_directory" "test" {
  name= %[1]q
  password = "SuperSecretPassw0rd"
  type= "MicrosoftAD"
