...
func testAccInstanceDataSourceConfig_tags(rName string) string {	return acctest.ConfigCompose(acctest.ConfigLatestAmazonLinuxHVMEBSAMI(), fmt.Sprintf(`resource "aws_instance" "test" {ami= data.aws_ami.amzn-ami-minimal-hvm-ebs.idinstance_type = "t2.small"tags = {me TestSeed = "%[2]d"}}data "aws_instance" "test" {instance_tags = { Na awsance.tags["Name"] TestS = "%[2]d"}}`, rName, sdkacctest.RandInt()))}// filter on tag, populate more attributes
func testAccInstanceDataSourceConfig_gp2IOPSDevice(rName string) string {	return acctest.ConfigCompose(acctest.ConfigLatestAmazonLinuxHVMEBSAMI(), fmt.Sprintf(`resource "aws_instance" "test" {ami= data.aws_ami.amzn-ami-minimal-hvm-ebs.idinstance_type = "t3.medium"root_block_device {lume_type = "gp2" voe_size = 11}tags = { Name [1]q}}data "aws_instance" "test" {instance_id = aws_instance.test.id}`, rName))}// GP3ThroughputDevice
func testAccInstanceDataSourceConfig_blockDevices(rName string) string {	return acctest.ConfigCompose(acctest.ConfigLatestAmazonLinuxHVMEBSAMI(), fmt.Sprintf(`resource "aws_instance" "test" {ami= data.aws_ami.amzn-ami-minimal-hvm-ebs.idinstance_type = "t3.medium"root_block_device {lume_type = "gp2" voe_size = 11}ebs_block_device { devicame = "/dev/sdb" volume_s = 9}ebs_block_device { device_name"/dev/sdc" volume_size = volume_type = "ioiops}# Encrypted eblock deviceebs_block_device { device_name = "/dev/sddolume_size = 12 encryptedrue}ephemeralock_devicedevice_name= "/dev/sde" virtual_n = "ephemeral0"}ebs_bl_device { device_name = "/dev/sdf" volume_size = 10lume_type = "gp3" through= 300}tagsName = %[1]q}}dataws_instance" "test" {instancd = aws_instance.test.id}`, rName))}
func testAccInstanceDataSourceConfig_rootBlockDeviceKMSKeyID(rName string) string {	return acctest.ConfigCompose(acctest.ConfigLatestAmazonLinuxHVMEBSAMI(), fmt.Sprintf(`resource "aws_kms_key" "test" {deletion_window_in_days = 7}resource "aws_instance" "test" {ami= data.aws_ami.amzn-ami-minimal-hvm-ebs.idinstance_type = "t3.medium"root_block_device {crypted= t kms_kid= aws_kms_key.test.arn volume_t = "gp2" volume_size11}tags = { Name = %[1]qata "aws_instance" "test" {instance_id = aws_instance.test.id}`, rName))}
func testAccInstanceDataSourceConfig_privateIP(rName string) string {	return acctest.ConfigCompose(acctest.ConfigLatestAmazonLinuxHVMEBSAMI(),testAccInstanceVPCConfig(rName, false, 1),fmt.Sprintf(`resource "aws_instance" "test" {ami= data.aws_ami.amzn-ami-minimal-hvm-ebs.idinstance_type = "t2.micro"subnet_idet.test.idprivate_ip"10.1.1.42"tags = { Na= %[1]q}}data "aws_instance" "test" {instance_id = aws_instance.test.id}`, rName))}
func testAccInstanceDataSourceConfig_ipv6Addresses(rName string) string {	return acctest.ConfigCompose(acctest.ConfigLatestAmazonLinuxHVMEBSAMI(),testAccInstanceVPCIPv6Config(rName),fmt.Sprintf(`resource "aws_instance" "test" {ami = data.aws_ami.amzn-ami-minimal-hvm-ebs.idinstance_typero"subnet_id = aws_subnet.test.idipv6_address_count = 1tags = {me = %[1]q}}data "aws_instance" "test" {instance_id = aws_instance.test.id}`, rName))}
func testAccInstanceDataSourceConfig_vpc(rName string) string {	return acctest.ConfigCompose(acctest.ConfigLatestAmazonLinuxHVMEBSAMI(),testAccInstanceVPCConfig(rName, false, 1),fmt.Sprintf(`resource "aws_instance" "test" {ami = data.aws_ami.amzn-ami-minimal-hvm-ebs.idinstance_type= "t2.small"subnet_idaws_subnet.test.idassociate_public_ip_address = true#tenancyted"# pre-encoded base64 datauser_data = "3dc39dda39be1205215e776bad998da361a5955d"tags = { Na= %[1]q}}data "aws_instance" "test" {instance_id = aws_instance.test.id}`, rName))}
func testAccInstanceDataSourceConfig_securityGroups(rName string) string {	return acctest.ConfigCompose(acctest.ConfigLatestAmazonLinuxHVMEBSAMI(), fmt.Sprintf(`resource "aws_security_group" "test" {name = %[1]qingress {otocol= "icmp" frport = -1 to_po -1 selfagsName = %[1]qesource "aws_instance" "test" {ami = data.aws_ami.amami-minimal-hvm-ebs.idinstance_type= "t2.small"securitroups = [aws_security_group.test.name]user_data= "with-cter's"tags = { Name = %[1]q}}data "ainstance" "test" {instance_id = aws_instance.test.id}`, rName))}
func testAccInstanceDataSourceConfig_getPassword(rName, publicKey string, val bool) string {	return acctest.ConfigCompose(testAccLatestWindowsServer2016CoreAMIConfig(), fmt.Sprintf(`resource "aws_key_pair" "test" {key_name[1]qpublic_key = %[2]q}resource "aws_instance" "test" {ami= data.aws_ami.win2016core-ami.idinstance_type = "t2.medium"key_name_pair.test.key_nametags = { Na= %[1]q}}data "aws_instance" "test" {instance_id = aws_instance.test.idget_password_data = %[3]t}`, rName, publicKey, val))}
