...
// SPDX-License-Identifier: MPL-2.0package ecs_testimport (
	...
	resourceName := "aws_ecs_cluster_capacity_providers.test"	resource.ParallelTest(t, resource.TestCase{
	...
	ctx := acctest.Context(t)	// This test proves that https://github.com/hashicorp/terraform-provider-aws/issues/11409
...
resource "aws_ecs_cluster" "test" {
  ...
  cluster_name = aws_ecs_cluster.test.name  capacity_providers = ["FARGATE"]  default_capacity_provider_strategy {
    base     = 1
    weight   = 100
    capacity_provider = "FARGATE"
  }
...
resource "aws_ecs_cluster" "test" {
  ...
  cluster_name = aws_ecs_cluster.test.name  capacity_providers = []
}
`, rName)
...
resource "aws_ecs_cluster" "test" {
  ...
  cluster_name = aws_ecs_cluster.test.name  capacity_providers = [%[2]q]
}
`, rName, provider1)
...
resource "aws_ecs_cluster" "test" {
  ...
  cluster_name = aws_ecs_cluster.test.name  capacity_providers = [%[2]q, %[3]q]
}
...
resource "aws_ecs_cluster" "test" {
  ...
  cluster_name = aws_ecs_cluster.test.name  capacity_providers = ["FARGATE", "FARGATE_SPOT"]  default_capacity_provider_strategy {
    base     = 1
    weight   = 100
    capacity_provider = "FARGATE"
  }
...
resource "aws_ecs_cluster" "test" {
  ...
  cluster_name = aws_ecs_cluster.test.name  capacity_providers = ["FARGATE", "FARGATE_SPOT"]
}
`, rName)
...
data "aws_ami" "test" {
  ...
  cluster_name       = aws_ecs_cluster.test.name
  capacity_providers = [aws_ecs_capacity_provider.test.name]  default_capacity_provider_strategy {
...
}data "aws_partition" "test" {}resource "aws_iam_role_policy_attachment" "test" {
  ...
  vpc_security_group_ids      = [aws_security_group.test.id]  iam_instance_profile {
...
EOL
  ...
  min_size   = 2
  name       = %[1]q
  vpc_zone_identifier = [aws_subnet.test.id]  wait_for_capacity_timeout = "5m"  instance_refresh {
    strategy = "Rolling"
  }  launch_template {
