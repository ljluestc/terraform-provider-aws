...
// +build sweeppackage ecsimport (
	...
	sweepResources := make([]sweep.Sweepable, 0)	err = describeCapacityProvidersPages(ctx, conn, input, func(page *ecs.DescribeCapacityProvidersOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
		}		for _, v := range page.CapacityProviders {
			arn := aws.StringValue(v.CapacityProviderArn)			if name := aws.StringValue(v.Name); name == "FARGATE" || name == "FARGATE_SPOT" {
	...
	sweepResources := make([]sweep.Sweepable, 0)	err = conn.ListClustersPagesWithContext(ctx, input, func(page *ecs.ListClustersOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
		}		for _, v := range page.ClusterArns {
			r := ResourceCluster()
			d := r.Data(nil)
			d.SetId(aws.StringValue(v))			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
	})	if sweep.SkipSweepError(err) {
		...
		if page == nil {
			...
			input := &ecs.ListServicesInput{
				...
				if page == nil {
					...
					d.Set("cluster", clusterARN)					sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
				}				return !lastPage
			})			if sweep.SkipSweepError(err) {
				continue
			}			if err != nil {
	...
	sweepResources := make([]sweep.Sweepable, 0)	err = conn.ListTaskDefinitionsPagesWithContext(ctx, input, func(page *ecs.ListTaskDefinitionsOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
			...
			d := r.Data(nil)
			d.SetId(aws.StringValue(v))
			d.Set("arn", v)			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
	})	if sweep.SkipSweepError(err) {
