...
// SPDX-License-Identifier: MPL-2.0package ecsimport (
	...
	capacityProviderUpdateTimeout = 10 * time.Minute	serviceCreateTimeout= 2 * time.Minute
	serviceInactiveMinTimeout = 1 * time.Second
	serviceDescribeTimeout= 2 * time.Minute
	serviceUpdateTimeout= 2 * time.Minute	clusterAvailableDelay= 10 * time.Second
	clusterAvailableTimeout = 10 * time.Minute
	clusterDeleteTimeout= 10 * time.Minute
	clusterReadTimeout= 2 * time.Second
	clusterUpdateTimeout= 10 * time.Minute	taskSetCreateTimeout = 10 * time.Minute
...
Timeout: capacityProviderDeleteTimeout,
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if v, ok := outputRaw.(*ecs.CapacityProvider); ok {
return v, err
	}	return nil, err
...
Timeout: timeout,
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if v, ok := outputRaw.(*ecs.Service); ok {
return v, err
	}	return nil, err
