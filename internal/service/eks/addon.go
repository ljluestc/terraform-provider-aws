...
DeleteWithoutTimeout: resourceAddonDelete,Importer: &schema.ResourceImporter{
...
conn := meta.(*conns.AWSClient).EKSConn(ctx)addonName := d.Get("addon_name").(string)
...
}d.SetId(id)if _, err := waitAddonCreated(ctx, conn, clusterName, addonName, d.Timeout(schema.TimeoutCreate)); err != nil {
...
// ConfigurationConflictApply failed with 1 conflict: conflict with "kubectl"...
//
...
var diags diag.Diagnostics
conn := meta.(*conns.AWSClient).EKSConn(ctx)clusterName, addonName, err := AddonParseResourceID(d.Id())if err != nil {
return sdkdiag.AppendFromErr(diags, err)
}addon, err := FindAddonByClusterNameAndAddonName(ctx, conn, clusterName, addonName)if !d.IsNewResource() && tfresource.NotFound(err) {
...
d.Set("service_account_role_arn", addon.ServiceAccountRoleArn)setTagsOut(ctx, addon.Tags)return diags
...
}var conflictResolutionAttr, conflictResolution stringif v, ok := d.GetOk("resolve_conflicts"); ok {
...
}output, err := conn.UpdateAddonWithContext(ctx, input)if err != nil {
...
_, err = conn.DeleteAddonWithContext(ctx, input)if err != nil {
