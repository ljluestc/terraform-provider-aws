...
funcTestAccEKSAddon_basic(t*testing.T){
...
funcTestAccEKSAddon_disappears(t*testing.T){
ctx:=acctest.Context(t)
varaddoneks.Addon
rName:=sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
...
funcTestAccEKSAddon_Disappears_cluster(t*testing.T){
ctx:=acctest.Context(t)
varaddoneks.Addon
rName:=sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName:="aws_eks_addon.test"
...
funcTestAccEKSAddon_addonVersion(t*testing.T){
ctx:=acctest.Context(t)
varaddon1,addon2eks.Addon
rName:=sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName:="aws_eks_addon.test"
addonName:="vpc-cni"
...
funcTestAccEKSAddon_preserve(t*testing.T){
ctx:=acctest.Context(t)
varaddoneks.Addon
rName:=sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName:="aws_eks_addon.test"
...
funcTestAccEKSAddon_deprecated(t*testing.T){
ctx:=acctest.Context(t)
varaddon1,addon2,addon3eks.Addon
rName:=sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName:="aws_eks_addon.test"
addonName:="vpc-cni"
...
funcTestAccEKSAddon_resolveConflicts(t*testing.T){
ctx:=acctest.Context(t)
varaddon1,addon2,addon3eks.Addon
rName:=sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName:="aws_eks_addon.test"
...
funcTestAccEKSAddon_serviceAccountRoleARN(t*testing.T){
ctx:=acctest.Context(t)
varaddoneks.Addon
rName:=sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName:="aws_eks_addon.test"
...
funcTestAccEKSAddon_configurationValues(t*testing.T){
ctx:=acctest.Context(t)
varaddoneks.Addon
rName:=sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName:="aws_eks_addon.test"
...
ExpectError:regexache.MustCompile(`InvalidParameterException:ConfigurationValueprovidedinrequestisnotsupported`),
},
},
})
}

funcTestAccEKSAddon_tags(t*testing.T){
ctx:=acctest.Context(t)
varaddon1,addon2,addon3eks.Addon
rName:=sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName:="aws_eks_addon.test"
...
functestAccCheckAddonExists(ctxcontext.Context,nstring,v*eks.Addon)resource.TestCheckFunc{
...
ifrs.Primary.ID==""{
returnfmt.Errorf("NoEKSAdd-OnIDisset")
}
...
functestAccCheckAddonDestroy(ctxcontext.Context)resource.TestCheckFunc{
returnfunc(s*terraform.State)error{
conn:=acctest.Provider.Meta().(*conns.AWSClient).EKSConn(ctx)

for_,rs:=ranges.RootModule().Resources{
ifrs.Type!="aws_eks_addon"{
continue
}
...
returnfmt.Errorf("EKSAdd-On%sstillexists",rs.Primary.ID)
}
...
functestAccPreCheckAddon(ctxcontext.Context,t*testing.T){
conn:=acctest.Provider.Meta().(*conns.AWSClient).EKSConn(ctx)
...
functestAccAddonConfig_base(rNamestring)string{
...
functestAccAddonConfig_basic(rName,addonNamestring)string{
returnacctest.ConfigCompose(testAccAddonConfig_base(rName),fmt.Sprintf(`
...
functestAccAddonConfig_version(rName,addonName,addonVersionstring)string{
returnacctest.ConfigCompose(testAccAddonConfig_base(rName),fmt.Sprintf(`
resource"aws_eks_addon""test"{
cluster_name=aws_eks_cluster.test.name
addon_name=%[2]q
...
functestAccAddonConfig_preserve(rName,addonNamestring)string{
returnacctest.ConfigCompose(testAccAddonConfig_base(rName),fmt.Sprintf(`
resource"aws_eks_addon""test"{
cluster_name=aws_eks_cluster.test.name
addon_name=%[2]q
...
functestAccAddonConfig_deprecated(rName,addonName,resolveConflictsstring)string{
returnacctest.ConfigCompose(testAccAddonConfig_base(rName),fmt.Sprintf(`
resource"aws_eks_addon""test"{
cluster_name=aws_eks_cluster.test.name
addon_name=%[2]q
...
functestAccAddonConfig_resolveConflicts(rName,addonName,resolveConflictsOnCreate,resolveConflictsOnUpdatestring)string{
returnacctest.ConfigCompose(testAccAddonConfig_base(rName),fmt.Sprintf(`
resource"aws_eks_addon""test"{
cluster_name=aws_eks_cluster.test.name
addon_name=%[2]q
...
functestAccAddonConfig_serviceAccountRoleARN(rName,addonNamestring)string{
returnacctest.ConfigCompose(testAccAddonConfig_base(rName),fmt.Sprintf(`
...
functestAccAddonConfig_tags1(rName,addonName,tagKey1,tagValue1string)string{
returnacctest.ConfigCompose(testAccAddonConfig_base(rName),fmt.Sprintf(`
resource"aws_eks_addon""test"{
cluster_name=aws_eks_cluster.test.name
addon_name=%[2]q
...
functestAccAddonConfig_tags2(rName,addonName,tagKey1,tagValue1,tagKey2,tagValue2string)string{
returnacctest.ConfigCompose(testAccAddonConfig_base(rName),fmt.Sprintf(`
resource"aws_eks_addon""test"{
cluster_name=aws_eks_cluster.test.name
addon_name=%[2]q
...
functestAccAddonConfig_configurationValues(rName,addonName,addonVersion,configurationValues,resolveConflictsstring)string{
returnacctest.ConfigCompose(testAccAddonConfig_base(rName),fmt.Sprintf(`
resource"aws_eks_addon""test"{
cluster_name=aws_eks_cluster.test.name
addon_name=%[2]q
