...
func TestAccEKSCluster_Encryption_versionUpdate(t *testing.T) {
	...
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_eks_cluster.test"
	controlPlaneInstanceType := "m5d.large"	resource.ParallelTest(t, resource.TestCase{
	...
	return func(s *terraform.State) error {
		...
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).EKSConn(ctx)		output, err := tfeks.FindClusterByName(ctx, conn, rs.Primary.ID)		if err != nil {
			return err
		}		*cluster = *output		return nil
	}
	...
	return func(s *terraform.State) error {
		for _, rs := range s.RootModule().Resources {
			if rs.Type != "aws_eks_cluster" {
				continue
			}			conn := acctest.Provider.Meta().(*conns.AWSClient).EKSConn(ctx)			_, err := tfeks.FindClusterByName(ctx, conn, rs.Primary.ID)			if tfresource.NotFound(err) {
				continue
			}			if err != nil {
				return err
	...
	conn := acctest.Provider.Meta().(*conns.AWSClient).EKSConn(ctx)	input := &eks.ListClustersInput{}	_, err := conn.ListClustersWithContext(ctx, input)	if acctest.PreCheckSkipError(err) {
...
resource "aws_eks_cluster" "test" {
  name= %[1]q
  role_arn= aws_iam_role.test.arn
  enabled_cluster_log_types = ["%[2]v"]  vpc_config {
subnet_ids = aws_subnet.test[*].id
  }  depends_on = [aws_iam_role_policy_attachment.test-AmazonEKSClusterPolicy]
}
...
resource "aws_kms_key" "test" {
  ...
  name= %[1]q
  role_arn = aws_iam_role.test.arn  encryption_config {
resources = ["secrets"]provider {
...
values = [data.aws_outposts_outpost.test.arn]
  ...
  name= %[1]q
  role_arn = data.aws_iam_role.test.arn  outpost_config {
