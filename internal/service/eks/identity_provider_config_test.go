...
resourceName:="aws_eks_identity_provider_config.test"resource.ParallelTest(t,resource.TestCase{
...
}clusterName,configName,err:=tfeks.IdentityProviderConfigParseResourceID(rs.Primary.ID)iferr!=nil{
returnerr
}conn:=acctest.Provider.Meta().(*conns.AWSClient).EKSConn(ctx)output,err:=tfeks.FindOIDCIdentityProviderConfigByClusterNameAndConfigName(ctx,conn,clusterName,configName)iferr!=nil{
returnerr
}*config=*outputreturnnil
}
...
returnerr
}_,err=tfeks.FindOIDCIdentityProviderConfigByClusterNameAndConfigName(ctx,conn,clusterName,configName)iftfresource.NotFound(err){
continue
}iferr!=nil{
returnerr
...
name=%[1]qassume_role_policy=jsonencode({
Statement=[{
...
cluster_name=aws_eks_cluster.test.nameoidc{
...
username_prefix="-"required_claims={
keyOne="valueOne"
keyTwo="valueTwo"
}
}
