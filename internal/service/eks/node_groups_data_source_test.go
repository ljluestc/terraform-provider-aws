...
dataSourceResourceName := "data.aws_eks_node_groups.test"resource.Test(t, resource.TestCase{
...
data "aws_eks_node_groups" "test" {
  cluster_name = aws_eks_cluster.test.name  depends_on = [aws_eks_node_group.test_a, aws_eks_node_group.test_b]
}
`)
...
resource "aws_eks_node_group" "test_a" {
  ...
  subnet_ids = aws_subnet.test[*].id  scaling_config {
desired_size = 1
max_size= 1
min_size= 1
  }  depends_on = [
