...
https://github.com/kubernetes-sigs/aws-iam-authenticator/blob/7547c74e660f8d34d9980f2c69aa008eed1f48d0/pkg/token/token.goWith the following modifications: - Removal of all Generator interface methods and implementations except GetWithSTS
...
*//*
Copyright 2017-2020 by the contributors.Licensed under the Apache License, Version 2.0 (the "License");
...
*/package eksimport (
	...
	"strconv"
	"strings"
	"time"	"github.com/aws/aws-sdk-go/service/sts"
...
type Identity struct {
	...
	ARN string	// CanonicalARN is the Amazon Resource Name converted to a more canonical
	...
	CanonicalARN string	// AccountID is the 12 digit AWS account number.
	AccountID string	// UserID is the unique user/role ID (e.g., "AROAAAAAAAAAAAAAAAAAA").
	UserID string	// SessionName is the STS session name (or "" if this is not a
	...
	SessionName string	// The AWS Access Key ID used to authenticate the request.  This can be used
...
func (g generator) GetWithSTS(ctx context.Context, clusterID string, stsAPI *sts.STS) (Token, error) {
	...
	request.HTTPRequest.Header.Add(clusterIDHeader, clusterID)	// Sign the request.  The expires parameter (sets the x-amz-expires header) is
...
func (v tokenVerifier) Verify(token string) (*Identity, error) {
	...
	accessKeyID := strings.Split(queryParamsLower.Get("x-amz-credential"), "/")[0]	dateParam, err := time.Parse(dateHeaderFormat, date)
	if err != nil {
	...
	req.Header.Set("accept", "application/json")	response, err := v.client.Do(req)
	if err != nil {
	...
	defer response.Body.Close()	responseBody, err := io.ReadAll(response.Body)
	if err != nil {
