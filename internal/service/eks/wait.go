...
Timeout: timeout,
}outputRaw, err := stateConf.WaitForStateContext(ctx)if output, ok := outputRaw.(*eks.Addon); ok {
...
Timeout: timeout,
}outputRaw, err := stateConf.WaitForStateContext(ctx)if output, ok := outputRaw.(*eks.Update); ok {
...
Timeout: timeout,
}outputRaw, err := stateConf.WaitForStateContext(ctx)if output, ok := outputRaw.(*eks.FargateProfile); ok {
return output, err
}return nil, err
...
Timeout: timeout,
}outputRaw, err := stateConf.WaitForStateContext(ctx)if output, ok := outputRaw.(*eks.Nodegroup); ok {
...
Timeout: timeout,
}outputRaw, err := stateConf.WaitForStateContext(ctx)if output, ok := outputRaw.(*eks.OidcIdentityProviderConfig); ok {
return output, err
}return nil, err
