...
func TestAccElastiCacheCluster_Engine_memcached(t *testing.T) {
	...
	resourceName := "aws_elasticache_cluster.test"	resource.ParallelTest(t, resource.TestCase{
		PreCheck: 
...
func TestAccElastiCacheCluster_ReplicationGroupID_availabilityZone(t *testing.T) {
	...
	replicationGroupResourceName := "aws_elasticache_replication_group.test"	resource.ParallelTest(t, resource.TestCase{
		PreCheck: 
func() { acctest.PreCheck(ctx, t) },
		ErrorCheck:acctest.ErrorCheck(t, elasticache.EndpointsID),
		ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckReplicationGroupExists(ctx, replicationGroupResourceName, &replicationGroup),					testAccCheckClusterExists(ctx, clusterResourceName1, &cluster1),
					...
					resource.TestCheckResourceAttr(clusterResourceName1, "port", "6379"),					testAccCheckClusterExists(ctx, clusterResourceName2, &cluster2),
...
func(s *terraform.State) error {
		conn := acctest.Provider.Meta().(*conns.AWSClient).ElastiCacheConn(ctx)		for _, rs := range s.RootModule().Resources {
			if rs.Type != "aws_elasticache_cluster" {
				continue
			}			_, err := tfelasticache.FindCacheClusterByID(ctx, conn, rs.Primary.ID)			if tfresource.NotFound(err) {
				continue
			}			if err != nil {
				return err
...
func(s *terraform.State) error {
		...
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).ElastiCacheConn(ctx)		output, err := tfelasticache.FindCacheClusterByID(ctx, conn, rs.Primary.ID)		if err != nil {
			return err
		}		*v = *output		return nil
	}
}
...
num_cache_nodes = 1
ip_discovery = %[2]q
network_type = %[3]qsubnet_group_name= aws_elasticache_subnet_group.test.name
...
engine = "memcached"
node_type = "cache.t3.small"
num_cache_nodes = 1final_snapshot_identifier = %[1]q
}
`, rName)
}
...
engine_version= "6.0"
node_type = "cache.t3.small"
num_cache_nodes = 1auto_minor_version_upgrade = %[2]t
}
`, rName, enable)
}
