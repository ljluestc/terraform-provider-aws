...
// it may also be a different identifier depending on the service.func listTags(ctx context.Context, conn elasticacheiface.ElastiCacheAPI, identifier string) (tftags.KeyValueTags, error) {
...
// It is called from outside this package.func (p *servicePackage) ListTags(ctx context.Context, meta any, identifier string) error {
	tags, err := listTags(ctx, meta.(*conns.AWSClient).ElastiCacheConn(ctx), identifier)	if err != nil {
		return err
	}	if inContext, ok := tftags.FromContext(ctx); ok {
		inContext.TagsOut = types.Some(tags)
	}	return nil
}// []*SERVICE.Tag handling// Tags returns elasticache service tags.func Tags(tags tftags.KeyValueTags) []*elasticache.Tag {
	result := make([]*elasticache.Tag, 0, len(tags))	for k, v := range tags.Map() {
		tag := &elasticache.Tag{
			Key:aws.String(k),
			Value: aws.String(v),
		}		result = append(result, tag)
	}	return result
}// KeyValueTags creates tftags.KeyValueTags from elasticache service tags.func KeyValueTags(ctx context.Context, tags []*elasticache.Tag) tftags.KeyValueTags {
	m := make(map[string]*string, len(tags))	for _, tag := range tags {
		m[aws.StringValue(tag.Key)] = tag.Value
	}	return tftags.New(ctx, m)
}// getTagsIn returns elasticache service tags from Context.
// nil is returned if there are no input tags.func getTagsIn(ctx context.Context) []*elasticache.Tag {
	if inContext, ok := tftags.FromContext(ctx); ok {
		if tags := Tags(inContext.TagsIn.UnwrapOrDefault()); len(tags) > 0 {
			return tags
		}
	}	return nil
}// setTagsOut sets elasticache service tags in Context.func setTagsOut(ctx context.Context, tags []*elasticache.Tag) {
	if inContext, ok := tftags.FromContext(ctx); ok {
		inContext.TagsOut = types.Some(KeyValueTags(ctx, tags))
	}
}// createTags creates elasticache service tags for new resources.func createTags(ctx context.Context, conn elasticacheiface.ElastiCacheAPI, identifier string, tags []*elasticache.Tag) error {
	if len(tags) == 0 {
		return nil
	}	return updateTags(ctx, conn, identifier, nil, KeyValueTags(ctx, tags))
}// updateTags updates elasticache service tags.
// The identifier is typically the Amazon Resource Name (ARN), although
// it may also be a different identifier depending on the service.func updateTags(ctx context.Context, conn elasticacheiface.ElastiCacheAPI, identifier string, oldTagsMap, newTagsMap any) error {
