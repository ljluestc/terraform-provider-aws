...
func TestAccElastiCacheUserGroupAssociation_basic(t *testing.T) {
	...
	resourceName := "aws_elasticache_user_group_association.test"	resource.ParallelTest(t, resource.TestCase{
		PreCheck: 
...
func TestAccElastiCacheUserGroupAssociation_multiple(t *testing.T) {
	...
	resourceName2 := "aws_elasticache_user_group_association.test2"	resource.ParallelTest(t, resource.TestCase{
		PreCheck: 
func() { acctest.PreCheck(ctx, t) },
		ErrorCheck:acctest.ErrorCheck(t, elasticache.EndpointsID),
		ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
func(s *terraform.State) error {
		conn := acctest.Provider.Meta().(*conns.AWSClient).ElastiCacheConn(ctx)		for _, rs := range s.RootModule().Resources {
			if rs.Type != "aws_elasticache_user_group_association" {
				continue
			}			userGroupID, userID, err := tfelasticache.UserGroupAssociationParseResourceID(rs.Primary.ID)			if err != nil {
				return err
			}			err = tfelasticache.FindUserGroupAssociation(ctx, conn, userGroupID, userID)			if tfresource.NotFound(err) {
				continue
			}			if err != nil {
				return err
...
func(s *terraform.State) error {
		...
		if !ok {
			...
			return err
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).ElastiCacheConn(ctx)		err = tfelasticache.FindUserGroupAssociation(ctx, conn, userGroupID, userID)		return err
	}
}
...
user_group_id = %[1]q
engine= "REDIS"
user_ids= [aws_elasticache_user.test1.user_id]lifecycle {
