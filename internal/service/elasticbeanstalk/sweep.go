...
// +build sweeppackage elasticbeanstalkimport (
	"fmt"
...
func sweepApplications(region string) error {
	...
	conn := client.ElasticBeanstalkConn(ctx)	resp, err := conn.DescribeApplicationsWithContext(ctx, &elasticbeanstalk.DescribeApplicationsInput{})
	if err != nil {
...
func sweepEnvironments(region string) error {
	...
	sweepResources := make([]sweep.Sweepable, 0)	err = describeEnvironmentsPages(ctx, conn, input, func(page *elasticbeanstalk.EnvironmentDescriptionsMessage, lastPage bool) bool {
		if page == nil {
			return !lastPage
			...
			d.Set("wait_for_ready_timeout", "5m")			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
	})	if sweep.SkipSweepError(err) {
