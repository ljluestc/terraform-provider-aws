...
package elasticbeanstalkimport (
"context"
...
tags, err := listTags(ctx, meta.(*conns.AWSClient).ElasticBeanstalkConn(ctx), identifier)if err != nil {
return err
}if inContext, ok := tftags.FromContext(ctx); ok {
inContext.TagsOut = types.Some(tags)
}return nil
}// []*SERVICE.Tag handling// Tags returns elasticbeanstalk service tags.
func Tags(tags tftags.KeyValueTags) []*elasticbeanstalk.Tag {
result := make([]*elasticbeanstalk.Tag, 0, len(tags))for k, v := range tags.Map() {
...
updatedTags = updatedTags.IgnoreSystem(names.ElasticBeanstalk)// Ensure we do not send empty requests.
...
}_, err := conn.UpdateTagsForResourceWithContext(ctx, input)if err != nil {
