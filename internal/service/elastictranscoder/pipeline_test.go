...
// SPDX-License-Identifier: MPL-2.0package elastictranscoder_testimport (
"context"
"fmt"
"reflect"
"sort"
...
ctx := acctest.Context(t)
pipeline := elastictranscoder.Pipeline{}
resourceName := "aws_elastictranscoder_pipeline.test"rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)resource.ParallelTest(t, resource.TestCase{
PreCheck:.PreCheck(ctx, t); testAccPreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, elastictranscoder.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckPipelineDestroy(ctx),
...
sort.Strings(notifications)if !reflect.DeepEqual(notes, notifications) {
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).ElasticTranscoderConn(ctx)out, err := conn.ReadPipelineWithContext(ctx, &elastictranscoder.ReadPipelineInput{
Id: aws.String(rs.Primary.ID),
})if err != nil {
return err
}*res = *out.Pipelinereturn nil
}
}func testAccCheckPipelineDestroy(ctx context.Context) resource.TestCheckFunc {
return func(s *terraform.State) error {
conn := acctest.Provider.Meta().(*conns.AWSClient).ElasticTranscoderConn(ctx)for _, rs := range s.RootModule().Resources {
...
conn := acctest.Provider.Meta().(*conns.AWSClient).ElasticTranscoderConn(ctx)input := &elastictranscoder.ListPipelinesInput{}_, err := conn.ListPipelinesWithContext(ctx, input)if acctest.PreCheckSkipError(err) {
...
resource "aws_elastictranscoder_pipeline" "test" {
  ...
  name= %[1]q
  role= aws_iam_role.test.arn  content_config {
bucketcontent_bucket.bucket
...
resource "aws_elastictranscoder_pipeline" "test" {
  ...
  output_bucket = aws_s3_bucket.test.bucket
  name = %[1]q
  role = aws_iam_role.test.arn  notifications {
