...
rorCheck:acctest.ErrorCheck(t, elastictranscoder.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckPresetDestroy(ctx),
...
nn := acctest.Provider.Meta().(*conns.AWSClient).ElasticTranscoderConn(ctx)rsok := s.RootModule().Resources[name]
 !ok {
return fmt.Errorf("Not found: %s", name) rs.Primary.ID == "" {
return fmt.Errorf("No Preset ID is set")
u err := conn.ReadPresetWithContext(ctx, &elastictranscoder.ReadPresetInput{
Id: aws.String(rs.Primary.ID),
ifrr != nil {
return err
pset = *out.Presetretu nil
	}
}func testAccCheckPresetDestroy(ctx context.Context) resource.TestCheckFunc {
	return func(s *terraform.State) error {
nn := acctest.Provider.Meta().(*conns.AWSClient).ElasticTranscoderConn(ctx)fo_, rs := range s.RootModule().Resources {
