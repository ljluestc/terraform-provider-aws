...
resourceName := "aws_app_cookie_stickiness_policy.test"resource.ParallelTest(t, resource.TestCase{
...
elbResourceName := "aws_elb.test"resource.ParallelTest(t, resource.TestCase{
PreCheck: func() { acctest.PreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, elb.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckAppCookieStickinessPolicyDestroy(ctx),
...
}func testAccCheckAppCookieStickinessPolicyDestroy(ctx context.Context) resource.TestCheck
func {
...
continue
}lbName, lbPort, policyName, err := tfelb.AppCookieStickinessPolicyParseResourceID(rs.Primary.ID)if err != nil {
return err
}_, err = tfelb.FindLoadBalancerListenerPolicyByThreePartKey(ctx, conn, lbName, lbPort, policyName)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
}func testAccCheckAppCookieStickinessPolicyExists(ctx context.Context, n string) resource.TestCheck
func {
returnfunc(s *terraform.State) error {
...
return err
}conn := acctest.Provider.Meta().(*conns.AWSClient).ELBConn(ctx)_, err = tfelb.FindLoadBalancerListenerPolicyByThreePartKey(ctx, conn, lbName, lbPort, policyName)return err
}
