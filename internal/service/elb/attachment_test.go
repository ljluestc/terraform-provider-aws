...
}// remove and instance and check that it's correctly re-attached.func TestAccELBAttachment_drift(t *testing.T) {
ctx := acctest.Context(t)
var conf elb.LoadBalancerDescription
resourceName := "aws_elb.test"testAccAttachmentConfig_deregInstance :=func() {
conn := acctest.Provider.Meta().(*conns.AWSClient).ELBConn(ctx)deRegisterInstancesOpts := elb.DeregisterInstancesFromLoadBalancerInput{
LoadBalancerName: conf.LoadBalancerName,
Instances:
}log.Printf("[DEBUG] deregistering instance %v from ELB", *conf.Instances[0].InstanceId)_, err := conn.DeregisterInstancesFromLoadBalancerWithContext(ctx, &deRegisterInstancesOpts)
if err != nil {
...
}func testAccAttachmentCheckInstanceCount(conf *elb.LoadBalancerDescription, expected int) resource.TestCheck
func {
...
}// add one attachmentfunc testAccAttachmentConfig_1() string {
...
`)
}// add a second attachmentfunc testAccAttachmentConfig_2() string {
return acctest.ConfigCompose(acctest.ConfigLatestAmazonLinuxHVMEBSAMI(), `
data "aws_availability_zones" "available" {
  state = "available"  filter {
name= "opt-in-status"
...
`)
}// swap attachments between resourcesfunc testAccAttachmentConfig_3() string {
return acctest.ConfigCompose(acctest.ConfigLatestAmazonLinuxHVMEBSAMI(), `
data "aws_availability_zones" "available" {
  state = "available"  filter {
name= "opt-in-status"
...
`)
}// destroy attachmentsfunc testAccAttachmentConfig_4() string {
return `
data "aws_availability_zones" "available" {
  state = "available"  filter {
name= "opt-in-status"
