...
resourceName := "aws_load_balancer_backend_server_policy.test"resource.ParallelTest(t, resource.TestCase{
...
}func testAccCheckBackendServerPolicyDestroy(ctx context.Context) resource.TestCheck
func {
...
continue
}lbName, instancePort, err := tfelb.BackendServerPolicyParseResourceID(rs.Primary.ID)if err != nil {
return err
}_, err = tfelb.FindLoadBalancerBackendServerPolicyByTwoPartKey(ctx, conn, lbName, instancePort)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
}func testAccCheckBackendServerPolicyExists(ctx context.Context, n string) resource.TestCheck
func {
returnfunc(s *terraform.State) error {
...
return err
}conn := acctest.Provider.Meta().(*conns.AWSClient).ELBConn(ctx)_, err = tfelb.FindLoadBalancerBackendServerPolicyByTwoPartKey(ctx, conn, lbName, instancePort)return err
}
