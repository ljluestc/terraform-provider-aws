...
// SPDX-License-Identifier: MPL-2.0package elb_testimport (
	...
	resourceName := "aws_lb_cookie_stickiness_policy.test"	resource.ParallelTest(t, resource.TestCase{
...
}func testAccCheckLBCookieStickinessPolicyDestroy(ctx context.Context) resource.TestCheck
func {
...
continue
	}	lbName, lbPort, policyName, err := tfelb.LBCookieStickinessPolicyParseResourceID(rs.Primary.ID)	if err != nil {
return err
...
}func testAccCheckLBCookieStickinessPolicyExists(ctx context.Context, n string) resource.TestCheck
func {
	returnfunc(s *terraform.State) error {
