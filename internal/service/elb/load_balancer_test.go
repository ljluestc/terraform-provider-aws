...
// SPDX-License-Identifier: MPL-2.0package elb_testimport ( // nosemgrep:ci.semgrep.aws.multiple-service-imports
	"context"
	"fmt"
	"math/rand"
	"reflect"
...
if leftHash == rightHash != tc.Match {
	...
	t.Parallel()	var n = "-Testing123"
	_, errors := tfelb.ValidName(n, "SampleKey")	if len(errors) != 1 {
t.Fatalf("Expected the ELB Name to trigger a validation error")
	}
}func TestValidLoadBalancerNameCanBeAnEmptyString(t *testing.T) {
	t.Parallel()	var n = ""
	_, errors := tfelb.ValidName(n, "SampleKey")	if len(errors) != 0 {
t.Fatalf("Expected the ELB Name to pass validation")
	...
	t.Parallel()	var n = "Testing123dddddddddddddddddddvvvv"
	_, errors := tfelb.ValidName(n, "SampleKey")	if len(errors) != 1 {
t.Fatalf("Expected the ELB Name to trigger a validation error")
	}
...
t.Fatalf("Expected the ELB Name to trigger a validation error")
	...
	t.Parallel()	type testCases struct {
Value    int
ErrCount int
...
if len(errors) != tc.ErrCount {
	...
	resourceName := "aws_elb.test"	resource.ParallelTest(t, resource.TestCase{
...
func(
	...
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_elb.test"	testCheck :=func(*terraform.State) error {
...
func(
	...
	var conf elb.LoadBalancerDescription
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_elb.test"	testCheckInstanceAttached :=func(count int) resource.TestCheck
func {
returnfunc(*terraform.State) error {
...
conn := acctest.Provider.Meta().(*conns.AWSClient).ELBConn(ctx)for _, rs := range s.RootModule().Resources {
	if rs.Type != "aws_elb" {
continue
	}	_, err := tfelb.FindLoadBalancerByName(ctx, conn, rs.Primary.ID)	if tfresource.NotFound(err) {
continue
	}	if err != nil {
return err
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).ELBConn(ctx)output, err := tfelb.FindLoadBalancerByName(ctx, conn, rs.Primary.ID)if err != nil {
	return err
}*v = *outputreturn nil
	}
...
resource "aws_elb" "test" {
  availability_zones = [data.aws_availability_zones.available.names[0], data.aws_availability_zones.available.names[1], data.aws_availability_zones.available.names[2]]  name = %[1]q  listener {
    instance_port     = 8000
    instance_protocol = "http"
    lb_port  = 80
    lb_protocol       = "http"
...
resource "aws_elb" "test" {
  name= %[1]q
  availability_zones = [data.aws_availability_zones.available.names[0], data.aws_availability_zones.available.names[1], data.aws_availability_zones.available.names[2]]  listener {
    instance_port     = 8000
    instance_protocol = "http"
    lb_port  = 80
    lb_protocol       = "http"
...
`, rName))
	...
	return fmt.Sprintf(`
data "aws_elb_service_account" "current" {}data "aws_partition" "current" {}resource "aws_s3_bucket" "accesslogs_bucket" {
  bucket        = %[1]q
  force_destroy = true
...
resource "aws_elb" "test" {
  ...
  depends_on = [aws_s3_bucket_policy.test]  availability_zones = [data.aws_availability_zones.available.names[0], data.aws_availability_zones.available.names[1], data.aws_availability_zones.available.names[2]]  name = %[1]q  listener {
    instance_port     = 8000
    instance_protocol = "http"
    lb_port  = 80
    lb_protocol       = "http"
...
`)func testAccLoadBalancerConfig_availabilityZonesUpdate(rName string) string {
	return acctest.ConfigCompose(acctest.ConfigAvailableAZsNoOptIn(), fmt.Sprintf(`
resource "aws_elb" "test" {
  availability_zones = [data.aws_availability_zones.available.names[0], data.aws_availability_zones.available.names[1]]  name = %[1]q  listener {
    instance_port     = 8000
    instance_protocol = "http"
    lb_port  = 80
    lb_protocol       = "http"
...
`, rName))
	...
	return acctest.ConfigCompose(acctest.ConfigAvailableAZsNoOptIn(), fmt.Sprintf(`
resource "aws_elb" "test" {
  availability_zones = [data.aws_availability_zones.available.names[0]]  name = %[1]q  listener {
    instance_port     = 8000
    instance_protocol = "http"
    lb_port  = 80
    lb_protocol       = "http"
...
resource "aws_subnet" "public_b_two" {
  vpc_id = aws_vpc.test.id  cidr_block        = "10.1.6.0/24"
  availability_zone = data.aws_availability_zones.available.names[1]  tags = {
    Name = %[1]q
  }
}resource "aws_elb" "test" {
