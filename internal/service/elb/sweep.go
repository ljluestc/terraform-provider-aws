...
// +build sweeppackage elbimport (
	...
	sweepResources := make([]sweep.Sweepable, 0)	err = conn.DescribeLoadBalancersPagesWithContext(ctx, input,
		func(page *elb.DescribeLoadBalancersOutput, lastPage bool) bool {
			if page == nil {
				...
				d := r.Data(nil)
				d.SetId(aws.StringValue(v.LoadBalancerName))				sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
			}			return !lastPage
