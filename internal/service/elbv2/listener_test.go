...
func TestAccELBV2Listener_LoadBalancerARN_gatewayLoadBalancer(t *testing.T) {
	...
	resourceName := "aws_lb_listener.test"	if testing.Short() {
...
func TestAccELBV2Listener_DefaultAction_order(t *testing.T) {
	...
	certificate := acctest.TLSRSAX509SelfSignedCertificatePEM(t, key, "example.com")
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_lb_listener.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck:  
func() { acctest.PreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, elbv2.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
var newDefaultActions []*elbv2.Actionfor i, action := range listener.DefaultActions {
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).ELBV2Conn(ctx)input := &elbv2.ModifyListenerInput{
	DefaultActions: newDefaultActions,
	ListenerArn: listener.ListenerArn,
}_, err := conn.ModifyListenerWithContext(ctx, input)return err
	}
}
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).ELBV2Conn(ctx)listener, err := tfelbv2.FindListenerByARN(ctx, conn, rs.Primary.ID)if err != nil {
...
continue
	}	listener, err := tfelbv2.FindListenerByARN(ctx, conn, rs.Primary.ID)	if tfawserr.ErrCodeEquals(err, elbv2.ErrCodeListenerNotFoundException) {
continue
	}	if err != nil {
return fmt.Errorf("reading ELBv2 Listener (%s): %w", rs.Primary.ID, err)
	}	if listener == nil {
...
resource "aws_vpc" "test" {
 ...
 protocol = "HTTP"
  ...
  load_balancer_arn = aws_lb.test.id  default_action {
 target_group_arn = aws_lb_target_group.test.id
 type = "forward"
  }  tags = {
 Name = %[1]q
...
resource "aws_acm_certificate" "test" {
 ...
 Name = %[1]q
  ...
  protocol = "TLS"
  ssl_policy  = "ELBSecurityPolicy-2016-08"
  alpn_policy = "HTTP2Preferred"  default_action {
