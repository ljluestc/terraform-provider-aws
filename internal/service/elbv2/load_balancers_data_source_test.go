...
func TestAccELBV2LoadBalancersDataSource_basic(t *testing.T) {
	...
	sharedTagVal := sdkacctest.RandString(32)	resourceLb1 := "aws_lb.test1"
	resourceLb2 := "aws_lb.test2"	dataSourceNameMatchFirstTag := "data.aws_lbs.tag_match_first"
	dataSourceNameMatchBothTag := "data.aws_lbs.tag_match_shared"
	dataSourceNameMatchNoneTag := "data.aws_lbs.tag_match_none"	resource.ParallelTest(t, resource.TestCase{
		PreCheck: 
func() {
