...
// +build sweeppackage elbv2import (
	"fmt"
...
func sweepLoadBalancers(region string) error {
	...
	conn := client.ELBV2Conn(ctx)	var sweeperErrs *multierror.Error
	err = conn.DescribeLoadBalancersPagesWithContext(ctx, &elbv2.DescribeLoadBalancersInput{}, 
func(page *elbv2.DescribeLoadBalancersOutput, lastPage bool) bool {
		if page == nil || len(page.LoadBalancers) == 0 {
			...
			name := aws.StringValue(loadBalancer.LoadBalancerName)			log.Printf("[INFO] Deleting LB: %s", name)
...
func sweepTargetGroups(region string) error {
	...
	conn := client.ELBV2Conn(ctx)	err = conn.DescribeTargetGroupsPagesWithContext(ctx, &elbv2.DescribeTargetGroupsInput{}, 
func(page *elbv2.DescribeTargetGroupsOutput, lastPage bool) bool {
		if page == nil || len(page.TargetGroups) == 0 {
			...
			name := aws.StringValue(targetGroup.TargetGroupName)			log.Printf("[INFO] Deleting LB Target Group: %s", name)
...
func sweepListeners(region string) error {
	...
	var errs *multierror.Error	err = conn.DescribeLoadBalancersPagesWithContext(ctx, &elbv2.DescribeLoadBalancersInput{}, 
func(page *elbv2.DescribeLoadBalancersOutput, lastPage bool) bool {
		if page == nil || len(page.LoadBalancers) == 0 {
			log.Print("[DEBUG] No LBs to sweep")
			return false
...
func(page *elbv2.DescribeListenersOutput, lastPage bool) bool {
				if page == nil {
					...
					d := r.Data(nil)
					d.SetId(aws.StringValue(listener.ListenerArn))					sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
				}				return !lastPage
			})			if err != nil {
