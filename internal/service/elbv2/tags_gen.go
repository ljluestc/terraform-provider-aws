...
func (p *servicePackage) ListTags(ctx context.Context, meta any, identifier string) error {
	tags, err := listTags(ctx, meta.(*conns.AWSClient).ELBV2Conn(ctx), identifier)	if err != nil {
		return err
	}	if inContext, ok := tftags.FromContext(ctx); ok {
		inContext.TagsOut = types.Some(tags)
	}	return nil
}// []*SERVICE.Tag handling// Tags returns elbv2 service tags.
func Tags(tags tftags.KeyValueTags) []*elbv2.Tag {
	result := make([]*elbv2.Tag, 0, len(tags))	for k, v := range tags.Map() {
		tag := &elbv2.Tag{
			Key:aws.String(k),
			Value: aws.String(v),
