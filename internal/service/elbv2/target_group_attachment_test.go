...
func TestAccELBV2TargetGroupAttachment_basic(t *testing.T) {
	...
	targetGroupName := fmt.Sprintf("test-target-group-%s", sdkacctest.RandString(10))	resource.ParallelTest(t, resource.TestCase{
		PreCheck:  
...
func(s *terraform.State) error {
		...
		targetGroupArn := rs.Primary.Attributes["target_group_arn"]		target := &elbv2.TargetDescription{
...
func(s *terraform.State) error {
		...
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).ELBV2Conn(ctx)		_, hasPort := rs.Primary.Attributes["port"]
		targetGroupArn := rs.Primary.Attributes["target_group_arn"]		target := &elbv2.TargetDescription{
			Id: aws.String(rs.Primary.Attributes["target_id"]),
		}
		if hasPort {
...
func testAccTargetGroupAttachmentConfig_idLambda(rName string) string {
	return fmt.Sprintf(`
data "aws_partition" "current" {}resource "aws_lambda_permission" "test" {
...
"Sid": ""
	...
	EOF}resource "aws_lb_target_group_attachment" "test" {
  depends_on = [aws_lambda_permission.test]  target_group_arn = aws_lb_target_group.test.arn
