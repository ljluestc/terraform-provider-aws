...
// SPDX-License-Identifier: MPL-2.0package emr_testimport (
	...
	resourceName := "aws_emr_instance_fleet.task"	resource.ParallelTest(t, resource.TestCase{
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).EMRConn(ctx)output, err := tfemr.FindInstanceFleetByTwoPartKey(ctx, conn, rs.Primary.Attributes["cluster_id"], rs.Primary.ID)if err != nil {
	return err
}*v = *outputreturn nil
	}
}func testAccInstanceFleetResourceImportStateId
...
}data "aws_partition" "current" {}resource "aws_iam_role" "emr_service" {
  ...
  "Statement": [
{
 ...
 "Effect": "Allow",
 "Principal": {
duce.${data.aws_partition.current.dns_suffix}"
 },
...
instance_type_configs {
 bid_price_as_percentage_of_on_demand_price = 100 ebs_config {
...
resource "aws_emr_instance_fleet" "task" {
  cluster_id = aws_emr_cluster.test.id  instance_type_configs {
instance_type= "m3.xlarge"
weighted_capacity = 1
...
bid_price_as_percentage_of_on_demand_price = 80ebs_config {
 size  = 10
 type  = "gp2"
 volumes_per_instance = 1
