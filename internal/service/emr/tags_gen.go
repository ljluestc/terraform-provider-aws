...
)// []*SERVICE.Tag handling// Tags returns emr service tags.func Tags(tags tftags.KeyValueTags) []*emr.Tag {
	result := make([]*emr.Tag, 0, len(tags))	for k, v := range tags.Map() {
tag := &emr.Tag{
Key:aws.String(k),
Value: aws.String(v),
elt = append(result, tag)
	}	return result
}// KeyValueTags creates tftags.KeyValueTags from emr service tags.func KeyValueTags(ctx context.Context, tags []*emr.Tag) tftags.KeyValueTags {
...
// nil is returned if there are no input tags.func getTagsIn(ctx context.Context) []*emr.Tag {
	if inContext, ok := tftags.FromContext(ctx); ok {
 tags := Tags(inContext.TagsIn.UnwrapOrDefault()); len(tags) > 0 {
return tags	}	return nil
}// setTagsOut sets emr service tags in Context.func setTagsOut(ctx context.Context, tags []*emr.Tag) {
	if inContext, ok := tftags.FromContext(ctx); ok {
Context.TagsOut = types.Some(KeyValueTags(ctx, tags))
	}
}// updateTags updates emr service tags.
// The identifier is typically the Amazon Resource Name (ARN), although
// it may also be a different identifier depending on the service.func updateTags(ctx context.Context, conn emriface.EMRAPI, identifier string, oldTagsMap, newTagsMap any) error {
...
put := &emr.RemoveTagsInput{
ResourceId: aws.String(identifier),
TagKeys:aws.StringSlice(removedTags.Keys()),
,rr := conn.RemoveTagsWithContext(ctx, input)if e != nil {
...
put := &emr.AddTagsInput{
ResourceId: aws.String(identifier),
Tags:Tags(updatedTags),
,rr := conn.AddTagsWithContext(ctx, input)if e != nil {
