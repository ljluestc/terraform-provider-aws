...
}conn := acctest.Provider.Meta().(*conns.AWSClient).EventsConn(ctx)_, err := tfevents.FindEventBusPolicyByName(ctx, conn, rs.Primary.ID)return err
}
}
...
if !ok {
return fmt.Errorf("Not found: %s", n)
}conn := acctest.Provider.Meta().(*conns.AWSClient).EventsConn(ctx)policy, err := tfevents.FindEventBusPolicyByName(ctx, conn, rs.Primary.ID)if err != nil {
return err
...
resource "aws_cloudwatch_event_bus" "test" {
  ...
  event_bus_name = aws_cloudwatch_event_bus.test.name  policy = jsonencode({
 Statement = [{
Sid = %[1]q
Action = [
  "events:PutEvents",
