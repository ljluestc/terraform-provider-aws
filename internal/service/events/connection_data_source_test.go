...
func TestAccEventsConnectionDataSource_Connection_basic(t *testing.T) {
	...
	resourceName := "aws_cloudwatch_event_connection.api_key"	name := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	...
	value := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)	resource.ParallelTest(t, resource.TestCase{
		PreCheck: 
