...
"context"
"fmt"
"testing""github.com/aws/aws-sdk-go/service/eventbridge"
...
resourceName := "aws_cloudwatch_event_endpoint.test"resource.ParallelTest(t, resource.TestCase{
PreCheck: 
...
continue
}_, err := tfevents.FindEndpointByName(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).EventsConn(ctx)output, err := tfevents.FindEndpointByName(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*v = *outputreturn nil
}
}
...
data "aws_partition" "current" {}resource "aws_cloudwatch_event_bus" "primary" {
  name = %[1]q
...
identifiers = ["events.${data.aws_partition.current.dns_suffix}"]
  ...
  name  = %[1]q
  assume_role_policy = data.aws_iam_policy_document.test_assume.json  inline_policy {
 name= %[1]q
...
aws_cloudwatch_event_bus.secondary.arn,
 ...
 actions = ["iam:PassRole"] resources = ["arn:${data.aws_partition.current.partition}:iam::*:role/%[1]s"] condition {
test= "StringLike"
variable = "iam:PassedToService"values = ["events.${data.aws_partition.current.dns_suffix}"]
 }
  }
...
resource "aws_cloudwatch_event_endpoint" "test" {
 ...
 event_bus_arn = aws_cloudwatch_event_bus.primary.arn
  ...
  event_bus {
 event_bus_arn = aws_cloudwatch_event_bus.secondary.arn
  }  role_arn = aws_iam_role.test.arn  routing_config {
 failover_config {
primary {
  health_check = aws_route53_health_check.test.arn
}secondary {
