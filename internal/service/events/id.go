...
func PermissionCreateResourceID(eventBusName, statementID string) string {
	...
	id := strings.Join(parts, permissionResourceIDSeparator)	return id
}
func PermissionParseResourceID(id string) (string, string, error) {
	parts := strings.Split(id, permissionResourceIDSeparator)	if len(parts) == 1 && parts[0] != "" {
...
func RuleCreateResourceID(eventBusName, ruleName string) string {
	...
	id := strings.Join(parts, ruleResourceIDSeparator)	return id
}
func RuleParseResourceID(id string) (string, string, error) {
	parts := strings.Split(id, ruleResourceIDSeparator)	if len(parts) == 1 && parts[0] != "" {
		return DefaultEventBusName, parts[0], nil
	}
	if len(parts) == 2 && parts[0] != "" && parts[1] != "" {
		return parts[0], parts[1], nil
...
}// Terraform resource IDs for Targets are not parseable as the separator used ("-") is also a valid character in both the rule name and the target ID.const (
	targetResourceIDSeparator = "-"
	targetImportIDSeparator= "/"
)
func TargetCreateResourceID(eventBusName, ruleName, targetID string) string {
	var parts []string	if eventBusName == "" || eventBusName == DefaultEventBusName {
		parts = []string{ruleName, targetID}
	} else {
		parts = []string{eventBusName, ruleName, targetID}
	}	id := strings.Join(parts, targetResourceIDSeparator)	return id
}
func TargetParseImportID(id string) (string, string, string, error) {
	parts := strings.Split(id, targetImportIDSeparator)	if len(parts) == 2 && parts[0] != "" && parts[1] != "" {
