...
"testing"tfevents "github.com/hashicorp/terraform-provider-aws/internal/service/events"
)
...
func(t *testing.T) {
t.Parallel()gotPart0, gotPart1, err := tfevents.PermissionParseResourceID(testCase.InputID)if err == nil && testCase.ExpectedError {
...
t.Run(testCase.TestName, 
func(t *testing.T) {
t.Parallel()gotPart0, gotPart1, err := tfevents.RuleParseResourceID(testCase.InputID)if err == nil && testCase.ExpectedError {
t.Fatalf("expected error, got no error")
}if err != nil && !testCase.ExpectedError {
t.Fatalf("got unexpected error: %s", err)
}if gotPart0 != testCase.ExpectedPart0 {
...
t.Run(testCase.TestName, 
func(t *testing.T) {
t.Parallel()gotPart0, gotPart1, gotPart2, err := tfevents.TargetParseImportID(testCase.InputID)if err == nil && testCase.ExpectedError {
t.Fatalf("expected error, got no error")
}if err != nil && !testCase.ExpectedError {
t.Fatalf("got unexpected error: %s", err)
}if gotPart0 != testCase.ExpectedPart0 {
