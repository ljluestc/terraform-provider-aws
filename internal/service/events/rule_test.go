...
t.Run(name, 
func(t *testing.T) {
t.Parallel()got, err := tfevents.RuleEventPatternJSONDecoder(test.input)
if err != nil {
t.Fatal(err)
...
resourceName := "aws_cloudwatch_event_rule.test"resource.ParallelTest(t, resource.TestCase{
PreCheck: 
...
acctest.MatchResourceAttrRegionalARN(resourceName, "arn", "events", regexache.MustCompile(fmt.Sprintf(`rule/%s/%s$`, busName1, rName1))),
),
},
{
ResourceName:resourceName,
...
acctest.MatchResourceAttrRegionalARN(resourceName, "arn", "events", regexache.MustCompile(fmt.Sprintf(`rule/%s/%s$`, busName2, rName2))),
),
},
},
})
...
iamRoleResourceName := "aws_iam_role.test"resource.ParallelTest(t, resource.TestCase{
PreCheck: 
func() { acctest.PreCheck(ctx, t) },
ErrorCheck:  acctest.ErrorCheck(t, eventbridge.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName := "aws_cloudwatch_event_rule.test"
eventBusName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)resource.ParallelTest(t, resource.TestCase{
PreCheck: 
func() { acctest.PreCheck(ctx, t) },
ErrorCheck:  acctest.ErrorCheck(t, eventbridge.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
}eventBusName, ruleName, err := tfevents.RuleParseResourceID(rs.Primary.ID)if err != nil {
return err
}conn := acctest.Provider.Meta().(*conns.AWSClient).EventsConn(ctx)output, err := tfevents.FindRuleByTwoPartKey(ctx, conn, eventBusName, ruleName)if err != nil {
return err
}*v = *outputreturn nil
}
}
...
return err
}_, err = tfevents.FindRuleByTwoPartKey(ctx, conn, eventBusName, ruleName)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
resource "aws_cloudwatch_event_rule" "test" {
  name= %[1]q
  schedule_expression = "rate(1 hour)"  tags = {
 %[2]q = %[3]q
  }
}
...
resource "aws_cloudwatch_event_bus" "test" {
  ...
  name= %[1]q
  event_bus_name = aws_cloudwatch_event_bus.test.arn  event_pattern = <<PATTERN
{
  "source": ["aws.ec2"]
}
PATTERN
