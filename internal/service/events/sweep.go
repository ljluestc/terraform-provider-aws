...
// +build sweeppackage eventsimport (
	"encoding/json"
	"fmt"
...
func sweepAPIDestination(region string) error {
	...
	conn := client.EventsConn(ctx)	var sweeperErrs *multierror.Error	input := &eventbridge.ListApiDestinationsInput{
		Limit: aws.Int64(100),
	}
	var apiDestinations []*eventbridge.ApiDestination
	for {
		output, err := conn.ListApiDestinationsWithContext(ctx, input)		if sweep.SkipSweepError(err) {
		...
		}		apiDestinations = append(apiDestinations, output.ApiDestinations...)		if aws.StringValue(output.NextToken) == "" {
			break
		}
		input.NextToken = output.NextToken
	}	for _, apiDestination := range apiDestinations {		input := &eventbridge.DeleteApiDestinationInput{
	...
	}	log.Printf("[INFO] Deleted %d EventBridge Api Destinations", len(apiDestinations))	return sweeperErrs.ErrorOrNil()
}
func sweepArchives(region string) error {
	...
	conn := client.EventsConn(ctx)	input := &eventbridge.ListArchivesInput{}	for {
		output, err := conn.ListArchivesWithContext(ctx, input)		if sweep.SkipSweepError(err) {
...
func sweepBuses(region string) error {
	...
	sweepResources := make([]sweep.Sweepable, 0)	input := &eventbridge.ListEventBusesInput{}
	err = listEventBusesPages(ctx, conn, input, 
func(page *eventbridge.ListEventBusesOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
...
func sweepConnection(region string) error {
	...
	conn := client.EventsConn(ctx)	var sweeperErrs *multierror.Error	input := &eventbridge.ListConnectionsInput{
		Limit: aws.Int64(100),
	}
	var connections []*eventbridge.Connection
	for {
		output, err := conn.ListConnectionsWithContext(ctx, input)		if sweep.SkipSweepError(err) {
	...
	}	log.Printf("[INFO] Deleted %d EventBridge Connections", len(connections))	return sweeperErrs.ErrorOrNil()
}
func sweepPermissions(region string) error {
	...
	conn := client.EventsConn(ctx)	output, err := conn.DescribeEventBusWithContext(ctx, &eventbridge.DescribeEventBusInput{})
	if err != nil {
		if sweep.SkipSweepError(err) {
	...
	}	policy := aws.StringValue(output.Policy)	if policy == "" {
	...
	if err != nil {
		...
		sid := statement.Sid		log.Printf("[INFO] Deleting EventBridge Permission %s", sid)
...
func sweepRules(region string) error {
	...
	var sweeperErrs *multierror.Error	err = listEventBusesPages(ctx, conn, input, 
func(page *eventbridge.ListEventBusesOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
		}		for _, eventBus := range page.EventBuses {
			eventBusName := aws.StringValue(eventBus.Name)			input := &eventbridge.ListRulesInput{
				EventBusName: aws.String(eventBusName),
			}			err := listRulesPages(ctx, conn, input, 
func(page *eventbridge.ListRulesOutput, lastPage bool) bool {
				if page == nil {
					return !lastPage
				}				for _, rule := range page.Rules {
					ruleName := aws.StringValue(rule.Name)					log.Printf("[DEBUG] Deleting EventBridge Rule: %s/%s", eventBusName, ruleName)
...
func(page *eventbridge.ListRulesOutput, lastPage bool) bool {
				if page == nil {
					...
					ruleName := aws.StringValue(rule.Name)					input := &eventbridge.ListTargetsByRuleInput{
						EventBusName: aws.String(eventBusName),
						Rule:aws.String(ruleName),
					}					err := listTargetsByRulePages(ctx, conn, input, 
func(page *eventbridge.ListTargetsByRuleOutput, lastPage bool) bool {
						if page == nil {
							return !lastPage
						}						for _, target := range page.Targets {
							targetID := aws.StringValue(target.Id)							log.Printf("[DEBUG] Deleting EventBridge Target: %s/%s/%s", eventBusName, ruleName, targetID)
