...
return false
}re := regexache.MustCompile(`^event-bus/(` + validBusNameCharClass + `)$`)return re.MatchString(arn.Resource)
}var validNameChars = `\.\-_A-Za-z0-9`
var validNameCharClass = `[` + validNameChars + `]+`var validBusNameCharPattern = `/` + validNameChars
