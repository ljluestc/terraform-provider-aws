...
// SPDX-License-Identifier: MPL-2.0package eventsimport (
	"context"
	"time"	"github.com/aws/aws-sdk-go/service/eventbridge"
...
func waitConnectionCreated(ctx context.Context, conn *eventbridge.EventBridge, id string) (*eventbridge.DescribeConnectionOutput, error) {
	...
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if v, ok := outputRaw.(*eventbridge.DescribeConnectionOutput); ok {
		return v, err
	}	return nil, err
}
