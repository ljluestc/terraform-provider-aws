...
// SPDX-License-Identifier: MPL-2.0package evidentlyimport (
	...
	}	output, err := conn.GetFeatureWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, cloudwatchevidently.ErrCodeResourceNotFoundException) {
	...
	input := &cloudwatchevidently.GetLaunchInput{
		...
		Project: aws.String(projectNameOrARN),
	}	output, err := conn.GetLaunchWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, cloudwatchevidently.ErrCodeResourceNotFoundException) {
		return nil, &retry.NotFoundError{
			LastError:   err,
			LastRequest: input,
		}
	...
	}	output, err := conn.GetProjectWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, cloudwatchevidently.ErrCodeResourceNotFoundException) {
		return nil, &retry.NotFoundError{
			LastError:   err,
			LastRequest: input,
		}
	...
	}	output, err := conn.GetSegmentWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, cloudwatchevidently.ErrCodeResourceNotFoundException) {
		return nil, &retry.NotFoundError{
			LastError:   err,
			LastRequest: input,
		}
