...
var project cloudwatchevidently.ProjectrName := sdkacctest.RandomWithPrefix("resource-test-terraform")
originalDescription := "original description"
updatedDescription := "updated description"
resourceName := "aws_evidently_project.test"resource.ParallelTest(t, resource.TestCase{
PreCheck: func() {
...
ctx := acctest.Context(t)
var project cloudwatchevidently.ProjectrName := sdkacctest.RandomWithPrefix("tf-test-bucket")
...
prefix := "tests3prefix"resource.ParallelTest(t, resource.TestCase{
PreCheck: func() {
acctest.PreCheck(ctx, t)
acctest.PreCheckPartitionHasService(t, cloudwatchevidently.EndpointsID)
},
...
updatedPrefix := "updated-prefix"resource.ParallelTest(t, resource.TestCase{
PreCheck: func() {
acctest.PreCheck(ctx, t)
acctest.PreCheckPartitionHasService(t, cloudwatchevidently.EndpointsID)
},
...
continue
}_, err := tfcloudwatchevidently.FindProjectByNameOrARN(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).EvidentlyConn(ctx)output, err := tfcloudwatchevidently.FindProjectByNameOrARN(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*v = *outputreturn nil
}
