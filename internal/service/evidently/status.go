...
// SPDX-License-Identifier: MPL-2.0package evidentlyimport (
	...
	return func() (interface{}, string, error) {
		featureName, projectNameOrARN, err := FeatureParseID(id)		if err != nil {
			return nil, "", err
		}		output, err := FindFeatureWithProjectNameorARN(ctx, conn, featureName, projectNameOrARN)		if tfresource.NotFound(err) {
			return nil, "", nil
		}		if err != nil {
			return nil, "", err
	...
	return func() (interface{}, string, error) {
		launchName, projectNameOrARN, err := LaunchParseID(id)		if err != nil {
			return nil, "", err
		}		output, err := FindLaunchWithProjectNameorARN(ctx, conn, launchName, projectNameOrARN)		if tfresource.NotFound(err) {
			return nil, "", nil
		}		if err != nil {
			return nil, "", err
		}		return output, aws.StringValue(output.Status), nil
	...
	return func() (interface{}, string, error) {
		output, err := FindProjectByNameOrARN(ctx, conn, id)		if tfresource.NotFound(err) {
			return nil, "", nil
		}		if err != nil {
			return nil, "", err
		}		return output, aws.StringValue(output.Status), nil
