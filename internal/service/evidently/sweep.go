...
// +build sweeppackage evidentlyimport (
"fmt"
...
sweepResources := make([]sweep.Sweepable, 0)err = conn.ListProjectsPagesWithContext(ctx, input, func(page *cloudwatchevidently.ListProjectsOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, project := range page.Projects {
r := ResourceProject()
d := r.Data(nil)
d.SetId(aws.StringValue(project.Name))sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
