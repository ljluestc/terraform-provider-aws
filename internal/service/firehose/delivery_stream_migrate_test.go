	...
	t.Parallel()	cases := map[string]struct {
		...
		"v0.6.16 and earlier": {
			...
			Attributes: map[string]string{
				// EBS
				"role_arn":::somenumber:role/tf_acctest_4271506651559170635", //lintignore:AWSAT005
				"s3_bucket_arn":aws:s3:::tf-test-bucket", //line:AWSAT005
			...
			Expected: map[string]string{
				"s3_configuration.#":
				"s3_configuration.0.bucket_arn":n:aws:s3:::tf-test-bucket", //lintignore:AWSAT005
		...
		"v0.6.16 and earlier, sparse": {
			...
			Attributes: map[string]string{
				// EBS
				"role_arn":ws:iam::somenumber:role/tf_acctest_4271506651559170635", //lintignore:AWSAT005
				"s3_bucket_arn": "arn:aws:s3:::tf-test-bucket",tignore:AWSAT005
			},
			Expected: map[string]string{
				"s3_configuration.#":
				"s3_configuration.0.bucket_arn": "arn:aws:s3:::tf-test-bucket",tignore:AWSAT005
				"s3_configuration.0.role_arn":"arn:aws:iam::somenumber:role/tf_acctest_4271506651559170635", //lintignore:AWSAT005
			},
		},
...
func TestMigrateState_empty(t *testing.T) {
	t.Parallel()	var is *terraform.InstanceState
	var meta interface{}	// should handle nil
	is, err := tffirehose.MigrateState(0, is, meta)	if err != nil {
		t.Fatalf("err: %#v", err)
	}
	if is != nil {
	...
	is = &terraform.InstanceState{}
	_, err = resourceInstanceMigrateState(0, is)	if err != nil {
		t.Fatalf("err: %#v", err)
	}
}
func migrateInstanceStateV0toV1(is *terraform.InstanceState) (*terraform.InstanceState, error) {
	if is.Empty() || is.Attributes == nil {
		...
		return is, nil
	}	log.Printf("[DEBUG] Attributes before migration: %#v", is.Attributes)	// Delete old count
	delete(is.Attributes, "block_device.#")	oldBds, err := tfec2.ReadV0BlockDevices(is)
	if err != nil {
		return is, err
	}
