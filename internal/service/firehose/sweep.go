...
// +build sweeppackage firehoseimport (
	"fmt"
...
func sweepDeliveryStreams(region string) error {
	...
	sweepResources := make([]sweep.Sweepable, 0)	err = listDeliveryStreamsPages(ctx, conn, input, 
func(page *firehose.ListDeliveryStreamsOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
