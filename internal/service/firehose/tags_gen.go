...
func listTags(ctx context.Context, conn firehoseiface.FirehoseAPI, identifier string) (tftags.KeyValueTags, error) {
	...
	}	output, err := conn.ListTagsForDeliveryStreamWithContext(ctx, input)	if err != nil {
		return tftags.New(ctx, nil), err
	}	return KeyValueTags(ctx, output.Tags), nil
}// ListTags lists firehose service tags and set them in Context.
// It is called from outside this package.
func (p *servicePackage) ListTags(ctx context.Context, meta any, identifier string) error {
	tags, err := listTags(ctx, meta.(*conns.AWSClient).FirehoseConn(ctx), identifier)	if err != nil {
		return err
	}	if inContext, ok := tftags.FromContext(ctx); ok {
		inContext.TagsOut = types.Some(tags)
	}	return nil
}// []*SERVICE.Tag handling// Tags returns firehose service tags.
func Tags(tags tftags.KeyValueTags) []*firehose.Tag {
	result := make([]*firehose.Tag, 0, len(tags))	for k, v := range tags.Map() {
		tag := &firehose.Tag{
			Key:aws.String(k),
			Value: aws.String(v),
...
func updateTags(ctx context.Context, conn firehoseiface.FirehoseAPI, identifier string, oldTagsMap, newTagsMap any) error {
	...
	if len(removedTags) > 0 {
		input := &firehose.UntagDeliveryStreamInput{
			DeliveryStreamName: aws.String(identifier),
			TagKeys:ice(removedTags.Keys()),
		}		_, err := conn.UntagDeliveryStreamWithContext(ctx, input)		if err != nil {
	...
	if len(updatedTags) > 0 {
		...
		}		_, err := conn.TagDeliveryStreamWithContext(ctx, input)		if err != nil {
