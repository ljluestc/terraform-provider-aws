...
// SPDX-License-Identifier: MPL-2.0package fmsimport (
	"encoding/json"	"github.com/hashicorp/terraform-plugin-sdk/v2/helper/schema"
...
return old == new
	}	old, new = removeEmptyFieldsFromJSON(old), removeEmptyFieldsFromJSON(new)	return verify.JSONStringsEqual(old, new)
...
func removeEmptyFieldsFromJSON(in string) string {
	...
	lenBeforeArray := 0	err := ujson.Walk([]byte(in), func(_ int, key, value []byte) bool {
n := len(out)// For valid JSON, value will never be empty.
skip := false
switch value[0] {
case 'n': // Null (null)
skip = true
...
out = append(out, value...)return true
	})	if err != nil {
return ""
	}	return string(out)
}
