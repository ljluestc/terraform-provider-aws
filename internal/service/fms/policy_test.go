...
// SPDX-License-Identifier: MPL-2.0package fms_testimport (
	"context"
	"fmt"
	...
	resourceName := "aws_fms_policy.test"	resource.Test(t, resource.TestCase{
PreCheck: func() {
...
Config: testAccPolicyConfig_updated(rName2, rName1),
	...
	acctest.Skip(t, "PolicyOption not returned from AWS API")	ctx := acctest.Context(t)
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_fms_policy.test"	resource.Test(t, resource.TestCase{
PreCheck: func() {
acctest.PreCheck(ctx, t)
...
conn := acctest.Provider.Meta().(*conns.AWSClient).FMSConn(ctx)for _, rs := range s.RootModule().Resources {
if rs.Type != "aws_fms_policy" {
continue
}_, err := tffms.FindPolicyByID(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).FMSConn(ctx)_, err := tffms.FindPolicyByID(ctx, conn, rs.Primary.ID)return err
	}
...
resource "aws_fms_policy" "test" {
  ...
  resource_type_list= ["AWS::ElasticLoadBalancingV2::LoadBalancer"]  exclude_map {
...
type
managed_service_data = "{\"type\": \"WAF\", \"ruleGroups\": [{\"id\":\"${aws_wafregional_rule_group.test.id}\", \"overrideAction\" : {\"type\": \"COUNT\"}}],\"defaultAction\": {\"type\": \"BLOCK\"}, \"overrideCustomerWebACLAssociation\": false}"policy_option {
...
`, policyName, ruleGroupName))
	...
	return acctest.ConfigCompose(testAccAdminAccountConfig_basic, fmt.Sprintf(`
data "aws_partition" "current" {}resource "aws_fms_policy" "test" {
  exclude_resource_tags = false
  name= %[1]q
  remediation_enabled= false
  resource_type= "AWS::CloudFront::Distribution"  security_service_policy_data {
type
...
Statement = [{
 ...
 Effect = "Allow"
 Principal = {
ta.aws_partition.current.dns_suffix}"
 }
 Sid = ""
}]
  })
  name = %[1]q  inline_policy {
name = "test"
policy = jsonencode({
 Version = "2012-10-17"
 Statement = [{
...
resource "aws_fms_policy" "test" {
  ...
  name= %[1]q
  remediation_enabled= false
  resource_type_list= ["AWS::ElasticLoadBalancingV2::LoadBalancer"]  include_map {
account = [data.aws_caller_identity.current.account_id]
  }  security_service_policy_data {
type
managed_service_data = "{\"type\": \"WAF\", \"ruleGroups\": [{\"id\":\"${aws_wafregional_rule_group.test.id}\", \"overrideAction\" : {\"type\": \"COUNT\"}}],\"defaultAction\": {\"type\": \"BLOCK\"}, \"overrideCustomerWebACLAssociation\": false}"
...
resource "aws_fms_policy" "test" {
  ...
  name= %[1]q
  remediation_enabled= false
  resource_type_list= ["AWS::ElasticLoadBalancingV2::LoadBalancer"]  security_service_policy_data {
type
managed_service_data = "{\"type\": \"WAF\", \"ruleGroups\": [{\"id\":\"${aws_wafregional_rule_group.test.id}\", \"overrideAction\" : {\"type\": \"COUNT\"}}],\"defaultAction\": {\"type\": \"BLOCK\"}, \"overrideCustomerWebACLAssociation\": false}"
  }  resource_tags = {
%[2]q = %[3]q
...
"disable" = ""
  ...
  delete_all_policy_resources = false  security_service_policy_data {
type
...
description = ""
  ...
  exclude_resource_tags= false
  remediation_enabled= false
  resource_type = "AWS::EC2::SecurityGroup"  security_service_policy_data {
type = "SECURITY_GROUPS_CONTENT_AUDIT"managed_service_data = jsonencode({
 type = "SECURITY_GROUPS_CONTENT_AUDIT", securityGroupAction = {
