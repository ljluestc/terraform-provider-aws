...
t.Skip("skipping long-running test in short mode")
	...
	resourceName := "aws_fsx_file_cache.test"	resource.Test(t, resource.TestCase{
PreCheck: func() {
...
}// Per Attribute Acceptance Testsfunc testAccFileCache_copyTagsToDataRepositoryAssociations(t *testing.T) {
	ctx := acctest.Context(t)
	if testing.Short() {
t.Skip("skipping long-running test in short mode")
...
}// helper functionsfunc testAccCheckFileCacheDestroy(ctx context.Context) resource.TestCheckFunc {
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).FSxConn(ctx)resp, err := conn.DescribeFileCachesWithContext(ctx, &fsx.DescribeFileCachesInput{
	FileCacheIds: []*string{aws.String(rs.Primary.ID)},
})if err != nil {
	return create.Error(names.FSx, create.ErrActionCheckingExistence, tffsx.ResNameFileCache, rs.Primary.ID, err)
}*filecache = *respreturn nil
	}
...
}// Test Configurationsfunc testAccFileCacheConfig_basic() string {
	return testAccFileCacheBaseConfig() +
`
resource "aws_fsx_file_cache" "test" {
  file_cache_type= "LUSTRE"
  file_cache_type_version = "2.12"  lustre_configuration {
...
fmt.Sprintf(`
resource "aws_fsx_file_cache" "test" {
  copy_tags_to_data_repository_associations = true  data_repository_association {
data_repository_path  = "nfs://filer.domain.com/"
