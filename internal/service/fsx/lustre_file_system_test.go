...
func TestAccFSxLustreFileSystem_importedFileChunkSize(t *testing.T) {
	...
	return func(s *terraform.State) error {
		...
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).FSxConn(ctx)		output, err := tffsx.FindLustreFileSystemByID(ctx, conn, rs.Primary.ID)		if err != nil {
			return err
		}		*v = *output		return nil
	}
	...
	return func(s *terraform.State) error {
		conn := acctest.Provider.Meta().(*conns.AWSClient).FSxConn(ctx)		for _, rs := range s.RootModule().Resources {
			if rs.Type != "aws_fsx_lustre_file_system" {
				continue
			}			_, err := tffsx.FindLustreFileSystemByID(ctx, conn, rs.Primary.ID)			if tfresource.NotFound(err) {
				continue
			}			if err != nil {
				return err
...
resource "aws_s3_bucket" "test" {
  ...
  storage_capacity = 1200
  subnet_ids   = aws_subnet.test[*].id
  deployment_type  = data.aws_partition.current.partition == "aws-us-gov" ? "SCRATCH_2" : null # GovCloud does not support SCRATCH_1  tags = {
Name = %[1]q
  }
}
`, rName, exportPrefix))
...
resource "aws_fsx_lustre_file_system" "test" {
  storage_capacity = %[2]d
  subnet_ids   = aws_subnet.test[*].id
  deployment_type  = "SCRATCH_2"  tags = {
Name = %[1]q
  }
}
`, rName, storageCapacity))
...
resource "aws_fsx_lustre_file_system" "test" {
  ...
  automatic_backup_retention_days   = 1  tags = {
Name = %[1]q
  }
}
...
resource "aws_fsx_lustre_file_system" "test" {
  ...
  deployment_type  = "PERSISTENT_1"
  per_unit_storage_throughput = 50
  automatic_backup_retention_days = %[2]d  tags = {
Name = %[1]q
  }
}
`, rName, retention))
...
resource "aws_fsx_lustre_file_system" "test" {
  ...
  subnet_ids   = aws_subnet.test[*].id
  deployment_type= "PERSISTENT_1"
  per_unit_storage_throughput = %[2]d  tags = {
Name = %[1]q
  }
}
...
resource "aws_fsx_lustre_file_system" "base" {
  ...
  subnet_ids   = aws_subnet.test[*].id
  deployment_type= "PERSISTENT_1"
  per_unit_storage_throughput = 50  tags = {
Name = %[1]q
  }
}resource "aws_fsx_backup" "test" {
  file_system_id = aws_fsx_lustre_file_system.base.id  tags = {
Name = %[1]q
  }
}resource "aws_fsx_lustre_file_system" "test" {
  storage_capacity   = 1200
  ...
  deployment_type= "PERSISTENT_1"
  per_unit_storage_throughput = 50
  backup_id= aws_fsx_backup.test.id  tags = {
Name = %[1]q
  }
}
`, rName))
...
resource "aws_kms_key" "test1" {
  ...
  deployment_type= "PERSISTENT_1"
  per_unit_storage_throughput = 50
  kms_key_id   = aws_kms_key.test1.arn  tags = {
Name = %[1]q
  }
}
`, rName))
...
resource "aws_kms_key" "test2" {
  ...
  deployment_type= "PERSISTENT_1"
  per_unit_storage_throughput = 50
  kms_key_id   = aws_kms_key.test2.arn  tags = {
Name = %[1]q
  }
}
`, rName))
...
resource "aws_fsx_lustre_file_system" "test" {
  ...
  subnet_ids   = aws_subnet.test[*].id
  per_unit_storage_throughput = 50
  copy_tags_to_backups= true  tags = {
Name = %[1]q
  }
}
`, rName))
...
resource "aws_fsx_lustre_file_system" "test" {
  ...
  subnet_ids   = aws_subnet.test[*].id
  deployment_type   = data.aws_partition.current.partition == "aws-us-gov" ? "SCRATCH_2" : null # GovCloud does not support SCRATCH_1
  data_compression_type = "LZ4"  tags = {
Name = %[1]q
  }
}
`, rName))
...
resource aws_cloudwatch_log_group "test" {
  ...
  storage_capacity = 1200
  subnet_ids   = aws_subnet.test[*].id
  deployment_type  = data.aws_partition.current.partition == "aws-us-gov" ? "SCRATCH_2" : null # GovCloud does not support SCRATCH_1  log_configuration {
...
resource "aws_fsx_lustre_file_system" "test" {
  storage_capacity = 1200
  subnet_ids   = aws_subnet.test[*].id
  deployment_type  = data.aws_partition.current.partition == "aws-us-gov" ? "SCRATCH_2" : null # GovCloud does not support SCRATCH_1  root_squash_configuration {
root_squash = %[2]q
  }  tags = {
Name = %[1]q
  }
