...
// SPDX-License-Identifier: MPL-2.0package fsx_testimport (
	...
	return func(s *terraform.State) error {
		...
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).FSxConn(ctx)		filesystem, err := tffsx.FindFileSystemByID(ctx, conn, rs.Primary.ID)
		if err != nil {
			return err
		}		if filesystem == nil {
			return fmt.Errorf("FSx ONTAP File System (%s) not found", rs.Primary.ID)
		}		*fs = *filesystem		return nil
	}
...
resource "aws_fsx_ontap_file_system" "test" {
  ...
  throughput_capacity   = 128
  preferred_subnet_id   = aws_subnet.test1.id
  endpoint_ip_address_range = "198.19.255.0/24"  tags = {
Name = %[1]q
  }
}
`, rName))
...
resource "aws_fsx_ontap_file_system" "test" {
  ...
  deployment_type = "MULTI_AZ_1"
  throughput_capacity = 128
  preferred_subnet_id = aws_subnet.test1.id  disk_iops_configuration {
...
Name = %[1]q
  ...
  deployment_type = "MULTI_AZ_1"
  throughput_capacity = 128
  preferred_subnet_id = aws_subnet.test1.id  tags = {
Name = %[1]q
  }
}
`, rName))
