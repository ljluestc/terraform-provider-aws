...
// +build sweeppackage fsximport (
	...
	input := &fsx.DescribeBackupsInput{}	err = conn.DescribeBackupsPagesWithContext(ctx, input, func(page *fsx.DescribeBackupsOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
		}		for _, fs := range page.Backups {
			r := ResourceBackup()
			d := r.Data(nil)
			d.SetId(aws.StringValue(fs.BackupId))			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
	})	if err != nil {
	...
	sweepResources := make([]sweep.Sweepable, 0)
	var errs *multierror.Error
	input := &fsx.DescribeFileSystemsInput{}	err = conn.DescribeFileSystemsPagesWithContext(ctx, input, func(page *fsx.DescribeFileSystemsOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
			...
			d := r.Data(nil)
			d.SetId(aws.StringValue(fs.FileSystemId))			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
	})	if err != nil {
	...
	sweepResources := make([]sweep.Sweepable, 0)
	var errs *multierror.Error
	input := &fsx.DescribeStorageVirtualMachinesInput{}	err = conn.DescribeStorageVirtualMachinesPagesWithContext(ctx, input, func(page *fsx.DescribeStorageVirtualMachinesOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
		}		for _, vm := range page.StorageVirtualMachines {
			r := ResourceOntapStorageVirtualMachine()
			d := r.Data(nil)
			d.SetId(aws.StringValue(vm.StorageVirtualMachineId))			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
	})	if err != nil {
	...
	sweepResources := make([]sweep.Sweepable, 0)
	var errs *multierror.Error
	input := &fsx.DescribeVolumesInput{}	err = conn.DescribeVolumesPagesWithContext(ctx, input, func(page *fsx.DescribeVolumesOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
			...
			d.Set("skip_final_backup", true)			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
	})	if err != nil {
	...
	input := &fsx.DescribeVolumesInput{}	err = conn.DescribeVolumesPagesWithContext(ctx, input, func(page *fsx.DescribeVolumesOutput, lastPage bool) bool {
		if page == nil {
			...
			d := r.Data(nil)
			d.SetId(aws.StringValue(v.VolumeId))			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
	})	if err != nil {
