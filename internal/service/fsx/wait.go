...
// SPDX-License-Identifier: MPL-2.0package fsximport (
	...
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if output, ok := outputRaw.(*fsx.Backup); ok {
		return output, err
	}	return nil, err
	...
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if output, ok := outputRaw.(*fsx.FileCache); ok {
	...
	stateConf := &retry.StateChangeConf{
		...
		Timeout: timeout,
		Delay:   30 * time.Second,
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if output, ok := outputRaw.(*fsx.StorageVirtualMachine); ok {
	...
	stateConf := &retry.StateChangeConf{
		...
		Timeout: timeout,
		Delay:   30 * time.Second,
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if output, ok := outputRaw.(*fsx.Volume); ok {
	...
	stateConf := &retry.StateChangeConf{
		...
		Timeout: timeout,
		Delay:   30 * time.Second,
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if output, ok := outputRaw.(*fsx.Snapshot); ok {
		return output, err
	}	return nil, err
