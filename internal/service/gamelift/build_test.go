...
// SPDX-License-Identifier: MPL-2.0package gamelift_testimport (
	...
	resourceName := "aws_gamelift_build.test"	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	...
	roleArn := *loc.RoleArn
	key := *loc.Key	resource.ParallelTest(t, resource.TestCase{
		PreCheck: func() {
	...
	ctx := acctest.Context(t)
	var conf gamelift.Build	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_gamelift_build.test"	region := acctest.Region()
	g, err := testAccSampleGame(region)	if tfresource.NotFound(err) {
		t.Skip(err)
	}	if err != nil {
		t.Fatal(err)
	...
	return func(s *terraform.State) error {
		...
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).GameLiftConn(ctx)		build, err := tfgamelift.FindBuildByID(ctx, conn, rs.Primary.ID)
		if err != nil {
			return err
		}		if aws.StringValue(build.BuildId) != rs.Primary.ID {
			return fmt.Errorf("GameLift Build not found")
		}		*res = *build		return nil
	}
	...
	return func(s *terraform.State) error {
		conn := acctest.Provider.Meta().(*conns.AWSClient).GameLiftConn(ctx)		for _, rs := range s.RootModule().Resources {
			if rs.Type != "aws_gamelift_build" {
				continue
			}			build, err := tfgamelift.FindBuildByID(ctx, conn, rs.Primary.ID)			if tfresource.NotFound(err) {
				continue
			}			if build != nil {
	...
	conn := acctest.Provider.Meta().(*conns.AWSClient).GameLiftConn(ctx)	input := &gamelift.ListBuildsInput{}	_, err := conn.ListBuildsWithContext(ctx, input)	if acctest.PreCheckSkipError(err) {
