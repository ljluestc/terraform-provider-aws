...
// SPDX-License-Identifier: MPL-2.0package gameliftimport (
	...
	}	output, err := conn.DescribeBuildWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, gamelift.ErrCodeNotFoundException) {
	...
	}	output, err := conn.DescribeFleetAttributesWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, gamelift.ErrCodeNotFoundException) {
		return nil, &retry.NotFoundError{
			LastError:   err,
			LastRequest: input,
		}
	...
	}	fleet := output.FleetAttributes[0]	if aws.StringValue(fleet.FleetId) != id {
	...
	}	output, err := conn.DescribeScriptWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, gamelift.ErrCodeNotFoundException) {
		return nil, &retry.NotFoundError{
			LastError:   err,
			LastRequest: input,
		}
