...
// SPDX-License-Identifier: MPL-2.0package gamelift_testimport (
	...
	t.Parallel()	testCases := []struct {
		...
		a, r := tfgamelift.DiffPortSettings(tc.Old, tc.New)		authsString := fmt.Sprintf("%+v", a)
	...
	}	var conf gamelift.FleetAttributes	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	rNameUpdated := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)	region := acctest.Region()
	g, err := testAccSampleGame(region)	if tfresource.NotFound(err) {
		t.Skip(err)
	}	if err != nil {
		t.Fatal(err)
	}	loc := g.Location
	...
	roleArn := *loc.RoleArn
	key := *loc.Key	launchPath := g.LaunchPath
	params := g.Parameters(33435)
	resourceName := "aws_gamelift_fleet.test"	resource.ParallelTest(t, resource.TestCase{
		PreCheck: func() {
	...
	if testing.Short() {
		t.Skip("skipping long-running test in short mode")
	}	var conf gamelift.FleetAttributes	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)	region := acctest.Region()
	g, err := testAccSampleGame(region)	if tfresource.NotFound(err) {
		t.Skip(err)
	}	if err != nil {
		t.Fatal(err)
	...
	params := g.Parameters(33435)	resourceName := "aws_gamelift_fleet.test"	resource.ParallelTest(t, resource.TestCase{
		PreCheck: func() {
			acctest.PreCheck(ctx, t)
			acctest.PreCheckPartitionHasService(t, gamelift.EndpointsID)
			testAccPreCheck(ctx, t)
	...
	rNameUpdated := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)	desc := fmt.Sprintf("Terraform Acceptance Test %s", sdkacctest.RandString(8))	region := acctest.Region()
	g, err := testAccSampleGame(region)	if tfresource.NotFound(err) {
		t.Skip(err)
	}	if err != nil {
		t.Fatal(err)
	...
	if testing.Short() {
		t.Skip("skipping long-running test in short mode")
	}	var conf gamelift.FleetAttributes	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)	resourceName := "aws_gamelift_fleet.test"	resource.ParallelTest(t, resource.TestCase{
		PreCheck: func() {
			acctest.PreCheck(ctx, t)
			acctest.PreCheckPartitionHasService(t, gamelift.EndpointsID)
			testAccPreCheck(ctx, t)
	...
	return func(s *terraform.State) error {
		...
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).GameLiftConn(ctx)		fleet, err := tfgamelift.FindFleetByID(ctx, conn, rs.Primary.ID)
		if err != nil {
			return err
		}		if aws.StringValue(fleet.FleetId) != rs.Primary.ID {
			return fmt.Errorf("GameLift Fleet not found")
		}		*res = *fleet		return nil
	}
	...
	return func(s *terraform.State) error {
		conn := acctest.Provider.Meta().(*conns.AWSClient).GameLiftConn(ctx)		for _, rs := range s.RootModule().Resources {
			if rs.Type != "aws_gamelift_fleet" {
				continue
			}			_, err := tfgamelift.FindFleetByID(ctx, conn, rs.Primary.ID)			if tfresource.NotFound(err) {
				continue
			}			return nil
		}		return nil
	}
...
resource "aws_gamelift_fleet" "test" {
  ...
  name = %[1]q  runtime_configuration {
server_process {
...
resource "aws_gamelift_fleet" "test" {
  ...
  new_game_session_protection_policy = "FullProtection"  resource_creation_limit_policy {
...
resource "aws_gamelift_fleet" "test" {
  ...
  fleet_type= "ON_DEMAND"  ec2_inbound_permission {
from_port = 8080
ip_range  = "8.8.8.8/32"
protocol  = "TCP"
to_port   = 8080
...
max_concurrent_game_session_activations = 99server_process {
  concurrent_executions = 1
  launch_path  = %q
  parameters   = "%s"
}
...
resource "aws_gamelift_fleet" "test" {
  build_id = aws_gamelift_build.test.id
  ec2_instance_type = "c4.large"  name = "%s"
  description   = "%s"
  instance_role_arn = aws_iam_role.test.arn
  fleet_type= "ON_DEMAND"  ec2_inbound_permission {
from_port = 8888
...
game_session_activation_timeout_seconds = 35
max_concurrent_game_session_activations = 98server_process {
  concurrent_executions = 1
  launch_path  = %q
  parameters   = "%s"
}
...
resource "aws_gamelift_build" "test" {
  name= %[1]q
  operating_system = "WINDOWS_2012"  storage_location {
bucket   = %[2]q
key  = %[3]q
role_arn = %[4]q
  }
...
EOF
  ...
  name= %[1]q
  path= "/"
  description = "GameLift Fleet PassRole Policy"  policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
{
...
resource "aws_gamelift_fleet" "test" {
  build_id = aws_gamelift_build.test.id
  ec2_instance_type = "c4.large"
  name = %[1]q  certificate_configuration {
