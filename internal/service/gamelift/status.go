...
// SPDX-License-Identifier: MPL-2.0package gameliftimport (
	...
	return func() (interface{}, string, error) {
		output, err := FindBuildByID(ctx, conn, id)		if tfresource.NotFound(err) {
			return nil, "", nil
		}		if err != nil {
			return nil, "", err
	...
	return func() (interface{}, string, error) {
		output, err := FindFleetByID(ctx, conn, id)		if tfresource.NotFound(err) {
			return nil, "", nil
		}		if err != nil {
			return nil, "", err
		}		return output, aws.StringValue(output.Status), nil
	...
	return func() (interface{}, string, error) {
		output, err := FindGameServerGroupByName(ctx, conn, name)		if tfresource.NotFound(err) {
			return nil, "", nil
		}		if err != nil {
			return nil, "", err
		}		return output, aws.StringValue(output.Status), nil
