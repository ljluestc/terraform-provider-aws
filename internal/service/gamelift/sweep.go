...
// +build sweeppackage gameliftimport (
	"context"
	"fmt"
	...
	conn := client.GameLiftConn(ctx)	err = listAliases(ctx, &gamelift.ListAliasesInput{}, conn, func(resp *gamelift.ListAliasesOutput) error {
		if len(resp.Aliases) == 0 {
			log.Print("[DEBUG] No GameLift Aliases to sweep")
			return nil
		}		log.Printf("[INFO] Found %d GameLift Aliases", len(resp.Aliases))		for _, alias := range resp.Aliases {
	...
	conn := client.GameLiftConn(ctx)	resp, err := conn.ListBuildsWithContext(ctx, &gamelift.ListBuildsInput{})
	if err != nil {
		if sweep.SkipSweepError(err) {
		...
		return nil
	}	log.Printf("[INFO] Found %d GameLift Builds", len(resp.Builds))	for _, build := range resp.Builds {
	...
	conn := client.GameLiftConn(ctx)	resp, err := conn.ListScriptsWithContext(ctx, &gamelift.ListScriptsInput{})
	if err != nil {
		if sweep.SkipSweepError(err) {
		...
		return nil
	}	log.Printf("[INFO] Found %d GameLift Scripts", len(resp.Scripts))	for _, build := range resp.Scripts {
	...
	var errs *multierror.Error	input := &gamelift.ListFleetsInput{}	for {
		output, err := conn.ListFleetsWithContext(ctx, input)		for _, fleet := range output.FleetIds {
			r := ResourceFleet()
			d := r.Data(nil)			id := aws.StringValue(fleet)
			d.SetId(id)			if err != nil {
	...
	conn := client.GameLiftConn(ctx)
	sweepResources := make([]sweep.Sweepable, 0)
	var errs *multierror.Error	input := &gamelift.ListGameServerGroupsInput{}	for {
		output, err := conn.ListGameServerGroupsWithContext(ctx, input)		for _, gameServerGroup := range output.GameServerGroups {
			r := ResourceGameServerGroup()
			d := r.Data(nil)			id := aws.StringValue(gameServerGroup.GameServerGroupName)
			d.SetId(id)			if err != nil {
	...
	conn := client.GameLiftConn(ctx)	out, err := conn.DescribeGameSessionQueuesWithContext(ctx, &gamelift.DescribeGameSessionQueuesInput{})	if sweep.SkipSweepError(err) {
		...
		return nil
	}	log.Printf("[INFO] Found %d GameLift Session Queue", len(out.GameSessionQueues))	for _, queue := range out.GameSessionQueues {
