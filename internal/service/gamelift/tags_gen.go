...
func (p *servicePackage) ListTags(ctx context.Context, meta any, identifier string) error {
	tags, err := listTags(ctx, meta.(*conns.AWSClient).GameLiftConn(ctx), identifier)	if err != nil {
		return err
	}	if inContext, ok := tftags.FromContext(ctx); ok {
		inContext.TagsOut = types.Some(tags)
	}	return nil
}// []*SERVICE.Tag handling// Tags returns gamelift service tags.
func Tags(tags tftags.KeyValueTags) []*gamelift.Tag {
	result := make([]*gamelift.Tag, 0, len(tags))	for k, v := range tags.Map() {
		tag := &gamelift.Tag{
			Key:   aws.String(k),
			Value: aws.String(v),
