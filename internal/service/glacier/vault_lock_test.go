...
// SPDX-License-Identifier: MPL-2.0package glacier_testimport (
	...
	resourceName := "aws_glacier_vault_lock.test"	resource.ParallelTest(t, resource.TestCase{
...
}func testAccCheckVaultLockExists(ctx context.Context, n string, v *glacier.GetVaultLockOutput) resource.TestCheck
func {
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).GlacierClient(ctx)output, err := tfglacier.FindVaultLockByName(ctx, conn, rs.Primary.ID)if err != nil {
	return err
}*v = *outputreturn nil
	}
}func testAccCheckVaultLockDestroy(ctx context.Context) resource.TestCheck
func {
	returnfunc(s *terraform.State) error {
...
continue
	}	_, err := tfglacier.FindVaultLockByName(ctx, conn, rs.Primary.ID)	if tfresource.NotFound(err) {
continue
	}	if err != nil {
return err
