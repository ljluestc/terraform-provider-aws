...
eckDestroy:testAccCheckPartitionDestroy(ctx),
...
nn := acctest.Provider.Meta().(*conns.AWSClient).GlueConn(ctx)fo_, rs := range s.RootModule().Resources {
...
return fmt.Errorf("still exists")turn nil
	}
...
t, err := tfglue.FindPartitionByValues(ctx, conn, rs.Primary.ID)
 err != nil {
return err
fut == nil {
...
stored_as_sub_directories = falseparameters = {
 param1 = "param1_val"
}columns {
 name= "my_column_1"
 type= "int"
