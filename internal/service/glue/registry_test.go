...
// SPDX-License-Identifier: MPL-2.0package glue_testimport (
	...
	var registry glue.GetRegistryOutput	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_glue_registry.test"	resource.ParallelTest(t, resource.TestCase{
...
ExpectNonEmptyPlan: true,
	...
	conn := acctest.Provider.Meta().(*conns.AWSClient).GlueConn(ctx)	_, err := conn.ListRegistriesWithContext(ctx, &glue.ListRegistriesInput{})	// Some endpoints that do not support Glue Registrys return InternalFailure
