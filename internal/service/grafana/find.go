...
// SPDX-License-Identifier: MPL-2.0package grafanaimport (
	...
	output, err := FindWorkspaceByID(ctx, conn, id)	if err != nil {
return nil, err
...
WorkspaceId: aws.String(id),
	}	output, err := conn.DescribeWorkspaceWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, managedgrafana.ErrCodeResourceNotFoundException) {
return nil, &retry.NotFoundError{
LastError:err,
LastRequest: input,
}
...
WorkspaceId: aws.String(id),
	}	output, err := conn.DescribeWorkspaceAuthenticationWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, managedgrafana.ErrCodeResourceNotFoundException) {
return nil, &retry.NotFoundError{
LastError:err,
LastRequest: input,
}
...
WorkspaceId: aws.String(workspaceID),
	...
	output := make(map[string][]string, 0)	err := conn.ListPermissionsPagesWithContext(ctx, input, func(page *managedgrafana.ListPermissionsOutput, lastPage bool) bool {
if page == nil {
return !lastPage
