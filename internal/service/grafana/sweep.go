...
// +build sweeppackage grafanaimport (
	"fmt"
...
return fmt.Errorf("error getting client: %s", err)
	...
	conn := client.GrafanaConn(ctx)	sweepResources := make([]sweep.Sweepable, 0)
	var errs *multierror.Error	input := &managedgrafana.ListWorkspacesInput{}	err = conn.ListWorkspacesPagesWithContext(ctx, input, func(page *managedgrafana.ListWorkspacesOutput, lastPage bool) bool {
if len(page.Workspaces) == 0 {
log.Printf("[INFO] No Grafana Workspaces to sweep")
return false
}
for _, workspace := range page.Workspaces {id := aws.StringValue(workspace.Id)
