...
// SPDX-License-Identifier: MPL-2.0package grafanaimport (
	"context"
	"time"	"github.com/aws/aws-sdk-go/service/managedgrafana"
...
Timeout: timeout,
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if output, ok := outputRaw.(*managedgrafana.WorkspaceDescription); ok {
return output, err
	}	return nil, err
...
Timeout: timeout,
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if output, ok := outputRaw.(*managedgrafana.SamlAuthentication); ok {
return output, err
	}	return nil, err
}
