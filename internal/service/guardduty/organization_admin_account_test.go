...
"context"
"fmt"
"testing""github.com/aws/aws-sdk-go/service/guardduty"
...
resourceName := "aws_guardduty_organization_admin_account.test"resource.Test(t, resource.TestCase{
PreCheck: func() {
...
CheckDestroy:izationAdminAccountDestroy(ctx),
...
continue
}adminAccount, err := tfguardduty.GetOrganizationAdminAccount(ctx, conn, rs.Primary.ID)if tfawserr.ErrMessageContains(err, guardduty.ErrCodeBadRequestException, "organization is not in use") {
continue
}if err != nil {
return err
}if adminAccount == nil {
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).GuardDutyConn(ctx)adminAccount, err := tfguardduty.GetOrganizationAdminAccount(ctx, conn, rs.Primary.ID)if err != nil {
return err
}if adminAccount == nil {
...
return `
data "aws_caller_identity" "current" {}data "aws_partition" "current" {}resource "aws_organizations_organization" "test" {
  aws_service_access_principals = ["guardduty.${data.aws_partition.current.dns_suffix}"]
  feature_set = "ALL"
}resource "aws_guardduty_detector" "test" {}resource "aws_guardduty_organization_admin_account" "test" {
  depends_on = [aws_organizations_organization.test, aws_guardduty_detector.test]  admin_account_id = data.aws_caller_identity.current.account_id
}
`
}
