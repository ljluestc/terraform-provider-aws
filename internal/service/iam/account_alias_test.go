...
)funcarallel()testCases := map[string]map[string]func(t *testing.T){
"DataSource": {
...
funcourceName := "aws_iam_account_alias.test"rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)resource.Test(t, resource.TestCase{
...
continue
}params := &iam.ListAccountAliasesInput{}resp, err := conn.ListAccountAliasesWithContext(ctx, params)if err != nil {
