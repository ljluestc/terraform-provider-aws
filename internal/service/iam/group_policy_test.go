...
func out iam.GetGroupPolicyOutput
rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)resource.ParallelTest(t, resource.TestCase{
PreCheck:func() { acctest.PreCheck(ctx, t) },
ErrorCheck:  acctest.ErrorCheck(t, iam.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
// during that step.func TestAccIAMGroupPolicy_unknownsInPolicy(t *testing.T) {
ctx := acctest.Context(t)
var gp iam.GetGroupPolicyOutput
rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName := "aws_iam_group_policy.test"
groupName := "aws_iam_group.test"resource.ParallelTest(t, resource.TestCase{
PreCheck:func() { acctest.PreCheck(ctx, t) },
ErrorCheck:  acctest.ErrorCheck(t, iam.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
conn := acctest.Provider.Meta().(*conns.AWSClient).IAMConn(ctx)funcf rs.Type != "aws_iam_group_policy" {
continue
...
return func(s *terraform.State) error {
conn := acctest.Provider.Meta().(*conns.AWSClient).IAMConn(ctx)params := &iam.DeleteGroupPolicyInput{
funcroupName:  out.GroupName,
...
funceturn fmt.Errorf("Not Found: %s", iamGroupResource)
}if rs.Primary.ID == "" {
...
funcc testAccCheckGroupPolicyNameMatches(i, j *iam.GetGroupPolicyOutput) resource.TestCheckFunc {
return func(s *terraform.State) error {
if aws.StringValue(i.PolicyName) != aws.StringValue(j.PolicyName) {
...
resource "aws_iam_group" "test" {
  ...
  group = aws_iam_group.test.name  policy = <<EOF
{functatement": {
"Effect": "Allow",
"Action": %[2]q,
"Resource": "*"
...
resource "aws_iam_group" "group" {
  ...
  name  = %[1]q
  group = aws_iam_group.group.name  policy = <<EOF
...
return fmt.Sprintf(`
data "aws_caller_identity" "current" {}data "aws_partition" "current" {}resource "aws_iam_group" "test" {
  name = %[1]q
  path = "/"
...
resource "aws_iam_group_policy" "test" {
  name  = %[1]q
  group = aws_iam_group.test.id  policy = jsonencode({
Version = "2012-10-17"
Statement = [{
 Sid= ""
 Effect = "Allow"
