...
resourceName := "aws_iam_openid_connect_provider.test"resource.ParallelTest(t, resource.TestCase{
...
funcring := sdkacctest.RandString(5)
resourceName := "aws_iam_openid_connect_provider.test"resource.ParallelTest(t, resource.TestCase{
PreCheck:func() { acctest.PreCheck(ctx, t) },
ErrorCheck:  acctest.ErrorCheck(t, iam.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
funcourceName := "aws_iam_openid_connect_provider.test"resource.ParallelTest(t, resource.TestCase{
PreCheck:func() { acctest.PreCheck(ctx, t) },
ErrorCheck:  acctest.ErrorCheck(t, iam.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckOpenIDConnectProviderDestroy(ctx),
...
conn := acctest.Provider.Meta().(*conns.AWSClient).IAMConn(ctx)funcf rs.Type != "aws_iam_openid_connect_provider" {
continue
}_, err := tfiam.FindOpenIDConnectProviderByARN(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).IAMConn(ctx)_, err := tfiam.FindOpenIDConnectProviderByARN(ctx, conn, rs.Primary.ID)return err
}
}func testAccOpenIDConnectProviderConfig_basic(rName string) string {
return fmt.Sprintf(`
resource "aws_iam_openid_connect_provider" "test" {
  url = "https://accounts.testle.com/%[1]s"  client_id_list = [
 func  thumbprint_list = ["cf23df2207d99a74fbe169e3eba035e633b65d94"]
}
`, rName)
...
funcumbprint_list = ["cf23df2207d99a74fbe169e3eba035e633b65d94"]  tags = {
%[2]q = %[3]q
  }
}
