...
t.Parallel()role, session := tfiam.RoleNameSessionFromARN(testCase.ARN)if testCase.ExpectedRoleName != role || testCase.ExpectedSessionName != session {
...
ctx := acctest.Context(t)
rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
dataSourceName := "data.aws_iam_session_context.test"funcheck:  func() { acctest.PreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, iam.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
Steps: []resource.TestStep{
{
...
data "aws_caller_identity" "current" {}data "aws_iam_session_context" "test" {
