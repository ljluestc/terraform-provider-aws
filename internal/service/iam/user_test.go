...
var conf iam.Username1 := fmt.Sprintf("test-user-%d", sdkacctest.RandInt())
...
func user iam.UserrName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName := "aws_iam_user.user"resource.ParallelTest(t, resource.TestCase{
PreCheck:  func() { acctest.PreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, iam.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
ctx := acctest.Context(t)
var user iam.UserfuncourceName := "aws_iam_user.test"resource.ParallelTest(t, resource.TestCase{
PreCheck:  func() { acctest.PreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, iam.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckUserDestroy(ctx),
...
ctx := acctest.Context(t)
var user iam.UserrName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
func
...
ctx := acctest.Context(t)
var conf iam.Username := fmt.Sprintf("test-user-%d", sdkacctest.RandInt())
path1 := "/"
path2 := "/updated/"
...
return func(s *terraform.State) error {
cfuncword, err := tfiam.GeneratePassword(32)
if err != nil {
return err
}
