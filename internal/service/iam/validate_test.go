...
_, errors := validOpenIDURL(tc.Value, "url")if len(errors) != tc.ErrCount {
...
_, errors := validRolePolicyRole(tc.Value, "role")if len(errors) != tc.ErrCount {
