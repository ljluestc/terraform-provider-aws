...
propagationTimeout = 2 * time.MinuteRoleStatusARNIsUniqueID = "uniqueid"
...
}outputRaw, err := stateConf.WaitForStateContext(ctx)if output, ok := outputRaw.(*iam.Role); ok {
return output, err
}return nil, err
}func statusRoleCreate(ctx context.Context, conn *iam.IAM, id string) retry.StateRefreshFunc {
funcle, err := FindRoleByName(ctx, conn, id)if tfresource.NotFound(err) {
