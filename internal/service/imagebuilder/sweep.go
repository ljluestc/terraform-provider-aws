...
// +build sweeppackage imagebuilderimport (
	"fmt"
...
return fmt.Errorf("error getting client: %s", err)
	...
	conn := client.ImageBuilderConn(ctx)	sweepResources := make([]sweep.Sweepable, 0)
	var sweeperErrs *multierror.Error	input := &imagebuilder.ListComponentsInput{
...
continue
}arn := aws.StringValue(componentSummary.Arn)r := ResourceComponent()
d := r.Data(nil)
d.SetId(arn)sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
})if err != nil {
...
return fmt.Errorf("error getting client: %s", err)
	...
	conn := client.ImageBuilderConn(ctx)	sweepResources := make([]sweep.Sweepable, 0)
	var sweeperErrs *multierror.Error	input := &imagebuilder.ListDistributionConfigurationsInput{}
...
continue
}arn := aws.StringValue(distributionConfigurationSummary.Arn)r := ResourceDistributionConfiguration()
d := r.Data(nil)
d.SetId(arn)sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
	})	if sweep.SkipSweepError(err) {
...
return fmt.Errorf("error getting client: %w", err)
	...
	conn := client.ImageBuilderConn(ctx)	sweepResources := make([]sweep.Sweepable, 0)
	var sweeperErrs *multierror.Error	input := &imagebuilder.ListImagePipelinesInput{}
...
continue
}arn := aws.StringValue(imagePipeline.Arn)r := ResourceImagePipeline()
d := r.Data(nil)
d.SetId(arn)sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
	})	if sweep.SkipSweepError(err) {
...
return fmt.Errorf("error getting client: %w", err)
	...
	conn := client.ImageBuilderConn(ctx)	sweepResources := make([]sweep.Sweepable, 0)
	var sweeperErrs *multierror.Error	input := &imagebuilder.ListImageRecipesInput{
Owner: aws.String(imagebuilder.OwnershipSelf),
	}
...
continue
}arn := aws.StringValue(imageRecipeSummary.Arn)r := ResourceImageRecipe()
d := r.Data(nil)
d.SetId(arn)sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
	})	if sweep.SkipSweepError(err) {
...
return fmt.Errorf("error getting client: %w", err)
	...
	conn := client.ImageBuilderConn(ctx)	sweepResources := make([]sweep.Sweepable, 0)
	var sweeperErrs *multierror.Error	input := &imagebuilder.ListContainerRecipesInput{
Owner: aws.String(imagebuilder.OwnershipSelf),
	}
...
continue
}arn := aws.StringValue(containerRecipeSummary.Arn)r := ResourceContainerRecipe()
d := r.Data(nil)
d.SetId(arn)sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
	})	if sweep.SkipSweepError(err) {
...
return fmt.Errorf("error getting client: %w", err)
	...
	var errs *multierror.Error	input := &imagebuilder.ListImagesInput{
Owner: aws.String(imagebuilder.OwnershipSelf),
...
imageVersionArn := aws.StringValue(imageVersion.Arn)input := &imagebuilder.ListImageBuildVersionsInput{
...
if imageSummary == nil {
continue
}imageBuildVersionArn := aws.StringValue(imageSummary.Arn)r := ResourceImage()
d := r.Data(nil)
d.SetId(imageBuildVersionArn)sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
})if err != nil {
...
return fmt.Errorf("error getting client: %s", err)
	...
	conn := client.ImageBuilderConn(ctx)	sweepResources := make([]sweep.Sweepable, 0)
	var sweeperErrs *multierror.Error	input := &imagebuilder.ListInfrastructureConfigurationsInput{}	err = conn.ListInfrastructureConfigurationsPagesWithContext(ctx, input, func(page *imagebuilder.ListInfrastructureConfigurationsOutput, lastPage bool) bool {
if page == nil {
return !lastPage
...
continue
}arn := aws.StringValue(infrastructureConfigurationSummary.Arn)r := ResourceInfrastructureConfiguration()
d := r.Data(nil)
d.SetId(arn)sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
	})	if sweep.SkipSweepError(err) {
