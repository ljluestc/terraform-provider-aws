...
// SPDX-License-Identifier: MPL-2.0package inspector_testimport (
	...
	resourceName := "aws_inspector_assessment_target.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck:k(ctx, t) },
rorCheck:acctest.ErrorCheck(t, inspector.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckTargetAssessmentDestroy(ctx),
...
nn := acctest.Provider.Meta().(*conns.AWSClient).InspectorConn(ctx)fo_, rs := range s.RootModule().Resources {
if rs.Type != "aws_inspector_assessment_target" {
continue
}assessmentTarget, err := tfinspector.DescribeAssessmentTarget(ctx, conn, rs.Primary.ID)if err != nil {
...
o := acctest.Provider.Meta().(*conns.AWSClient).InspectorConn(ctx)assementTarget, err := tfinspector.DescribeAssessmentTarget(ctx, conn, rs.Primary.ID)if err= nil {
return fmt.Errorf("finding Inspector Classic Assessment Target: %s", err)
fssessmentTarget == nil {
return fmt.Errorf("Inspector Classic Assessment Target (%s) not found", rs.Primary.ID)
tget = *assessmentTargetretu nil
	}
}func testAccCheckTargetDisappears(ctx context.Context, assessmentTarget *inspector.AssessmentTarget) resource.TestCheckFunc {
	return func(s *terraform.State) error {
nn := acctest.Provider.Meta().(*conns.AWSClient).InspectorConn(ctx)int := &inspector.DeleteAssessmentTargetInput{
AssessmentTargetArn: assessmentTarget.Arn,
,rr := conn.DeleteAssessmentTargetWithContext(ctx, input)retu err
	}
