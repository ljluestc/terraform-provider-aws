...
// SPDX-License-Identifier: MPL-2.0package inspector_testimport (
	...
	return func(s *terraform.State) error {
		...
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).InspectorConn(ctx)		assessmentTarget, err := tfinspector.DescribeAssessmentTarget(ctx, conn, rs.Primary.ID)		if err != nil {
			return fmt.Errorf("finding Inspector Classic Assessment Target: %s", err)
		}		if assessmentTarget == nil {
			return fmt.Errorf("Inspector Classic Assessment Target (%s) not found", rs.Primary.ID)
		}		*target = *assessmentTarget		return nil
	}
}func testAccCheckTargetDisappears(ctx context.Context, assessmentTarget *inspector.AssessmentTarget) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		conn := acctest.Provider.Meta().(*conns.AWSClient).InspectorConn(ctx)		input := &inspector.DeleteAssessmentTargetInput{
			AssessmentTargetArn: assessmentTarget.Arn,
		}		_, err := conn.DeleteAssessmentTargetWithContext(ctx, input)		return err
	}
