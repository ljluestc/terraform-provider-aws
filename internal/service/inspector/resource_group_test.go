...
// SPDX-License-Identifier: MPL-2.0package inspector_testimport (
	...
	resourceName := "aws_inspector_resource_group.test"	resource.ParallelTest(t, resource.TestCase{
...
conn := acctest.Provider.Meta().(*conns.AWSClient).InspectorConn(ctx)rs, ok := s.RootModule().Resources[name]
if !ok {
...
}*rg = *output.ResourceGroups[0]return nil
	}
