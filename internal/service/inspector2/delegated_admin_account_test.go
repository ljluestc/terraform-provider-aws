...
// SPDX-License-Identifier: MPL-2.0package inspector2_testimport (
	...
	resourceName := "aws_inspector2_delegated_admin_account.test"	resource.Test(t, resource.TestCase{
PreCheck: func() {
...
eckDestroy:testAccCheckDelegatedAdminAccountDestroy(ctx),
...
}rs := s[idx]if rs.Attributes["relationship_status"] != string(types.RelationshipStatusEnabled) {
...
continue
}st, _, err := tfinspector2.FindDelegatedAdminAccountStatusID(ctx, conn, rs.Primary.ID)if st == "" && errs.Contains(err, "admin account not found") {
return nil
}if err != nil {
return err
...
o := acctest.Provider.Meta().(*conns.AWSClient).Inspector2Client(ctx)_, _err := tfinspector2.FindDelegatedAdminAccountStatusID(ctx, conn, rs.Primary.ID)if err= nil {
...
ern nil
	...
	return `
data "aws_caller_identity" "current" {}resource "aws_inspector2_delegated_admin_account" "test" {
