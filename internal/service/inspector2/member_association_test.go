...
resourceName := "aws_inspector2_member_association.test"resource.Test(t, resource.TestCase{
PreCheck: func() {
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).Inspector2Client(ctx)_, err := tfinspector2.FindMemberByAccountID(ctx, conn, rs.Primary.ID)return err
}
}func testAccCheckMemberAssociationDestroy(ctx context.Context) resource.TestCheckFunc {
return func(s *terraform.State) error {
conn := acctest.Provider.Meta().(*conns.AWSClient).Inspector2Client(ctx)for _, rs := range s.RootModule().Resources {
if rs.Type != "aws_inspector2_member_association" {
continue
}_, err := tfinspector2.FindMemberByAccountID(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
