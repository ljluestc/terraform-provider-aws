...
resourceName := "aws_iot_authorizer.test"resource.ParallelTest(t, resource.TestCase{
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).IoTConn(ctx)output, err := tfiot.FindAuthorizerByName(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*v = *outputreturn nil
}
...
continue
}_, err := tfiot.FindAuthorizerByName(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
resource "aws_iot_authorizer" "test" {
  ...
  token_key_name = "Token-Header-1"  token_signing_public_keys = {
...
resource "aws_iot_authorizer" "test" {
  ...
  status = "INACTIVE"
  enable_caching_for_http = true  token_signing_public_keys = {
Key1 = file("test-fixtures/iot-authorizer-signing-key.pem")
