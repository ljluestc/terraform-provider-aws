...
// SPDX-License-Identifier: MPL-2.0package iot_testimport (
	...
	policyName2 := sdkacctest.RandomWithPrefix("PolicyName2-")	resource.Test(t, resource.TestCase{
	...
	return func(s *terraform.State) error {
		...
		for _, rs := range s.RootModule().Resources {
			...
			policyName := rs.Primary.Attributes["policy"]			input := &iot.ListAttachedPoliciesInput{
	...
	return func(s *terraform.State) error {
		...
		target := rs.Primary.Attributes["target"]
		policyName := rs.Primary.Attributes["policy"]		policy, err := tfiot.GetPolicyAttachment(ctx, conn, target, policyName)		if err != nil {
	...
	return func(s *terraform.State) error {
		conn := acctest.Provider.Meta().(*conns.AWSClient).IoTConn(ctx)		rs, ok := s.RootModule().Resources[n]		if !ok {
			return fmt.Errorf("Not found: %s", n)
		}		if rs.Primary.ID == "" {
			return fmt.Errorf("No ID is set")
		}		certARN := rs.Primary.Attributes["arn"]		out, err := conn.ListAttachedPoliciesWithContext(ctx, &iot.ListAttachedPoliciesInput{
...
EOF}resource "aws_iot_policy_attachment" "att" {
...
EOF}resource "aws_iot_policy" "policy2" {
  name = "%s"  policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
...
EOF}resource "aws_iot_policy_attachment" "att2" {
  policy = aws_iot_policy.policy2.name
  target = aws_iot_certificate.cert.arn
}
`, policyName2)
