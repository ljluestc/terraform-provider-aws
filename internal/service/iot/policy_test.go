...
// SPDX-License-Identifier: MPL-2.0package iot_testimport (
	...
	resourceName := "aws_iot_policy.test"	resource.ParallelTest(t, resource.TestCase{
	...
	return func(s *terraform.State) error {
		conn := acctest.Provider.Meta().(*conns.AWSClient).IoTConn(ctx)		for _, rs := range s.RootModule().Resources {
			...
			}			resp, err := conn.GetPolicyWithContext(ctx, GetPolicyOpts)			if err == nil {
	...
	return func(s *terraform.State) error {
		...
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).IoTConn(ctx)		resp, err := conn.GetPolicyWithContext(ctx, &iot.GetPolicyInput{
