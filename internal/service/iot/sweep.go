...
// +build sweeppackage iotimport (
	...
	var errs *multierror.Error	input := &iot.ListCertificatesInput{}	err = conn.ListCertificatesPagesWithContext(ctx, input, func(page *iot.ListCertificatesOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
		}		for _, certificate := range page.Certificates {
			r := ResourceCertificate()
			d := r.Data(nil)			d.SetId(aws.StringValue(certificate.CertificateId))			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
	})	if err != nil {
	...
	sweepResources := make([]sweep.Sweepable, 0)
	var errs *multierror.Error	input := &iot.ListPoliciesInput{}	err = conn.ListPoliciesPagesWithContext(ctx, input, func(page *iot.ListPoliciesOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
			...
			input := &iot.ListTargetsForPolicyInput{
				...
				if page == nil {
					...
					d := r.Data(nil)					d.SetId(fmt.Sprintf("%s|%s", aws.StringValue(policy.PolicyName), aws.StringValue(target)))
					d.Set("policy", policy.PolicyName)
					d.Set("target", target)					sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
				}				return !lastPage
			})			if err != nil {
	...
	var errs *multierror.Error	input := &iot.ListPoliciesInput{}	err = conn.ListPoliciesPagesWithContext(ctx, input, func(page *iot.ListPoliciesOutput, lastPage bool) bool {
		if page == nil {
			...
			r := ResourcePolicy()
			d := r.Data(nil)			d.SetId(aws.StringValue(policy.PolicyName))			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
	})	if err != nil {
	...
	sweepResources := make([]sweep.Sweepable, 0)
	var errs *multierror.Error	input := &iot.ListRoleAliasesInput{}	err = conn.ListRoleAliasesPagesWithContext(ctx, input, func(page *iot.ListRoleAliasesOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
		}		for _, roleAlias := range page.RoleAliases {
			r := ResourceRoleAlias()
			d := r.Data(nil)			d.SetId(aws.StringValue(roleAlias))			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
	})	if err != nil {
	...
	sweepResources := make([]sweep.Sweepable, 0)
	var errs *multierror.Error	input := &iot.ListThingsInput{}	err = conn.ListThingsPagesWithContext(ctx, input, func(page *iot.ListThingsOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
			...
			input := &iot.ListThingPrincipalsInput{
				...
				if page == nil {
					...
					d := r.Data(nil)					d.SetId(fmt.Sprintf("%s|%s", aws.StringValue(thing.ThingName), aws.StringValue(principal)))
					d.Set("principal", principal)
					d.Set("thing", thing.ThingName)					sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
				}				return !lastPage
			})			if err != nil {
	...
	var errs *multierror.Error	input := &iot.ListThingsInput{}	err = conn.ListThingsPagesWithContext(ctx, input, func(page *iot.ListThingsOutput, lastPage bool) bool {
		if page == nil {
			...
			r := ResourceThing()
			d := r.Data(nil)			d.SetId(aws.StringValue(thing.ThingName))			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
	})	if err != nil {
	...
	sweepResources := make([]sweep.Sweepable, 0)
	var errs *multierror.Error	input := &iot.ListThingTypesInput{}	err = conn.ListThingTypesPagesWithContext(ctx, input, func(page *iot.ListThingTypesOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
		}		for _, thingTypes := range page.ThingTypes {
			r := ResourceThingType()
			d := r.Data(nil)			d.SetId(aws.StringValue(thingTypes.ThingTypeName))			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
	})	if err != nil {
	...
	var sweeperErrs *multierror.Error	for {
		...
		if err != nil {
			...
			name := aws.StringValue(rule.RuleName)			log.Printf("[INFO] Deleting IoT Topic Rule: %s", name)
	...
	sweepResources := make([]sweep.Sweepable, 0)	err = conn.ListThingGroupsPagesWithContext(ctx, input, func(page *iot.ListThingGroupsOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
		}		for _, group := range page.ThingGroups {
			r := ResourceThingGroup()
			d := r.Data(nil)
			d.SetId(aws.StringValue(group.GroupName))			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
	})	if sweep.SkipSweepError(err) {
	...
	sweepResources := make([]sweep.Sweepable, 0)	err = conn.ListTopicRuleDestinationsPagesWithContext(ctx, input, func(page *iot.ListTopicRuleDestinationsOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
		}		for _, v := range page.DestinationSummaries {
			r := ResourceTopicRuleDestination()
			d := r.Data(nil)
			d.SetId(aws.StringValue(v.Arn))			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
	})	if sweep.SkipSweepError(err) {
