...
// SPDX-License-Identifier: MPL-2.0package iot_testimport (
	...
	resourceName := "aws_iot_thing_group.test"	resource.ParallelTest(t, resource.TestCase{
	...
	grandparentResourceName := "aws_iot_thing_group.grandparent"	resource.ParallelTest(t, resource.TestCase{
		PreCheck:func() { acctest.PreCheck(ctx, t) },
		ErrorCheck:  acctest.ErrorCheck(t, iot.EndpointsID),
		ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
		CheckDestroy:testAccCheckThingGroupDestroy(ctx),
	...
	return func(s *terraform.State) error {
		...
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).IoTConn(ctx)		output, err := tfiot.FindThingGroupByName(ctx, conn, rs.Primary.ID)		if err != nil {
			return err
		}		*v = *output		return nil
	}
	...
	return func(s *terraform.State) error {
		conn := acctest.Provider.Meta().(*conns.AWSClient).IoTConn(ctx)		for _, rs := range s.RootModule().Resources {
			if rs.Type != "aws_iot_thing_group" {
				continue
			}			_, err := tfiot.FindThingGroupByName(ctx, conn, rs.Primary.ID)			if tfresource.NotFound(err) {
				continue
			}			if err != nil {
				return err
...
resource "aws_iot_thing_group" "grandparent" {
  ...
  name = "%[1]s-parent"  parent_group_name = aws_iot_thing_group.grandparent.name
}resource "aws_iot_thing_group" "test" {
  name = %[1]q  parent_group_name = aws_iot_thing_group.parent.name
}
`, rName)
