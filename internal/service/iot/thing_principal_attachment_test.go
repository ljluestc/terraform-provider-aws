...
// SPDX-License-Identifier: MPL-2.0package iot_testimport (
	...
	thingName2 := sdkacctest.RandomWithPrefix("tf-acc2")	resource.Test(t, resource.TestCase{
	...
	return func(s *terraform.State) error {
		conn := acctest.Provider.Meta().(*conns.AWSClient).IoTConn(ctx)		for _, rs := range s.RootModule().Resources {
			...
			thing := rs.Primary.Attributes["thing"]			found, err := tfiot.GetThingPricipalAttachment(ctx, conn, thing, principal)			if err != nil {
	...
	return func(s *terraform.State) error {
		...
		principal := rs.Primary.Attributes["principal"]		found, err := tfiot.GetThingPricipalAttachment(ctx, conn, thing, principal)		if err != nil {
	...
	return func(s *terraform.State) error {
		conn := acctest.Provider.Meta().(*conns.AWSClient).IoTConn(ctx)		principalARNs := make(map[string]string)		for _, p := range principals {
