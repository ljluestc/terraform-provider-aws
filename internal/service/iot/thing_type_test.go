...
resourceName := "aws_iot_thing_type.test"resource.ParallelTest(t, resource.TestCase{
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).IoTConn(ctx)_, err := tfiot.FindThingTypeByName(ctx, conn, rs.Primary.ID)return err
}
...
continue
}_, err := tfiot.FindThingTypeByName(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
