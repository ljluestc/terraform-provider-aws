...
resourceName := "aws_iot_topic_rule_destination.test"resource.ParallelTest(t, resource.TestCase{
...
continue
}_, err := tfiot.FindTopicRuleDestinationByARN(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).IoTConn(ctx)_, err := tfiot.FindTopicRuleDestinationByARN(ctx, conn, rs.Primary.ID)return err
}
