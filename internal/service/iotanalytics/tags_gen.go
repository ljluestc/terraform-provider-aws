...
func (p *servicePackage) ListTags(ctx context.Context, meta any, identifier string) error {
	tags, err := listTags(ctx, meta.(*conns.AWSClient).IoTAnalyticsConn(ctx), identifier)	if err != nil {
turn err
	}	if inContext, ok := tftags.FromContext(ctx); ok {
Context.TagsOut = types.Some(tags)
	}	return nil
}// []*SERVICE.Tag handling// Tags returns iotanalytics service tags.
func Tags(tags tftags.KeyValueTags) []*iotanalytics.Tag {
	result := make([]*iotanalytics.Tag, 0, len(tags))	for k, v := range tags.Map() {
g := &iotanalytics.Tag{
Key:aws.String(k),
Value: aws.String(v),
...
put := &iotanalytics.UntagResourceInput{
ResourceArn: aws.String(identifier),
...
put := &iotanalytics.TagResourceInput{
ResourceArn: aws.String(identifier),
Tags:
