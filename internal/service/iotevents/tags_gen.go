...
func (p *servicePackage) ListTags(ctx context.Context, meta any, identifier string) error {
	tags, err := listTags(ctx, meta.(*conns.AWSClient).IoTEventsConn(ctx), identifier)	if err != nil {
		return err
	}	if inContext, ok := tftags.FromContext(ctx); ok {
		inContext.TagsOut = types.Some(tags)
	}	return nil
}// []*SERVICE.Tag handling// Tags returns iotevents service tags.
func Tags(tags tftags.KeyValueTags) []*iotevents.Tag {
	result := make([]*iotevents.Tag, 0, len(tags))	for k, v := range tags.Map() {
		tag := &iotevents.Tag{
			Key:aws.String(k),
			Value: aws.String(v),
