...
// SPDX-License-Identifier: MPL-2.0package kafkaconnect_testimport (
"context"
"fmt"
"testing""github.com/aws/aws-sdk-go/service/kafkaconnect"
...
resourceName := "aws_mskconnect_connector.test"resource.ParallelTest(t, resource.TestCase{
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).KafkaConnectConn(ctx)_, err := tfkafkaconnect.FindConnectorByARN(ctx, conn, rs.Primary.ID)return err
}
}func testAccCheckConnectorDestroy(ctx context.Context) resource.TestCheckFunc {
return func(s *terraform.State) error {
conn := acctest.Provider.Meta().(*conns.AWSClient).KafkaConnectConn(ctx)for _, rs := range s.RootModule().Resources {
if rs.Type != "aws_mskconnect_connector" {
continue
}_, err := tfkafkaconnect.FindConnectorByARN(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
Name = %[1]q
  ...
  vpc_id   = aws_vpc.test.id
  cidr_block0.3.0/24"
  availability_zone = data.aws_availability_zones.available.names[2]  tags = {
Name = %[1]q
  }
...
Name = %[1]q
  ...
  vpc_id   = aws_vpc.test.id
  service_nameazonaws.${data.aws_region.current.name}.s3"
  vpc_endpoint_type = "Interface"  security_group_ids = [
...
EOF
  ...
  cluster_name  = %[1]q
  kafka_version = "2.2.1"
  number_of_broker_nodes = 3  broker_node_group_info {
client_subnets  = [aws_subnet.test1.id, aws_subnet.test2.id, aws_subnet.test3.id]
instance_type   = "kafka.m5.large"
security_groups = [aws_security_group.test.id]storage_info {
ge_info {
size = 10}
  }
}
...
resource "aws_mskconnect_connector" "test" {
  name = %[1]q  kafkaconnect_version = "2.7.1"  capacity {
autoscaling {
r_count = 1
r_count = 2
}
...
= aws_mskconnect_custom_plugin.test.latest_revision
  ...
  }  service_execution_role_arn = aws_iam_role.test.arn  depends_on = [aws_iam_role_policy.test, aws_vpc_endpoint.test]
}
`, rName))
...
= aws_mskconnect_custom_plugin.test.latest_revision
  ...
  }  service_execution_role_arn = aws_iam_role.test.arn  worker_configuration {
