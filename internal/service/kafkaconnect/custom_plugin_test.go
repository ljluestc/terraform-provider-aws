...
resourceName := "aws_mskconnect_custom_plugin.test"resource.ParallelTest(t, resource.TestCase{
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).KafkaConnectConn(ctx)_, err := tfkafkaconnect.FindCustomPluginByARN(ctx, conn, rs.Primary.ID)return err
}
...
continue
}_, err := tfkafkaconnect.FindCustomPluginByARN(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
resource "aws_s3_bucket" "test" {
  ...
  bucket = aws_s3_bucket.test.bucket  versioning_configuration {
