...
}output, err := conn.DescribeConnectorWithContext(ctx, input)if tfawserr.ErrCodeEquals(err, kafkaconnect.ErrCodeNotFoundException) {
...
}output, err := conn.DescribeCustomPluginWithContext(ctx, input)if tfawserr.ErrCodeEquals(err, kafkaconnect.ErrCodeNotFoundException) {
return nil, &retry.NotFoundError{
LastError:   err,
LastRequest: input,
}
...
}output, err := conn.DescribeWorkerConfigurationWithContext(ctx, input)if tfawserr.ErrCodeEquals(err, kafkaconnect.ErrCodeNotFoundException) {
return nil, &retry.NotFoundError{
LastError:   err,
LastRequest: input,
}
