...
// +build sweeppackage kafkaconnectimport (
"fmt"
...
sweepResources := make([]sweep.Sweepable, 0)err = conn.ListConnectorsPagesWithContext(ctx, input, func(page *kafkaconnect.ListConnectorsOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, v := range page.Connectors {
r := ResourceConnector()
d := r.Data(nil)
d.SetId(aws.StringValue(v.ConnectorArn))sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
...
sweepResources := make([]sweep.Sweepable, 0)err = conn.ListCustomPluginsPagesWithContext(ctx, input, func(page *kafkaconnect.ListCustomPluginsOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, v := range page.CustomPlugins {
r := ResourceCustomPlugin()
d := r.Data(nil)
d.SetId(aws.StringValue(v.CustomPluginArn))sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
})if sweep.SkipSweepError(err) {
