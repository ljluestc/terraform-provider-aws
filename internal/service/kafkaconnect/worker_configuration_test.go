...
resourceName := "aws_mskconnect_worker_configuration.test"resource.ParallelTest(t, resource.TestCase{
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).KafkaConnectConn(ctx)_, err := tfkafkaconnect.FindWorkerConfigurationByARN(ctx, conn, rs.Primary.ID)return err
}
}func testAccWorkerConfigurationConfig_basic(rName string) string {
return fmt.Sprintf(`
resource "aws_mskconnect_worker_configuration" "test" {
  name = %[1]q  properties_file_content = <<EOF
...
resource "aws_mskconnect_worker_configuration" "test" {
  name
  description = "testing"  properties_file_content = <<EOF
key.converter=org.apache.kafka.connect.storage.StringConverter
value.converter=org.apache.kafka.connect.storage.StringConverter
EOF
}
