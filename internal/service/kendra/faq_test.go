...
// SPDX-License-Identifier: MPL-2.0package kendra_testimport (
	...
	resourceName := "aws_kendra_faq.test"	resource.ParallelTest(t, resource.TestCase{
		PreCheck:
...
func(s *terraform.State) error {
		conn := acctest.Provider.Meta().(*conns.AWSClient).KendraClient(ctx)		for _, rs := range s.RootModule().Resources {
			...
			_, err = tfkendra.FindFaqByID(ctx, conn, id, indexId)			if tfresource.NotFound(err) {
				continue
			}			if err != nil {
				return err
			}
...
func(s *terraform.State) error {
		...
		if err != nil {
			return err
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).KendraClient(ctx)		_, err = tfkendra.FindFaqByID(ctx, conn, id, indexId)		if err != nil {
