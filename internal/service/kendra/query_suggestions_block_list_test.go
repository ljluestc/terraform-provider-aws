...
t.Skip("skipping long-running test in short mode")
	...
	resourceName := "aws_kendra_query_suggestions_block_list.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
func() {
...
continue
	...
	if err != nil {
return err
	}	_, err = tfkendra.FindQuerySuggestionsBlockListByID(ctx, conn, id, indexId)	if tfresource.NotFound(err) {
continue
	}	if err != nil {
return err
...
if err != nil {
	return err
}conn := acctest.Provider.Meta().(*conns.AWSClient).KendraClient(ctx)_, err = tfkendra.FindQuerySuggestionsBlockListByID(ctx, conn, id, indexId)if err != nil {
...
index_id = aws_kendra_index.test.id
name= %[1]q
role_arn = aws_iam_role.test2.arnsource_s3_path {
 bucket = aws_s3_bucket.test.id
 key = aws_s3_object.test.key
}
}
