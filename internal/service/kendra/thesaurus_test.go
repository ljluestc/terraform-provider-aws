...
func TestAccKendraThesaurus_basic(t *testing.T) {
	...
	resourceName := "aws_kendra_thesaurus.test"	resource.ParallelTest(t, resource.TestCase{
		PreCheck: 
func() {
...
func(s *terraform.State) error {
		conn := acctest.Provider.Meta().(*conns.AWSClient).KendraClient(ctx)		for _, rs := range s.RootModule().Resources {
			...
			if err != nil {
				return err
			}			_, err = tfkendra.FindThesaurusByID(ctx, conn, id, indexId)			if tfresource.NotFound(err) {
				continue
			}			if err != nil {
				return err
			}
...
func(s *terraform.State) error {
		...
		if err != nil {
			return err
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).KendraClient(ctx)		_, err = tfkendra.FindThesaurusByID(ctx, conn, id, indexId)		if err != nil {
...
func testAccThesaurusBaseConfig(rName string) string {
	return fmt.Sprintf(`
data "aws_partition" "current" {}data "aws_iam_policy_document" "assume_role" {
statement {
