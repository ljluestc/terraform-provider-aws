...
continue
}_, err := tfkinesis.FindStreamConsumerByARN(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
}_, err := tfkinesis.FindStreamConsumerByARN(ctx, conn, rs.Primary.ID)return err
}
