...
resourceName := "aws_kinesis_stream.test"resource.ParallelTest(t, resource.TestCase{
PreCheck:func() { acctest.PreCheck(ctx, t) },
ErrorCheck:  acctest.ErrorCheck(t, kinesis.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckStreamDestroy(ctx),
...
ctx := acctest.Context(t)
var stream kinesis.StreamDescriptionSummary
var updatedStream kinesis.StreamDescriptionSummarytestCheckStreamNotDestroyed :=func() resource.TestCheck
func {
...
conn := acctest.Provider.Meta().(*conns.AWSClient).KinesisConn(ctx)rs, ok := s.RootModule().Resources[n]
if !ok {
return fmt.Errorf("Not found: %s", n)
}if rs.Primary.ID == "" {
return fmt.Errorf("No Kinesis Stream ID is set")
}output, err := tfkinesis.FindStreamByName(ctx, conn, rs.Primary.Attributes["name"])if err != nil {
return err
}*v = *outputreturn nil
}
...
func {
returnfunc(s *terraform.State) error {
conn := acctest.Provider.Meta().(*conns.AWSClient).KinesisConn(ctx)for _, rs := range s.RootModule().Resources {
if rs.Type != "aws_kinesis_stream" {
continue
}_, err := tfkinesis.FindStreamByName(ctx, conn, rs.Primary.Attributes["name"])if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
func {
returnfunc(s *terraform.State) error {
conn := acctest.Provider.Meta().(*conns.AWSClient).KinesisConn(ctx)if _, err := conn.RegisterStreamConsumerWithContext(ctx, &kinesis.RegisterStreamConsumerInput{
...
resource "aws_kinesis_stream" "test" {
  name  = %[1]q
  shard_count = 2  shard_level_metrics = [
"IncomingBytes",
"IncomingRecords",
"OutgoingBytes",
"OutgoingRecords",
...
resource "aws_kms_key" "key" {
  count = 2  description= "%[1]s-${count.index}"
...
resource "aws_kinesis_stream" "test" {
  name  = %[1]q
  shard_count = 2  stream_mode_details {
...
resource "aws_kinesis_stream" "test" {
  name  = %[1]q
  shard_count = 1  stream_mode_details {
stream_mode = "ON_DEMAND"
  }
}
`, rName)
