...
// +build sweeppackage kinesisanalyticsimport (
	"fmt"
	"log"
...
func sweepApplications(region string) error {
	...
	conn := client.KinesisAnalyticsConn(ctx)	sweepResources := make([]sweep.Sweepable, 0)
	var sweeperErrs *multierror.Error	input := &kinesisanalytics.ListApplicationsInput{}
	err = ListApplicationsPages(ctx, conn, input, func(page *kinesisanalytics.ListApplicationsOutput, lastPage bool) bool {
		if page == nil {
			...
			name := aws.StringValue(applicationSummary.ApplicationName)			application, err := FindApplicationDetailByName(ctx, conn, name)			if tfawserr.ErrMessageContains(err, kinesisanalytics.ErrCodeUnsupportedOperationException, "was created/updated by kinesisanalyticsv2 SDK") {
				continue
			}			if err != nil {
			...
			d.Set("name", name)			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
