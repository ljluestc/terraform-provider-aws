...
package kinesisanalyticsimport (
	"context"
...
func (p *servicePackage) ListTags(ctx context.Context, meta any, identifier string) error {
	tags, err := listTags(ctx, meta.(*conns.AWSClient).KinesisAnalyticsConn(ctx), identifier)	if err != nil {
		return err
	}	if inContext, ok := tftags.FromContext(ctx); ok {
		inContext.TagsOut = types.Some(tags)
	}	return nil
}// []*SERVICE.Tag handling// Tags returns kinesisanalytics service tags.
func Tags(tags tftags.KeyValueTags) []*kinesisanalytics.Tag {
	result := make([]*kinesisanalytics.Tag, 0, len(tags))	for k, v := range tags.Map() {
