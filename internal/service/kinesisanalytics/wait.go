...
// SPDX-License-Identifier: MPL-2.0package kinesisanalyticsimport (
	"context"
	"time"	"github.com/aws/aws-sdk-go/service/kinesisanalytics"
...
func waitApplicationDeleted(ctx context.Context, conn *kinesisanalytics.KinesisAnalytics, name string) (*kinesisanalytics.ApplicationDetail, error) {
	...
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if v, ok := outputRaw.(*kinesisanalytics.ApplicationDetail); ok {
		return v, err
	}	return nil, err
...
func waitIAMPropagation(ctx context.Context, f func() (interface{}, error)) (interface{}, error) {
	var output interface{}	err := retry.RetryContext(ctx, propagationTimeout, func() *retry.RetryError {
