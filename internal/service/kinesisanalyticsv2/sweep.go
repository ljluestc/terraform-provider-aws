...
// +build sweeppackage kinesisanalyticsv2import (
	"fmt"
	"log"
	...
	conn := client.KinesisAnalyticsV2Conn(ctx)	sweepResources := make([]sweep.Sweepable, 0)
	var sweeperErrs *multierror.Error	input := &kinesisanalyticsv2.ListApplicationsInput{}
	err = listApplicationsPages(ctx, conn, input,(page *kinesisanalyticsv2.ListApplicationsOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
		}		for _, applicationSummary := range page.ApplicationSummaries {
			arn := aws.StringValue(applicationSummary.ApplicationARN)
			name := aws.StringValue(applicationSummary.ApplicationName)			application, err := FindApplicationDetailByName(ctx, conn, name)			if err != nil {
