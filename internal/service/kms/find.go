...
var output *kms.AliasListEntryerr := conn.ListAliasesPagesWithContext(ctx, input, func(page *kms.ListAliasesOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, alias := range page.Aliases {
if aws.StringValue(alias.AliasName) == name {
output = aliasreturn false
}
}return !lastPage
})if err != nil {
return nil, err
...
out, err := conn.DescribeCustomKeyStoresWithContext(ctx, in)if tfawserr.ErrCodeEquals(err, kms.ErrCodeCustomKeyStoreNotFoundException) {
return nil, &retry.NotFoundError{
LastError:err,
LastRequest: in,
}
...
KeyId: aws.String(id),
}output, err := conn.DescribeKeyWithContext(ctx, input)if tfawserr.ErrCodeEquals(err, kms.ErrCodeNotFoundException) {
return nil, &retry.NotFoundError{
LastError:err,
LastRequest: input,
}
...
}keyMetadata := output.KeyMetadata// Once the CMK is in the pending (replica) deletion state Terraform considers it logically deleted.
...
conn := meta.(*conns.AWSClient).KMSConn(ctx)if aws.StringValue(conn.Config.Region) != region {
...
}output, err := conn.GetKeyPolicyWithContext(ctx, input)if tfawserr.ErrCodeEquals(err, kms.ErrCodeNotFoundException) {
return nil, &retry.NotFoundError{
LastError:err,
LastRequest: input,
}
...
KeyId: aws.String(keyID),
}output, err := conn.GetKeyRotationStatusWithContext(ctx, input)if tfawserr.ErrCodeEquals(err, kms.ErrCodeNotFoundException) {
return nil, &retry.NotFoundError{
LastError:err,
LastRequest: input,
}
