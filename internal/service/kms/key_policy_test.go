...
eckDestroy:acctest.CheckDestroyNoop,
...
func TestAccKMSKeyPolicy_bypass(t *testing.T) {
	...
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	keyResourceName := "aws_kms_key.test"
	attachmentResourceName := "aws_kms_key_policy.test"	resource.ParallelTest(t, resource.TestCase{
eCheck:k(ctx, t) },
rorCheck: acctest.ErrorCheck(t, kms.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:acctest.CheckDestroyNoop,
...
data "aws_caller_identity" "current" {}resource "aws_kms_key" "test" {
  ...
  key_id= aws_kms_key.test.id
  bypass_policy_lockout_safety_check = %[2]t  policy = jsonencode({
Id = %[1]q
Statement = [
 {
