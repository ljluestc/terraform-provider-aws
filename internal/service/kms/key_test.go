...
func TestAccKMSKey_basic(t *testing.T) {
	...
	resourceName := "aws_kms_key.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck:func() { acctest.PreCheck(ctx, t) },
rorCheck:  acctest.ErrorCheck(t, kms.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckKeyDestroy(ctx),
...
func TestAccKMSKey_Policy_basic(t *testing.T) {
	...
	expectedPolicyText := fmt.Sprintf(`{"Version":"2012-10-17","Id":%[1]q,"Statement":[{"Sid":"Enable IAM User Permissions","Effect":"Allow","Principal":{"AWS":"*"},"Action":"kms:*","Resource":"*"}]}`, rName)	resource.ParallelTest(t, resource.TestCase{
eCheck:func() { acctest.PreCheck(ctx, t) },
rorCheck:  acctest.ErrorCheck(t, kms.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckKeyDestroy(ctx),
...
o := acctest.Provider.Meta().(*conns.AWSClient).KMSConn(ctx)out,rr := conn.GetKeyPolicyWithContext(ctx, &kms.GetKeyPolicyInput{
KeyId: aws.String(rs.Primary.ID),
PolicyName: aws.String("default"), err != nil {
return err
calPolicyText := aws.StringValue(out.Policy)equilent, err := awspolicy.PoliciesAreEquivalent(actualPolicyText, expectedPolicyText)
 err != nil {
return fmt.Errorf("Error testing policy equivalence: %s", err) !equivalent {
...
func testAccCheckKeyDestroy(ctx context.Context) resource.TestCheckFunc {
	return func(s *terraform.State) error {
nn := acctest.Provider.Meta().(*conns.AWSClient).KMSConn(ctx)fo_, rs := range s.RootModule().Resources {
...
fs.Primary.ID == "" {
return fmt.Errorf("No KMS Key ID is set")
o := acctest.Provider.Meta().(*conns.AWSClient).KMSConn(ctx)outpRaw, err := tfresource.RetryWhenNotFound(ctx, tfkms.PropagationTimeout, func() (interface{}, error) {
return tfkms.FindKeyByID(ctx, conn, rs.Primary.ID)
ifrr != nil {
return err
k = *(outputRaw.(*kms.KeyMetadata))retu nil
	}
}
func testAccCheckKeyAddTag(ctx context.Context, key *kms.KeyMetadata, tagKey, tagValue string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
nn := acctest.Provider.Meta().(*conns.AWSClient).KMSConn(ctx)int := &kms.TagResourceInput{
KeyId: key.KeyId,
Tags: []*kms.Tag{{
TagKey:aws.String(tagKey),
TagValue: aws.String(tagValue),
}},
,rr := conn.TagResourceWithContext(ctx, input)retu err
	}
}
...
resource "aws_kms_key" "test" {
  description= %[1]q
  deletion_window_in_days = 7  key_usage = "SIGN_VERIFY"
...
resource "aws_kms_key" "test" {
  description= %[1]q
  deletion_window_in_days = 7  key_usage = "GENERATE_VERIFY_MAC"
  customer_master_key_spec = "HMAC_256"
}
`, rName)
}
func testAccKeyConfig_policy(rName string) string {
	return fmt.Sprintf(`resource "aws_kms_key" "test" {
  description= %[1]q
  deletion_window_in_days = 7  policy = jsonencode({
Id = %[1]q
Statement = [{
...
func testAccKeyConfig_policyBypass(rName string, bypassFlag bool) string {
	return fmt.Sprintf(`
data "aws_caller_identity" "current" {}resource "aws_kms_key" "test" {
  description= %[1]q
  deletion_window_in_days = 7  bypass_policy_lockout_safety_check = %[2]t  policy = jsonencode({
Id = %[1]q
Statement = [
 {
...
func testAccKeyConfig_policyIAMRole(rName string) string {
	return fmt.Sprintf(`
data "aws_partition" "current" {}data "aws_caller_identity" "current" {}resource "aws_iam_role" "test" {
...
func testAccKeyConfig_policyIAMMultiRole(rName string) string {
	return fmt.Sprintf(`
data "aws_partition" "current" {}resource "aws_iam_role" "test1" {
  name = "%[1]s-sultan"  assume_role_policy = jsonencode({
Statement = [{
 Action = "sts:AssumeRole"
 Effect = "Allow"
 Principal = {
...
Version = "2012-10-17"
  ...
  name = "%[1]s-shepard"  assume_role_policy = jsonencode({
Statement = [{
 Action = "sts:AssumeRole"
 Effect = "Allow"
 Principal = {
...
Version = "2012-10-17"
  ...
  name = "%[1]s-tritonal"  assume_role_policy = jsonencode({
Statement = [{
 Action = "sts:AssumeRole"
 Effect = "Allow"
 Principal = {
...
Version = "2012-10-17"
  ...
  name = "%[1]s-artlec"  assume_role_policy = jsonencode({
Statement = [{
 Action = "sts:AssumeRole"
 Effect = "Allow"
 Principal = {
...
Version = "2012-10-17"
  ...
  name = "%[1]s-cazzette"  assume_role_policy = jsonencode({
Statement = [{
 Action = "sts:AssumeRole"
 Effect = "Allow"
 Principal = {
...
resources = ["*"]
  ...
  description= %[1]q
  deletion_window_in_days = 7  policy = data.aws_iam_policy_document.test.json
}
`, rName)
}
func testAccKeyConfig_policyIAMServiceLinkedRole(rName string) string {
	return fmt.Sprintf(`
data "aws_partition" "current" {}data "aws_caller_identity" "current" {}resource "aws_iam_service_linked_role" "test" {
...
func testAccKeyConfig_policyBooleanCondition(rName string) string {
	return fmt.Sprintf(`
data "aws_caller_identity" "current" {}data "aws_partition" "current" {}resource "aws_kms_key" "test" {
  description= %[1]q
  deletion_window_in_days = 7  policy = jsonencode({
Id = %[1]q
Statement = [
...
le IAM User Permissions"Conditio
 Bool = {
