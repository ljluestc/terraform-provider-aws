...
funcTestValidGrantName(t*testing.T){
t.Parallel()validValues:=[]string{
"123",
"Abc",
"grant_1",
"grant:/-",
}for_,s:=rangevalidValues{
_,errors:=validGrantName(s,"name")
iflen(errors)>0{
t.Fatalf("%qAWSKMSGrantNameshouldhavebeenvalid:%v",s,errors)
}
...
";",
"whitespace",
}for_,s:=rangeinvalidValues{
_,errors:=validGrantName(s,"name")
iflen(errors)==0{
t.Fatalf("%qshouldnotbeavalidAWSKMSGrantName",s)
}
}
}
funcTestValidNameForDataSource(t*testing.T){
...
t.Fatalf("AWSKMSAliasNamevalidationfailed:%v",errors)
}
}
}
funcTestValidNameForResource(t*testing.T){
t.Parallel()cases:=[]struct{
Valuestring
ErrCountint
}{
...
funcTestValidateKeyOrAlias(t*testing.T){
t.Parallel()cases:=[]struct{
Valuestring
ErrCountint
validbool
...
t.Parallel()_,errors:=ValidateKeyOrAlias(tc.Value,"key_id")
if(len(errors)==0)!=tc.valid{
t.Errorf("%qValidateKMSKeyOrAliasfailed:%v",tc.Value,errors)
}
})
}
}
funcTestValidateKeyARN(t*testing.T){
...
t.Parallel()aWs,aEs:=validateKeyARN(testcase.in,"field")
iflen(aWs)!=0{
t.Errorf("expectednowarnings,got%v",aWs)
}
iftestcase.valid{
iflen(aEs)!=0{
t.Errorf("expectednoerrors,got%v",aEs)
}
}else{
iflen(aEs)==0{
t.Error("expectederrors,gotnone")
}
}
})
}
