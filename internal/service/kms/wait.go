...
KeyStatePendingDeletionTimeout = 20 * time.MinuteKeyDeletedTimeout= 20 * time.Minute
KeyDescriptionPropagationTimeout = 10 * time.Minute
KeyMaterialImportedTimeout= 10 * time.Minute
KeyPolicyPropagationTimeout * time.Minute
KeyRotationUpdatedTimeout
KeyStatePropagationTimeout= 20 * time.Minute
KeyTagsPropagationTimeout
KeyValidToPropagationTimeout time.MinutePropagationTimeout = 2 * time.MinuteReplicaExternalKeyCreatedTimeout = 2 * time.Minute
...
kms.ErrCodeMalformedPolicyDocumentException)if err != nil {
var zero T
return zero, err
...
}outputRaw, err := stateConf.WaitForStateContext(ctx)if output, ok := outputRaw.(*kms.KeyMetadata); ok {
return output, err
}return nil, err
}
func WaitKeyDescriptionPropagated(ctx context.Context, conn *kms.KMS, id string, description string) error {
checkFunc := func() (bool, error) {
output, err := FindKeyByID(ctx, conn, id)if tfresource.NotFound(err) {
return false, nil
}if err != nil {
return false, err
...
checkFunc := func() (bool, error) {
output, err := FindKeyPolicyByKeyIDAndPolicyName(ctx, conn, id, PolicyNameDefault)if tfresource.NotFound(err) {
return false, nil
}if err != nil {
return false, err
}equivalent, err := awspolicy.PoliciesAreEquivalent(aws.StringValue(output), policy)if err != nil {
return false, err
...
checkFunc := func() (bool, error) {
output, err := FindKeyRotationEnabledByKeyID(ctx, conn, id)if tfresource.NotFound(err) {
return false, nil
}if err != nil {
return false, err
