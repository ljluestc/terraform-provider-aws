...
acctest.CheckResourceAttrAccountID(resourceName, "catalog_id"),
	...
	resourceName := "aws_lakeformation_resource_lf_tags.test"	resource.Test(t, resource.TestCase{
PreCheck:func() { acctest.PreCheck(ctx, t); acctest.PreCheckPartitionHasService(t, lakeformation.EndpointsID) },
ErrorCheck:  acctest.ErrorCheck(t, lakeformation.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
_, err := conn.GetResourceLFTagsWithContext(ctx, input)return err
	}
...
data "aws_caller_identity" "current" {}data "aws_iam_session_context" "current" {
  ...
  key= %[1]q
  values = [%[2]s]  # for consistency, ensure that admins are set up before testing
  depends_on = [aws_lakeformation_data_lake_settings.test]
}resource "aws_lakeformation_resource_lf_tags" "test" {
  catalog_id = data.aws_caller_identity.current.account_id  database {
...
data "aws_caller_identity" "current" {}data "aws_iam_session_context" "current" {
  ...
  key= "%[1]s-2"
  values = [%[3]s]  # for consistency, ensure that admins are set up before testing
  depends_on = [aws_lakeformation_data_lake_settings.test]
}resource "aws_lakeformation_resource_lf_tags" "test" {
  database {
name = aws_glue_catalog_database.test.name
...
columns {
 ...
 = "double"
  ...
  key= "%[1]s-3"
  values = [%[6]s]  # for consistency, ensure that admins are set up before testing
  depends_on = [aws_lakeformation_data_lake_settings.test]
