...
}output, err := conn.DescribeMapWithContext(ctx, input)if tfawserr.ErrCodeEquals(err, locationservice.ErrCodeResourceNotFoundException) {
continue
}if err != nil {
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).LocationConn(ctx)input := &locationservice.DescribeMapInput{
MapName: aws.String(rs.Primary.ID),
}_, err := conn.DescribeMapWithContext(ctx, input)if err != nil {
return fmt.Errorf("error getting Location Service Map (%s): %w", rs.Primary.ID, err)
}return nil
	}
