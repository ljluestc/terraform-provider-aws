...
// +build sweeppackage locationimport (
	"fmt"
...
return fmt.Errorf("error getting client: %w", err)
	...
	var errs *multierror.Error	input := &locationservice.ListGeofenceCollectionsInput{}	err = conn.ListGeofenceCollectionsPagesWithContext(ctx, input, func(page *locationservice.ListGeofenceCollectionsOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, entry := range page.Entries {
r := ResourceGeofenceCollection()
d := r.Data(nil)id := aws.StringValue(entry.CollectionName)
...
return fmt.Errorf("error getting client: %w", err)
	...
	sweepResources := make([]sweep.Sweepable, 0)
	var errs *multierror.Error	input := &locationservice.ListMapsInput{}	err = conn.ListMapsPagesWithContext(ctx, input, func(page *locationservice.ListMapsOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, entry := range page.Entries {
r := ResourceMap()
d := r.Data(nil)id := aws.StringValue(entry.MapName)
d.SetId(id)sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
	})	if err != nil {
...
return fmt.Errorf("error getting client: %w", err)
	...
	sweepResources := make([]sweep.Sweepable, 0)
	var errs *multierror.Error	input := &locationservice.ListPlaceIndexesInput{}	err = conn.ListPlaceIndexesPagesWithContext(ctx, input, func(page *locationservice.ListPlaceIndexesOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, entry := range page.Entries {
r := ResourcePlaceIndex()
d := r.Data(nil)id := aws.StringValue(entry.IndexName)
d.SetId(id)sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
	})	if err != nil {
...
return fmt.Errorf("error getting client: %w", err)
	...
	sweepResources := make([]sweep.Sweepable, 0)
	var errs *multierror.Error	input := &locationservice.ListRouteCalculatorsInput{}	err = conn.ListRouteCalculatorsPagesWithContext(ctx, input, func(page *locationservice.ListRouteCalculatorsOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, entry := range page.Entries {
r := ResourceRouteCalculator()
d := r.Data(nil)id := aws.StringValue(entry.CalculatorName)
d.SetId(id)sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
	})	if err != nil {
...
return fmt.Errorf("error getting client: %w", err)
	...
	sweepResources := make([]sweep.Sweepable, 0)
	var errs *multierror.Error	input := &locationservice.ListTrackersInput{}	err = conn.ListTrackersPagesWithContext(ctx, input, func(page *locationservice.ListTrackersOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, entry := range page.Entries {
r := ResourceTracker()
d := r.Data(nil)id := aws.StringValue(entry.TrackerName)
d.SetId(id)sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
	})	if err != nil {
...
d := r.Data(nil)d.SetId(fmt.Sprintf("%s|%s", aws.StringValue(entry.TrackerName), aws.StringValue(arn)))sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
})if err != nil {
