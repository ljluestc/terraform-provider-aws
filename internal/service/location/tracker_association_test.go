...
t.Parallel()got, err := tflocation.TrackerAssociationParseID(testCase.Input)if err != nil && !testCase.Error {
...
continue
}trackerAssociationId, err := tflocation.TrackerAssociationParseID(rs.Primary.ID)if err != nil {
...
}err = tflocation.FindTrackerAssociationByTrackerNameAndConsumerARN(ctx, conn, trackerAssociationId.TrackerName, trackerAssociationId.ConsumerARN)if err != nil {
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).LocationConn(ctx)err = tflocation.FindTrackerAssociationByTrackerNameAndConsumerARN(ctx, conn, trackerAssociationId.TrackerName, trackerAssociationId.ConsumerARN)if err != nil {
