...
t.Skip("skipping long-running test in short mode")
	...
	resourceName := "aws_medialive_input_security_group.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck: func() {
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).MediaLiveClient(ctx)resp, err := tfmedialive.FindInputSecurityGroupByID(ctx, conn, rs.Primary.ID)if err != nil {
...
}*inputSecurityGroup = *respreturn nil
	}
}func testAccInputSecurityGroupsPreCheck(ctx context.Context, t *testing.T) {
	conn := acctest.Provider.Meta().(*conns.AWSClient).MediaLiveClient(ctx)	input := &medialive.ListInputSecurityGroupsInput{}
	_, err := conn.ListInputSecurityGroups(ctx, input)	if acctest.PreCheckSkipError(err) {
