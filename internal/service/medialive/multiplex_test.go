...
t.Skip("skipping long-running test in short mode")
	...
	resourceName := "aws_medialive_multiplex.test"	resource.Test(t, resource.TestCase{
PreCheck: func() {
...
continue
	}	_, err := tfmedialive.FindInputSecurityGroupByID(ctx, conn, rs.Primary.ID)	if tfresource.NotFound(err) {
continue
	}	if err != nil {
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).MediaLiveClient(ctx)resp, err := tfmedialive.FindMultiplexByID(ctx, conn, rs.Primary.ID)if err != nil {
	return create.Error(names.MediaLive, create.ErrActionCheckingExistence, tfmedialive.ResNameMultiplex, rs.Primary.ID, err)
}*multiplex = *respreturn nil
	}
}func testAccMultiplexesPreCheck(ctx context.Context, t *testing.T) {
	conn := acctest.Provider.Meta().(*conns.AWSClient).MediaLiveClient(ctx)	input := &medialive.ListMultiplexesInput{}
	_, err := conn.ListMultiplexes(ctx, input)	if acctest.PreCheckSkipError(err) {
