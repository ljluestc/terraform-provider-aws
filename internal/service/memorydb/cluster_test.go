...
// SPDX-License-Identifier: MPL-2.0package memorydb_testimport (
	...
	resourceName := "aws_memorydb_cluster.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck:func() { acctest.PreCheck(ctx, t); testAccPreCheck(t) },
rorCheck:  acctest.ErrorCheck(t, memorydb.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckClusterDestroy(ctx),
...
ImportStateVerify: true,
	...
	rName := "tf-test-" + sdkacctest.RandString(8)
	resourceName := "aws_memorydb_cluster.test"	resource.ParallelTest(t, resource.TestCase{
eCheck:func() { acctest.PreCheck(ctx, t); testAccPreCheck(t) },
rorCheck:  acctest.ErrorCheck(t, memorydb.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckClusterDestroy(ctx),
eps: []resource.TestStep{
...
func TestAccMemoryDBCluster_create_noTLS(t *testing.T) {
	ctx := acctest.Context(t)	rName := "tf-test-" + sdkacctest.RandString(8)
	resourceName := "aws_memorydb_cluster.test"	resource.ParallelTest(t, resource.TestCase{
eCheck:func() { acctest.PreCheck(ctx, t); testAccPreCheck(t) },
rorCheck:  acctest.ErrorCheck(t, memorydb.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
ImportStateVerify: true,
	...
	rName2 := "tf-test-" + sdkacctest.RandString(8)	resource.ParallelTest(t, resource.TestCase{
eCheck:func() { acctest.PreCheck(ctx, t); testAccPreCheck(t) },
rorCheck:  acctest.ErrorCheck(t, memorydb.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckClusterDestroy(ctx),
...
nn := acctest.Provider.Meta().(*conns.AWSClient).MemoryDBConn(ctx)fo_, rs := range s.RootModule().Resources {
if rs.Type != "aws_memorydb_cluster" {
continue
}_, err := tfmemorydb.FindClusterByName(ctx, conn, rs.Primary.Attributes["name"])if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
o := acctest.Provider.Meta().(*conns.AWSClient).MemoryDBConn(ctx)_, e := tfmemorydb.FindClusterByName(ctx, conn, rs.Primary.Attributes["name"])returnrr
	}
}func testAccCheckSnapshotExistsByName(ctx context.Context, snapshotName string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
nn := acctest.Provider.Meta().(*conns.AWSClient).MemoryDBConn(ctx)_,rr := tfmemorydb.FindSnapshotByName(ctx, conn, snapshotName)if tesource.NotFound(err) {
...
ctest.ConfigVPCWithSubnets(rName, 2),resource "aws_memorydb_subnet_group" "test" {
...
`, rName)
	...
	return acctest.ConfigCompose(
stAccClusterConfig_baseNetwork(rName),
stAccClusterConfigBaseUserAndACL(rName),
t.Sprintf(`
resource "aws_security_group" "test" {
  ...
  subnet_group_name = aws_memorydb_subnet_group.test.id  tags = {
Test = "test"
  }
}
`, rName),
...
`, rName),
	...
	return acctest.ConfigCompose(
stAccClusterConfig_baseNetwork(rName),resource "aws_memorydb_cluster" "test" {
  acl_name= "open-access"
  node_type = "db.t4g.small"
...
`, rName, retentionLimit),
	...
	return acctest.ConfigCompose(
stAccClusterConfig_baseNetwork(rName1),
t.Sprintf(`
