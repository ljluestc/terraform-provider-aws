...
eckDestroy:testAccCheckParameterGroupDestroy(ctx),
...
rorCheck:  acctest.ErrorCheck(t, memorydb.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckSubnetGroupDestroy(ctx),
eps: []resource.TestStep{
{
...
o := acctest.Provider.Meta().(*conns.AWSClient).MemoryDBConn(ctx)_, e := tfmemorydb.FindParameterGroupByName(ctx, conn, rs.Primary.Attributes["name"])returnrr
	}
...
d*schema.Set
w*schema.Set
pectedRemove  []*memorydb.ParameterNameValue
pectedAddOrUpdate []*memorydb.ParameterNameValue
	}{Name:"Empty",
Old:new(schema.Set),
New:new(schema.Set),
...
move, addOrUpdate := tfmemorydb.ParameterChanges(tc.Old, tc.New)
 !reflect.DeepEqual(remove, tc.ExpectedRemove) {
t.Errorf("Case %q: Remove did not match\n%#v\n\nGot:\n%#v", tc.Name, tc.ExpectedRemove, remove) !reflect.DeepEqual(addOrUpdate, tc.ExpectedAddOrUpdate) {
