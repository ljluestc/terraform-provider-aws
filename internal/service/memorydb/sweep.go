...
// +build sweeppackage memorydbimport (
"fmt"
"log"
...
sweepResources := make([]sweep.Sweepable, 0)err = describeACLsPages(ctx, conn, input, func(page *memorydb.DescribeACLsOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, v := range page.ACLs {
id := aws.StringValue(v.Name)if id == "open-access" {
...
sweepResources := make([]sweep.Sweepable, 0)err = describeClustersPages(ctx, conn, input, func(page *memorydb.DescribeClustersOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, v := range page.Clusters {
r := ResourceCluster()
d := r.Data(nil)
d.SetId(aws.StringValue(v.Name))sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
})if sweep.SkipSweepError(err) {
...
sweepResources := make([]sweep.Sweepable, 0)err = describeParameterGroupsPages(ctx, conn, input, func(page *memorydb.DescribeParameterGroupsOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, v := range page.ParameterGroups {
id := aws.StringValue(v.Name)if strings.HasPrefix(id, "default.") {
...
sweepResources := make([]sweep.Sweepable, 0)err = describeSnapshotsPages(ctx, conn, input, func(page *memorydb.DescribeSnapshotsOutput, lastPage bool) bool {
if page == nil {
return !lastPage
...
sweepResources := make([]sweep.Sweepable, 0)err = describeSubnetGroupsPages(ctx, conn, input, func(page *memorydb.DescribeSubnetGroupsOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, v := range page.SubnetGroups {
id := aws.StringValue(v.Name)if id == "default" {
...
sweepResources := make([]sweep.Sweepable, 0)err = describeUsersPages(ctx, conn, input, func(page *memorydb.DescribeUsersOutput, lastPage bool) bool {
if page == nil {
return !lastPage
