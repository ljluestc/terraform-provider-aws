...
eckDestroy:testAccCheckUserDestroy(ctx),
...
o := acctest.Provider.Meta().(*conns.AWSClient).MemoryDBConn(ctx)_, e := tfmemorydb.FindUserByName(ctx, conn, rs.Primary.Attributes["user_name"])returnrr
	}
