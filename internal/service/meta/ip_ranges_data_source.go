...
// SPDX-License-Identifier: MPL-2.0package metaimport (
	...
	"sort"
	"strconv"
	"strings"	cleanhttp "github.com/hashicorp/go-cleanhttp"
...
func (d *dataSourceIPRanges) Read(ctx context.Context, request datasource.ReadRequest, response *datasource.ReadResponse) {
	var data dataSourceIPRangesData	response.Diagnostics.Append(request.Config.Get(ctx, &data)...)	if response.Diagnostics.HasError() {
return
	}	var url string	if data.URL.IsNull() {
// Data sources make no use of AttributePlanModifiers to set default values.
url = "https://ip-ranges.amazonaws.com/ip-ranges.json"
	} else {
url = data.URL.ValueString()
	}	bytes, err := readAll(ctx, url)	if err != nil {
response.Diagnostics.AddError("downloading IP ranges", err.Error())return
	}	ipRanges := new(ipRanges)	if err := json.Unmarshal(bytes, ipRanges); err != nil {
response.Diagnostics.AddError("parsing JSON", err.Error())return
	}	syncToken, err := strconv.Atoi(ipRanges.SyncToken)	if err != nil {
response.Diagnostics.AddError("parsing SyncToken", err.Error())return
...
matchService := slices.Contains(services, strings.ToLower(service))return matchRegion && matchService
	}	var ipv4Prefixes []string	for _, v := range ipRanges.IPv4Prefixes {
if matchFilter(v.Region, v.Service) {
ipv4Prefixes = append(ipv4Prefixes, v.Prefix)
}
	}	sort.Strings(ipv4Prefixes)	var ipv6Prefixes []string	for _, v := range ipRanges.IPv6Prefixes {
if matchFilter(v.Region, v.Service) {
ipv6Prefixes = append(ipv6Prefixes, v.Prefix)
}
	}	sort.Strings(ipv6Prefixes)	data.CreateDate = types.StringValue(ipRanges.CreateDate)
	...
	data.URL = types.StringValue(url)	response.Diagnostics.Append(response.State.Set(ctx, &data)...)
	...
	Servicestypes.Set`tfsdk:"services"`
	SyncToken  types.Int64  `tfsdk:"sync_token"`
	URLtypes.String `tfsdk:"url"`
}func readAll(ctx context.Context, url string) ([]byte, error) {
	request, err := http.NewRequestWithContext(ctx, http.MethodGet, url, nil)	if err != nil {
return nil, err
	}	response, err := cleanhttp.DefaultClient().Do(request)	if err != nil {
return nil, fmt.Errorf("HTTP GET (%s): %w", url, err)
	}	defer response.Body.Close()	bytes, err := io.ReadAll(response.Body)	if err != nil {
