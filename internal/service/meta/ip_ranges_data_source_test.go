...
// SPDX-License-Identifier: MPL-2.0package meta_testimport (
	...
	dataSourceName := "data.aws_ip_ranges.test"	resource.ParallelTest(t, resource.TestCase{
	...
	return func(s *terraform.State) error {
		...
		}		cidrBlocks = make([]string, cidrBlockSize)		for i := range cidrBlocks {
			cidrBlock := a[fmt.Sprintf("%s.%d", attribute, i)]			_, _, err := net.ParseCIDR(cidrBlock)
			if err != nil {
