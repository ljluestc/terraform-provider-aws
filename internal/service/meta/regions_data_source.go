...
func (d *dataSourceRegions) Read(ctx context.Context, request datasource.ReadRequest, response *datasource.ReadResponse) {
	var data dataSourceRegionsData	response.Diagnostics.Append(request.Config.Get(ctx, &data)...)	if response.Diagnostics.HasError() {
		return
	}	conn := d.Meta().EC2Conn(ctx)	input := &ec2.DescribeRegionsInput{
		AllRegions: flex.BoolFromFramework(ctx, data.AllRegions),
		Filters:tfec2.BuildCustomFilters(ctx, data.Filters),
	}	output, err := conn.DescribeRegionsWithContext(ctx, input)	if err != nil {
		response.Diagnostics.AddError("reading Regions", err.Error())		return
	}	var names []string
	for _, v := range output.Regions {
		names = append(names, aws.StringValue(v.RegionName))
	}	data.ID = types.StringValue(d.Meta().Partition)
	data.Names = flex.FlattenFrameworkStringValueSetLegacy(ctx, names)	response.Diagnostics.Append(response.State.Set(ctx, &data)...)
