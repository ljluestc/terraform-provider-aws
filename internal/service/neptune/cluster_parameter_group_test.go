...
// SPDX-License-Identifier: MPL-2.0package neptune_testimport (
	...
	var v neptune.DBClusterParameterGroup	parameterGroupName := sdkacctest.RandomWithPrefix("cluster-parameter-group-test-terraform")	resourceName := "aws_neptune_cluster_parameter_group.test"	resource.ParallelTest(t, resource.TestCase{
		PreCheck:acctest.PreCheck(ctx, t) },
		ErrorCheck:orCheck(t, neptune.EndpointsID),
		ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
		CheckDestroy:lusterParameterGroupDestroy(ctx),
	...
	ctx := acctest.Context(t)
	var v neptune.DBClusterParameterGroup	resourceName := "aws_neptune_cluster_parameter_group.test"	resource.ParallelTest(t, resource.TestCase{
		PreCheck:acctest.PreCheck(ctx, t) },
		ErrorCheck:orCheck(t, neptune.EndpointsID),
		ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
		CheckDestroy:lusterParameterGroupDestroy(ctx),
	...
	ctx := acctest.Context(t)
	var v neptune.DBClusterParameterGroup	resourceName := "aws_neptune_cluster_parameter_group.test"	parameterGroupName := sdkacctest.RandomWithPrefix("cluster-parameter-group-test-terraform")	resource.ParallelTest(t, resource.TestCase{
		PreCheck:acctest.PreCheck(ctx, t) },
		ErrorCheck:orCheck(t, neptune.EndpointsID),
		ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
		CheckDestroy:lusterParameterGroupDestroy(ctx),
...
func TestAccNeptuneClusterParameterGroup_NamePrefix_parameter(t *testing.T) {
	ctx := acctest.Context(t)
	var v neptune.DBClusterParameterGroup	resourceName := "aws_neptune_cluster_parameter_group.test"
	prefix := acctest.ResourcePrefix	resource.ParallelTest(t, resource.TestCase{
		PreCheck:acctest.PreCheck(ctx, t) },
		ErrorCheck:orCheck(t, neptune.EndpointsID),
		ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
		CheckDestroy:lusterParameterGroupDestroy(ctx),
	...
	ctx := acctest.Context(t)
	var v neptune.DBClusterParameterGroup	resourceName := "aws_neptune_cluster_parameter_group.test"	parameterGroupName := sdkacctest.RandomWithPrefix("cluster-parameter-group-test-tf")	resource.ParallelTest(t, resource.TestCase{
		PreCheck:acctest.PreCheck(ctx, t) },
		ErrorCheck:orCheck(t, neptune.EndpointsID),
		ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
		CheckDestroy:lusterParameterGroupDestroy(ctx),
	...
	return func(s *terraform.State) error {
		...
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).NeptuneConn(ctx)		opts := neptune.DescribeDBClusterParameterGroupsInput{
			DBClusterParameterGroupName: aws.String(rs.Primary.ID),
		}		resp, err := conn.DescribeDBClusterParameterGroupsWithContext(ctx, &opts)		if err != nil {
			return err
		}		if len(resp.DBClusterParameterGroups) != 1 ||
			aws.StringValue(resp.DBClusterParameterGroups[0].DBClusterParameterGroupName) != rs.Primary.ID {
			return errors.New("Neptune Cluster Parameter Group not found")
		}		*v = *resp.DBClusterParameterGroups[0]		return nil
	}
