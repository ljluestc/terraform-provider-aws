...
Name = %[1]q
  ...
  global_network_id = aws_networkmanager_global_network.test.id  description = "%[1]s-3"  site_id = aws_networkmanager_site.test.id  tags = {
Name = %[1]q
  }  # Create one device at a time.
  depends_on = [aws_networkmanager_device.test2]
}resource "aws_networkmanager_device" "test4" {
  global_network_id = aws_networkmanager_global_network.test.id  description = "%[1]s-4"  site_id = aws_networkmanager_site.test.id  tags = {
Name = %[1]q
  }  # Create one device at a time.
  ...
  connected_device_id = aws_networkmanager_device.test2.id  description = "%[1]s-1"
}resource "aws_networkmanager_connection" "test2" {
  global_network_id= aws_networkmanager_global_network.test.id
  device_id  = aws_networkmanager_device.test3.id
  connected_device_id = aws_networkmanager_device.test4.id  description = "%[1]s-2"  tags = {
Name = %[1]q
  }  # Create one connection at a time.
  depends_on = [aws_networkmanager_connection.test1]
}data "aws_networkmanager_connections" "all" {
  global_network_id = aws_networkmanager_global_network.test.id  depends_on = [aws_networkmanager_connection.test1, aws_networkmanager_connection.test2]
