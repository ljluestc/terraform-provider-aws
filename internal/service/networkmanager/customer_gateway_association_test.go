...
isappears":sociation_disappears,
isappears_CustomerGateway": testAccCustomerGatewayAssociation_Disappears_customerGateway,
...
eckDestroy:testAccCheckCustomerGatewayAssociationDestroy(ctx),
...
continue
}globalNetworkID, customerGatewayARN, err := tfnetworkmanager.CustomerGatewayAssociationParseResourceID(rs.Primary.ID)if err != nil {
return err
}_, err = tfnetworkmanager.FindCustomerGatewayAssociationByTwoPartKey(ctx, conn, globalNetworkID, customerGatewayARN)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
o := acctest.Provider.Meta().(*conns.AWSClient).NetworkManagerConn(ctx)globNetworkID, customerGatewayARN, err := tfnetworkmanager.CustomerGatewayAssociationParseResourceID(rs.Primary.ID)if err= nil {
return err
,rr = tfnetworkmanager.FindCustomerGatewayAssociationByTwoPartKey(ctx, conn, globalNetworkID, customerGatewayARN)retu err
	}
...
Name = %[1]q
  ...
  site_id  = aws_networkmanager_site.test.id  tags = {
Name = %[1]q
  }
...
Name = %[1]q
  ...
  static_routes_only  = true  tags = {
Name = %[1]q
  }
}resource "aws_networkmanager_transit_gateway_registration" "test" {
  global_network_id= aws_networkmanager_global_network.test.id
  transit_gateway_arn = aws_ec2_transit_gateway.test.arn  depends_on = [aws_vpn_connection.test]
}resource "aws_networkmanager_customer_gateway_association" "test" {
  global_network_id= aws_networkmanager_global_network.test.id
  customer_gateway_arn = aws_customer_gateway.test.arn
  device_id= aws_networkmanager_device.test.id  depends_on = [aws_networkmanager_transit_gateway_registration.test]
}
`, rName)
}
