...
// SPDX-License-Identifier: MPL-2.0package networkmanager_testimport (
	...
	resourceName := "aws_networkmanager_global_network.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck:k(ctx, t) },
rorCheck:acctest.ErrorCheck(t, networkmanager.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckGlobalNetworkDestroy(ctx),
...
continue
}_, err := tfnetworkmanager.FindGlobalNetworkByID(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
o := acctest.Provider.Meta().(*conns.AWSClient).NetworkManagerConn(ctx)_, e := tfnetworkmanager.FindGlobalNetworkByID(ctx, conn, rs.Primary.ID)returnrr
	}
