...
eckDestroy:testAccCheckSiteToSiteVPNAttachmentDestroy(ctx),
...
o := acctest.Provider.Meta().(*conns.AWSClient).NetworkManagerConn(ctx)outp, err := tfnetworkmanager.FindSiteToSiteVPNAttachmentByID(ctx, conn, rs.Primary.ID)if err= nil {
return err
v *outputretu nil
	}
...
continue
}_, err := tfnetworkmanager.FindSiteToSiteVPNAttachmentByID(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
data "aws_region" "current" {}resource "aws_customer_gateway" "test" {
  bgp_asn= %[2]d
  ip_address  = %[3]q
  type
  device_name = %[1]q  tags = {
Name = %[1]q
  }
...
resource "aws_networkmanager_site_to_site_vpn_attachment" "test" {
  ...
  vpn_connection_arn = aws_vpn_connection.test.arn  tags = {
segment = "shared"
  }
