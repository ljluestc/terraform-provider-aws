...
)// []*SERVICE.Tag handling// Tags returns networkmanager service tags.
func Tags(tags tftags.KeyValueTags) []*networkmanager.Tag {
	result := make([]*networkmanager.Tag, 0, len(tags))	for k, v := range tags.Map() {
...
put := &networkmanager.UntagResourceInput{
...
put := &networkmanager.TagResourceInput{
ResourceArn: aws.String(identifier),
Tags:
