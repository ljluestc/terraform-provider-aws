...
eckDestroy:testAccCheckTransitGatewayPeeringDestroy(ctx),
...
o := acctest.Provider.Meta().(*conns.AWSClient).NetworkManagerConn(ctx)outp, err := tfnetworkmanager.FindTransitGatewayPeeringByID(ctx, conn, rs.Primary.ID)if err= nil {
return err
v *outputretu nil
	}
...
continue
}_, err := tfnetworkmanager.FindTransitGatewayPeeringByID(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
ern nil
	...
	return fmt.Sprintf(`
data "aws_region" "current" {}resource "aws_ec2_transit_gateway" "test" {
  tags = {
Name = %[1]q
  }
}resource "aws_ec2_transit_gateway_policy_table" "test" {
  transit_gateway_id = aws_ec2_transit_gateway.test.id  tags = {
Name = %[1]q
  }
...
asn_ranges = ["65022-65534"]edge_locations {
...
resource "aws_networkmanager_transit_gateway_peering" "test" {
  core_network_id= aws_networkmanager_core_network.test.id
  transit_gateway_arn = aws_ec2_transit_gateway.test.arn  depends_on = [aws_ec2_transit_gateway_policy_table.test]
}
`)
...
resource "aws_networkmanager_transit_gateway_peering" "test" {
  core_network_id= aws_networkmanager_core_network.test.id
  transit_gateway_arn = aws_ec2_transit_gateway.test.arn  tags = {
%[1]q = %[2]q
