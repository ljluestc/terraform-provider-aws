...
eckDestroy:testAccCheckVPCAttachmentDestroy(ctx),
...
o := acctest.Provider.Meta().(*conns.AWSClient).NetworkManagerConn(ctx)outp, err := tfnetworkmanager.FindVPCAttachmentByID(ctx, conn, rs.Primary.ID)if err= nil {
return err
v *outputretu nil
	}
...
continue
}_, err := tfnetworkmanager.FindVPCAttachmentByID(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
data "aws_region" "current" {}resource "aws_vpc" "test" {
...
Name = %[1]q
  ...
  cidr_blockt.cidr_block, 8, count.index)  ipv6_cidr_block= cidrsu_vpc.tesidr_blocunt.index)
...
resource "aws_networkmanager_vpc_attachment" "test" {
  ...
  core_network_id = aws_networkmanager_core_network_policy_attachment.test.core_network_id
  vpc_arn= aws_vpc.test.arn  options {
