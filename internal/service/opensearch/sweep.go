...
// +build sweeppackage opensearchimport (
	"fmt"
...
func sweepDomains(region string) error {
	...
	var errs *multierror.Error	input := &opensearchservice.ListDomainNamesInput{}	// ListDomainNames has no pagination support whatsoever
	output, err := conn.ListDomainNamesWithContext(ctx, input)	if sweep.SkipSweepError(err) {
		...
		if domainInfo == nil {
			continue
		}		name := aws.StringValue(domainInfo.DomainName)		// OpenSearch Domains have regularly gotten stuck in a "being deleted" state
		// e.g. Deleted and Processing are both true for days in the API
		// Filter out domains that are Deleted already.		output, err := FindDomainByName(ctx, conn, name)
		if err != nil {
		...
		d := r.Data(nil)
		d.SetId(name)
		d.Set("domain_name", name)		sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
