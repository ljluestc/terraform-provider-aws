...
resourceName := "aws_opsworks_custom_layer.test"resource.ParallelTest(t, resource.TestCase{
...
logGroupResourceName := "aws_cloudwatch_log_group.test"resource.ParallelTest(t, resource.TestCase{
PreCheck: func() { acctest.PreCheck(ctx, t); acctest.PreCheckPartitionHasService(t, opsworks.EndpointsID) },
ErrorCheck:acctest.ErrorCheck(t, opsworks.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckCustomLayerDestroy(ctx),
...
func testAccCheckCustomLayerDestroy(ctx context.Context) resource.TestCheck
func {
returnfunc(s *terraform.State) error { return testAccCheckLayerDestroy(ctx, "aws_opsworks_custom_layer", s) }
}
...
resource "aws_opsworks_custom_layer" "test" {
  ...
  auto_assign_public_ips = false  custom_security_group_ids = aws_security_group.test[*].id  drain_elb_on_shutdown= true
  instance_shutdown_timeout = 300  system_packages = [
"git",
"golang",
  ]  ebs_volume {
type= "gp2"
...
Name = %[1]q
  ...
  name= %[1]q
  short_name= "tf-ops-acc-custom-layer"
  auto_assign_public_ips = true  custom_security_group_ids = concat(aws_security_group.test[*].id, [aws_security_group.extra.id])  drain_elb_on_shutdown= false
  instance_shutdown_timeout = 120  system_packages = [
"git",
"golang",
"subversion",
  ]  ebs_volume {
...
resource "aws_cloudwatch_log_group" "test" {
  ...
  instance_shutdown_timeout = 300  cloudwatch_configuration {
enabled = %[2]tlog_streams {
