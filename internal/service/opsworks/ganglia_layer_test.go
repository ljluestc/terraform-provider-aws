...
resourceName := "aws_opsworks_ganglia_layer.test"resource.ParallelTest(t, resource.TestCase{
...
func testAccCheckGangliaLayerDestroy(ctx context.Context) resource.TestCheck
func {
returnfunc(s *terraform.State) error { return testAccCheckLayerDestroy(ctx, "aws_opsworks_ganglia_layer", s) }
}
...
resource "aws_opsworks_ganglia_layer" "test" {
  ...
  password = "avoid-plaintext-passwords"  custom_security_group_ids = aws_security_group.test[*].id
}
`)
}
