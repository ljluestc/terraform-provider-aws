...
"context"
"fmt""github.com/aws/aws-sdk-go/service/opsworks"
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).OpsWorksConn(ctx)output, err := tfopsworks.FindLayerByID(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*v = *outputreturn nil
}
}
...
continue
}_, err := tfopsworks.FindLayerByID(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
resource "aws_opsworks_stack" "test" {
  ...
  count = 2  provider = "awsalternate"  name= "%[1]s-${count.index}"
  vpc_id = aws_vpc.test.id  ingress {
from_port= 8
to_port= -1
protocol= "icmp"
