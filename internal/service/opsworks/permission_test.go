...
var opsperm opsworks.Permissionresource.ParallelTest(t, resource.TestCase{
...
}// Reference: https://github.com/hashicorp/terraform-provider-aws/issues/4804func TestAccOpsWorksPermission_self(t *testing.T) {
ctx := acctest.Context(t)
var opsperm opsworks.Permission
rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName := "aws_opsworks_permission.test"resource.ParallelTest(t, resource.TestCase{
PreCheck: func() { acctest.PreCheck(ctx, t); acctest.PreCheckPartitionHasService(t, opsworks.EndpointsID) },
ErrorCheck:acctest.ErrorCheck(t, opsworks.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:ctest.CheckDestroyNoop,
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).OpsWorksConn(ctx)output, err := tfopsworks.FindPermissionByTwoPartKey(ctx, conn, rs.Primary.Attributes["user_arn"], rs.Primary.Attributes["stack_id"])if err != nil {
return err
}*v = *outputreturn nil
}
}
...
resource "aws_opsworks_permission" "test" {
  stack_id = aws_opsworks_stack.test.id  allow_ssh  = %[1]t
  allow_sudo = %[2]t
...
testAccStackConfig_vpcCreate(rName),
fmt.Sprintf(`
data "aws_caller_identity" "current" {}resource "aws_opsworks_permission" "test" {
  allow_ssh  = %[1]t
  allow_sudo = %[2]t
