...
}conn := acctest.Provider.Meta().(*conns.AWSClient).OpsWorksConn(ctx)output, err := tfopsworks.FindRDSDBInstanceByTwoPartKey(ctx, conn, rs.Primary.Attributes["rds_db_instance_arn"], rs.Primary.Attributes["stack_id"])if err != nil {
return err
}*v = *outputreturn nil
}
}
...
continue
}_, err := tfopsworks.FindRDSDBInstanceByTwoPartKey(ctx, conn, rs.Primary.Attributes["rds_db_instance_arn"], rs.Primary.Attributes["stack_id"])if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
data "aws_rds_engine_version" "default" {
  ...
  stack_id = aws_opsworks_stack.test.id  rds_db_instance_arn = aws_db_instance.test.arn
  db_user= %[2]q
  db_password= %[3]q
}
