...
// +build sweeppackage opsworksimport (
"context"
"fmt"
"log"
"strings"
...
sweepResources := make([]sweep.Sweepable, 0)output, err := conn.DescribeStacksWithContext(ctx, &opsworks.DescribeStacksInput{})if err != nil {
...
}var sweeperErrs *multierror.Errorfor _, stack := range output.Stacks {
input := &opsworks.DescribeAppsInput{
StackId: stack.StackId,
}appOutput, err := conn.DescribeAppsWithContext(ctx, input)if err != nil {
...
d := r.Data(nil)
d.SetId(aws.StringValue(app.AppId))sweepResources = append(sweepResources, sdk.NewSweepResource(r, d, client))
}
...
StackId: stack.StackId,
}instanceOutput, err := conn.DescribeInstancesWithContext(ctx, input)if err != nil {
...
d.Set("status", instance.Status)sweepResources = append(sweepResources, sdk.NewSweepResource(r, d, client))
}
}return sweep.SweepOrchestrator(ctx, sweepResources)
}
...
StackId: stack.StackId,
}dbInstOutput, err := conn.DescribeRdsDbInstancesWithContext(ctx, input)if err != nil {
...
d.Set("rds_db_instance_arn", dbInstance.RdsDbInstanceArn)sweepResources = append(sweepResources, sdk.NewSweepResource(r, d, client))
}
}return sweep.SweepOrchestrator(ctx, sweepResources)
}
...
d := r.Data(nil)
d.SetId(aws.StringValue(stack.StackId))if aws.StringValue(stack.VpcId) != "" {
...
StackId: stack.StackId,
}layerOutput, err := conn.DescribeLayersWithContext(ctx, input)if err != nil {
...
r := l.resourceSchema()
d := r.Data(nil)
d.SetId(aws.StringValue(layer.LayerId))if layer.Attributes != nil {
...
sweepResources := make([]sweep.Sweepable, 0)output, err := conn.DescribeUserProfilesWithContext(ctx, &opsworks.DescribeUserProfilesInput{})if err != nil {
if sweep.SkipSweepError(err) {
