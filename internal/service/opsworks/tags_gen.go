...
// it may also be a different identifier depending on the service.func listTags(ctx context.Context, conn opsworksiface.OpsWorksAPI, identifier string) (tftags.KeyValueTags, error) {
input := &opsworks.ListTagsInput{
ResourceArn: aws.String(identifier),
}output, err := conn.ListTagsWithContext(ctx, input)if err != nil {
...
tags, err := listTags(ctx, meta.(*conns.AWSClient).OpsWorksConn(ctx), identifier)if err != nil {
return err
}if inContext, ok := tftags.FromContext(ctx); ok {
inContext.TagsOut = types.Some(tags)
}return nil
}// map[string]*string handling// Tags returns opsworks service tags.func Tags(tags tftags.KeyValueTags) map[string]*string {
return aws.StringMap(tags.Map())
}// KeyValueTags creates tftags.KeyValueTags from opsworks service tags.func KeyValueTags(ctx context.Context, tags map[string]*string) tftags.KeyValueTags {
...
}// setTagsOut sets opsworks service tags in Context.func setTagsOut(ctx context.Context, tags map[string]*string) {
if inContext, ok := tftags.FromContext(ctx); ok {
inContext.TagsOut = types.Some(KeyValueTags(ctx, tags))
}
}// createTags creates opsworks service tags for new resources.func createTags(ctx context.Context, conn opsworksiface.OpsWorksAPI, identifier string, tags map[string]*string) error {
if len(tags) == 0 {
return nil
}return updateTags(ctx, conn, identifier, nil, tags)
}// updateTags updates opsworks service tags.
// The identifier is typically the Amazon Resource Name (ARN), although
// it may also be a different identifier depending on the service.func updateTags(ctx context.Context, conn opsworksiface.OpsWorksAPI, identifier string, oldTagsMap, newTagsMap any) error {
