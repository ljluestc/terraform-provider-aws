...
resourceName := "aws_opsworks_user_profile.test"resource.ParallelTest(t, resource.TestCase{
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).OpsWorksConn(ctx)_, err := tfopsworks.FindUserProfileByARN(ctx, conn, rs.Primary.ID)return err
}
}
...
continue
}_, err := tfopsworks.FindUserProfileByARN(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
