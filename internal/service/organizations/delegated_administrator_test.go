...
// SPDX-License-Identifier: MPL-2.0package organizations_testimport (
	...
	dataSourceIdentity := "data.aws_caller_identity.delegated"	resource.Test(t, resource.TestCase{
PreCheck:func() {
...
continue
}accountID, servicePrincipal, err := tforganizations.DelegatedAdministratorParseResourceID(rs.Primary.ID)if err != nil {
return err
}_, err = tforganizations.FindDelegatedAdministratorByTwoPartKey(ctx, conn, accountID, servicePrincipal)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
return err
}conn := acctest.Provider.Meta().(*conns.AWSClient).OrganizationsConn(ctx)output, err := tforganizations.FindDelegatedAdministratorByTwoPartKey(ctx, conn, accountID, servicePrincipal)if err != nil {
return err
}*v = *outputreturn nil
	}
