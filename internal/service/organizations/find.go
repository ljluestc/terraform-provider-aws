...
}output, err := conn.DescribeAccountWithContext(ctx, input)if tfawserr.ErrCodeEquals(err, organizations.ErrCodeAccountNotFoundException) {
...
var output *organizations.PolicyTargetSummaryerr := conn.ListTargetsForPolicyPagesWithContext(ctx, input,func(page *organizations.ListTargetsForPolicyOutput, lastPage bool) bool {
if page == nil {
return !lastPage
