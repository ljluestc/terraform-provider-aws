...
resourceName := "aws_qldb_ledger.test"resource.ParallelTest(t, resource.TestCase{
...
continue
}_, err := tfqldb.FindLedgerByName(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).QLDBClient(ctx)output, err := tfqldb.FindLedgerByName(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*v = *outputreturn nil
}
