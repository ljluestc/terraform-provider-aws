...
if !ok {
return nil
}sourceEntity := &quicksight.AnalysisSourceEntity{}if v, ok := tfMap["source_template"].([]interface{}); ok && len(v) > 0 {
...
if !ok {
return nil
}definition := &quicksight.AnalysisDefinition{}if v, ok := tfMap["analysis_defaults"].([]interface{}); ok && len(v) > 0 {
