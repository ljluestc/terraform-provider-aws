...
if !ok {
return nil
}visual := &quicksight.ComboChartVisual{}if v, ok := tfMap["visual_id"].(string); ok && v != "" {
...
if !ok {
return nil
}config := &quicksight.ComboChartConfiguration{}if v, ok := tfMap["bars_arrangement"].(string); ok && v != "" {
...
if !ok {
return nil
}config := &quicksight.ComboChartFieldWells{}if v, ok := tfMap["combo_chart_aggregated_field_wells"].([]interface{}); ok && len(v) > 0 {
...
if !ok {
return nil
}config := &quicksight.ComboChartAggregatedFieldWells{}if v, ok := tfMap["bar_values"].([]interface{}); ok && len(v) > 0 {
...
if !ok {
return nil
}config := &quicksight.ComboChartSortConfiguration{}if v, ok := tfMap["category_items_limit"].([]interface{}); ok && len(v) > 0 {
