...
if !ok {
return nil
}visual := &quicksight.ScatterPlotVisual{}if v, ok := tfMap["visual_id"].(string); ok && v != "" {
...
if !ok {
return nil
}config := &quicksight.ScatterPlotConfiguration{}if v, ok := tfMap["data_labels"].([]interface{}); ok && len(v) > 0 {
...
if !ok {
return nil
}config := &quicksight.ScatterPlotFieldWells{}if v, ok := tfMap["scatter_plot_categorically_aggregated_field_wells"].([]interface{}); ok && len(v) > 0 {
...
if !ok {
return nil
}config := &quicksight.ScatterPlotCategoricallyAggregatedFieldWells{}if v, ok := tfMap["category"].([]interface{}); ok && len(v) > 0 {
...
if !ok {
return nil
}config := &quicksight.ScatterPlotUnaggregatedFieldWells{}if v, ok := tfMap["size"].([]interface{}); ok && len(v) > 0 {
config.Size = expandMeasureFields(v)
}
if v, ok := tfMap["x_axis"].([]interface{}); ok && len(v) > 0 {
