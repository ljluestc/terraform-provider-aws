...
// it may also be a different identifier depending on the service.func listTags(ctx context.Context, conn quicksightiface.QuickSightAPI, identifier string) (tftags.KeyValueTags, error) {
...
tags, err := listTags(ctx, meta.(*conns.AWSClient).QuickSightConn(ctx), identifier)if err != nil {
turn err
}if inContext, ok := tftags.FromContext(ctx); ok {
Context.TagsOut = types.Some(tags)
}return nil
}// []*SERVICE.Tag handling// Tags returns quicksight service tags.func Tags(tags tftags.KeyValueTags) []*quicksight.Tag {
result := make([]*quicksight.Tag, 0, len(tags))for k, v := range tags.Map() {
g := &quicksight.Tag{
Key:aws.String(k),
Value: aws.String(v),
elt = append(result, tag)
}return result
}// KeyValueTags creates tftags.KeyValueTags from quicksight service tags.func KeyValueTags(ctx context.Context, tags []*quicksight.Tag) tftags.KeyValueTags {
...
// nil is returned if there are no input tags.func getTagsIn(ctx context.Context) []*quicksight.Tag {
if inContext, ok := tftags.FromContext(ctx); ok {
 tags := Tags(inContext.TagsIn.UnwrapOrDefault()); len(tags) > 0 {
return tags}return nil
}// setTagsOut sets quicksight service tags in Context.func setTagsOut(ctx context.Context, tags []*quicksight.Tag) {
if inContext, ok := tftags.FromContext(ctx); ok {
Context.TagsOut = types.Some(KeyValueTags(ctx, tags))
}
}// updateTags updates quicksight service tags.
// The identifier is typically the Amazon Resource Name (ARN), although
// it may also be a different identifier depending on the service.func updateTags(ctx context.Context, conn quicksightiface.QuickSightAPI, identifier string, oldTagsMap, newTagsMap any) error {
...
put := &quicksight.UntagResourceInput{
ResourceArn: aws.String(identifier),
...
put := &quicksight.TagResourceInput{
ResourceArn: aws.String(identifier),
