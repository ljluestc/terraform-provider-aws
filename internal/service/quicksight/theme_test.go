...
funcTestAccQuickSightTheme_basic(t*testing.T){
	ctx:=acctest.Context(t)

	varthemequicksight.Theme
...
funcTestAccQuickSightTheme_disappears(t*testing.T){
	ctx:=acctest.Context(t)
...
funcTestAccQuickSightTheme_fullConfig(t*testing.T){
	ctx:=acctest.Context(t)
...
funcTestAccQuickSightTheme_update(t*testing.T){
	ctx:=acctest.Context(t)
...
functestAccCheckThemeDestroy(ctxcontext.Context)resource.TestCheck
func{
	return
...
for_,rs:=ranges.RootModule().Resources{
	ifrs.Type!="aws_quicksight_theme"{
continue
	}
...
returnerr
	...
	ifoutput!=nil{
returnfmt.Errorf("QuickSightTheme(%s)stillexists",rs.Primary.ID)
	}
}
...
functestAccCheckThemeExists(ctxcontext.Context,namestring,theme*quicksight.Theme)resource.TestCheck
func{
	return
func(s*terraform.State)error{
rs,ok:=s.RootModule().Resources[name]
if!ok{
	returncreate.Error(names.QuickSight,create.ErrActionCheckingExistence,tfquicksight.ResNameTheme,name,errors.New("notfound"))
}

ifrs.Primary.ID==""{
	returncreate.Error(names.QuickSight,create.ErrActionCheckingExistence,tfquicksight.ResNameTheme,name,errors.New("notset"))
}
...
iferr!=nil{
	returncreate.Error(names.QuickSight,create.ErrActionCheckingExistence,tfquicksight.ResNameTheme,rs.Primary.ID,err)
}
...
functestAccThemeConfig_basic(rId,rName,baseThemIdstring)string{
...
functestAccThemeConfig_fullConfig(rId,rName,baseThemIdstring)string{
	returnacctest.ConfigCompose(
fmt.Sprintf(`
resource"aws_quicksight_theme""test"{
theme_id=%[1]q
...
functestAccThemeConfig_update(rId,rName,baseThemId,emptyFillColorstring)string{
	returnacctest.ConfigCompose(
fmt.Sprintf(`
resource"aws_quicksight_theme""test"{
theme_id=%[1]q
