...
)type cleanupWaiterFunc 
func
type cleanupWaiterErrFunc 
func(context.Context, ...tfresource.OptionsFunc) error //nolint:unused // WIP
func blueGreenOrchestrator struct {
...
func := createOut.BlueGreenDeployment
	return dep, nil
}
...
func(err error) (bool, error) {
	...
	i
functurn nil, fmt.Errorf("switching over Blue/Green Deployment: %s", err)
	}	d
funcerr != nil {
...
funcInstanceIdentifier: aws.String(d.Get("identifier").(string)),
	}	// Backups must be enabled for Blue/Green Deployments. Enable them first.
	o, n := d.GetChange("backup_retention_period")
	if o.(int) == 0 && n.(int) > 0 {
funcput.BackupRetentionPeriod = aws.Int32(int32(d.Get("backup_retention_period").(int)))
...
func (h *instanceHandler) modifyTarget(ctx context.Context, identifier string, d *schema.ResourceData, timeout time.Duration, operation string) error {
	...
	}	needsModify := dbInstancePopulateModify(modifyInput, d)	if needsModify {
		log.Printf("[DEBUG] %s: Updating Green environment", operation)		err := dbInstanceModify(ctx, h.conn, d.Id(), modifyInput, timeout)
funceturn fmt.Errorf("updating Green environment: %s", err)
		}
	}	return nil
}
