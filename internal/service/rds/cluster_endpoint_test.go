...
func := acctest.Context(t)
	...
	defaultResourceName := "aws_rds_cluster_endpoint.default"	resource.ParallelTest(t, resource.TestCase{
		PreCheck: 
...
funcSkip("skipping long-running test in short mode")
	...
	var customReaderEndpoint rds.DBClusterEndpoint
	resourceName := "aws_rds_cluster_endpoint.reader"	resource.ParallelTest(t, resource.TestCase{
		PreCheck: 
func() { acctest.PreCheck(ctx, t) },
...
func
		if aws
funcws.StringValue(v.CustomEndpointType) != "ANY" {
...
func
func, err := tfrds.FindDBClusterEndpointByID(ctx, conn, rs.Primary.ID)			if tfresource.NotFound(err) {
				continue
			}			if err != nil {
				return err
...
func
funcnn := acctest.Provider.Meta().(*conns.AWSClient).RDSConn(ctx)		output, err := tfrds.FindDBClusterEndpointByID(ctx, conn, rs.Primary.ID)
		if err != nil {
			return err
		}		*v = *output		return nil
	}
...
funcuster_identifier = "tf-aurora-cluster-%[1]d"
availability_zones = [
...
custom_endpoint_type= "READER"static_members = [aws_rds_cluster_instance.test2.id]
}
funcurce "aws_rds_cluster_endpoint" "default" {
cluster_identifier = aws_rds_cluster.default.id
cluster_endpoint_identifier = "default-%[1]d"
custom_endpoint_type= "ANY"excluded_members = [aws_rds_cluster_instance.test2.id]
}
`, n))
}
...
cluster_identifier = aws_rds_cluster.default.id
cluster_endpoint_identifier = "reader-%[1]d"
custom_endpoint_type= "READER"static_members = [aws_rds_cluster_instance.test2.id]tags = {
func
}
`, n, tagKey1, tagValue1))
}
