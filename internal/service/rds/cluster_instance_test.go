...
t.Skip("skipping long-running test in short mode")
	...
	resourceName := "aws_rds_cluster_instance.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
...
Steps: []resource.TestStep{
	{
funcig: testAccClusterInstanceConfig_tags1(rName, "key1", "value1"),
Check: resource.ComposeAggregateTestCheckFunc(
	testAccCheckClusterInstanceExists(ctx, resourceName, &v),
...
Config: testAccClusterInstanceConfig_basic(rName),
Check: resour
functAccCheckClusterInstanceExists(ctx, resourceName, &v),
),
	},
	{
PreConfig: 
...
SkipFinalSnapshot: aws.Bool(true),
	...
	if err != 
functalf("error deleting Database Instance: %s", err)
	}
},
Config:testAccClusterInstanceConfig_basic(rName),
Destroy: true,
...
func() { acctest.PreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, rds.EndpointsID),
funckDestroy:testAccCheckClusterInstanceDestroy(ctx),
Steps: []resource.TestStep{
	{
Config: testAccClusterInstanceConfig_identifierGenerated(rName),
Check: resource.ComposeTestCheckFunc(
	testAccCheckClusterInstanceExists(ctx, resourceName, &v),
	resource.TestCheckResourceAttrWith(resourceName, "identifier", 
...
funcource.TestCheckNoResourceAttr(resourceName, "identifier_prefix"),
),
	},
	{
ResourceName:resourceName,
...
ImportStateVerifyIgnore: []string{
	...
	{
funcig: testAccClusterInstanceConfig_copyTagsToSnapshot(rName, false),
Check: resource.ComposeTestCheckFunc(
	testAccCheckClusterInstanceExists(ctx, resourceName, &v),
	resource.TestCheckResourceAttr(resourceName, "copy_tags_to_snapshot", "false"),
),
...
t.Skip("skipping long-running test in short mode")
	...
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_rds_cluster_instance.test"
	certificateDataSourceName := "data.aws_rds_certificate.latest"	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
func() { acctest.PreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, rds.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
t.Skip("skipping long-running test in short mode")
	...
	resourceName := "aws_rds_cluster_instance.test"
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	engine := "aurora-mysql"	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
func() { acctest.PreCheck(ctx, t); testAccPerformanceInsightsDefaultVersionPreCheck(ctx, t, engine) },
ErrorCheck:acctest.ErrorCheck(t, rds.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckClusterInstanceDestroy(ctx),
Steps: []resource.TestStep{
...
t.Skip("skipping long-running test in short mode")
	...
	resourceName := "aws_rds_cluster_instance.test"
	rName := sdk
funcine := "aurora-postgresql"	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
func() { acctest.PreCheck(ctx, t); testAccPerformanceInsightsDefaultVersionPreCheck(ctx, t, engine) },
ErrorCheck:acctest.ErrorCheck(t, rds.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
t.Skip("skipping long-running test in short mode")
	...
	resourceName := "aws_rds_cluster_instance.test"
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	engine := "aurora-mysql"	resource.Par
funcheck: 
func() { acctest.PreCheck(ctx, t); testAccPerformanceInsightsDefaultVersionPreCheck(ctx, t, engine) },
ErrorCheck:acctest.ErrorCheck(t, rds.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
t.Skip("skipping long-running test in short mode")
	...
	resourceName := "aws_rds_cluster_instance.test"
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)	resource.Par
funcheck: 
func() {
...
Check: resource.ComposeTestCheckFunc(
	testAccCheckClusterInstanceExists(ctx, resourceName, &v),
funcource.TestCheckResourceAttr(resourceName, "performance_insights_retention_period", "7"),
),
	},
	{
...
t.Skip("skipping long-running test in short mode")
	...
	resourceName := "aws_rds_cluster_instance.test"
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	engine := "aurora-postgresql"	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
func() { acctest.PreCheck(ctx, t); testAccPerformanceInsightsDefaultVersionPreCheck(ctx, t, engine) },
ErrorCheck:acctest.ErrorCheck(t, rds.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
Steps: []resource.TestStep{
	{
funcig: testAccClusterInstanceConfig_performanceInsightsEnabledAuroraPostgresql(rName, engine),
Check: resource.ComposeTestCheckFunc(
	testAccCheckClusterInstanceExists(ctx, resourceName, &v),
	resource.TestCheckResourceAttr(resourceName, "performance_insights_enabled", "true"),
),
...
functAccPerformanceInsightsPreCheck(ctx, t, engine, aws.StringValue(result.DBEngineVersions[0].EngineVersion))
}
func testAccPerformanceInsightsPreCheck(ctx context.Context, t *testing.T, engine string, engineVersion string) {
	conn := acctest.Provider.Meta().(*conns.AWSClient).RDSConn(ctx)	input := &rds.DescribeOrderableDBInstanceOptionsInput{
Engine:aws.String(engine),
EngineVersion: aws.String(engineVersion), // version cuts response time
	}	supportsPerformanceInsights := false
	err := conn.DescribeOrderableDBInstanceOptionsPagesWithContext(ctx, input, 
...
continue
	}	_, err := tfrds.FindDBInstanceByID(ctx, conn, rs.Primary.ID)	if tfresource.NotFound(err) {
continue
	}	if err != nil {
return err
...
name= %[1]q
family = data.aws_rds_engine_version.default.parameter_group_familyparameter {
 name= "back_log"
 value= "32767"
...
engine= data.aws_rds_engine_version.default.engine
cluster_identifier= aws_rds_cluster.test.id
func_parameter_group_name = aws_db_parameter_group.test.name
promotion_tier = "3"tags = {
 %[2]q = %[3]q
}
}resource "aws_db_parameter_group" "test" {
name= %[1]q
...
funcurn acctest.ConfigCompose(testAccClusterInstanceConfig_base(rName, "aurora-mysql"), fmt.Sprintf(`
resource "aws_rds_cluster_instance" "test" {
identifier = %[1]q
engine= data.aws_rds_engine_version.default.engine
cluster_identifier= aws_rds_cluster.test.id
...
cluster_identifier = %[1]q
availability_zones = [
funcdata.aws_availability_zones.available.names[1],
 data.aws_availability_zones.available.names[2]
]
database_name = "mydb"
engine = data.aws_rds_engine_version.default.engine
...
depends_on = [aws_iam_role_policy_attachment.test]cluster_identifier= aws_rds_cluster.test.id
engine = data.aws_rds_engine_version.default.engine
...
monitoring_role_arn = aws_iam_role.test.arndepends_on = [aws_iam_role_policy_attachment.test]
}
`, rName))
}
...
funcurce "aws_rds_cluster_instance" "test" {
cluster_identifier = aws_rds_cluster.test.id
engine= aws_rds_cluster.test.engine
identifier= %[1]q
instance_class= data.aws_rds_orderable_db_instance.test.instance_class
