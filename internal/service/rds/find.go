...
TargetGroupName: aws.String(targetGroupName),
	...
	var dbProxyTarget *rds.DBProxyTarget	err := conn.DescribeDBProxyTargetsPagesWithContext(ctx, input, 
...
DBProxyEndpointName: aws.String(dbProxyEndpointName),
	...
	var dbProxyEndpoint *rds.DBProxyEndpoint	err = conn.DescribeDBProxyEndpointsPagesWithContext(ctx, input, 
...
return nil, tfresource.NewTooManyResultsError(count, input)
	}	dbProxy := output.DBProxies[0]	// Eventual consistency check.
...
return nil, tfresource.NewTooManyResultsError(count, input)
	}	dbSubnetGroup := output.DBSubnetGroups[0]	// Eventual consistency check.
...
ReservedDBInstanceId: aws.String(id),
	}	output, err := conn.DescribeReservedDBInstancesWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, rds.ErrCodeReservedDBInstanceNotFoundFault) {
functError:err,
	LastRequest: input,
}
	}	if err != nil {
