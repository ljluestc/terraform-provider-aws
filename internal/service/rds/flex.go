...
functfMap == nil {
		return nil
	}	apiObject := &rds.ScalingConfiguration{}	if v, ok := tfMap["auto_pause"].(bool); ok {
...
func flattenScalingConfigurationInfo(apiObject *rds.ScalingConfigurationInfo) map[string]interface{} {
	if apiObject == nil {
func	tfMap := map[string]interface{}{}	if v := apiObject.AutoPause; v != nil {
...
func expandServerlessV2ScalingConfiguration(tfMap map[string]interface{}) *rds.ServerlessV2ScalingConfiguration {
	if tfMap == nil {
		return nil
func
	apiObject := &rds.ServerlessV2ScalingConfiguration{}	if v, ok := tfMap["max_capacity"].(float64); ok && v != 0.0 {
...
funcap := map[string]interface{}{}	if v := apiObject.MaxCapacity; v != nil {
...
func expandOptionConfiguration(configured []interface{}) []*rds.OptionConfiguration {
	var option []*rds.OptionConfiguration	for _, pRaw := range configured {
...
func flattenOptions(apiOptions []*rds.Option, optionConfigurations []*rds.OptionConfiguration) []map[string]interface{} {
	result := make([]map[string]interface{}, 0)	for _, apiOption := range apiOptions {
		if apiOption == nil || apiOption.OptionName == nil {
			continue
func
		var configuredOption *rds.OptionConfiguration		for _, optionConfiguration := range optionConfigurations {
		...
		for _, apiOptionSetting := range apiOption.OptionSettings {
			...
			var configuredOptionSetting *rds.OptionSetting			if configuredOption != nil {
...
func expandOptionSetting(list []interface{}) []*rds.OptionSetting {
	options := make([]*rds.OptionSetting, 0, len(list))	for _, oRaw := range list {
		data := oRaw.(map[string]interface{})		o := &rds.OptionSetting{
...
func expandParameters(configured []interface{}) []*rds.Parameter {
	var parameters []*rds.Parameter	// Loop over our configured parameters and create
