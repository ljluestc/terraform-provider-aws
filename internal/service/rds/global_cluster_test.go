...
func(t *testing.T) {
	t.Parallel()
func
	gotID, gotRegion, gotErr := tfrds.ClusterIDRegionFromARN(testCase.Input)	if gotErr != nil && !testCase.ExpectedErr {
...
funcme := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_rds_global_cluster.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
...
Steps: []resource.TestStep{
	{
funcig: testAccGlobalClusterConfig_basic(rName),
Check: resource.ComposeTestCheckFunc(
	testAccCheckGlobalClusterExists(ctx, resourceName, &globalCluster1),
...
Check: resour
functAccCheckGlobalClusterExists(ctx, resourceName, &globalCluster1),
...
Check: resource.ComposeTestCheckFunc(
	testAccCheck
functAccCheckGlobalClusterNotRecreated(&globalCluster1, &globalCluster2),
...
func TestAccRDSGlobalCluster_sourceDBClusterIdentifier(t *testing.T) {
	...
	resourceName := "aws_rds_global_cluster.test"	resource.Par
funcheck: 
func() { acctest.PreCheck(ctx, t); testAccPreCheckGlobalCluster(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, rds.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
Check: resource.ComposeTestCheckFunc(
	testAccCheckGlobalClusterExists(ctx, resourceName, &globalCluster2),
funcource.TestCheckResourceAttr(resourceName, "storage_encrypted", "false"),
),
	},
},
	})
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).RDSConn(ctx)output, err := tfrds.FindGlobalClusterByID(ctx, conn, rs.Primary.ID)if err != nil {
	return err
}*v = *outputreturn nil
	}
}
...
func testAccPreCheckGlobalCluster(ctx context.Context, t *testing.T) {
	conn := acctest.Provider.Meta().(*conns.AWSClient).RDSConn(ctx)	input := &rds.DescribeGlobalClustersInput{}	_, err := conn.DescribeGlobalClustersWithContext(ctx, input)	if acctest.PreCheckSkipError(err) || tfawserr.ErrMessageContains(err, "InvalidParameterValue", "Access Denied to API Version: APIGlobalDatabases") {
...
funcurce "aws_rds_global_cluster" "test" {
databa
funcobal_cluster_identifier = %[2]q
engine = "aurora-postgresql"
}
`, databaseName, rName)
}
...
skip_final_snapshot= trueglobal_cluster_identifier = aws_rds_global_cluster.test.idlifecycle {
...
skip_final_snapshot= truelifecycle {
 ignore_changes = [engine_version]
}
...
master_password= "mustbeeightcharacters"
master_username= "test"
skip_final_snapshot = true# global_cluster_identifier cannot be Computedlifecycle {
 ignore_changes = [global_cluster_identifier]
}
}resource "aws_rds_global_cluster" "test" {
force_destroy= true
...
storage_encrypted= true# global_cluster_identifier cannot be Computed
funcignore_changes = [global_cluster_identifier]
}
}resource "aws_rds_global_cluster" "test" {
force_destroy= true
global_cluster_identifier = %[1]q
