...
func := acctest.Context(t)
	...
	resourceName := "aws_db_instance_automated_backups_replication.test"	resource.ParallelTest(t, resource.TestCase{
		PreCheck: 
func() {
			acctest.P
funccctest.PreCheckMultipleRegion(t, 2)
		},
...
func() {
		...
		ErrorCheck:acctest.ErrorCheck(t, rds.EndpointsID),
		ProtoV5Pro
funceckDestroy:testAccCheckInstanceAutomatedBackupsReplicationDestroy(ctx),
		Steps: []resource.TestStep{
			{
...
funcourceName := "aws_db_instance_automated_backups_replication.test"	resource.ParallelTest(t, resource.TestCase{
		PreCheck: 
func() {
			acctest.PreCheck(ctx, t)
			acctest.PreCheckMultipleRegion(t, 2)
...
func
funcnn := acctest.Provider.Meta().(*conns.AWSClient).RDSConn(ctx)		_, err := tfrds.FindDBInstanceAutomatedBackupByARN(ctx, conn, rs.Primary.ID)		return err
	}
}
...
count = 2cidr_block= "10.1.${count.index}.0/24"
...
engine = "postgres"provider = "awsalternate"
}data "aws_rds_orderable_db_instance" "test" {
engine= data.aws_rds_engine_version.default.engine
engine_version = data.aws_rds_engine_version.default.version
license_model= "postgresql-license"
storage_type= "standard"preferred_instance_classes = [%[3]s]provider = "awsalternate"
...
db_subnet_group_name = aws_db_subnet_group.test.nameprovider = "awsalternate"
}
...
funcurce "aws_db_instance_automated_backups_replication" "test" {
source_db_instance_arn = aws_db_instance.test.arn
