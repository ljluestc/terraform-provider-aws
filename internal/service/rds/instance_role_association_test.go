...
func := acctest.Context(t)
	...
	resourceName := "aws_db_instance_role_association.test"	resource.ParallelTest(t, resource.TestCase{
		PreCheck: 
...
func
		...
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).RDSConn(ctx)		role, err := tfrds.DescribeDBInstanceRole(ctx, conn, dbInstanceIdentifier, roleArn)
		...
		}		*dbInstanceRole = *role		return nil
	}
}
...
func testAccCheckInstanceRoleAssociationDisappears(ctx context.Context, dbInstance *rds.DBInstance, dbInstanceRole *rds.DBInstanceRole) resource.TestCheckFunc {
	return 
func(s *terraform.State) error {
		conn := acctest.Provider.Meta().(*conns.AWSClient).RDSConn(ctx)		input := &rds.RemoveRoleFromDBInstanceInput{
