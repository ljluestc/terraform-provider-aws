...
func := acctest.Context(t)
	...
	storageType := "standard"	resource.ParallelTest(t, resource.TestCase{
		PreCheck: 
func() { acctest.PreCheck(ctx, t); testAccOrderableInstancePreCheck(ctx, t) },
		ErrorCheck:
...
funcSkip("skipping long-running test in short mode")
	...
	preferredClass := "db.t3.micro"	resource.ParallelTest(t, resource.TestCase{
		PreCheck: 
func() { acctest.PreCheck(ctx, t); testAccOrderableInstancePreCheck(ctx, t) },
...
func
	dataSourceName := "data.aws_rds_orderable_db_instance.test"	resource.ParallelTest(t, resource.TestCase{
		PreCheck: 
func() { acctest.PreCheck(ctx, t); testAccOrderableInstancePreCheck(ctx, t) },
		ErrorCheck:acctest.ErrorCheck(t, rds.EndpointsID),
		ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
		...
		Steps: []resource.TestStep{
			{
funcConfig: testAccOrderableInstanceDataSourceConfig_preferredVersion(),
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttrPair(dataSourceName, "engine_version", "data.aws_rds_engine_version.default", "version"),
				),
			},
		...
		Steps: []resource.TestStep{
			{
				...
				Check: reso
func	resource.TestCheckResourceAttrPair(dataSourceName, "instance_class", "data.aws_rds_orderable_db_instance.dynamic", "instance_class"),
					resource.TestCheckResourceAttrPair(dataSourceName, "engine_version", "data.aws_rds_engine_version.default", "version"),
				),
			},
		},
...
func() { acctest.PreCheck(ctx, t); testAccOrderableInstancePreCheck(ctx, t) },
		...
		CheckDestroy:nil,
		Steps: []resource.TestStep{
funcConfig: testAccOrderableInstanceDataSourceConfig_supportsKerberosAuthentication(),
				Check: resource.ComposeTestCheckFunc(
...
func testAccOrderableInstancePreCheck(ctx context.Context, t *testing.T) {
	conn := acctest.Provider.Meta().(*conns.AWSClient).RDSConn(ctx)	input := &rds.DescribeOrderableDBInstanceOptionsInput{
		...
		DBInstanceCla
func	_, err := conn.DescribeOrderableDBInstanceOptionsWithContext(ctx, input)	if acctest.PreCheckSkipError(err) {
...
engine= data.aws_rds_engine_version.default.engine
engine_version = data.aws_rds_engine_version.default.version
license_model= "general-public-license"preferred_instance_classes = [
 "db.xyz.xlarge",
 %[1]q,
 "db.t3.small",
]
...
storage_type= "standard"preferred_engine_versions = [
 "18.42.32",
...
func "aws_rds_orderable_db_instance" "dynamic" {
engine= data.aws_rds_engine_version.default.engine
engine_version= data.aws_rds_engine_version.default.version
...
funcySQLPreferredInstanceClasses)
}
...
supports_enhanced_monitoring = truepreferred_engine_versions= ["8.0.25", "8.0.26", data.aws_rds_engine_version.default.version]
preferred_instance_classes = [%[1]s]
}
...
engine= data.aws_rds_engine_version.default.engine
license_model= "general-public-license"
funcpports_iam_database_authentication = truepreferred_engine_versions= ["8.0.25", "8.0.26", data.aws_rds_engine_version.default.version]
preferred_instance_classes = [%[1]s]
}
`, mySQLPreferredInstanceClasses)
}
...
license_model = "general-public-license"
supports_iops = true
funceferred_engine_versions= ["8.0.20", "8.0.19", data.aws_rds_engine_version.default.version]
preferred_instance_classes = [%[1]s]
}
`, mySQLPreferredInstanceClasses)
}
...
funceferred_instance_classes = [%[1]s]
}
...
engine= data.aws_rds_engine_version.default.engine
license_model = "general-public-license"
supports_performance_insights = truepreferred_engine_versions= ["8.0.25", "8.0.26", data.aws_rds_engine_version.default.version]
func
`, mySQLPreferredInstanceClasses)
}
...
engine = data.aws_rds_engine_version.default.engine
license_model= "general-public-license"
supports_storage_autoscaling = truepreferred_engine_versions= ["8.0.20", "8.0.19", data.aws_rds_engine_version.default.version]
preferred_instance_classes = [%[1]s]
}
func
...
license_model= "general-public-license"
storage_type= "standard"
supports_storage_encryption = truepreferred_engine_versions= ["8.0.25", "8.0.26", data.aws_rds_engine_version.default.version]
preferred_instance_classes = [%[1]s]
}
func
func