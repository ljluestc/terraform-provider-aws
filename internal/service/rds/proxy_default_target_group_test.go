...
Steps: []resource.TestStep{
	{
funcig: testAccProxyDefaultTargetGroupConfig_connectionBorrowTimeout(rName, 120),
Check: resource.ComposeTestCheckFunc(
	testAccCheckProxyTargetGroupExists(ctx, resourceName, &dbProxyTargetGroup),
...
funcourceName := "aws_db_proxy_default_target_group.test"
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)	resource.ParallelTest(t, resource.TestCase{
PreCheck: 
func() { acctest.PreCheck(ctx, t); testAccDBProxyPreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, rds.EndpointsID),
...
Check: resour
functAccCheckProxyTargetGroupExists(ctx, resourceName, &dbProxyTargetGroup),
...
t.Skip("skipping long-running test in short mode")
	...
	resourceName := "aws_db_proxy_default_target_group.test"
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	sessionPinningFilters := "EXCLUDE_VARIABLE_SETS"	resource.ParallelTest(t, resource.TestCase{
...
funcsp.TargetGroups[0].DBProxyName == rs.Primary.ID {
	return 
func
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).RDSConn(ctx)opts := rds.DescribeDBProxyTargetGroupsInput{
	DBProxyName:aws.String(rs.Primary.ID),
	TargetGroupName: aws.String("default"),
}
funcrr != nil {
	return 
funcif len(resp.TargetGroups) != 1 ||
	*resp.TargetGroups[0].DBProxyName != rs.Primary.ID {
	return fmt.Errorf("DB Proxy Target Group not found")
}*v = *resp.TargetGroups[0]return nil
	}
}
...
vpc_subnet_ids= aws_subnet.test[*].idauth {
 auth_scheme = "SECRETS"
 description = "test"
funcsecret_arn= aws_secretsmanager_secret.test.arn
}tags = {
 Name = "%[1]s"
}
...
func testAccProxyDefaultTargetGroupConfig_emptyConnectionPoolConfig(rName string) string {
	return testAccProxyDefaultTargetGroupBaseConfig(rName) + `
resource "aws_db_proxy_default_target_group" "test" {
db_proxy_name = aws_db_proxy.test.nameconnection_pool_config {
}
}
`
}
...
func_proxy_name = aws_db_proxy.test.nameconnection_pool_config {
...
funcurn testAccProxyDefaultTargetGroupBaseConfig(rName) + fmt.Sprintf(`
resource "aws_db_proxy_default_target_group" "test" {
db_proxy_name = aws_db_proxy.test.nameconnection_pool_config {
...
funcurce "aws_db_proxy_default_target_group" "test" {
db_proxy_name = aws_db_proxy.test.nameconnection_pool_config {
