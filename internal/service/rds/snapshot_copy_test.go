...
func
funcerr := tfrds.FindDBSnapshotByID(ctx, conn, rs.Primary.ID)	if tfresource.NotFound(err) {
continue
	}	if err != nil {
return err
...
func
func := acctest.Provider.Meta().(*conns.AWSClient).RDSConn(ctx)output, err := tfrds.FindDBSnapshotByID(ctx, conn, rs.Primary.ID)
if err != nil {
	return err
}*v = *outputreturn nil
	}
...
func testAccSnapshotCopyConfig_tags1(rName, tagKey, tagValue string) string {
	return acctest.ConfigCompose(testAccSnapshotCopyConfig_base(rName), fmt.Sprintf(`
resource "aws_db_snapshot_copy" "test" {
funcrget_db_snapshot_identifier = "%[1]s-target"tags = {
 %[2]q = %[3]q
}
...
resource "aws_db_snapshot_copy" "test" {
source_db_snapshot_identifier = aws_db_snapshot.test.db_snapshot_arn
target_db_snapshot_identifier = "%[1]s-target"tags = {
 %[2]q = %[3]q
 %[4]q = %[5]q
}
