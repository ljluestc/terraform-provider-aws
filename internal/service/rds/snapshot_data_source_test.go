...
func := acctest.Context(t)
	...
	ds2Name := "data.aws_db_snapshot.by_tags"	resource.ParallelTest(t, resource.TestCase{
		PreCheck: 
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					resource.TestCheckResourceAttr(ds1Name, "tags.Name", rName),					resource.TestCheckResourceAttrPair(ds2Name, "db_instance_identifier", resourceName, "db_instance_identifier"),
...
db_snapshot_identifier = %[1]qtags = {
 Name = %[1]q
 Test = "true"
}
}resource "aws_db_snapshot_copy" "test" {
source_db_snapshot_identifier = aws_db_snapshot.test.db_snapshot_arn
target_db_snapshot_identifier = "%[1]s-copy"tags = {
 Name = "%[1]s-copy"
 Test = "true"
}
}data "aws_db_snapshot" "by_id" {
most_recent= "true"
db_snapshot_identifier = aws_db_snapshot.test.iddepends_on = [aws_db_snapshot_copy.test]
}data "aws_db_snapshot" "by_tags" {
most_recent = "true"tags = {
 Name = %[1]q
