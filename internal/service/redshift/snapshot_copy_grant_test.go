...
continue
}_, err := tfredshift.FindSnapshotCopyGrantByName(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).RedshiftConn(ctx)_, err := tfredshift.FindSnapshotCopyGrantByName(ctx, conn, rs.Primary.ID)return err
}
...
return fmt.Sprintf(`
resource "aws_redshift_snapshot_copy_grant" "test" {
  snapshot_copy_grant_name = %[1]q  tags = {
%[2]q = %[3]q
  }
}
