...
)// []*SERVICE.Tag handling// Tags returns redshift service tags.
func Tags(tags tftags.KeyValueTags) []*redshift.Tag {
result := make([]*redshift.Tag, 0, len(tags))for k, v := range tags.Map() {
tag := &redshift.Tag{
Key:aws.String(k),
Value: aws.String(v),
...
}_, err := conn.CreateTagsWithContext(ctx, input)if err != nil {
