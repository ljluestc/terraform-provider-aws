...
// SPDX-License-Identifier: MPL-2.0package redshiftimport (
	...
	clusterInvalidClusterStateFaultTimeout = 15 * time.Minute	clusterRelocationStatusResolvedTimeout = 1 * time.Minute
	...
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if output, ok := outputRaw.(*redshift.Cluster); ok {
		tfresource.SetLastError(err, errors.New(aws.StringValue(output.ClusterStatus)))		return output, err
	}	return nil, err
	...
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if output, ok := outputRaw.(*redshift.EndpointAccess); ok {
		tfresource.SetLastError(err, errors.New(aws.StringValue(output.EndpointStatus)))		return output, err
	}	return nil, err
	...
	stateConf := &retry.StateChangeConf{
		...
		MinTimeout: 10 * time.Second,
		Delay:  30 * time.Second,
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if output, ok := outputRaw.(*redshift.Snapshot); ok {
		tfresource.SetLastError(err, errors.New(aws.StringValue(output.Status)))		return output, err
	}	return nil, err
