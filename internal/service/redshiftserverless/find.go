...
// SPDX-License-Identifier: MPL-2.0package redshiftserverlessimport (
	...
	}	output, err := conn.GetNamespaceWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, redshiftserverless.ErrCodeResourceNotFoundException) {
	...
	}	output, err := conn.GetEndpointAccessWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, redshiftserverless.ErrCodeResourceNotFoundException) {
		return nil, &retry.NotFoundError{
			LastError:   err,
			LastRequest: input,
		}
	...
	}	output, err := conn.GetUsageLimitWithContext(ctx, input)	if tfawserr.ErrMessageContains(err, redshiftserverless.ErrCodeValidationException, "does not exist") {
		return nil, &retry.NotFoundError{
			LastError:   err,
			LastRequest: input,
		}
	...
	}	output, err := conn.GetSnapshotWithContext(ctx, input)	if tfawserr.ErrMessageContains(err, redshiftserverless.ErrCodeResourceNotFoundException, "snapshot") {
		return nil, &retry.NotFoundError{
			LastError:   err,
			LastRequest: input,
		}
	...
	}	output, err := conn.GetResourcePolicyWithContext(ctx, input)	if tfawserr.ErrMessageContains(err, redshiftserverless.ErrCodeResourceNotFoundException, "does not exist") {
		return nil, &retry.NotFoundError{
			LastError:   err,
			LastRequest: input,
		}
