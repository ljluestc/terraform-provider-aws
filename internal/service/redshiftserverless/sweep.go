...
// +build sweeppackage redshiftserverlessimport (
	"fmt"
	...
	var errs *multierror.Error	err = conn.ListNamespacesPagesWithContext(ctx, input, func(page *redshiftserverless.ListNamespacesOutput, lastPage bool) bool {
		if len(page.Namespaces) == 0 {
			...
			r := ResourceNamespace()
			d := r.Data(nil)
			d.SetId(aws.StringValue(namespace.NamespaceName))			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
	})	if err != nil {
	...
	sweepResources := make([]sweep.Sweepable, 0)
	var errs *multierror.Error	err = conn.ListWorkgroupsPagesWithContext(ctx, input, func(page *redshiftserverless.ListWorkgroupsOutput, lastPage bool) bool {
		if len(page.Workgroups) == 0 {
			...
			r := ResourceWorkgroup()
			d := r.Data(nil)
			d.SetId(aws.StringValue(workgroup.WorkgroupName))			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
	})	if err != nil {
	...
	sweepResources := make([]sweep.Sweepable, 0)
	var errs *multierror.Error	err = conn.ListSnapshotsPagesWithContext(ctx, input, func(page *redshiftserverless.ListSnapshotsOutput, lastPage bool) bool {
		if len(page.Snapshots) == 0 {
			...
			r := ResourceSnapshot()
			d := r.Data(nil)
			d.SetId(aws.StringValue(workgroup.SnapshotName))			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
	})	if err != nil {
