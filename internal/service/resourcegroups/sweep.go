...
// +build sweeppackage resourcegroupsimport (
"fmt"
...
sweepResources := make([]sweep.Sweepable, 0)err = conn.ListGroupsPagesWithContext(ctx, input, func(page *resourcegroups.ListGroupsOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, v := range page.GroupIdentifiers {
r := ResourceGroup()
d := r.Data(nil)
d.SetId(aws.StringValue(v.GroupName))sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
