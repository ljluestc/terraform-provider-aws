...
func testAccCluster_basic(t *testing.T) {
	...
	resourceName := "aws_route53recoverycontrolconfig_cluster.test"	resource.Test(t, resource.TestCase{
		PreCheck:
...
func testAccCheckClusterDestroy(ctx context.Context) resource.TestCheckFunc {
	returnfunc(s *terraform.State) error {
		conn := acctest.Provider.Meta().(*conns.AWSClient).Route53RecoveryControlConfigConn(ctx)		for _, rs := range s.RootModule().Resources {
			...
			}			_, err := conn.DescribeClusterWithContext(ctx, input)			if err == nil {
...
func testAccCheckClusterExists(ctx context.Context, name string) resource.TestCheckFunc {
	returnfunc(s *terraform.State) error {
		...
		}		conn := acctest.Provider.Meta().(*conns.AWSClient).Route53RecoveryControlConfigConn(ctx)		input := &r53rcc.DescribeClusterInput{
			ClusterArn: aws.String(rs.Primary.ID),
		}		_, err := conn.DescribeClusterWithContext(ctx, input)		return err
	}
}
