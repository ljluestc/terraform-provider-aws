...
resourceName := "aws_route53recoverycontrolconfig_control_panel.test"resource.Test(t, resource.TestCase{
PreCheck:
...
}_, err := conn.DescribeControlPanelWithContext(ctx, input)if err == nil {
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).Route53RecoveryControlConfigConn(ctx)input := &r53rcc.DescribeControlPanelInput{
ControlPanelArn: aws.String(rs.Primary.ID),
}_, err := conn.DescribeControlPanelWithContext(ctx, input)return err
}
}
