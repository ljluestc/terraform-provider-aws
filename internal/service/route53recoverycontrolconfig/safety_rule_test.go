...
resourceName := "aws_route53recoverycontrolconfig_safety_rule.test"resource.Test(t, resource.TestCase{
PreCheck:
...
}_, err := conn.DescribeSafetyRuleWithContext(ctx, input)if err == nil {
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).Route53RecoveryControlConfigConn(ctx)input := &r53rcc.DescribeSafetyRuleInput{
SafetyRuleArn: aws.String(rs.Primary.ID),
}_, err := conn.DescribeSafetyRuleWithContext(ctx, input)return err
}
}
...
resource "aws_route53recoverycontrolconfig_cluster" "test" {
  ...
  control_panel_arn = aws_route53recoverycontrolconfig_control_panel.test.arn
  wait_period_ms= 5000
  asserted_controls = [aws_route53recoverycontrolconfig_routing_control.test.arn]  rule_config {
inverted  = false
threshold = 0
type = "AND"
  }
...
resource "aws_route53recoverycontrolconfig_cluster" "test" {
  ...
  target_controls= [aws_route53recoverycontrolconfig_routing_control.test.arn]  rule_config {
inverted  = false
threshold = 0
type = "AND"
  }
