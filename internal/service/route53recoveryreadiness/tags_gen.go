...
package route53recoveryreadinessimport (
	"context"
...
func listTags(ctx context.Context, conn route53recoveryreadinessiface.Route53RecoveryReadinessAPI, identifier string) (tftags.KeyValueTags, error) {
	...
	}	output, err := conn.ListTagsForResourcesWithContext(ctx, input)	if err != nil {
		return tftags.New(ctx, nil), err
	}	return KeyValueTags(ctx, output.Tags), nil
}// ListTags lists route53recoveryreadiness service tags and set them in Context.
// It is called from outside this package.
func (p *servicePackage) ListTags(ctx context.Context, meta any, identifier string) error {
	tags, err := listTags(ctx, meta.(*conns.AWSClient).Route53RecoveryReadinessConn(ctx), identifier)	if err != nil {
		return err
	}	if inContext, ok := tftags.FromContext(ctx); ok {
		inContext.TagsOut = types.Some(tags)
	}	return nil
}// map[string]*string handling// Tags returns route53recoveryreadiness service tags.
