...
resourceName := "aws_route53_resolver_firewall_domain_list.test"resource.ParallelTest(t, resource.TestCase{
...
domainName2 := acctest.RandomFQDomainName()resource.ParallelTest(t, resource.TestCase{
PreCheck: func() { acctest.PreCheck(ctx, t); testAccPreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, route53resolver.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:    testAccCheckFirewallDomainListDestroy(ctx),
...
}func testAccCheckFirewallDomainListDestroy(ctx context.Context) resource.TestCheck
func {
...
continue
}_, err := tfroute53resolver.FindFirewallDomainListByID(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
}func testAccCheckFirewallDomainListExists(ctx context.Context, n string, v *route53resolver.FirewallDomainList) resource.TestCheck
func {
returnfunc(s *terraform.State) error {
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).Route53ResolverConn(ctx)output, err := tfroute53resolver.FindFirewallDomainListByID(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*v = *outputreturn nil
}
