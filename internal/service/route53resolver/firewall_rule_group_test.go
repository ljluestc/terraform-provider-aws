...
resourceName := "aws_route53_resolver_firewall_rule_group.test"resource.ParallelTest(t, resource.TestCase{
...
}func testAccCheckFirewallRuleGroupDestroy(ctx context.Context) resource.TestCheck
func {
...
continue
}_, err := tfroute53resolver.FindFirewallRuleGroupByID(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
}func testAccCheckFirewallRuleGroupExists(ctx context.Context, n string, v *route53resolver.FirewallRuleGroup) resource.TestCheck
func {
returnfunc(s *terraform.State) error {
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).Route53ResolverConn(ctx)output, err := tfroute53resolver.FindFirewallRuleGroupByID(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*v = *outputreturn nil
}
