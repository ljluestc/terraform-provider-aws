...
var v route53resolver.ResolverQueryLogConfigAssociation
rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName := "aws_route53_resolver_query_log_config_association.test"resource.ParallelTest(t, resource.TestCase{
PreCheck: func() { acctest.PreCheck(ctx, t); testAccPreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, route53resolver.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckQueryLogConfigAssociationDestroy(ctx),
...
}func testAccCheckQueryLogConfigAssociationDestroy(ctx context.Context) resource.TestCheck
func {
...
continue
}_, err := tfroute53resolver.FindResolverQueryLogConfigAssociationByID(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
}func testAccCheckQueryLogConfigAssociationExists(ctx context.Context, n string, v *route53resolver.ResolverQueryLogConfigAssociation) resource.TestCheck
func {
returnfunc(s *terraform.State) error {
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).Route53ResolverConn(ctx)output, err := tfroute53resolver.FindResolverQueryLogConfigAssociationByID(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*v = *outputreturn nil
}
