...
continue
}_, err := tfroute53resolver.FindResolverRuleAssociationByID(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).Route53ResolverConn(ctx)output, err := tfroute53resolver.FindResolverRuleAssociationByID(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*v = *outputreturn nil
}
