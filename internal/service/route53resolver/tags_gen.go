...
// Thisfunction will optimise the handling over listTags, if possible.
// The identifier is typically the Amazon Resource Name (ARN), although
// it may also be a different identifier depending on the service.func GetTag(ctx context.Context, conn route53resolveriface.Route53ResolverAPI, identifier, key string) (*string, error) {
listTags, err := listTags(ctx, conn, identifier)if err != nil {
return nil, err
...
// The identifier is typically the Amazon Resource Name (ARN), although
// it may also be a different identifier depending on the service.func listTags(ctx context.Context, conn route53resolveriface.Route53ResolverAPI, identifier string) (tftags.KeyValueTags, error) {
...
tags, err := listTags(ctx, meta.(*conns.AWSClient).Route53ResolverConn(ctx), identifier)if err != nil {
return err
...
}// KeyValueTags creates tftags.KeyValueTags from route53resolver service tags.func KeyValueTags(ctx context.Context, tags []*route53resolver.Tag) tftags.KeyValueTags {
...
// nil is returned if there are no input tags.func getTagsIn(ctx context.Context) []*route53resolver.Tag {
if inContext, ok := tftags.FromContext(ctx); ok {
if tags := Tags(inContext.TagsIn.UnwrapOrDefault()); len(tags) > 0 {
return tags
}
}return nil
}// setTagsOut sets route53resolver service tags in Context.func setTagsOut(ctx context.Context, tags []*route53resolver.Tag) {
if inContext, ok := tftags.FromContext(ctx); ok {
inContext.TagsOut = types.Some(KeyValueTags(ctx, tags))
}
}// updateTags updates route53resolver service tags.
// The identifier is typically the Amazon Resource Name (ARN), although
// it may also be a different identifier depending on the service.func updateTags(ctx context.Context, conn route53resolveriface.Route53ResolverAPI, identifier string, oldTagsMap, newTagsMap any) error {
