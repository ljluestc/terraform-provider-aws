...
// SPDX-License-Identifier: MPL-2.0package rum_testimport (
	"context"
	"fmt"
	"testing"	"github.com/aws/aws-sdk-go/service/cloudwatchrum"
	...
	resourceName := "aws_rum_app_monitor.test"	resource.ParallelTest(t, resource.TestCase{
...
conn := acctest.Provider.Meta().(*conns.AWSClient).RUMConn(ctx)for _, rs := range s.RootModule().Resources {
if rs.Type != "aws_rum_app_monitor" {
continue
}_, err := tfcloudwatchrum.FindAppMonitorByName(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).RUMConn(ctx)output, err := tfcloudwatchrum.FindAppMonitorByName(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*v = *outputreturn nil
	}
...
resource "aws_rum_app_monitor" "test" {
  name= %[1]q
  domain = "localhost"  custom_events {
status = %[2]q
  }
}
`, rName, enabled)
