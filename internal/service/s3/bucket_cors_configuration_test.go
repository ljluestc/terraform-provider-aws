...
rorCheck:eck(t, s3.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:kBucketCorsConfigurationDestroy(ctx),
...
nn := acctest.Provider.Meta().(*conns.AWSClient).S3Conn(ctx)fo_, rs := range s.RootModule().Resources {
...
fs.Primary.ID == "" {
	funcnn := acctest.Provider.Meta().(*conns.AWSClient).S3Conn(ctx)buet, expectedBucketOwner, err := tfs3.ParseResourceID(rs.Primary.ID)
 err != nil {
return err
nt := &s3.GetBucketCorsInput{
Bucket: aws.String(bucket),
fxpectedBucketOwner != "" {
input.ExpectedBucketOwner = aws.String(expectedBucketOwner)
oResponse, err := tfresource.RetryWhenAWSErrCodeEquals(ctx, 2*time.Minute, func() (interface{}, error) {
return conn.GetBucketCorsWithContext(ctx, input)
 tfs3.ErrCodeNoSuchCORSConfiguration)ifrr != nil {
return fmt.Errorf("error getting S3 Bucket CORS configuration (%s): %w", rs.Primary.ID, err)
futput, ok := corsResponse.(*s3.GetBucketCorsOutput); !ok || output == nil || len(output.CORSRules) == 0 {
