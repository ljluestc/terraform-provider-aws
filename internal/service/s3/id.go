...
// SPDX-License-Identifier: MPL-2.0package s3import (
	"fmt"
	"strings"
)const resourceIDSeparator = ","// CreateResourceID is a generic method for creating an ID string for a bucket-related resource e.g. aws_s3_bucket_versioning.
...
return bucket
	...
	id := strings.Join(parts, resourceIDSeparator)	return id
...
// for a bucket name and accountID if provided.func ParseResourceID(id string) (bucket, expectedBucketOwner string, err error) {
	parts := strings.Split(id, resourceIDSeparator)	if len(parts) == 1 && parts[0] != "" {
cket = parts[0]
turn
	}	if len(parts) == 2 && parts[0] != "" && parts[1] != "" {
cket = parts[0]
pectedBucketOwner = parts[1]
turn
