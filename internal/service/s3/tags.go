...
// The identifier is the bucket name.func BucketUpdateTags(ctx context.Context, conn s3iface_sdkv1.S3API, identifier string, oldTagsMap, newTagsMap any) error {
	...
	allTags, err := BucketListTags(ctx, conn, identifier)	if err != nil {
		...
		}		_, err := conn.PutBucketTaggingWithContext(ctx, input)		if err != nil {
		...
		input := &s3_sdkv1.DeleteBucketTaggingInput{
			Bucket: aws_sdkv1.String(identifier),
		}		_, err := conn.DeleteBucketTaggingWithContext(ctx, input)		if err != nil {
	...
	allTags, err := ObjectListTagsV1(ctx, conn, bucket, key)	if err != nil {
		...
		}		_, err := conn.PutObjectTaggingWithContext(ctx, input)		if err != nil {
		...
		input := &s3_sdkv1.DeleteObjectTaggingInput{
			Bucket: aws_sdkv1.String(bucket),
			Key:    aws_sdkv1.String(key),
		}		_, err := conn.DeleteObjectTaggingWithContext(ctx, input)		if err != nil {
