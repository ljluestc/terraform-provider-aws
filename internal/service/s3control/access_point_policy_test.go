...
return err
}_, _, err = tfs3control.FindAccessPointPolicyAndStatusByTwoPartKey(ctx, conn, accountID, name)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
return err
}conn := acctest.Provider.Meta().(*conns.AWSClient).S3ControlConn(ctx)_, _, err = tfs3control.FindAccessPointPolicyAndStatusByTwoPartKey(ctx, conn, accountID, name)return err
	}
...
resource "aws_s3control_access_point_policy" "test" {
  access_point_arn = aws_s3_access_point.test.arn  policy = jsonencode({
Version = "2008-10-17"
Statement = [{
 Effect = "Allow"
