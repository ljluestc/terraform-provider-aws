...
// SPDX-License-Identifier: MPL-2.0package s3control_testimport (
	...
	resourceName := "aws_s3control_bucket_policy.test"	resource.ParallelTest(t, resource.TestCase{
...
return err
	}	_, err = tfs3control.FindBucketPolicyByTwoPartKey(ctx, conn, parsedArn.AccountID, rs.Primary.ID)	if tfresource.NotFound(err) {
continue
	}	if err != nil {
return err
...
if !ok {
	...
	return err
}_, err = tfs3control.FindBucketPolicyByTwoPartKey(ctx, conn, parsedArn.AccountID, rs.Primary.ID)return err
	}
