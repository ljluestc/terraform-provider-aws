...
PreCheck:k(ctx, t); acctest.PreCheckPartitionNot(t, endpoints.AwsUsGovPartitionID) },
...
conn, err := tfs3control.ConnForMRAP(ctx, acctest.Provider.Meta().(*conns.AWSClient))if err != nil {
return err
}for _, rs := range s.RootModule().Resources {
if rs.Type != "aws_s3control_multi_region_access_point" {
continue
}accountID, name, err := tfs3control.MultiRegionAccessPointParseResourceID(rs.Primary.ID)if err != nil {
return err
}_, err = tfs3control.FindMultiRegionAccessPointByTwoPartKey(ctx, conn, accountID, name)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
return err
}output, err := tfs3control.FindMultiRegionAccessPointByTwoPartKey(ctx, conn, accountID, name)if err != nil {
return err
}*v = *outputreturn nil
	}
...
resource "aws_s3_bucket" "test1" {
  ...
  provider = awsalternate  bucket
  force_destroy = true
}resource "aws_s3_bucket" "test3" {
  provider = awsthird  bucket
  force_destroy = true
}resource "aws_s3control_multi_region_access_point" "test" {
  provider = aws  details {
name = %[4]qregion {
