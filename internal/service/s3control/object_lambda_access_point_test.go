...
// SPDX-License-Identifier: MPL-2.0package s3control_testimport (
	...
	lambdaFunctionResourceName := "aws_lambda_function.test"	resource.ParallelTest(t, resource.TestCase{
...
ImportStateVerify: true,
	...
	var v s3control.ObjectLambdaConfiguration
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_s3control_object_lambda_access_point.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck:  func() { acctest.PreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, s3control.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckObjectLambdaAccessPointDestroy(ctx),
...
return err
	}	_, err = tfs3control.FindObjectLambdaAccessPointByTwoPartKey(ctx, conn, accountID, name)	if tfresource.NotFound(err) {
continue
	}	if err != nil {
return err
...
if !ok {
	...
	return err
}conn := acctest.Provider.Meta().(*conns.AWSClient).S3ControlConn(ctx)output, err := tfs3control.FindObjectLambdaAccessPointByTwoPartKey(ctx, conn, accountID, name)if err != nil {
	return err
}*v = *outputreturn nil
	}
