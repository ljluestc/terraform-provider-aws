...
// +build sweeppackage s3controlimport (
	"fmt"
	...
	var sweeperErrs *multierror.Error	err = conn.ListAccessPointsPagesWithContext(ctx, input, func(page *s3control.ListAccessPointsOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
	...
	sweepResources := make([]sweep.Sweepable, 0)	err = conn.ListMultiRegionAccessPointsPagesWithContext(ctx, input, func(page *s3control.ListMultiRegionAccessPointsOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
		}		for _, v := range page.AccessPoints {
			r := resourceMultiRegionAccessPoint()
			d := r.Data(nil)
			d.SetId(MultiRegionAccessPointCreateResourceID(accountID, aws.StringValue(v.Name)))			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
	})	if sweep.SkipSweepError(err) {
	...
	sweepResources := make([]sweep.Sweepable, 0)	conn.ListAccessPointsForObjectLambdaPagesWithContext(ctx, input, func(page *s3control.ListAccessPointsForObjectLambdaOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
		}		for _, v := range page.ObjectLambdaAccessPointList {
			r := resourceObjectLambdaAccessPoint()
			d := r.Data(nil)
			d.SetId(ObjectLambdaAccessPointCreateResourceID(accountID, aws.StringValue(v.Name)))			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
	})	if sweep.SkipSweepError(err) {
	...
	sweepResources := make([]sweep.Sweepable, 0)	conn.ListStorageLensConfigurationsPagesWithContext(ctx, input, func(page *s3control.ListStorageLensConfigurationsOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
		}		for _, v := range page.StorageLensConfigurationList {
			configID := aws.StringValue(v.Id)			if configID == "default-account-dashboard" {
				continue
			}			r := resourceStorageLensConfiguration()
			d := r.Data(nil)
			d.SetId(StorageLensConfigurationCreateResourceID(accountID, configID))			sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
		}		return !lastPage
	})	if sweep.SkipSweepError(err) {
