//Codegeneratedbyinternal/generate/servicepackages/main.go;DONOTEDIT.packages3outpostsimport(
	...
	returnnames.S3Outposts
}//NewConnreturnsanewAWSSDKforGov1clientforthisservicepackage'sAWSAPI.
func(p*servicePackage)NewConn(ctxcontext.Context,configmap[string]any)(*s3outposts_sdkv1.S3Outposts,error){
	sess:=config["session"].(*session_sdkv1.Session)	returns3outposts_sdkv1.New(sess.Copy(&aws_sdkv1.Config{Endpoint:aws_sdkv1.String(config["endpoint"].(string))})),nil
