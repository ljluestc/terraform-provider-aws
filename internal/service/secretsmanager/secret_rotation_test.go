...
lambdaFunctionResourceName := "aws_lambda_function.test1"
days01 := 7
duration := "3h"resource.ParallelTest(t, resource.TestCase{
PreCheck:k(ctx, t); testAccPreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, secretsmanager.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckSecretRotationDestroy(ctx),
...
continue
}output, err := tfsecretsmanager.FindSecretByID(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
resource "aws_lambda_function" "test1" {
  ...
  principalcretsmanager.amazonaws.com"
...
resource "aws_lambda_function" "test2" {
  ...
  rotation_lambda_arn = aws_lambda_function.test1.arn  rotation_rules {
