...
// SPDX-License-Identifier: MPL-2.0package secretsmanager_testimport (
"context"
"fmt"
...
resourceName := "aws_secretsmanager_secret.test"resource.ParallelTest(t, resource.TestCase{
...
rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName := "aws_secretsmanager_secret.test"resource.ParallelTest(t, resource.TestCase{
PreCheck:k(ctx, t); testAccPreCheck(ctx, t); acctest.PreCheckMultipleRegion(t, 2) },
ErrorCheck:acctest.ErrorCheck(t, secretsmanager.EndpointsID),
...
rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName := "aws_secretsmanager_secret.test"resource.ParallelTest(t, resource.TestCase{
PreCheck:k(ctx, t); testAccPreCheck(ctx, t); acctest.PreCheckMultipleRegion(t, 3) },
ErrorCheck:acctest.ErrorCheck(t, secretsmanager.EndpointsID),
...
conn := acctest.Provider.Meta().(*conns.AWSClient).SecretsManagerConn(ctx)for _, rs := range s.RootModule().Resources {
if rs.Type != "aws_secretsmanager_secret" {
continue
}_, err := tfsecretsmanager.FindSecretByID(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).SecretsManagerConn(ctx)output, err := tfsecretsmanager.FindSecretByID(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*v = *outputreturn nil
}
}func testAccPreCheck(ctx context.Context, t *testing.T) {
conn := acctest.Provider.Meta().(*conns.AWSClient).SecretsManagerConn(ctx)input := &secretsmanager.ListSecretsInput{}_, err := conn.ListSecretsWithContext(ctx, input)if acctest.PreCheckSkipError(err) {
...
resource "aws_kms_key" "test" {
  ...
  name= %[1]q
  force_overwrite_replica_secret = %[2]t  replica {
kms_key_id = aws_kms_key.test.key_id
regiona.aws_region.alternate.name
  }
}
...
Sid= ""
  ...
  name
  description = "San Holo feat. Duskus"  policy = jsonencode({
Version = "2012-10-17"
Statement = [{
