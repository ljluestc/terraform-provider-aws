...
// SPDX-License-Identifier: MPL-2.0package secretsmanager_testimport (
	...
	secretResourceName := "aws_secretsmanager_secret.test"	resource.ParallelTest(t, resource.TestCase{
...
ImportStateVerify: true,
	...
	var version secretsmanager.GetSecretValueOutput
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_secretsmanager_secret_version.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck:k(ctx, t); testAccPreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, secretsmanager.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckSecretVersionDestroy(ctx),
...
}output, err := conn.GetSecretValueWithContext(ctx, input)if err != nil {
...
if err != nil {
return err
}conn := acctest.Provider.Meta().(*conns.AWSClient).SecretsManagerConn(ctx)input := &secretsmanager.GetSecretValueInput{
SecretId:  aws.String(secretID),
VersionId: aws.String(versionID),
}output, err := conn.GetSecretValueWithContext(ctx, input)if err != nil {
return err
...
resource "aws_secretsmanager_secret" "test" {
  ...
  secret_id_secretsmanager_secret.test.id
  secret_string = "test-string"  version_stages = ["one", "AWSCURRENT"]
}
`, rName)
...
resource "aws_secretsmanager_secret" "test" {
  ...
  secret_id_secretsmanager_secret.test.id
  secret_string = "test-string"  version_stages = ["two", "AWSCURRENT"]
}
`, rName)
...
resource "aws_secretsmanager_secret" "test" {
  ...
  secret_id_secretsmanager_secret.test.id
  secret_string = "test-string"  version_stages = ["one", "two", "AWSCURRENT"]
}
`, rName)
}
