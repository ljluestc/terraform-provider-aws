...
// +build sweeppackage secretsmanagerimport (
	"fmt"
...
return fmt.Errorf("error getting client: %s", err)
	...
	conn := client.SecretsManagerConn(ctx)	err = conn.ListSecretsPagesWithContext(ctx, &secretsmanager.ListSecretsInput{}, func(page *secretsmanager.ListSecretsOutput, lastPage bool) bool {
if len(page.SecretList) == 0 {
log.Print("[DEBUG] No Secrets Manager Secrets to sweep")
return true
}for _, secret := range page.SecretList {
name := aws.StringValue(secret.Name)log.Printf("[INFO] Deleting Secrets Manager Secret Policy: %s", name)
...
name := aws.StringValue(secret.Name)log.Printf("[INFO] Deleting Secrets Manager Secret: %s", name)
