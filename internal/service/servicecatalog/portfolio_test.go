...
var dpo servicecatalog.DescribePortfolioOutputresource.ParallelTest(t, resource.TestCase{
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).ServiceCatalogConn(ctx)output, err := tfservicecatalog.FindPortfolioByID(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*v = *outputreturn nil
}
...
continue
}_, err := tfservicecatalog.FindPortfolioByID(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
