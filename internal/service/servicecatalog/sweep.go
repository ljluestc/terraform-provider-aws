...
// +build sweeppackage servicecatalogimport (
"fmt"
"log"
...
var errs *multierror.Errorinput := &servicecatalog.ListPortfoliosInput{}err = conn.ListPortfoliosPagesWithContext(ctx, input, func(page *servicecatalog.ListPortfoliosOutput, lastPage bool) bool {
if page == nil {
return !lastPage
...
d := r.Data(nil)
d.SetId(BudgetResourceAssociationID(aws.StringValue(budget.BudgetName), aws.StringValue(port.Id)))sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
})
}return !lastPage
})if err != nil {
...
}prodInput := &servicecatalog.SearchProductsAsAdminInput{}err = conn.SearchProductsAsAdminPagesWithContext(ctx, prodInput, func(page *servicecatalog.SearchProductsAsAdminOutput, lastPage bool) bool {
if page == nil {
return !lastPage
...
d := r.Data(nil)
d.SetId(BudgetResourceAssociationID(aws.StringValue(budget.BudgetName), aws.StringValue(pvd.ProductViewSummary.ProductId)))sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
})
}return !lastPage
})if err != nil {
...
sweepResources := make([]sweep.Sweepable, 0)
var errs *multierror.Error// no paginator or list operation for constraints directly, have to list portfolios and constraints of portfoliosinput := &servicecatalog.ListPortfoliosInput{}err = conn.ListPortfoliosPagesWithContext(ctx, input, func(page *servicecatalog.ListPortfoliosOutput, lastPage bool) bool {
if page == nil {
return !lastPage
...
d := r.Data(nil)
d.SetId(aws.StringValue(detail.ConstraintId))sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
})
}return !lastPage
})if err != nil {
...
d := r.Data(nil)
d.SetId(PrincipalPortfolioAssociationCreateResourceID(AcceptLanguageEnglish, aws.StringValue(principal.PrincipalARN), aws.StringValue(detail.Id), aws.StringValue(principal.PrincipalType)))sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
})if err != nil {
...
sweepResources := make([]sweep.Sweepable, 0)
var errs *multierror.Error// no paginator or list operation for associations directly, have to list products and associations of productsinput := &servicecatalog.SearchProductsAsAdminInput{}err = conn.SearchProductsAsAdminPagesWithContext(ctx, input, func(page *servicecatalog.SearchProductsAsAdminOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, detail := range page.ProductViewDetails {
if detail == nil {
continue
}productARN, err := arn.Parse(aws.StringValue(detail.ProductARN))if err != nil {
errs = multierror.Append(errs, fmt.Errorf("error parsing Product ARN for %s: %w", aws.StringValue(detail.ProductARN), err))
continue
}// arn:aws:catalog:us-west-2:187416307283:product/prod-t5thhvquxw2x2resourceParts := strings.SplitN(productARN.Resource, "/", 2)if len(resourceParts) != 2 {
errs = multierror.Append(errs, fmt.Errorf("error parsing Product ARN resource for %s: %w", aws.StringValue(detail.ProductARN), err))
continue
}productID := resourceParts[1]assocInput := &servicecatalog.ListPortfoliosForProductInput{
...
d := r.Data(nil)
d.SetId(ProductPortfolioAssociationCreateID(AcceptLanguageEnglish, aws.StringValue(detail.Id), productID))sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
})if err != nil {
...
sweepResources := make([]sweep.Sweepable, 0)
var errs *multierror.Errorinput := &servicecatalog.SearchProductsAsAdminInput{}err = conn.SearchProductsAsAdminPagesWithContext(ctx, input, func(page *servicecatalog.SearchProductsAsAdminOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, pvd := range page.ProductViewDetails {
if pvd == nil || pvd.ProductViewSummary == nil {
continue
}id := aws.StringValue(pvd.ProductViewSummary.ProductId)r := ResourceProduct()
d := r.Data(nil)
d.SetId(id)sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
})if err != nil {
...
sweepResources := make([]sweep.Sweepable, 0)
var errs *multierror.Errorinput := &servicecatalog.SearchProvisionedProductsInput{
...
d := r.Data(nil)
d.SetId(aws.StringValue(detail.Id))sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
})if err != nil {
...
continue
}productID := aws.StringValue(pvd.ProductViewSummary.ProductId)artInput := &servicecatalog.ListProvisioningArtifactsInput{
ProductId: aws.String(productID),
}// there's no paginator for ListProvisioningArtifacts
for {
output, err := conn.ListProvisioningArtifactsWithContext(ctx, artInput)if err != nil {
...
d := r.Data(nil)d.SetId(aws.StringValue(pad.Id))
d.Set("product_id", productID)sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}/*
...
sweepResources := make([]sweep.Sweepable, 0)
var errs *multierror.Errorinput := &servicecatalog.ListServiceActionsInput{}err = conn.ListServiceActionsPagesWithContext(ctx, input, func(page *servicecatalog.ListServiceActionsOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, sas := range page.ServiceActionSummaries {
if sas == nil {
continue
}id := aws.StringValue(sas.Id)r := ResourceServiceAction()
d := r.Data(nil)
d.SetId(id)sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
})if err != nil {
...
sweepResources := make([]sweep.Sweepable, 0)
var errs *multierror.Errorinput := &servicecatalog.ListTagOptionsInput{}err = conn.ListTagOptionsPagesWithContext(ctx, input, func(page *servicecatalog.ListTagOptionsOutput, lastPage bool) bool {
if page == nil {
return !lastPage
...
d := r.Data(nil)
d.SetId(aws.StringValue(resource.Id))sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
})
}return !lastPage
...
if tod == nil {
continue
}id := aws.StringValue(tod.Id)r := ResourceTagOption()
d := r.Data(nil)
d.SetId(id)sweepResources = append(sweepResources, sweep.NewSweepResource(r, d, client))
}return !lastPage
})if tfawserr.ErrCodeEquals(err, servicecatalog.ErrCodeTagOptionNotMigratedException) {
