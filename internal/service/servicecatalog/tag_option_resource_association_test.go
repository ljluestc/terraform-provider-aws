...
)// add sweeper to delete known test servicecat tag option resource associationsfunc TestAccServiceCatalogTagOptionResourceAssociation_basic(t *testing.T) {
...
continue
}tagOptionID, resourceID, err := tfservicecatalog.TagOptionResourceAssociationParseID(rs.Primary.ID)if err != nil {
return fmt.Errorf("could not parse ID (%s): %w", rs.Primary.ID, err)
}err = tfservicecatalog.WaitTagOptionResourceAssociationDeleted(ctx, conn, tagOptionID, resourceID, tfservicecatalog.TagOptionResourceAssociationDeleteTimeout)if tfresource.NotFound(err) {
continue
}if err != nil {
...
return fmt.Errorf("could not parse ID (%s): %w", rs.Primary.ID, err)
}conn := acctest.Provider.Meta().(*conns.AWSClient).ServiceCatalogConn(ctx)_, err = tfservicecatalog.WaitTagOptionResourceAssociationReady(ctx, conn, tagOptionID, resourceID, tfservicecatalog.TagOptionResourceAssociationReadyTimeout)if err != nil {
