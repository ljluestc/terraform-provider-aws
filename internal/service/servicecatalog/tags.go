...
// +build !generatepackage servicecatalogimport (
"context"
...
)// Custom Service Catalog tag service update functions using the same format as generated code.func productUpdateTags(ctx context.Context, conn servicecatalogiface.ServiceCatalogAPI, identifier string, oldTagsMap, newTagsMap any) error {
oldTags := tftags.New(ctx, oldTagsMap)
newTags := tftags.New(ctx, newTagsMap)input := &servicecatalog.UpdateProductInput{
...
}_, err := conn.UpdateProductWithContext(ctx, input)if err != nil {
