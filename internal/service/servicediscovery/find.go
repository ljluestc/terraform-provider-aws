...
}output, err := conn.GetInstanceWithContext(ctx, input)if tfawserr.ErrCodeEquals(err, servicediscovery.ErrCodeInstanceNotFound) {
...
var output []*servicediscovery.NamespaceSummaryerr := conn.ListNamespacesPagesWithContext(ctx, input, func(page *servicediscovery.ListNamespacesOutput, lastPage bool) bool {
if page == nil {
return !lastPage
...
Name:ring(servicediscovery.NamespaceFilterNameType),
...
output, err := findNamespacesByType(ctx, conn, nsType)if err != nil {
return nil, err
...
Id: aws.String(id),
}output, err := conn.GetNamespaceWithContext(ctx, input)if tfawserr.ErrCodeEquals(err, servicediscovery.ErrCodeNamespaceNotFound) {
return nil, &retry.NotFoundError{
LastError:err,
LastRequest: input,
}
...
}output, err := conn.GetOperationWithContext(ctx, input)if tfawserr.ErrCodeEquals(err, servicediscovery.ErrCodeOperationNotFound) {
return nil, &retry.NotFoundError{
LastError:err,
LastRequest: input,
}
...
var output []*servicediscovery.ServiceSummaryerr := conn.ListServicesPagesWithContext(ctx, input, func(page *servicediscovery.ListServicesOutput, lastPage bool) bool {
if page == nil {
return !lastPage
...
Condition: aws.String(servicediscovery.FilterConditionEq),
Name:ring(servicediscovery.ServiceFilterNameNamespaceId),
...
output, err := findServicesByNamespaceID(ctx, conn, namespaceID)if err != nil {
return nil, err
}for _, v := range output {
if aws.StringValue(v.Name) == name {
return v, nil
...
Id: aws.String(id),
}output, err := conn.GetServiceWithContext(ctx, input)if tfawserr.ErrCodeEquals(err, servicediscovery.ErrCodeServiceNotFound) {
return nil, &retry.NotFoundError{
LastError:err,
LastRequest: input,
}
