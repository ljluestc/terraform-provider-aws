...
// +build sweeppackage servicediscoveryimport (
"fmt"
...
sweepResources := make([]sweep.Sweepable, 0)namespaces, err := findNamespacesByType(ctx, conn, servicediscovery.NamespaceTypeHttp)if sweep.SkipSweepError(err) {
...
conn := client.ServiceDiscoveryConn(ctx)
sweepResources := make([]sweep.Sweepable, 0)namespaces, err := findNamespacesByType(ctx, conn, servicediscovery.NamespaceTypeDnsPrivate)if sweep.SkipSweepError(err) {
...
conn := client.ServiceDiscoveryConn(ctx)
sweepResources := make([]sweep.Sweepable, 0)namespaces, err := findNamespacesByType(ctx, conn, servicediscovery.NamespaceTypeDnsPublic)if sweep.SkipSweepError(err) {
...
sweepResources := make([]sweep.Sweepable, 0)services, err := findServices(ctx, conn, input)if sweep.SkipSweepError(err) {
