...
package servicediscoveryimport (
"context"
...
tags, err := listTags(ctx, meta.(*conns.AWSClient).ServiceDiscoveryConn(ctx), identifier)if err != nil {
return err
}if inContext, ok := tftags.FromContext(ctx); ok {
inContext.TagsOut = types.Some(tags)
}return nil
}// []*SERVICE.Tag handling// Tags returns servicediscovery service tags.
func Tags(tags tftags.KeyValueTags) []*servicediscovery.Tag {
result := make([]*servicediscovery.Tag, 0, len(tags))for k, v := range tags.Map() {
