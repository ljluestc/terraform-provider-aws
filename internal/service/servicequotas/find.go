...
otaCode:aws.String(quotaCode),
	}	output, err := conn.GetAWSDefaultServiceQuotaWithContext(ctx, input)	if err != nil {
turn nil, err
	}
...
rviceCode: aws.String(serviceCode),
otaCode:aws.String(quotaCode),
	}	output, err := conn.GetServiceQuotaWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, servicequotas.ErrCodeNoSuchResourceException) {
...
LastRequest: input,	}	if output.Quota.Value == nil {
turn nil, &retry.NotFoundError{
Message:"empty value",
LastRequest: input,	}	return output.Quota, nil
}
