...
func TestAccSESDomainIdentityVerification_basic(t *testing.T) {
	...
	domain := fmt.Sprintf("tf-acc-%d.%s", sdkacctest.RandInt(), rootDomain)	resource.ParallelTest(t, resource.TestCase{
		PreCheck: 
...
func(s *terraform.State) error {
		...
		conn := acctest.Provider.Meta().(*conns.AWSClient).SESConn(ctx)		params := &ses.GetIdentityVerificationAttributesInput{
			Identities: []*string{
				aws.String(domain),
			},
...
data "aws_route53_zone" "test" {
  ...
  domain = aws_ses_domain_identity.test.id  depends_on = [aws_route53_record.domain_identity_verification]
}
`, rootDomain, domain)
}
...
resource "aws_ses_domain_identity" "test" {
  ...
  domain = aws_ses_domain_identity.test.id  timeouts {
 create = "5s"
  }
}
`, domain)
