...
// SPDX-License-Identifier: MPL-2.0package ses_testimport (
"context"
"fmt"
...
resourceName := "aws_ses_receipt_rule.test"resource.ParallelTest(t, resource.TestCase{
PreCheck: 
func() {
...
resource.TestCheckTypeSetElemAttrPair(resourceName, "lambda_action.*.
...
ruleSetResourceName := "aws_ses_receipt_rule_set.test"resource.ParallelTest(t, resource.TestCase{
PreCheck: 
func() {
acctest.PreCheck(ctx, t)
testAccPreCheck(ctx, t)
...
conn := acctest.Provider.Meta().(*conns.AWSClient).SESConn(ctx)for _, rs := range s.RootModule().Resources {
if rs.Type != "aws_ses_receipt_rule" {
continue
}_, err := tfses.FindReceiptRuleByTwoPartKey(ctx, conn, rs.Primary.ID, rs.Primary.Attributes["rule_set_name"])if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).SESConn(ctx)output, err := tfses.FindReceiptRuleByTwoPartKey(ctx, conn, rs.Primary.ID, rs.Primary.Attributes["rule_set_name"])if err != nil {
return err
}*v = *outputreturn nil
}
}
...
conn := acctest.Provider.Meta().(*conns.AWSClient).SESConn(ctx)input := &ses.DescribeReceiptRuleInput{
RuleName: aws.String("MyRule"),
RuleSetName: aws.String("MyRuleSet"),
}_, err := conn.DescribeReceiptRuleWithContext(ctx, input)if acctest.PreCheckSkipError(err) {
...
function.test.arn
  ...
  enabled = true
  scan_enabled  = true
  tls_policy = "Require"  lambda_action {
...
resource "aws_ses_receipt_rule_set" "test" {
  ...
  enabled = true
  scan_enabled  = true
  tls_policy = "Require"  stop_action {
 topic_arn = aws_sns_topic.test.arn
 scope= "RuleSet"
 position  = 1
  }
...
resource "aws_ses_receipt_rule_set" "test" {
  ...
  name = %[1]q
  rule_set_name = aws_ses_receipt_rule_set.test.rule_set_name  add_header_action {
 header_name  = "Added-By"
 header_value = "Terraform"
 position= 2
  }  add_header_action {
