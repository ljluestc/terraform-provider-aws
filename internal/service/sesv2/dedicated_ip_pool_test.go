...
// SPDX-License-Identifier: MPL-2.0package sesv2_testimport (
	"context"
	"errors"
	"fmt"
	...
	resourceName := "aws_sesv2_dedicated_ip_pool.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck:k(ctx, t); testAccPreCheckDedicatedIPPool(ctx, t) },
rorCheck: acctest.ErrorCheck(t, names.SESV2EndpointID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckDedicatedIPPoolDestroy(ctx),
...
ImportStateVerify: true,
	...
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_sesv2_dedicated_ip_pool.test"	resource.ParallelTest(t, resource.TestCase{
eCheck:k(ctx, t); testAccPreCheckDedicatedIPPool(ctx, t) },
rorCheck: acctest.ErrorCheck(t, names.SESV2EndpointID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckDedicatedIPPoolDestroy(ctx),
eps: []resource.TestStep{
...
nn := acctest.Provider.Meta().(*conns.AWSClient).SESV2Client(ctx)fo_, rs := range s.RootModule().Resources {
...
, ok := s.RootModule().Resources[name]
 !ok {
return create.Error(names.SESV2, create.ErrActionCheckingExistence, tfsesv2.ResNameDedicatedIPPool, name, errors.New("not found")) rs.Primary.ID == "" {
return create.Error(names.SESV2, create.ErrActionCheckingExistence, tfsesv2.ResNameDedicatedIPPool, name, errors.New("not set"))
o := acctest.Provider.Meta().(*conns.AWSClient).SESV2Client(ctx)_, e := tfsesv2.FindDedicatedIPPoolByID(ctx, conn, rs.Primary.ID)
 err != nil {
...
ern nil
	...
	conn := acctest.Provider.Meta().(*conns.AWSClient).SESV2Client(ctx)	_, err := conn.ListDedicatedIpPools(ctx, &sesv2.ListDedicatedIpPoolsInput{})
