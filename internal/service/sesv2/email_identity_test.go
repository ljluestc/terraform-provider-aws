...
resourceName := "aws_sesv2_email_identity.test"resource.ParallelTest(t, resource.TestCase{
PreCheck:k(ctx, t) },
...
ctx := acctest.Context(t)
rName := acctest.RandomDomainName()
resourceName := "aws_sesv2_email_identity.test"key1 := verify.Base64Encode([]byte(acctest.TLSRSAPrivateKeyPEM(t, 2048)))
selector1 := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)key2 := verify.Base64Encode([]byte(acctest.TLSRSAPrivateKeyPEM(t, 2048)))
selector2 := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)resource.ParallelTest(t, resource.TestCase{
PreCheck:k(ctx, t) },
ErrorCheck: acctest.ErrorCheck(t, names.SESV2EndpointID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckEmailIdentityDestroy(ctx),
...
conn := acctest.Provider.Meta().(*conns.AWSClient).SESV2Client(ctx)for _, rs := range s.RootModule().Resources {
if rs.Type != "aws_sesv2_email_identity" {
continue
}_, err := tfsesv2.FindEmailIdentityByID(ctx, conn, rs.Primary.ID)if err != nil {
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).SESV2Client(ctx)_, err := tfsesv2.FindEmailIdentityByID(ctx, conn, rs.Primary.ID)if err != nil {
...
return fmt.Sprintf(`
resource "aws_sesv2_email_identity" "test" {
  email_identity = %[1]q  dkim_signing_attributes {
