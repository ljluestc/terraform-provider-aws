...
resourceName := "aws_shield_drt_access_log_bucket_association.test"resource.Test(t, resource.TestCase{
PreCheck: func() {
...
resourceName2 := "aws_shield_drt_access_log_bucket_association.test2"resource.Test(t, resource.TestCase{
PreCheck: func() {
acctest.PreCheck(ctx, t)
testAccPreCheckLogBucket(ctx, t)
},
...
}*drtaccesslogbucketassociation = *respreturn nil
}
...
_, err := conn.DescribeDRTAccessWithContext(ctx, input)if acctest.PreCheckSkipError(err) {
...
return fmt.Sprintf(`
data "aws_partition" "current" {}resource "aws_s3_bucket" "test1" {
  bucket = %[2]q
  ...
  role_arn_association_id = aws_shield_drt_access_role_arn_association.test.id  depends_on = [aws_shield_drt_access_log_bucket_association.test1]
}
