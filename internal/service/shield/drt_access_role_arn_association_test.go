...
// SPDX-License-Identifier: MPL-2.0package shield_testimport (
	...
	"errors"
	"fmt"
	"testing"	"github.com/aws/aws-sdk-go/service/shield"
...
func TestAccShieldDRTAccessRoleARNAssociation_basic(t *testing.T) {
	ctx := acctest.Context(t)	if testing.Short() {
t.Skip("skipping long-running test in short mode")
	}	var drtaccessrolearnassociation shield.DescribeDRTAccessOutput
	rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	resourceName := "aws_shield_drt_access_role_arn_association.test"	resource.Test(t, resource.TestCase{
eCheck: func() {
acctest.PreCheck(ctx, t)
testAccPreCheckRoleARN(ctx, t)otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckDRTAccessRoleARNAssociationDestroy(ctx),
...
nn := acctest.Provider.Meta().(*conns.AWSClient).ShieldConn(ctx)
...
resp, err := conn.DescribeDRTAccessWithContext(ctx, input)if errs.IsA[*shield.ResourceNotFoundException](err) {
return nil
}if resp != nil && (resp.RoleArn == nil || *resp.RoleArn == "") {
return nil
}return create.Error(names.Shield, create.ErrActionCheckingDestroyed, tfshield.ResNameDRTAccessRoleARNAssociation, rs.Primary.ID, errors.New("not destroyed"))turn nil
	}
...
fs.Primary.ID == "" {
return create.Error(names.Shield, create.ErrActionCheckingExistence, tfshield.ResNameDRTAccessLogBucketAssociation, name, errors.New("not set"))nn := acctest.Provider.Meta().(*conns.AWSClient).ShieldConn(ctx)
sp, err := conn.DescribeDRTAccessWithContext(ctx, &shield.DescribeDRTAccessInput{})
 err != nil {
...
daccessrolearnassociation = *resp
turn nil
	}
}func testAccPreCheckRoleARN(ctx context.Context, t *testing.T) {
	conn := acctest.Provider.Meta().(*conns.AWSClient).ShieldConn(ctx)	input := &shield.DescribeDRTAccessInput{}
...
AssumeRole"
  ...
  role_arn = aws_iam_role.test.arn  depends_on = [aws_iam_role_policy_attachment.test]
}
`, rName)
}
