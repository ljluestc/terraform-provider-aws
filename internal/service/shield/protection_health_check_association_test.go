...
// SPDX-License-Identifier: MPL-2.0package shield_testimport (
	...
	resourceName := "aws_shield_protection_health_check_association.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck: func() {
...
eckDestroy:testAccCheckProtectionHealthCheckAssociationDestroy(ctx),
...
continue
}protectionId, _, err := tfshield.ProtectionHealthCheckAssociationParseResourceID(rs.Primary.ID)if err != nil {
return err
...
rectionId, _, err := tfshield.ProtectionHealthCheckAssociationParseResourceID(rs.Primary.ID)if e != nil {
return err
o := acctest.Provider.Meta().(*conns.AWSClient).ShieldConn(ctx)inpu:= &shield.DescribeProtectionInput{
ProtectionId: aws.String(protectionId),
e, err := conn.DescribeProtectionWithContext(ctx, input)if e != nil {
return err
fesp == nil || resp.Protection == nil {
return fmt.Errorf("The Shield protection does not exist")
fesp.Protection.HealthCheckIds == nil || len(aws.StringValueSlice(resp.Protection.HealthCheckIds)) != 1 {
...
data "aws_partition" "current" {}resource "aws_eip" "test" {
  domain = "vpc"  tags = {
foo  = "bar"
Name = %[1]q
  }
