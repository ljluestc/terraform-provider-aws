...
// SPDX-License-Identifier: MPL-2.0package shield_testimport (
	...
	rName := sdkacctest.RandString(10)	resource.ParallelTest(t, resource.TestCase{
PreCheck: func() {
acctest.PreCheck(ctx, t)
acctest.PreCheckPartitionHasService(t, shield.EndpointsID)
testAccPreCheck(ctx, t)rorCheck: acctest.ErrorCheck(t, shield.EndpointsID),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckProtectionDestroy(ctx),
...
acctest.PreCheck(ctx, t)
acctest.PreCheckPartitionHasService(t, shield.EndpointsID)
testAccPreCheck(ctx, t)rorCheck: acctest.ErrorCheck(t, shield.EndpointsID, "route53"),
otoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
eckDestroy:testAccCheckProtectionDestroy(ctx),
eps: []resource.TestStep{
{
...
nn := acctest.Provider.Meta().(*conns.AWSClient).ShieldConn(ctx)fo_, rs := range s.RootModule().Resources {
if rs.Type != "aws_shield_protection" {
continue
}input := &shield.DescribeProtectionInput{
ProtectionId: aws.String(rs.Primary.ID),
}resp, err := conn.DescribeProtectionWithContext(ctx, input)if tfawserr.ErrCodeEquals(err, shield.ErrCodeResourceNotFoundException) {
continue
}if err != nil {
return err
...
o := acctest.Provider.Meta().(*conns.AWSClient).ShieldConn(ctx)inpu:= &shield.DescribeProtectionInput{
ProtectionId: aws.String(rs.Primary.ID),
,rr := conn.DescribeProtectionWithContext(ctx, input)retu err
	}
}func testAccPreCheck(ctx context.Context, t *testing.T) {
	conn := acctest.Provider.Meta().(*conns.AWSClient).ShieldConn(ctx)	input := &shield.ListProtectionsInput{}	_, err := conn.ListProtectionsWithContext(ctx, input)	if acctest.PreCheckSkipError(err) || tfawserr.ErrMessageContains(err, shield.ErrCodeResourceNotFoundException, "subscription does not exist") {
...
resource "aws_route53_zone" "test" {
  name= "%[1]s.com."
  comment = "Terraform Acceptance Tests"  tags = {
foo  = "bar"
Name = %[1]q
  }
}data "aws_partition" "current" {}resource "aws_shield_protection" "test" {
  name= %[1]q
...
Name = %[1]q
  ...
  name = %[1]q  subnets  = aws_subnet.test[*].id
  internal = true  listener {
instance_port= 8000
...
custom_origin_config {
 http_port= 80
 https_port= 443
 origin_protocol_policy = "https-only" origin_ssl_protocols = [
...
origin_id= %[1]q
  ...
  wait_for_deployment = false  default_cache_behavior {
allowed_methods  = ["HEAD", "DELETE", "POST", "GET", "OPTIONS", "PUT", "PATCH"]
cached_methods= ["GET", "HEAD"]
target_origin_id = %[1]qforwarded_values {
 query_string = false
 headers = ["*"] cookies {
...
cloudfront_default_certificate = true
  ...
  name= %[1]q
  resource_arn = aws_cloudfront_distribution.test.arn  tags = {
%[3]q = %[4]q
  }
}
...
}data "aws_region" "current" {}data "aws_caller_identity" "current" {}data "aws_partition" "current" {}resource "aws_eip" "test" {
  domain = "vpc"  tags = {
foo  = "bar"
Name = %[1]q
  }
