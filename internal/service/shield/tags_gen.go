...
tags, err := listTags(ctx, meta.(*conns.AWSClient).ShieldConn(ctx), identifier)if err != nil {
return err
}if inContext, ok := tftags.FromContext(ctx); ok {
inContext.TagsOut = types.Some(tags)
}return nil
}// []*SERVICE.Tag handling// Tags returns shield service tags.
func Tags(tags tftags.KeyValueTags) []*shield.Tag {
result := make([]*shield.Tag, 0, len(tags))for k, v := range tags.Map() {
tag := &shield.Tag{
Key:aws.String(k),
Value: aws.String(v),
