...
// SPDX-License-Identifier: MPL-2.0package sns_testimport (
 ...
 Before running this test, at least one of these ENV variables combinations must be set: GCM_API_KEY - Google Cloud Messaging API Key APNS_SANDBOX_CREDENTIAL - Apple Push Notification Sandbox Private Key
 APNS_SANDBOX_PRINCIPAL - Apple Push Notification Sandbox Certificate APNS_SANDBOX_CREDENTIAL_PATH - Apple Push Notification Sandbox Private Key file location
 APNS_SANDBOX_PRINCIPAL_PATH - Apple Push Notification Sandbox Certificate file location APNS_SANDBOX_TOKEN_CREDENTIAL - Apple signing key
...
**/type testAccPlatformApplicationPlatform struct {
	...
	platforms := make([]*testAccPlatformApplicationPlatform, 0, 2)	if os.Getenv("APNS_SANDBOX_CREDENTIAL") != "" && allowedApnsAuthType == "certificate" {
if os.Getenv("APNS_SANDBOX_PRINCIPAL") == "" {
t.Fatalf("APNS_SANDBOX_CREDENTIAL set but missing APNS_SANDBOX_PRINCIPAL")
lform := &testAccPlatformApplicationPlatform{
Name:"APNS_SANDBOX",
...
ApnsAuthType: "certificate",atforms = append(platforms, platform)
	} else if os.Getenv("APNS_SANDBOX_CREDENTIAL_PATH") != "" && allowedApnsAuthType == "certificate" {
 os.Getenv("APNS_SANDBOX_PRINCIPAL_PATH") == "" {
...
ApnsAuthType: "certificate",atforms = append(platforms, platform)
 ...
 os.Getenv("APNS_SANDBOX_TOKEN_PRINCIPAL") == "" {
...
ApnsAuthType: "token",atforms = append(platforms, platform)
	}	if os.Getenv("GCM_API_KEY") != "" {
atform := &testAccPlatformApplicationPlatform{
Name:"GCM",
Credential: strconv.Quote(os.Getenv("GCM_API_KEY")),atforms = append(platforms, platform)
	}	if len(platforms) == 0 {
Skipf("no SNS Platform Application environment variables found")
	}
	return platforms
...
ExpectedName:"",
ExpectedPlatform: "",
ErrCount:1,	}	for _, tc := range testCases {
n, name, platform, err := tfsns.DecodePlatformApplicationID(tc.Input)
 tc.ErrCount == 0 && err != nil {
t.Fatalf("expected %q not to trigger an error, received: %s", tc.Input, err) tc.ErrCount > 0 && err == nil {
t.Fatalf("expected %q to trigger an error", tc.Input) arn != tc.ExpectedArn {
t.Fatalf("expected %q to return arn: %s", tc.Input, arn) name != tc.ExpectedName {
t.Fatalf("expected %q to return name: %s", tc.Input, name) platform != tc.ExpectedPlatform {
t.Fatalf("expected %q to return platform: %s", tc.Input, platform)	}
	...
	if apiKey == "" {
Skipf("Environment variable %s is not set", key)
...
eckDestroy:testAccCheckPlatformApplicationDestroy(ctx),
...
resource.TestCheckResourceAttr(resourceName, "success_feedback_sample_rate", "50"),
	...
	resourceName := "aws_sns_platform_application.test"	for _, platform := range platforms { //nolint:paralleltest
me := fmt.Sprintf("tf-acc-%d", sdkacctest.RandInt())
atformPrincipalCheck := resource.TestCheckNoResourceAttr(resourceName, "platform_principal")
 platform.Principal != "" {
platformPrincipalCheck = resource.TestCheckResourceAttrSet(resourceName, "platform_principal")
.n(platform.Name, func(*testing.T) {
...
ImportStateVerifyIgnore: []string{"platform_credential", "platform_principal"},
	...
	ctx := acctest.Context(t)
	platforms := testAccPlatformApplicationPlatformFromEnv(t, "certificate")
	resourceName := "aws_sns_platform_application.test"	var testCases = []struct {
tributeKeystring
tributeValuestring
tributeValueUpdate string
	}{AttributeKey:"success_feedback_sample_rate",
AttributeValue:"100",
AttributeValueUpdate: "99",	}	for _, platform := range platforms { //nolint:paralleltest
Run(platform.Name, func(*testing.T) {
t.Parallel()for _, tc := range testCases {
t.Run(fmt.Sprintf("%s/%s", platform.Name, tc.AttributeKey), func(*testing.T) {
name := fmt.Sprintf("tf-acc-%d", sdkacctest.RandInt())resource.ParallelTest(t, resource.TestCase{
PreCheck:k(ctx, t) },
ErrorCheck: acctest.ErrorCheck(t, sns.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckPlatformApplicationDestroy(ctx),
Steps: []resource.TestStep{
	{
nfig: testAccPlatformApplicationConfig_basicAttribute(name, platform, tc.AttributeKey, tc.AttributeValue),
eck: resource.ComposeTestCheckFunc(
testAccCheckPlatformApplicationExists(ctx, resourceName),
resource.TestCheckResourceAttr(resourceName, tc.AttributeKey, tc.AttributeValue),	},
	{
nfig: testAccPlatformApplicationConfig_basicAttribute(name, platform, tc.AttributeKey, tc.AttributeValueUpdate),
eck: resource.ComposeTestCheckFunc(
testAccCheckPlatformApplicationExists(ctx, resourceName),
...
portState:true,
portStateVerify:true,
portStateVerifyIgnore: []string{"platform_credential", "platform_principal"},
	},
},
})
})
	...
	updatedApplePlatformBundleId := "com.bundle2.name2"	for _, platform := range platforms { //nolint:paralleltest
me := fmt.Sprintf("tf-acc-%d", sdkacctest.RandInt())t.n(platform.Name, func(*testing.T) {
resource.ParallelTest(t, resource.TestCase{
PreCheck:k(ctx, t) },
ErrorCheck: acctest.ErrorCheck(t, sns.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
...
o := acctest.Provider.Meta().(*conns.AWSClient).SNSConn(ctx)_, e := tfsns.FindPlatformApplicationAttributesByARN(ctx, conn, rs.Primary.ID)returnrr
	}
}func testAccCheckPlatformApplicationDestroy(ctx context.Context) resource.TestCheckFunc {
	return func(s *terraform.State) error {
nn := acctest.Provider.Meta().(*conns.AWSClient).SNSConn(ctx)fo_, rs := range s.RootModule().Resources {
if rs.Type != "aws_sns_platform_application" {
continue
}_, err := tfsns.FindPlatformApplicationAttributesByARN(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
resource "aws_sns_topic" "test" {
  ...
  count = 2  name = "%[1]s-${count.index}"  assume_role_policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": {
"Effect": "Allow",
...
EOF
  ...
  count = 2  policy_arn = "arn:${data.aws_partition.current.partition}:iam::aws:policy/CloudWatchLogsFullAccess"
...
resource "aws_sns_platform_application" "test" {
  name = %[1]q
  platform= "GCM"
  platform_credential = %[2]q  event_delivery_failure_topic_arn = aws_sns_topic.test[0].arn
  event_endpoint_created_topic_arn = aws_sns_topic.test[1].arn
  event_endpoint_deleted_topic_arn = aws_sns_topic.test[0].arn
  event_endpoint_updated_topic_arn = aws_sns_topic.test[1].arn  failure_feedback_role_arn = aws_iam_role.test[0].arn
  success_feedback_role_arn = aws_iam_role.test[1].arn  success_feedback_sample_rate = 25
}
`, rName, credentials))
...
resource "aws_sns_platform_application" "test" {
  name = %[1]q
  platform= "GCM"
  platform_credential = %[2]q  event_delivery_failure_topic_arn = aws_sns_topic.test[1].arn
  event_endpoint_created_topic_arn = aws_sns_topic.test[0].arn
  event_endpoint_deleted_topic_arn = aws_sns_topic.test[1].arn
  event_endpoint_updated_topic_arn = aws_sns_topic.test[0].arn  failure_feedback_role_arn = aws_iam_role.test[1].arn
  success_feedback_role_arn = aws_iam_role.test[0].arn  success_feedback_sample_rate = 50
}
`, rName, credentials))
