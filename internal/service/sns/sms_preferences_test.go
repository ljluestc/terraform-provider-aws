...
"deliveryRole":testAccSMSPreferences_deliveryRole,
	...
	resourceName := "aws_sns_sms_preferences.test"	resource.Test(t, resource.TestCase{
...
continue
	}	conn := acctest.Provider.Meta().(*conns.AWSClient).SNSConn(ctx)	attrs, err := conn.GetSMSAttributesWithContext(ctx, &sns.GetSMSAttributesInput{})	if err != nil {
return err
	}	if attrs == nil || len(attrs.Attributes) == 0 {
return nil
	}	var attrErrs *multierror.Error	// The API is returning undocumented keys, e.g. "UsageReportS3Enabled". Only check the keys we're aware of.
...
resource "aws_sns_sms_preferences" "test" {
  ...
  name = %[1]q
  path = "/"  assume_role_policy = <<POLICY
{
  "Version": "2012-10-17",
  "Statement": [
{
...
POLICY
  ...
  name = %[1]q
  role = aws_iam_role.test.id  policy = <<POLICY
{
  "Version": "2012-10-17",
  "Statement": [
{
