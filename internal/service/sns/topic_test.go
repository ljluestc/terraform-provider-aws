...
resourceName := "aws_sns_topic.test"resource.ParallelTest(t, resource.TestCase{
...
var attributes map[string]string
resourceName := "aws_sns_topic.test"
rName := "tf-acc-test-prefix-"resource.ParallelTest(t, resource.TestCase{
PreCheck:  func() { acctest.PreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, sns.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckTopicDestroy(ctx),
...
expectedPolicy := fmt.Sprintf(`{"Statement":[{"Sid":"Stmt1445931846145","Effect":"Allow","Principal":{"AWS":"*"},"Action":"sns:Publish","Resource":"arn:%s:sns:%s::example"}],"Version":"2012-10-17","Id":"Policy1445931846145"}`, acctest.Partition(), acctest.Region())resource.ParallelTest(t, resource.TestCase{
PreCheck:  func() { acctest.PreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, sns.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckTopicDestroy(ctx),
...
resourceName := "aws_sns_topic.test"
rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
expectedPolicy := `{"http":{"defaultHealthyRetryPolicy": {"minDelayTarget": 20,"maxDelayTarget": 20,"numMaxDelayRetries": 0,"numRetries": 3,"numNoDelayRetries": 0,"numMinDelayRetries": 0,"backoffFunction": "linear"},"disableSubscriptionOverrides": false}}`resource.ParallelTest(t, resource.TestCase{
PreCheck:  func() { acctest.PreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, sns.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckTopicDestroy(ctx),
...
var attributes map[string]string
resourceName := "aws_sns_topic.test"
rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix) + tfsns.FIFOTopicNameSuffixresource.ParallelTest(t, resource.TestCase{
PreCheck:  func() { acctest.PreCheck(ctx, t) },
ErrorCheck:acctest.ErrorCheck(t, sns.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckTopicDestroy(ctx),
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).SNSConn(ctx)attributes, err := tfsns.FindTopicAttributesByARN(ctx, conn, rs.Primary.ID)if err != nil {
return err
}actualPolicyText := attributes[tfsns.TopicAttributeNamePolicy]if actualPolicyText == "" {
return fmt.Errorf("SNS Topic Policy (%s) not found", rs.Primary.ID)
}equivalent, err := awspolicy.PoliciesAreEquivalent(actualPolicyText, expectedPolicyText)if err != nil {
...
return err
}actualPolicyText := attributes[tfsns.TopicAttributeNameDeliveryPolicy]if actualPolicyText == "" {
return fmt.Errorf("SNS Topic Delivery Policy (%s) not found", rs.Primary.ID)
}equivalent := verify.SuppressEquivalentJSONDiffs("", actualPolicyText, expectedPolicyText, nil)if !equivalent {
...
return fmt.Errorf("No SNS Topic ID is set")
}conn := acctest.Provider.Meta().(*conns.AWSClient).SNSConn(ctx)output, err := tfsns.GetTopicAttributesByARN(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*v = outputreturn nil
}
...
return fmt.Sprintf(`
data "aws_partition" "current" {}data "aws_region" "current" {}resource "aws_sns_topic" "test" {
  name = %[1]q  policy = <<EOF
{
  "Statement": [
{
...
return fmt.Sprintf(`
data "aws_partition" "current" {}resource "aws_sns_topic" "test" {
  name = %[1]q  policy = <<EOF
{
  "Statement": [
{
  ...
  "Statement": [
{
 ...
 "Principal": {
ws_partition.current.dns_suffix}"
 },
 "Effect": "Allow",
 "Sid": ""
}
  ...
  "Statement": [
{
 ...
 "Effect": "Allow",
 "Principal": {
artition.current.partition}:iam::012345678901:role/wooo"
 },
 "Action": "sns:Publish",
 "Resource": "arn:${data.aws_partition.current.partition}:sns:${data.aws_region.current.name}::example"
}
...
return fmt.Sprintf(`
resource "aws_sns_topic" "test" {
  depends_on = [aws_iam_role_policy.example]  name= %[1]q
  ...
  firehose_success_feedback_role_arn= aws_iam_role.example.arn  tracing_config = "Active"
}data "aws_partition" "current" {}resource "aws_iam_role" "example" {
  name = %[1]q
...
EOF
  ...
  name = %[1]q
  role = aws_iam_role.example.id  policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
{
