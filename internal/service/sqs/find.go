...
// SPDX-License-Identifier: MPL-2.0package sqsimport (
	...
	}	output, err := conn.GetQueueAttributesWithContext(ctx, input)	if tfawserr.ErrCodeEquals(err, sqs.ErrCodeQueueDoesNotExist) {
	...
	}	v, ok := output.Attributes[attributeName]	if !ok || aws.StringValue(v) == "" {
		return "", tfresource.NewEmptyResultError(input)
