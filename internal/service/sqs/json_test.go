...
t.Parallel()unquoted := `{"test": "test"}`
quoted := "{\"test\": \"test\"}"if !BytesEqual([]byte(unquoted), []byte(quoted)) {
t.Errorf("Expected BytesEqual to return true for %s == %s", unquoted, quoted)
}unquotedDiff := `{"test": "test"}`
quotedDiff := "{\"test\": \"tested\"}"if BytesEqual([]byte(unquotedDiff), []byte(quotedDiff)) {
...
t.Parallel()noWhitespace := `{"test":"test"}`
whitespace := `
{
  "test": "test"
