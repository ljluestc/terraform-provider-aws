...
// +build sweeppackage sqsimport (
	...
	conn := client.SQSConn(ctx)	input := &sqs.ListQueuesInput{}
	var sweeperErrs *multierror.Error	err = conn.ListQueuesPagesWithContext(ctx, input, func(page *sqs.ListQueuesOutput, lastPage bool) bool {
		if page == nil {
			return !lastPage
			...
			err = sdk.DeleteResource(ctx, r, d, client)			if err != nil {
