...
func listTags(ctx context.Context, conn sqsiface.SQSAPI, identifier string) (tftags.KeyValueTags, error) {
	...
	}	output, err := conn.ListQueueTagsWithContext(ctx, input)	if err != nil {
		return tftags.New(ctx, nil), err
	}	return KeyValueTags(ctx, output.Tags), nil
}// ListTags lists sqs service tags and set them in Context.
// It is called from outside this package.
func (p *servicePackage) ListTags(ctx context.Context, meta any, identifier string) error {
	tags, err := listTags(ctx, meta.(*conns.AWSClient).SQSConn(ctx), identifier)	if err != nil {
		return err
	}	if inContext, ok := tftags.FromContext(ctx); ok {
		inContext.TagsOut = types.Some(tags)
	}	return nil
}// map[string]*string handling// Tags returns sqs service tags.
...
func updateTags(ctx context.Context, conn sqsiface.SQSAPI, identifier string, oldTagsMap, newTagsMap any) error {
	...
	if len(removedTags) > 0 {
		...
		}		_, err := conn.UntagQueueWithContext(ctx, input)		if err != nil {
	...
	if len(updatedTags) > 0 {
		...
		}		_, err := conn.TagQueueWithContext(ctx, input)		if err != nil {
