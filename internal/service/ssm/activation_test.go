...
// SPDX-License-Identifier: MPL-2.0package ssm_testimport (
	...
	"fmt"
	"testing"
	"time"	"github.com/aws/aws-sdk-go/service/ssm"
	...
	resourceName := "aws_ssm_activation.test"	resource.ParallelTest(t, resource.TestCase{
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).SSMConn(ctx)output, err := tfssm.FindActivationByID(ctx, conn, rs.Primary.ID)if err != nil {
	return err
}*v = *outputreturn nil
	}
...
continue
	}	_, err := tfssm.FindActivationByID(ctx, conn, rs.Primary.ID)	if tfresource.NotFound(err) {
continue
	}	if err != nil {
return err
...
EOF}resource "aws_iam_role_policy_attachment" "test" {
...
resource "aws_ssm_activation" "test" {
  ...
  registration_limit = "5"  depends_on = [aws_iam_role_policy_attachment.test]
}
`, rName))
...
resource "aws_ssm_activation" "test" {
  ...
  description
  iam_role  = aws_iam_role.test.name
  registration_limit = "5"  depends_on = [aws_iam_role_policy_attachment.test]  tags = {
Name = %[1]q
  }
}
`, rName))
