...
resourceName := "aws_ssm_document.test"resource.ParallelTest(t, resource.TestCase{
PreCheck:.PreCheck(ctx, t) },
...
ids := "123456789012"resource.ParallelTest(t, resource.TestCase{
PreCheck:.PreCheck(ctx, t) },
ErrorCheck: acctest.ErrorCheck(t, ssm.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckDocumentDestroy(ctx),
...
idsAdd := "123456789012,123456789014"resource.ParallelTest(t, resource.TestCase{
PreCheck:.PreCheck(ctx, t) },
ErrorCheck: acctest.ErrorCheck(t, ssm.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckDocumentDestroy(ctx),
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).SSMConn(ctx)_, err := tfssm.FindDocumentByName(ctx, conn, rs.Primary.ID)return err
}
...
continue
}_, err := tfssm.FindDocumentByName(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
resource "aws_ssm_document" "test" {
  name = %[1]q
  document_type = "Command"  permissions = {
type
account_ids = "all"
  }  content = <<DOC
{
...
"
 ...
 "Resource": [
partition.current.partition}:s3:::${aws_s3_bucket.test.id}/*",
partition.current.partition}:s3:::${aws_s3_bucket.test.id}"
 ]
}
  ]
...
EOF
  ...
  name = %[1]q
  document_type = "Package"  attachments_source {
key= "SourceUrl"
...
"test.zip": {
 "checksums": {
esha256("test-fixtures/ssm-doc-acc-test.zip")}"
 }
}
  },
  "packages": {
...
DOC  depends_on = [aws_iam_role_policy.test]
}
`, rName, rInt)
...
resource "aws_ssm_document" "test" {
  name = %[1]q
  document_type = "Session"  content = <<DOC
{
  "schemaVersion": "1.0",
