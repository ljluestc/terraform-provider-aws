...
targets := make([]*ssm.Target, 0)for _, tConfig := range in {
config := tConfig.(map[string]interface{})target := &ssm.Target{
...
t["values"] = flex.FlattenStringList(target.Values)result = append(result, t)
}return result
}
