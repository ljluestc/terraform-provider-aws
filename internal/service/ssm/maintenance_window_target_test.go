...
}conn := acctest.Provider.Meta().(*conns.AWSClient).SSMConn(ctx)resp, err := conn.DescribeMaintenanceWindowTargetsWithContext(ctx, &ssm.DescribeMaintenanceWindowTargetsInput{
...
resource "aws_ssm_maintenance_window" "test" {
  ...
  resource_type = "INSTANCE"  targets {
key= "tag:Name"
...
resource "aws_ssm_maintenance_window" "test" {
  ...
  description= "This resource is for test purpose only"
  window_id= aws_ssm_maintenance_window.test.id
  resource_type = "RESOURCE_GROUP"  targets {
...
resource "aws_ssm_maintenance_window" "test" {
  ...
  owner_information = "something"  targets {
key= "tag:Name"
values = ["acceptance_test"]
  }  targets {
key= "tag:Updated"
