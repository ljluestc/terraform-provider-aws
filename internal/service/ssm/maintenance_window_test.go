...
// SPDX-License-Identifier: MPL-2.0package ssm_testimport (
	...
	resourceName := "aws_ssm_maintenance_window.test"	resource.ParallelTest(t, resource.TestCase{
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).SSMConn(ctx)output, err := tfssm.FindMaintenanceWindowByID(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*v = *outputreturn nil
	}
...
continue
}_, err := tfssm.FindMaintenanceWindowByID(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
