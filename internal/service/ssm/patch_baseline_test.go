...
Check: resource.ComposeTestCheckFunc(
	...
	name := sdkacctest.RandString(10)
	resourceName := "aws_ssm_patch_baseline.test"	resource.ParallelTest(t, resource.TestCase{
PreCheck:.PreCheck(ctx, t) },
ErrorCheck: acctest.ErrorCheck(t, ssm.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckPatchBaselineDestroy(ctx),
...
func testAccSSMPatchBaseline_deleteDefault(t *testing.T) {
	...
	var ssmPatch ssm.PatchBaselineIdentity
	name := sdkacctest.RandString(10)
	resourceName := "aws_ssm_patch_baseline.test"	resource.Test(t, resource.TestCase{
PreCheck:.PreCheck(ctx, t) },
ErrorCheck: acctest.ErrorCheck(t, ssm.EndpointsID),
ProtoV5ProviderFactories: acctest.ProtoV5ProviderFactories,
CheckDestroy:testAccCheckPatchBaselineDestroy(ctx),
...
{
PreConfig: func() {
	conn := acctest.Provider.Meta().(*conns.AWSClient).SSMConn(ctx)	input := &ssm.RegisterDefaultPatchBaselineInput{
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).SSMConn(ctx)resp, err := conn.DescribePatchBaselinesWithContext(ctx, &ssm.DescribePatchBaselinesInput{
...
resource "aws_ssm_patch_baseline" "test" {
  ...
  descriptionaining all updates approved for production systems"
...
resource "aws_ssm_patch_baseline" "test" {
  ...
  descriptionaining all updates approved for production systems"
  approved_patches= ["KB123456"]
  approved_patches_compliance_level = "CRITICAL"  tags = {
%[2]q = %[3]q
  }
}
...
resource "aws_ssm_patch_baseline" "test" {
  ...
  descriptionaining all updates approved for production systems - August 2017"
...
resource "aws_ssm_patch_baseline" "test" {
  ...
  description = "Baseline containing all updates approved for production systems"  tags = {
Name = "My Patch Baseline"
  }  approval_rule {
approve_after_days  = 7
enable_non_security = true
compliance_level= "CRITICAL"patch_filter {
 key= "PRODUCT"
...
approve_after_days = 7
compliance_level= "INFORMATIONAL"patch_filter {
 key= "PRODUCT"
...
resource "aws_ssm_patch_baseline" "test" {
  ...
  operating_system= "AMAZON_LINUX"  source {
name = "My-AL2017.09"
