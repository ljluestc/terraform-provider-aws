...
resourceName := "aws_ssm_resource_data_sync.test"resource.ParallelTest(t, resource.TestCase{
...
continue
}syncItem, err := tfssm.FindResourceDataSyncItem(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
}if syncItem != nil {
...
resource "aws_s3_bucket" "hoge" {
  buckett-%[1]d"
  force_destroy = true
}data "aws_partition" "current" {}resource "aws_s3_bucket_policy" "hoge" {
  bucket = aws_s3_bucket.hoge.bucket  policy = <<EOF
{
  "Version": "2012-10-17",
  "Statement": [
{
...
${data.aws_partition.current.dns_suffix}"
 ...
 "Action": "s3:PutObject",
 "Resource": [
partition.current.partition}:s3:::tf-test-bucket-%[1]d/*"
 ],
 "Condition": {
{
 ...
 EOF}resource "aws_ssm_resource_data_sync" "test" {
  name = "tf-test-ssm-%[2]s"  s3_destination {
