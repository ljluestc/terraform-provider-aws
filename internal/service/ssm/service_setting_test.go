...
resourceName := "aws_ssm_service_setting.test"resource.ParallelTest(t, resource.TestCase{
...
continue
}output, err := tfssm.FindServiceSettingByID(ctx, conn, rs.Primary.Attributes["setting_id"])if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
}conn := acctest.Provider.Meta().(*conns.AWSClient).SSMConn(ctx)output, err := tfssm.FindServiceSettingByID(ctx, conn, rs.Primary.Attributes["setting_id"])if err != nil {
...
data "aws_caller_identity" "current" {}resource "aws_ssm_service_setting" "test" {
