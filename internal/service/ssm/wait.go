...
Timeout: timeout,
}outputRaw, err := stateConf.WaitForStateContext(ctx)if output, ok := outputRaw.(*ssm.AssociationDescription); ok && output.Overview != nil {
...
Timeout: timeout,
}outputRaw, err := stateConf.WaitForStateContext(ctx)if output, ok := outputRaw.(*ssm.ServiceSetting); ok {
return output, err
}return nil, err
