...
resourceName := "aws_ssmincidents_response_plan.test"snsTopic1 := "aws_sns_topic.topic1"
snsTopic2 := "aws_sns_topic.topic2"displayName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
chatChannelTopic := "aws_sns_topic.channel_topic"resource.Test(t, resource.TestCase{
PreCheck: func() {
...
resource.TestCheckResourceAttrPair(resourceName, "tags.b", dataSourceName, "tags.b"),acctest.MatchResourceAttrGlobalARN(dataSourceName, "arn", "ssm-incidents", regexache.MustCompile(`response-plan/+.`)),
),
},
},
})
