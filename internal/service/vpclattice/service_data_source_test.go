...
dataSourceName := "data.aws_vpclattice_service.test"resource.ParallelTest(t, resource.TestCase{
PreCheck: func() {
...
Name = %[1]q
  ...
  provider = "awsalternate"  service_identifier = aws_vpclattice_service.test.id  depends_on = [aws_ram_resource_association.test, aws_ram_principal_association.test]
}
`, rName))
}
