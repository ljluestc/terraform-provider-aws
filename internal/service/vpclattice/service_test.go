...
ctx := acctest.Context(t)var service vpclattice.GetServiceOutput
...
continue
}_, err := tfvpclattice.FindServiceByID(ctx, conn, rs.Primary.ID)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
...
resp, err := tfvpclattice.FindServiceByID(ctx, conn, rs.Primary.ID)if err != nil {
return err
}*service = *respreturn nil
}
}func testAccPreCheck(ctx context.Context, t *testing.T) {
conn := acctest.Provider.Meta().(*conns.AWSClient).VPCLatticeClient(ctx)input := &vpclattice.ListServicesInput{}
_, err := conn.ListServices(ctx, input)if acctest.PreCheckSkipError(err) {
