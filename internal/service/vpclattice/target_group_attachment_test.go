...
instanceResourceName := "aws_instance.test"resource.ParallelTest(t, resource.TestCase{
PreCheck: func() {
...
rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName := "aws_vpclattice_target_group_attachment.test"
lambdaResourceName := "aws_lambda_function.test"resource.ParallelTest(t, resource.TestCase{
PreCheck: func() {
acctest.PreCheck(ctx, t)
acctest.PreCheckPartitionHasService(t, names.VPCLatticeEndpointID)
testAccPreCheck(ctx, t)
...
rName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
resourceName := "aws_vpclattice_target_group_attachment.test"
albResourceName := "aws_lb.test"resource.ParallelTest(t, resource.TestCase{
PreCheck: func() {
acctest.PreCheck(ctx, t)
acctest.PreCheckPartitionHasService(t, names.VPCLatticeEndpointID)
testAccPreCheck(ctx, t)
...
return fmt.Sprintf(`
data "aws_partition" "current" {}resource "aws_vpclattice_target_group" "test" {
  name = %[1]q
  type = "LAMBDA"
...
port, err = strconv.Atoi(v)if err != nil {
return err
}
}conn := acctest.Provider.Meta().(*conns.AWSClient).VPCLatticeClient(ctx)_, err = tfvpclattice.FindTargetByThreePartKey(ctx, conn, rs.Primary.Attributes["target_group_identifier"], rs.Primary.Attributes["target.0.id"], port)return err
}
...
}_, err = tfvpclattice.FindTargetByThreePartKey(ctx, conn, rs.Primary.Attributes["target_group_identifier"], rs.Primary.Attributes["target.0.id"], port)if tfresource.NotFound(err) {
continue
}if err != nil {
return err
