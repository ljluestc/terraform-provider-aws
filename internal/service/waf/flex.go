...
func ExpandAction(l []interface{}) *waf.WafAction {
	if len(l) == 0 || l[0] == nil {
		return nil
	}	m := l[0].(map[string]interface{})	return &waf.WafAction{
...
func expandOverrideAction(l []interface{}) *waf.WafOverrideAction {
	if len(l) == 0 || l[0] == nil {
		return nil
	}	m := l[0].(map[string]interface{})	return &waf.WafOverrideAction{
		Type: aws.String(m["type"].(string)),
	}
}
func ExpandWebACLUpdate(updateAction string, aclRule map[string]interface{}) *waf.WebACLUpdate {
	var rule *waf.ActivatedRule	switch aclRule["type"].(string) {
	case waf.WafRuleTypeGroup:
		rule = &waf.ActivatedRule{
			OverrideAction: expandOverrideAction(aclRule["override_action"].([]interface{})),
			Priority:nt64(int64(aclRule["priority"].(int))),
			RuleId:.String(aclRule["rule_id"].(string)),
			Type:ws.String(aclRule["type"].(string)),
		}
	default:
		rule = &waf.ActivatedRule{
...
func FlattenWebACLRules(ts []*waf.ActivatedRule) []map[string]interface{} {
	...
	for i, r := range ts {
		m := make(map[string]interface{})		switch aws.StringValue(r.Type) {
		case waf.WafRuleTypeGroup:
