...
func TestAccWAFRuleGroup_basic(t *testing.T) {
	...
	resourceName := "aws_waf_rule_group.test"	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeAggregateTestCheckFunc(
					...
					testAccCheckRuleGroupExists(ctx, resourceName, &groupAfter),					testAccCheckRuleExists(ctx, "aws_waf_rule.test", &rule1),
...
func computeActivatedRuleWithRuleId(rule *waf.Rule, actionType string, priority int, idx *int) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		ruleResource := tfwaf.ResourceRuleGroup().SchemaMap()["activated_rule"].Elem.(*schema.Resource)		m := map[string]interface{}{
			"action": []interface{}{
...
func testAccCheckRuleGroupDisappears(ctx context.Context, group *waf.RuleGroup) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		conn := acctest.Provider.Meta().(*conns.AWSClient).WAFConn(ctx)		rResp, err := conn.ListActivatedRulesInRuleGroupWithContext(ctx, &waf.ListActivatedRulesInRuleGroupInput{
