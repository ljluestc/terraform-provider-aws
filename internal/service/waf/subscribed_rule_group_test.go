...
func TestAccWAFSubscribedRuleGroupDataSource_basic(t *testing.T) {
	...
	if os.Getenv("WAF_SUBSCRIBED_RULE_GROUP_NAME") == "" {
	...
	}	metricName := os.Getenv("WAF_SUBSCRIBED_RULE_GROUP_METRIC_NAME")	datasourceName := "data.aws_waf_subscribed_rule_group.rulegroup"	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config:cSubscribedRuleGroupDataSourceConfig_nonexistent,
			...
			{
				Config:cSubscribedRuleGroupDataSourceConfig_nameAndMismatchingMetricName(ruleGroupName),
				ExpectError: regexache.MustCompile(`no matches found`),
			},
		},
	})
