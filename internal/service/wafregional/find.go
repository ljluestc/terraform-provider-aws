...
RegexMatchSetId: aws.String(id),
	...
	hasMatch := false	if !hasName && !hasMetricName {
return nil, errors.New("must specify either name or metricName")
	}	input := &waf.ListSubscribedRuleGroupsInput{}	matchingRuleGroup := &waf.SubscribedRuleGroupSummary{}	for {
output, err := conn.ListSubscribedRuleGroupsWithContext(ctx, input)if tfawserr.ErrCodeContains(err, waf.ErrCodeNonexistentItemException) {
...
respMetricName := aws.StringValue(ruleGroup.MetricName)if hasName && respName != name {
continue
}
