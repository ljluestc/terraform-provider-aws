...
// +build sweeppackage wafregionalimport (
	"context"
	"fmt"
...
return fmt.Errorf("error getting client: %s", err)
	...
	conn := client.WAFRegionalConn(ctx)	input := &waf.ListRateBasedRulesInput{}	for {
output, err := conn.ListRateBasedRulesWithContext(ctx, input)if sweep.SkipSweepError(err) {
...
wr := NewRetryer(conn, region)_, err := wr.RetryWithToken(ctx,func(token *string) (interface{}, error) {
...
getRateBasedRuleInput := &waf.GetRateBasedRuleInput{
	RuleId: rule.RuleId,
}getRateBasedRuleOutput, getRateBasedRuleErr := conn.GetRateBasedRuleWithContext(ctx, getRateBasedRuleInput)if getRateBasedRuleErr != nil {
...
return fmt.Errorf("error getting client: %s", err)
	...
	conn := client.WAFRegionalConn(ctx)	var sweeperErrs *multierror.Error	err = listRegexMatchSetsPages(ctx, conn, &waf.ListRegexMatchSetsInput{},func(page *waf.ListRegexMatchSetsOutput, lastPage bool) bool {
if page == nil {
return !lastPage
}for _, r := range page.RegexMatchSets {
id := aws.StringValue(r.RegexMatchSetId)set, err := FindRegexMatchSetByID(ctx, conn, id)
if err != nil {
...
return fmt.Errorf("error getting client: %s", err)
	...
	conn := client.WAFRegionalConn(ctx)	req := &waf.ListRuleGroupsInput{}
...
return fmt.Errorf("error getting client: %s", err)
	...
	conn := client.WAFRegionalConn(ctx)	input := &waf.ListRulesInput{}	for {
output, err := conn.ListRulesWithContext(ctx, input)if sweep.SkipSweepError(err) {
...
getRuleInput := &waf.GetRuleInput{
	RuleId: rule.RuleId,
}getRuleOutput, getRuleErr := conn.GetRuleWithContext(ctx, getRuleInput)if getRuleErr != nil {
...
return fmt.Errorf("error getting client: %s", err)
	...
	conn := client.WAFRegionalConn(ctx)	input := &waf.ListWebACLsInput{}	for {
output, err := conn.ListWebACLsWithContext(ctx, input)if sweep.SkipSweepError(err) {
...
getWebACLInput := &waf.GetWebACLInput{
	WebACLId: webACL.WebACLId,
}getWebACLOutput, getWebACLErr := conn.GetWebACLWithContext(ctx, getWebACLInput)if getWebACLErr != nil {
...
}func listRegexMatchSetsPages(ctx context.Context, conn *wafregional.WAFRegional, input *waf.ListRegexMatchSetsInput, fnfunc(*waf.ListRegexMatchSetsOutput, bool) bool) error {
	for {
