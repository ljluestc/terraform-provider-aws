...
func testAccIPGroup_basic(t *testing.T) {
	...
	resourceName := "aws_workspaces_ip_group.test"	resource.Test(t, resource.TestCase{
...
func testAccIPGroup_MultipleDirectories(t *testing.T) {
	ctx := acctest.Context(t)
	var v types.WorkspacesIpGroup
	var d1, d2 types.WorkspaceDirectory	ipGroupName := sdkacctest.RandomWithPrefix(acctest.ResourcePrefix)
	domain := acctest.RandomDomainName()	resourceName := "aws_workspaces_ip_group.test"
	directoryResourceName1 := "aws_workspaces_directory.test1"
	directoryResourceName2 := "aws_workspaces_directory.test2"	resource.Test(t, resource.TestCase{
		PreCheck: func() {
...
resource "aws_workspaces_ip_group" "test" {
  ...
  directory_id = aws_directory_service_directory.main.id  ip_group_ids = [
