...
// SPDX-License-Identifier: MPL-2.0package workspacesimport (
	"context"
	"time"	"github.com/aws/aws-sdk-go-v2/service/workspaces"
	"github.com/aws/aws-sdk-go-v2/service/workspaces/types"
	"github.com/hashicorp/terraform-plugin-sdk/v2/helper/retry"
	"github.com/hashicorp/terraform-provider-aws/internal/enum"
)const (
	DirectoryDeregisterInvalidResourceStateTimeout = 2 * time.Minute
	DirectoryRegisterInvalidResourceStateTimeout   = 2 * time.Minute	// Maximum amount of time to wait for a Directory to return Registered
	DirectoryRegisteredTimeout = 10 * time.Minute	// Maximum amount of time to wait for a Directory to return Deregistered
	DirectoryDeregisteredTimeout = 10 * time.Minute	// Maximum amount of time to wait for a WorkSpace to return Available
	WorkspaceAvailableTimeout = 30 * time.Minute	// Maximum amount of time to wait for a WorkSpace while returning Updating
	WorkspaceUpdatingTimeout = 10 * time.Minute	// Amount of time to delay before checking WorkSpace when updating
	WorkspaceUpdatingDelay = 1 * time.Minute	// Maximum amount of time to wait for a WorkSpace to return Terminated
...
func WaitDirectoryRegistered(ctx context.Context, conn *workspaces.Client, directoryID string) (*types.WorkspaceDirectory, error) {
	...
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if v, ok := outputRaw.(*types.WorkspaceDirectory); ok {
		return v, err
	}	return nil, err
}
...
func WaitWorkspaceAvailable(ctx context.Context, conn *workspaces.Client, workspaceID string, timeout time.Duration) (*types.Workspace, error) {
	stateConf := &retry.StateChangeConf{
		...
		Timeout: timeout,
	}	outputRaw, err := stateConf.WaitForStateContext(ctx)	if v, ok := outputRaw.(*types.Workspace); ok {
		return v, err
	}	return nil, err
}
